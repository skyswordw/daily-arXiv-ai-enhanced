...
Motivation: 茶叶是孟加拉国的重要经济作物，但茶叶易受各种叶部病害影响，导致产量和质量下降。人工检测病害耗时且易出错，因此需要开发自动化深度学习模型来高效准确地识别茶叶病害。

Method: 研究采用TeaLeafBD数据集（5278张图像，7个类别：6种病害+健康叶片）。方法包括：数据预处理、数据分割、对抗训练、数据增强、模型训练、评估以及可解释AI分析。使用DenseNet201和EfficientNetB3两种深度学习架构进行分类，通过对抗训练增强模型鲁棒性，并采用Grad-CAM可视化分析模型预测依据。

Result: 实验结果显示，EfficientNetB3达到最高的93%分类准确率，DenseNet201达到91%。对抗训练使模型对噪声或干扰输入具有更好的鲁棒性。Grad-CAM可视化成功识别了影响模型预测的关键图像区域。

Conclusion: 提出的深度学习方法能够准确检测茶叶病害，为现代农业管理提供了实用解决方案。该方法证明了自动化病害检测的可行性，有助于提高茶叶生产的效率和质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的自对弈方法依赖静态图像集的被动交互，导致对初始数据集的强依赖和学习效率低下。模型无法主动获取适合其当前能力发展的视觉数据，浪费计算资源在过于简单或过难的任务上。

Method: 提出Active-Zero框架，包含三个协同进化的智能体：1) Searcher从开放世界库中基于模型能力边界检索图像；2) Questioner合成校准的推理任务；3) Solver通过准确性奖励进行精炼。形成闭环的自支架自动课程学习系统。
...
TL;DR: ReTracing是一个多智能体具身表演艺术项目，采用考古学方法研究AI如何塑造、约束和产生身体运动。通过LLM生成提示词，扩散模型转化为人类表演者和四足机器人的动作指导，在镜面地板上表演并记录运动轨迹，揭示生成系统中的社会文化偏见。


<details>
  <summary>Details</summary>
Motivation: 该项目旨在探索人工智能如何影响和塑造身体运动，揭示生成系统中编码的社会文化偏见。通过人类、机器人和AI的互动，探讨在AI也能运动、思考和留下痕迹的时代，作为人类意味着什么这一关键问题。

Method: 1. 从科幻小说中提取描述人机交互的句子；2. 使用大语言模型为每个摘录生成"做什么"和"不做什么"的配对提示；3. 基于扩散的文本到视频模型将这些提示转化为人类表演者的编舞指导和四足机器人的运动指令；4. 在镜面地板上进行表演，通过多摄像头运动捕捉系统记录；5. 重建为3D点云和运动轨迹，形成数字运动档案。

Result: 创建了一个数字运动痕迹档案，通过人类和机器人在镜面地板上的同步表演，捕捉了AI生成的动作指导在实际物理空间中的体现。项目展示了生成系统如何通过编排的动作编码社会文化偏见，并提供了研究AI与身体运动关系的新方法。

Conclusion: ReTracing提供了一种新颖的方法来揭示生成系统如何通过编排的动作编码社会文化偏见。这个项目不仅是一个艺术表演，更是一个研究框架，通过沉浸式的人机交互，探索AI时代身体运动的意义和人类身份的本质问题。
...
TL;DR: REVEAL是一个诊断基准，通过五个压力测试揭示视频语言模型在理解视频内容、时间序列和运动方面的基本弱点。


<details>
  <summary>Details</summary>
Motivation: 研究视频语言模型是否能够稳健地理解视频内容、时间序列和运动，发现现有模型存在显著缺陷，而人类在这些任务上表现优异。

Method: 开发REVEAL诊断基准，包含五个压力测试：时间期望偏差、语言捷径依赖、视频谄媚、相机运动敏感性、时空遮挡鲁棒性。提供自动生成诊断示例的数据管道。

Result: 测试主流开源和闭源视频语言模型发现：模型会自信地将反向场景描述为正向、回答问题忽视视频内容、同意错误主张、难以处理基本相机运动、在简单时空遮挡下无法聚合时间信息。

Conclusion: 当前视频语言模型在视频内容理解方面存在根本性弱点，需要更稳健的评估方法。REVEAL基准和代码将公开发布以支持未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D重建方法主要通过摄影测量实现，但不同方法之间的差异主要依赖定性评估，缺乏可重复、可量化的比较基准

Method: 开发了一个新流程：从高质量3D模型出发，通过程序化生成的相机位姿生成合成图像，从而创建可重复的实验环境

Result: 该流程能够实现多种可重复、可量化的实验，可以比较虚拟相机参数和虚拟对象的真实值与重建估计值之间的差异

Conclusion: 提出的合成图像生成流程为摄影测量方法的性能评估提供了可靠的量化基准，解决了现有方法依赖定性评估的问题
...
<details>
  <summary>Details</summary>
Motivation: 传统单目VIO系统在低纹理环境中表现不佳，稀疏视觉特征不足以进行准确位姿估计。虽然基于ViT的复杂基础模型能提供密集、几何一致的深度信息，但其计算需求通常无法满足实时边缘部署要求

Method: 提出将学习到的深度先验直接集成到VINS-Mono优化后端的新框架，强制实施仿射不变深度一致性和成对序数约束，通过基于方差的门控机制显式过滤不稳定伪影

Result: 在TartanGround和M3ED数据集上的大量实验表明，该方法在挑战性场景中防止发散，显著提高精度，将绝对轨迹误差(ATE)降低高达28.3%

Conclusion: 该方法在严格遵循边缘设备计算限制的同时，鲁棒地恢复度量尺度，为低纹理环境中的实时VIO系统提供了有效的解决方案
...
TL;DR: 提出StreamSR数据集和EfRLFN模型，解决压缩视频实时超分辨率问题，在YouTube流媒体场景中实现高效性能提升。
...
Result: 1) 创建了具有代表性的流媒体数据集；2) EfRLFN模型在视觉质量和运行时性能上均有提升；3) 在其他模型上使用该数据集微调能显著提升性能并良好泛化到标准基准测试。

Conclusion: StreamSR数据集填补了流媒体超分辨率评估的空白，EfRLFN模型为实时压缩视频超分辨率提供了高效解决方案，相关资源已开源。
...
TL;DR: ArtContext：基于开放获取艺术史文章和Wikidata知识构建的艺术作品上下文标注管道，通过定制化的PaintingCLIP模型提升艺术作品理解


<details>
  <summary>Details</summary>
Motivation: 艺术史文章常讨论艺术作品整体及具体方面（布局、图像学、物质文化等），但观看艺术品时难以快速识别不同文章对同一作品的论述，需要系统化标注方法

Method: 提出ArtContext管道：1）新颖的语料库收集管道整合开放获取艺术史文章和Wikidata知识；2）使用低秩适应（LoRA）技术定制CLIP模型，创建领域特定的PaintingCLIP模型；3）通过收集的语料库进行弱监督训练

Result: 新模型PaintingCLIP在性能上超越原始CLIP模型，能够为给定艺术作品提供有效上下文信息

Conclusion: 提出的管道具有通用性，可轻松应用于众多人文学科领域，为艺术作品理解提供了系统化的上下文标注解决方案
...
TL;DR: Latent Forcing是一种改进潜在扩散模型的方法，通过联合处理潜在表示和原始像素，使用分离的噪声调度，在保持潜在扩散效率的同时直接在原始自然图像上操作。


<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型虽然能生成高质量图像，但存在三个主要问题：1) 在图像编码过程中丢弃信息；2) 需要单独训练的编码器；3) 对原始数据的辅助分布进行建模，失去了端到端建模的优势。

Method: 提出Latent Forcing方法，通过联合处理潜在表示和像素，使用分离的噪声调度来排序去噪轨迹。潜在表示作为中间计算的"草稿纸"，在高频像素特征生成前发挥作用。特别研究了条件信号顺序的重要性。

Result: 在ImageNet上，Latent Forcing在扩散变换器基像素生成方面达到了新的最先进水平（在作者的计算规模下）。该方法还解释了REPA蒸馏在分词器和扩散模型中的差异、条件与非条件生成的区别，以及分词器重建质量与可扩散性的关系。

Conclusion: Latent Forcing通过简单的架构修改，既保持了潜在扩散的效率，又能在原始自然图像上操作，解决了传统潜在扩散模型的局限性，为高质量图像生成提供了更优的端到端解决方案。
...
TL;DR: 利用CTA数据训练神经隐式函数，从各向异性的短轴CMRI重建心脏形状，提高右心室和心肌的3D形状分析精度


<details>
  <summary>Details</summary>
Motivation: 短轴心血管磁共振成像的各向异性特性限制了心脏形状分析，需要利用近各向同性、高分辨率CT血管造影数据来改善心脏形状重建

Method: 使用CTA数据训练单个神经隐式函数，联合表示任何分辨率的CMRI心脏形状；通过提取四腔切片的右心室和心肌来评估重建效果

Result: 右心室Dice系数0.91±0.07，Hausdorff距离6.21±3.97mm；心肌Dice系数0.75±0.13，Hausdorff距离7.53±5.13mm；模型能重建准确、平滑且解剖学合理的形状

Conclusion: 该方法能有效改善心脏形状分析，支持从各向异性的短轴CMRI重建高质量的心脏3D形状
...
TL;DR: Ctrl&Shift：首个无需显式3D建模、统一几何控制与真实世界泛化的物体级操作扩散框架，通过两阶段分解（物体移除+参考引导修复）实现背景保持、几何一致性和用户可控变换


<details>
  <summary>Details</summary>
Motivation: 现有物体级操作方法无法同时实现三个核心目标：背景保持、视角变换下的几何一致性、用户可控变换。基于几何的方法需要显式3D重建且泛化差，基于扩散的方法泛化好但缺乏细粒度几何控制。需要一种能统一几何控制与真实世界泛化的框架。

Method: 将物体操作分解为两阶段：物体移除和参考引导修复，统一编码到扩散过程中。设计多任务多阶段训练策略，分离背景、身份和姿态信号。引入可扩展的真实世界数据集构建流程，生成带有估计相对相机姿态的配对图像和视频样本。

Result: 广泛实验表明Ctrl&Shift在保真度、视角一致性和可控性方面达到最先进水平。这是首个无需任何显式3D建模就能统一细粒度几何控制和真实世界泛化的物体操作框架。

Conclusion: Ctrl&Shift通过创新的两阶段扩散框架成功解决了物体级操作中的核心挑战，实现了背景保持、几何一致性和用户可控性的统一，为电影后期制作、AR和创意编辑等应用提供了强大的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有特征压缩方法通常需要为特定压缩比训练专用模型，缺乏灵活性且泛化能力有限，当需要适应新压缩比时必须重新训练。这在实际应用中效率低下且不实用。
...
Conclusion: ARFC框架有效解决了现有特征压缩方法缺乏灵活性的问题，通过单一模型支持任意压缩比，在资源受限的实际场景中具有显著优势和应用价值。
...
TL;DR: 提出VasoMIM框架，一种融合血管解剖知识的掩码图像建模方法，用于X射线血管造影的自监督预训练，并构建了XA-170K大规模数据集，在多个下游任务中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: X射线血管造影是心血管疾病诊断的金标准，但当前深度学习方法受限于标注数据稀缺。大规模自监督学习虽具潜力，但在该领域应用不足，主要缺乏有效的SSL框架和大规模数据集。
...
Result: 在四个下游任务（跨六个数据集）上验证VasoMIM，相比现有方法表现出优越的迁移能力，并取得最先进的性能。
...
TL;DR: 提出MFdiff模型，通过监督辅助的多模态融合扩散模型，从低剂量PET和MR图像中恢复高质量标准剂量PET图像


<details>
  <summary>Details</summary>
Motivation: PET成像存在辐射暴露问题，降低剂量或扫描时间会降低图像质量。利用MR图像辅助恢复标准剂量PET面临多模态融合的结构纹理不一致性以及分布外数据不匹配的挑战

Method: 1) 设计多模态特征融合模块学习优化融合特征；2) 以融合特征为条件，基于扩散模型迭代生成高质量SPET；3) 引入两阶段监督辅助学习策略，结合模拟分布内数据的广义先验和体内分布外数据的特定先验
...
TL;DR: 提出基于生成压缩框架的感知去噪方法，通过熵编码潜在表示强制低复杂度结构，结合生成解码器恢复真实纹理，在保持竞争性失真性能的同时实现感知改进。


<details>
  <summary>Details</summary>
Motivation: 传统失真驱动去噪方法在强噪声和分布偏移下容易产生过度平滑的重建结果，缺乏感知真实性。需要一种能够同时处理失真和感知质量的去噪框架。

Method: 提出生成压缩框架：1) 基于条件Wasserstein GAN的压缩去噪器，显式控制率-失真-感知权衡；2) 基于条件扩散的重建策略，通过压缩潜在进行迭代去噪指导。建立加性高斯噪声下压缩最大似然去噪器的非渐近保证。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在视觉质量上虽有进步，但超高分辨率视频生成仍面临运动建模、语义规划和细节合成的复合挑战，需要更高效的解决方案。

Method: 采用三阶段架构：1)低分辨率运动生成用于运动一致的潜在合成；2)视频潜在上采样直接在潜在空间进行以减少内存和计算开销；3)高分辨率内容细化集成低频和高频专家共同增强语义连贯性和细粒度细节生成。

Result: 大量实验表明LUVE在超高分辨率视频生成中实现了卓越的逼真度和内容保真度，全面的消融研究进一步验证了各组件有效性。

Conclusion: LUVE通过潜在级联架构和双频专家系统有效解决了UHR视频生成的挑战，为高质量视频生成提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 涂鸦注释虽然降低了密集像素标注成本，但存在噪声和不完整监督问题。现有基于伪标签的方法受限于伪标签质量，需要提升伪标签的可靠性和空间一致性

Method: 提出PLESS框架，通过层次化图像分区将图像分解为空间一致的区域层次结构，在语义一致区域内传播涂鸦信息来细化伪标签。该框架模型无关，可轻松集成到现有伪标签方法中

Result: 在两个公共心脏MRI数据集（ACDC和MSCMRseg）上，对四种涂鸦监督算法进行实验，均显示分割准确性的持续改进

Conclusion: PLESS是一种有效的伪标签增强策略，能够提高基于涂鸦注释的医学图像分割性能，具有通用性和易集成性
...
TL;DR: GR-Diffusion：一种结合3D离散高斯表示几何先验和扩散模型生成能力的新型低剂量全身PET重建框架，通过分层引导机制提升图像质量和生理细节保留。
...
Method: 提出GR-Diffusion框架：1）使用3D离散高斯表示从投影数据生成参考PET图像，建立物理基础和结构明确的基准；2）在扩散过程中采用分层引导机制：细粒度引导利用差异图优化局部细节，粗粒度引导使用多尺度差异图校正偏差；3）扩散模型顺序集成GR的强几何先验并恢复亚体素信息。

Result: 在UDPET和临床数据集上的实验表明，GR-Diffusion在不同剂量水平下均优于现有最先进方法，显著提升了3D全身PET图像质量并更好地保留了生理细节。

Conclusion: GR-Diffusion成功整合了离散高斯表示的几何先验和扩散模型的生成能力，通过分层引导机制实现了高质量的3D低剂量全身PET重建，克服了传统方法的局限性。
...
TL;DR: SToRM提出首个用于多模态大语言模型的监督式令牌缩减框架，通过重要性预测、监督训练和锚点-上下文合并三个关键组件，在保持全令牌性能的同时将计算成本降低30倍。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶系统在意外场景中依赖人类自然语言指令时需要使用多模态大语言模型，但MLLM依赖大量视觉令牌导致计算资源需求过高，现有令牌缩减方法往往导致任务性能下降。
...
Conclusion: SToRM是首个用于多模态大语言模型的监督式令牌缩减框架，能够在保持端到端自动驾驶性能的同时显著降低计算开销，解决了MLLM在资源受限自动驾驶系统中的部署难题。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成方法难以捕捉细微情感语义和细粒度控制，限制了创建情感丰富的沉浸式VR内容

Method: 采用分层情感表示，学习丰富的可学习原型；开发可控生成流程，包含多原型引导、时间混合和注意力重加权
...
Method: 提出层次化实例树结构，利用噪声掩码作为信息线索而非直接修正；通过跨视图分组和条件替换抑制过分割和欠分割；为每个实例添加开放词汇语义嵌入；引入一致性感知更新机制处理多阶段任务中的场景变化

Result: 在合成和真实数据集上均优于现有方法；在最具挑战性的重度杂乱序列中AP@25达到61.66，比基线高2.2倍以上；仅用4个输入视图就超越使用8个视图的MaskClustering方法2倍以上

Conclusion: Clutt3R-Seg通过层次化实例树和语义线索处理，实现了在杂乱场景中鲁棒的3D实例分割，支持语言引导的机器人抓取任务，并在各种设置下均优于现有方法
...
TL;DR: 提出颈戴式视角视线估计新任务，收集首个数据集，评估GLC模型并提出两个扩展：视线越界分类和多视角协同学习，发现前者有效而后者无效


<details>
  <summary>Details</summary>
Motivation: 现有第一人称视线估计主要关注头戴式摄像头，而其他视角（如颈戴式）尚未充分探索。颈戴式视角具有独特优势和应用潜力，需要建立相应数据集和方法

Method: 1) 收集首个颈戴式视线估计数据集（8名参与者，约4小时日常活动视频）；2) 评估基于Transformer的GLC模型；3) 提出两个扩展：视线越界分类辅助任务和多视角协同学习方法（联合训练头戴式和颈戴式模型，使用几何感知辅助损失）

Result: 视线越界分类任务能提升性能，优于标准微调；但多视角协同学习方法未能带来性能增益。对结果进行了深入分析，讨论了颈戴式视线估计的启示

Conclusion: 颈戴式视线估计是一个有前景的新任务，视线越界分类是有效的改进策略，但多视角协同学习在当前设置下效果有限，需要进一步研究颈戴式视角的特性
...
TL;DR: 提出TD-FusionUNet模型，通过可训练的Hadamard变换和DCT变换层在正交化潜在空间中捕获频率分量，用于轻量级次日野火蔓延预测，在参数较少情况下优于UNet基线。
...
Method: 提出Transform Domain Fusion UNet (TD-FusionUNet)，包含可训练的Hadamard变换和离散余弦变换层，在正交化潜在空间中应用二维变换捕获频率分量。采用随机边缘裁剪和高斯混合模型等自定义预处理技术丰富稀疏预火掩码表示并增强泛化能力。

Result: 在Google Research 2023年发布的Next-Day Wildfire Spread数据集和WildfireSpreadTS数据集上评估。TD-FusionUNet以37万参数获得0.591的F1分数，优于使用ResNet18编码器的UNet基线，且参数显著减少。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到形状检索方法需要规范姿态且支持类别有限，而现实世界中物体类别多样且姿态随机，限制了实际应用。需要开发能够处理任意旋转姿态且支持多类别的检索方法。

Method: 提出RI-Mamba旋转不变状态空间模型：1) 定义全局和局部参考系解耦姿态与几何；2) 使用希尔伯特排序构建具有几何结构的token序列；3) 引入方向嵌入计算策略并通过特征线性调制恢复空间上下文；4) 采用跨模态对比学习与自动三元组生成进行训练。

Result: 在OmniObject3D基准测试中，RI-Mamba在200多个物体类别上实现最先进性能，表现出卓越的表征能力和鲁棒性，能够处理任意旋转姿态。

Conclusion: RI-Mamba是首个旋转不变的状态空间模型，通过解耦姿态与几何、构建几何结构序列、恢复空间上下文等创新方法，有效解决了多类别任意姿态下的文本到3D形状检索问题，具有线性时间复杂度和良好的可扩展性。
...
TL;DR: 开发了一种语义条件化的潜在扩散模型，用于在解剖循环和C臂位置控制下合成动脉期脑DSA图像，评估显示生成的图像具有临床真实性和分布相似性。


<details>
  <summary>Details</summary>
Motivation: DSA在脑血管疾病诊断和治疗中至关重要，但其侵入性和高采集成本限制了大规模数据收集和公共数据共享，需要开发合成DSA图像的方法来支持算法开发、研究和培训。

Method: 使用包含99,349帧的单中心DSA数据集，训练语义条件化的潜在扩散模型，通过文本嵌入编码解剖结构（前循环vs后循环）和采集几何（C臂位置）来控制图像生成。

Result: 生成的合成DSA图像获得3.1-3.3的总体Likert评分（5分制），具有高的评分者间可靠性（ICC(2,k)=0.80-0.87），与真实DSA帧的分布相似性表现为低FID分数（中位数15.27）。

Conclusion: 语义控制的潜在扩散模型能够生成具有临床真实性的合成DSA图像，适用于下游算法开发、研究和培训应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的3D场景生成方法通常缺乏领域特定推理、验证机制和模块化设计，导致控制性差和可扩展性不足。本研究旨在解决这些限制，专门针对农业领域开发能够从自然语言提示生成合成仿真环境的系统。

Method: 开发了模块化多LLM管道，集成3D资产检索、领域知识注入和Unreal渲染引擎API代码生成。采用混合策略结合few-shot提示、检索增强生成、微调和验证等LLM优化技术。系统架构支持结构化数据处理、中间验证和灵活扩展。

Result: 系统能够基于输入提示和领域知识生成具有真实种植布局和环境背景的3D环境。通过结构化提示和语义准确性指标评估，用户研究显示生成的场景在真实感和熟悉度方面表现良好，专家比较证明相比手动场景设计显著节省时间。
...
<details>
  <summary>Details</summary>
Motivation: 现有SLAM方法存在两种问题：1）使用统一场景表示耦合跟踪与建图导致计算成本高；2）将跟踪框架与场景表示松散集成引入冗余。需要一种既能实时运行又能保持高重建质量的方法。

Method: 1）双向耦合视觉里程计（VO）与高斯泼溅（GS）；2）在期望最大化（EM）框架下制定联合优化，同时细化VO生成的半稠密深度估计和GS表示；3）提出高斯泼溅初始化，利用VO提供的图像信息、关键帧位姿和像素关联生成接近最终高斯场景的近似，避免启发式方法。

Result: 1）系统能够实时运行；2）在重建场景的几何/光度保真度和跟踪精度方面达到最先进水平；3）通过广泛实验验证了方法的有效性。

Conclusion: GSO-SLAM通过双向耦合VO和GS，在EM框架下实现联合优化，不仅实现了实时性能，而且在场景重建质量和跟踪精度方面超越了现有方法，为实时单目稠密SLAM提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在密集预测任务（如时空视频定位）中的幻觉问题，避免传统方法需要额外可训练模块带来的标注成本和计算开销

Method: 1) 视觉提示范式：将逐帧坐标预测转化为紧凑的实例级识别问题，为每个对象分配唯一、时间一致的ID并嵌入视频作为视觉提示；2) STVG-R1强化学习框架：使用任务驱动奖励联合优化时间精度、空间一致性和结构格式正则化

Result: 在六个基准测试中验证有效性，STVG-R1在HCSTVG-v2基准上超越基线Qwen2.5-VL-7B达20.9% m_IoU，达到SOTA；在MeViS基准上零样本泛化到多对象参考视频对象分割任务，达到47.3% J&F的SOTA性能

Conclusion: 提出的视觉提示范式有效避免了跨模态坐标对齐难题，STVG-R1框架在时空视频定位任务中表现出色，同时具备强大的零样本泛化能力
...
TL;DR: JEPA-VLA：通过整合预测性视频嵌入（V-JEPA 2）来增强视觉-语言-动作模型，解决现有VLA模型样本效率低和泛化能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于预训练视觉语言模型构建的视觉-语言-动作模型存在样本效率低和泛化能力有限的问题。作者认为这些限制与预训练视觉表示密切相关，现有表示在环境理解和策略先验两方面都提供不足的知识。

Method: 提出JEPA-VLA方法，自适应地将预测性嵌入（特别是V-JEPA 2）整合到现有VLA模型中。V-JEPA 2通过视频预训练，能够灵活丢弃不可预测的环境因素并编码任务相关的时序动态。

Result: 在多个基准测试中（LIBERO、LIBERO-plus、RoboTwin2.0和真实机器人任务）都取得了显著的性能提升，证明了方法的有效性。

Conclusion: 预测性视频嵌入能够有效补偿现有视觉表示在VLA中的关键缺陷，通过整合这类嵌入可以显著提升VLA模型的性能和泛化能力。
...
TL;DR: 通过针对性的适配策略，将通用视觉语言模型有效应用于电商产品理解任务，在保持广泛多模态能力的同时显著提升电商性能


<details>
  <summary>Details</summary>
Motivation: 电商产品理解需要强大的多模态理解能力（文本、图像、结构化属性），但通用视觉语言模型缺乏针对电商数据特性（属性中心、多图像、噪声）的适配策略，且现有方法往往以牺牲通用性能为代价

Method: 提出针对性的通用VLM适配方法，通过大规模实验研究验证策略有效性；同时设计了一个新颖的全面评估套件，涵盖深度产品理解、严格指令遵循和动态属性提取

Result: 研究表明，通过针对性的适配策略，可以显著提升通用VLM在电商任务上的性能，同时保持其广泛的多模态能力

Conclusion: 提出了有效的通用VLM电商适配策略和全面的评估框架，为电商多模态理解提供了系统性的解决方案
...
TL;DR: DiffPlace是一个用于地点可控多视角图像生成的新框架，通过引入place-ID控制器在CLIP空间中映射地点嵌入，实现背景建筑一致而前景对象和天气条件可灵活修改的城市场景生成，显著提升视觉地点识别任务的训练效果。


<details>
  <summary>Details</summary>
Motivation: 当前多视角扩散模型在从文本、BEV地图和物体边界框生成地点感知和背景一致的城市场景方面存在困难，这限制了它们在为地点识别任务生成真实样本方面的有效性。需要解决生成模型在场景级和地点感知合成方面的不足。

Method: 提出DiffPlace框架，引入place-ID控制器，该控制器采用线性投影、感知器变换器和对比学习，将place-ID嵌入映射到固定的CLIP空间中。这使得模型能够合成具有一致背景建筑的图像，同时灵活修改前景对象和天气条件。

Result: 广泛的实验（包括定量比较和增强训练评估）表明，DiffPlace在生成质量和视觉地点识别训练支持方面均优于现有方法。结果突显了生成模型在增强场景级和地点感知合成方面的潜力。

Conclusion: DiffPlace为改进自动驾驶中的地点识别提供了一种有价值的方法，通过地点可控的多视角图像生成，能够有效提升视觉地点识别任务的性能。
...
Method: 利用自监督视觉Transformer中的对象中心注意力，移除最显著视觉证据构建辅助视图，破坏不受支持的token并产生更强的对比信号

Result: 在两个流行的物体幻觉基准测试和两个MLLM上均取得一致性能提升

Conclusion: 该方法具有提示无关、模型无关、计算开销小等优点，可无缝集成到现有VCD流程中
...
TL;DR: Code2Worlds：通过语言到模拟代码生成的框架，解决4D世界生成中的多尺度上下文纠缠和语义-物理执行差距问题，实现物理感知的动态场景生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注静态3D场景生成，但实现空间智能需要超越视觉合理性，构建基于物理定律的世界模拟器。4D动态生成面临两个核心挑战：多尺度上下文纠缠（整体生成难以平衡局部对象结构与全局环境布局）和语义-物理执行差距（开环代码生成导致物理幻觉，缺乏动态保真度）。

Method: 提出Code2Worlds框架，将4D生成表述为语言到模拟代码生成。采用双流架构：检索增强的对象生成流与分层环境编排流解耦。建立物理感知闭环机制：PostProcess Agent编写动态脚本，VLM-Motion Critic进行自我反思以迭代优化模拟代码。

Result: 在Code4D基准测试中，Code2Worlds优于基线方法，获得41%的SGS增益和49%更高的丰富度评分，且能生成先前静态方法所缺乏的物理感知动态效果。

Conclusion: Code2Worlds通过语言到模拟代码生成框架，成功解决了4D世界生成中的关键挑战，实现了物理感知的动态场景生成，为构建基于物理定律的世界模拟器提供了有效途径。
...
TL;DR: Light4D是一个无需训练即可合成在目标光照下保持时间一致性的4D视频框架，通过解耦流引导和时间一致性注意力机制，能够处理极端视角变化。
...
Method: 提出Light4D框架：1) 解耦流引导：时间感知策略，将光照控制注入潜在空间同时保持几何完整性；2) 时间一致性注意力：在IC-Light架构中开发，结合确定性正则化消除外观闪烁。
...
<details>
  <summary>Details</summary>
Motivation: SAM模型需要海量数据集（1100万图像）且仅依赖RGB输入，现有高效变体虽减少计算但仍需大规模训练。深度信息能为分割提供几何先验，但当前方法未充分利用

Method: 提出轻量级RGB-D融合框架：1) 使用预训练深度估计器生成深度图；2) 通过专用深度编码器在中间层将深度特征与RGB特征融合；3) 基于EfficientViT-SAM架构，仅需11.2k样本训练

Result: 仅用11.2k样本（少于SA-1B的0.1%）训练，该方法在分割精度上超越了EfficientViT-SAM，证明深度线索为分割提供了强几何先验

Conclusion: 深度信息作为几何先验能显著提升分割性能，轻量级RGB-D融合框架在极少量训练数据下即可超越仅用RGB的模型，为高效分割提供了新方向
...
TL;DR: 使用3D分形作为合成数据预训练动作识别模型，提出Targeted Smart Filtering方法解决生成速度慢和多样性不足问题


<details>
  <summary>Details</summary>
Motivation: 合成数据集在深度学习中被视为真实数据的宝贵替代品，FDSL方法能提供无限完美标注数据且无人工标注、隐私等伦理问题。但现有3D分形生成方法速度慢且产生退化分形，需要改进

Method: 使用3D迭代函数系统生成分形，通过时间变换形成视频作为预训练数据集。提出Targeted Smart Filtering方法，解决生成速度和分形多样性问题，比传统过滤方法快约100倍

Result: Targeted Smart Filtering方法实现了约100倍的采样速度提升，并在下游任务性能上优于其他3D分形过滤方法

Conclusion: 通过Targeted Smart Filtering方法，能够高效生成多样化的3D分形数据，有效提升动作识别模型的预训练效果，为合成数据在计算机视觉任务中的应用提供了新思路
...
TL;DR: WorldTree：一种统一的时空分解框架，通过时间划分树和空间祖先链实现从粗到细的动态重建优化，在单目输入下显著提升重建质量。
...
Method: 提出WorldTree统一框架，包含两个核心组件：1) 时间划分树(TPT)：基于继承的划分树结构实现从粗到细的层次时间分解优化；2) 空间祖先链(SAC)：递归查询祖先层次结构，提供互补的空间动态，同时在祖先节点间专门化运动表示。

Result: 在不同数据集上的实验结果表明，该方法在NVIDIA-LS数据集上LPIPS指标提升8.26%，在DyCheck数据集上mLPIPS指标提升9.09%，均优于次优方法。

Conclusion: WorldTree通过统一的时空分解框架有效解决了单目动态重建中的挑战，实现了更高效的运动表示和更好的重建质量，为实际应用提供了更实用的解决方案。
...
TL;DR: 提出Proximal-Mean Inversion (PMI)和mimic-CFG方法，解决Rectified-Flow模型中训练自由反转的累积误差问题，提升重建和编辑质量


<details>
  <summary>Details</summary>
Motivation: Rectified-Flow模型虽然支持训练自由反转，但现有反转方法存在跨时间步的累积近似误差，导致速度场不稳定，重建和编辑质量下降

Method: 提出Proximal-Mean Inversion (PMI)：训练自由的梯度校正方法，通过将速度场引导到历史速度的运行平均值来稳定速度场，约束在理论推导的球形高斯分布内；引入mimic-CFG：轻量级速度校正方案，在当前速度与其在历史平均值上的投影之间插值，平衡编辑效果和结构一致性

Result: 在PIE-Bench上的大量实验表明，该方法显著提高了反转稳定性、图像重建质量和编辑保真度，同时减少了所需的神经函数评估次数，在PIE-Bench上实现了最先进的性能

Conclusion: 提出的PMI和mimic-CFG方法有效解决了Rectified-Flow模型中反转的累积误差问题，在提升重建和编辑质量的同时增强了效率和理论严谨性
...
TL;DR: 提出了DynaHOI-Gym平台和DynaHOI-10M基准，用于评估动态手-物交互任务，包含参数化运动生成器和基于滚动的评估指标，并提出了ObAct基线方法。
...
Method: 1. 开发DynaHOI-Gym统一在线闭环平台，包含参数化运动生成器和基于滚动的评估指标；2. 构建DynaHOI-10M大规模基准，包含1000万帧和18万条手部捕捉轨迹，目标运动分为8个主要类别和22个细分子类别；3. 提出ObAct基线方法，通过时空注意力将短期观察与当前帧整合来预测动作。

Result: ObAct基线方法在位置成功率上实现了8.1%的提升，验证了DynaHOI-Gym平台的有效性，为动态手-物交互研究提供了标准化评估框架。

Conclusion: DynaHOI-Gym平台和DynaHOI-10M基准填补了动态手-物交互评估的空白，为未来研究提供了标准化测试环境和评估指标，推动了该领域的发展。
...
TL;DR: 提出基于隐式神经表示和去噪扩散模型的LGE图像与分割掩码合成框架，用于缓解心肌瘢痕分割任务中标注数据稀缺问题
...
Method: 首先训练隐式神经表示（INRs）捕捉LGE数据及相应心肌和纤维化掩码的连续空间表示；将这些INRs压缩为紧凑的潜在嵌入；在潜在空间上使用扩散模型生成新表示；解码为具有解剖一致分割掩码的合成LGE图像
...
TL;DR: 评估视觉语言模型在法语PDF转Markdown任务上的表现，针对复杂文档布局提出专门基准测试方法


<details>
  <summary>Details</summary>
Motivation: 文档解析是RAG管道的关键步骤，现有基准测试多关注英语或中文，且过度惩罚格式和线性化选择，而法语文档特别是复杂布局的解析缺乏专门评估

Method: 从60,000份文档中通过模型分歧采样选择困难页面构建法语基准，使用单元测试式检查针对具体失败模式（文本存在、阅读顺序、局部表格约束），结合类别特定归一化来忽略仅呈现差异

Result: 评估15个模型显示，最强专有模型在手写体和表单处理上具有显著更高的鲁棒性，而多个开源权重系统在标准印刷布局上仍保持竞争力

Conclusion: 需要专门针对法语复杂文档的基准测试方法，专有模型在困难任务上表现更优，但开源模型在常规布局上仍有竞争力，为文档解析评估提供了更实用的框架
...
TL;DR: 提出一个基于贝叶斯深度学习的概率优化框架，通过CUB-Loss和DTS策略解决医学影像AI模型校准问题，提升预测可靠性和临床可接受性。


<details>
  <summary>Details</summary>
Motivation: 医学影像AI决策支持系统中，模型可靠性（不确定性量化）与预测准确性同等重要。现有深度学习模型常存在校准问题（错误预测过度自信），阻碍临床接受。需要让模型的不确定性估计与预测正确性相关，使临床医生能识别不可靠输出进行复核。

Method: 提出通用概率优化框架：1) 训练时引入置信度-不确定性边界损失（CUB-Loss），惩罚高置信度错误和低置信度正确预测，强制对齐预测正确性与不确定性估计；2) 后处理采用双温度缩放（DTS）策略，细化后验分布，提升直观可解释性。

Result: 在三个医学影像任务验证：肺炎自动筛查、糖尿病视网膜病变检测、皮肤病变识别。实验表明：1) 在不同模态上实现一致的校准改进；2) 在数据稀缺场景下保持鲁棒性能；3) 在严重不平衡数据集上仍有效。

Conclusion: 该框架通过CUB-Loss和DTS策略有效解决医学影像AI模型的校准问题，提升不确定性估计与预测正确性的对齐，增强临床可接受性，具有实际临床部署潜力。
...
Method: 提出SCoT即插即用框架：1) 使用交错文本-坐标指令格式训练扩散模型增强布局感知能力；2) 利用先进MLLMs作为规划器生成全面布局计划，将其空间规划能力直接转移到生成过程。
...
TL;DR: 本研究探索使用生成式AI方法改进新生儿复苏视频中的活动识别，通过结合本地视觉语言模型和大型语言模型，在模拟数据集上达到0.91的F1分数，优于传统监督方法。


<details>
  <summary>Details</summary>
Motivation: 新生儿复苏的准确记录对质量改进和临床指南遵循至关重要，但实践中仍未被充分利用。现有3D-CNN和Vision Transformer方法在识别细粒度活动方面存在挑战，因此需要探索生成式AI方法来改进活动识别性能。

Method: 研究探索使用本地视觉语言模型结合大型语言模型，并与监督式TimeSFormer基线比较。在包含13.26小时新生儿复苏视频的模拟数据集上，评估了多种零样本VLM策略和带有分类头的微调VLM，包括使用低秩适应技术。

Result: 小型本地VLM存在幻觉问题，但使用LoRA微调后，F1分数达到0.91，显著优于TimeSFormer的0.70结果。

Conclusion: 生成式AI方法特别是微调的视觉语言模型，在新生儿复苏视频活动识别方面具有显著优势，能够有效改进医疗质量监控和临床指南遵循。
...
Result: 在标准基准测试中超越现有基于扩散的新视角合成方法，在重建保真度和修复质量上均有显著提升，并能从稀疏、无位姿的图像集合中实现鲁棒合成

Conclusion: 利用外部表征作为条件引导扩散模型进行新视角合成是有效的，ReNoV框架通过表征投影模块显著提升了生成视角的几何一致性和合成质量
...
Method: 开发基于数字微镜器件(DMD)的HDR成像系统，包含两个协同子系统：DMD光学调制单元和自适应计算成像管道，实现自主区域分割和自适应曝光控制

Result: 系统实现127dB可测量动态范围，有效消除高眩光下的饱和伪影，实验显示应变误差减少78%，DIC定位精度提高，在极端强度变化下表现可靠

Conclusion: DMD基系统提供高保真自适应HDR成像，克服传统传感器关键限制，在传统方法不足的高眩光环境中具有光学计量和应力分析的强大潜力
...
<details>
  <summary>Details</summary>
Motivation: 数字产业对高质量、多样化的模块化3D资产需求增长，特别是用户生成内容领域。现有方法难以生成符合约束设计参数的模块化资产，需要更有效的自动化生成方案。

Method: 采用自回归Transformer架构，从在线平台收集真实模块化资产数据。创新性地借鉴语言模型的模块序列化和解码技术，通过自回归建模提升资产生成质量。

Result: 初步结果表明AssetFormer在专业开发和用户生成内容场景中能有效简化资产创建流程，生成质量得到提升。
...
<details>
  <summary>Details</summary>
Motivation: 图像到海报生成需要同时处理局部调整和高级设计理解，涉及文本、布局、风格和视觉元素的生成，同时保持语义保真度和美学一致性。现有方法难以同时满足实体保持编辑和概念驱动创建的双重需求

Method: 提出PosterOmni框架，通过高效的数据-蒸馏-奖励管道整合局部编辑和全局创建：(1)构建覆盖六种任务类型的多场景图像到海报数据集；(2)在局部和全局专家之间进行知识蒸馏用于监督微调；(3)应用统一的PosterOmni奖励反馈来联合对齐视觉实体保持和美学偏好

Result: PosterOmni显著提升了参考依从性、全局构图质量和美学和谐性，超越了所有开源基线，甚至优于多个专有系统。同时建立了PosterOmni-Bench统一评估基准

Conclusion: PosterOmni成功解决了图像到海报生成中实体保持编辑与概念驱动创建的双重挑战，通过统一框架实现了多任务图像到海报生成，为艺术海报创作提供了有效的解决方案
...
TL;DR: FAIL提出了一种基于流匹配的对抗模仿学习框架，通过对抗训练最小化策略与专家分布之间的差异，无需显式奖励或成对比较，实现了高效的模型后训练。
...
Result: 仅使用13,000个Nano Banana pro演示微调FLUX模型，在提示跟随和美学基准上取得竞争性表现。框架有效泛化到离散图像和视频生成，并可作为鲁棒正则化器缓解基于奖励的优化中的奖励攻击问题。

Conclusion: FAIL提供了一种无需显式奖励或成对比较的有效后训练方法，在多种生成任务中表现优异，并能缓解奖励攻击问题，具有广泛的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前主流的基于多视图扩散的3D纹理生成方法存在视图不一致性问题。现有表示方法要么依赖UV贴图（在展开时会产生扭曲），要么使用点云方法（纹理保真度与几何密度紧密耦合，限制了高分辨率纹理生成）。需要一种新的表示方法来克服这些限制。

Method: 提出TexSpot框架，核心是Texlet表示方法：将点云3D纹理的几何表达能力与UV表示的紧凑性相结合。每个Texlet潜在向量通过2D编码器编码局部纹理块，再通过3D编码器聚合全局形状上下文。使用级联的3D到2D解码器重建高质量纹理块，实现Texlet空间学习。基于此表示训练条件扩散变换器，用于增强多视图扩散方法生成的纹理。

Result: 大量实验表明，TexSpot在视觉保真度、几何一致性和鲁棒性方面显著优于现有的最先进3D纹理生成和增强方法。

Conclusion: TexSpot通过创新的Texlet表示方法有效解决了3D纹理生成中的关键挑战，为高质量3D纹理生成提供了新的解决方案。
...
TL;DR: DreamID-Omni是一个统一的可控人像音频-视频生成框架，通过对称条件扩散Transformer、双级解耦策略和多任务渐进训练方案，解决了多人物身份与音色绑定失败问题，在参考音频-视频生成、视频编辑和音频驱动视频动画等任务上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型在联合音频-视频生成方面取得进展，但将人像相关任务（参考音频-视频生成、视频编辑、音频驱动视频动画）视为孤立目标。此外，在单一框架内实现对多个人物身份和音色的精确解耦控制仍是一个开放挑战。

Method: 1. 对称条件扩散Transformer：通过对称条件注入方案整合异构条件信号；2. 双级解耦策略：信号级采用同步RoPE确保刚性注意力空间绑定，语义级采用结构化描述建立显式属性-主体映射；3. 多任务渐进训练方案：利用弱约束生成先验正则化强约束任务，防止过拟合并协调不同目标。

Result: 大量实验表明，DreamID-Omni在视频、音频和音视频一致性方面实现了全面的最先进性能，甚至超越了领先的专有商业模型。

Conclusion: DreamID-Omni提出了一个统一的框架，解决了人像音频-视频生成中的多任务统一和身份-音色解耦控制问题，通过创新的架构设计和训练策略实现了卓越性能，将促进学术研究与商业级应用之间的桥梁。
...
<details>
  <summary>Details</summary>
Motivation: 当前统一多模态模型通常参数规模巨大（>10B），导致训练成本高昂且部署困难。需要开发轻量级但性能强大的模型来降低研究门槛。
...
TL;DR: UniDFlow是一个统一的离散流匹配框架，通过任务特定低秩适配器解耦理解和生成，避免目标干扰和表示纠缠，并通过基于参考的多模态偏好对齐优化相同条件下的相对结果，无需大规模重训练即可提升忠实度和可控性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态系统在理解、生成和编辑任务中存在目标干扰和表示纠缠问题，需要统一的框架来同时处理这些任务，同时保持高忠实度和可控性。

Method: 1. 使用任务特定低秩适配器解耦理解和生成；2. 提出基于参考的多模态偏好对齐方法，在相同条件下优化相对结果；3. 采用离散流匹配框架统一多模态任务。

Result: 在八个基准测试中达到最先进性能，并在零样本设置下表现出强大的泛化能力，包括修复、上下文图像生成、基于参考的编辑和组合生成等任务。

Conclusion: UniDFlow通过统一的离散流匹配框架有效解决了多模态理解、生成和编辑任务中的目标干扰和表示纠缠问题，实现了高性能和强泛化能力，为多模态系统提供了统一的解决方案。
...
TL;DR: Monarch-RT：一种用于实时视频生成的Monarch矩阵结构化注意力参数化方法，通过因子化注意力解决3D自注意力的二次计算成本问题，在保持高质量的同时实现高达95%的注意力稀疏性。


<details>
  <summary>Details</summary>
Motivation: 实时视频生成中的扩散变换器受限于3D自注意力的二次计算成本，特别是在少步和自回归的实时场景中。现有稀疏注意力近似方法在这种设置下失效，因为视频注意力具有复杂的时空结构，包含周期性模式、动态稀疏语义对应和密集混合。

Method: 提出Monarch-RT，一种基于Monarch矩阵的结构化注意力参数化方法。通过适当对齐的块结构和扩展的平铺Monarch参数化，在保持计算效率的同时实现高表达能力。通过微调和自定义Triton内核克服参数化开销。

Result: Monarch-RT在双向模型设计的现有稀疏基线中表现优异，在应用于最先进的Self-Forcing模型时，实现高达95%的注意力稀疏性且无质量损失。优化实现相比FlashAttention-2/3/4内核在多种GPU上获得1.4-11.8倍加速，首次在单RTX 5090上实现16 FPS的真正实时视频生成。
...
Method: 提出UniT框架，包含三个核心组件：1）智能数据合成生成多轮推理轨迹；2）统一模型训练学习推理、验证和精炼能力；3）灵活测试时推理支持链式思维推理。框架结合认知行为包括验证、子目标分解和内容记忆。

Result: 关键发现：1）在短推理轨迹上训练的统一模型能泛化到测试时的长推理链；2）顺序链式思维推理比并行采样更具可扩展性和计算效率；3）生成和编辑轨迹训练提升分布外视觉推理能力。

Conclusion: 多模态测试时扩展是推进统一模型生成和理解能力的有效范式，UniT框架通过迭代推理、验证和精炼机制，为复杂多模态任务提供了新的解决方案。
...
TL;DR: 提出渐进式语义错觉任务和Stroke of Surprise生成框架，通过矢量笔划优化实现单一草图在不同绘制阶段的语义转换，解决双重约束问题


<details>
  <summary>Details</summary>
Motivation: 传统视觉错觉依赖空间操作，本文旨在将视觉字谜从空间维度扩展到时间维度，创建在绘制过程中发生语义转变的矢量草图

Method: 提出序列感知联合优化框架，采用双分支分数蒸馏采样机制，动态调整前缀笔划以发现两个目标的共同结构子空间，并引入覆盖损失确保空间互补性

Result: 方法在可识别性和错觉强度方面显著优于现有基线，成功实现了从空间到时间维度的视觉字谜扩展

Conclusion: 渐进式语义错觉为视觉错觉研究开辟了新方向，Stroke of Surprise框架有效解决了双重约束问题，展示了时间维度上的语义转换能力
...
TL;DR: VRFT-Aug：针对医学图像领域的视觉强化微调框架，通过感知增强和推理优化策略提升模型性能


<details>
  <summary>Details</summary>
Motivation: 当前基于规则的强化微调方法在跨模态视觉领域应用不足，特别是在医学图像领域需要同时具备强大的视觉感知和结构化推理能力。现有方法在这一高风险的医学应用领域存在局限性。

Method: VRFT-Aug框架包含：1) 先验知识注入；2) 感知驱动的策略优化；3) 医学信息指导的奖励塑造；4) 行为模仿。这些策略共同稳定和改善强化微调过程。

Result: 在多个医学数据集上的实验表明，该方法持续优于标准监督微调和强化微调基线，并为其他医学图像任务提供了可推广的训练启发式方法。

Conclusion: 该工作为开发可靠、具备推理能力的医学应用模型提供了可操作的指导和新的灵感，推动了高风险医学领域模型的发展。
...
TL;DR: HybridRAG：一种新颖实用的RAG框架，通过预生成QA知识库和分层文本块处理，实现更准确、更快速的聊天机器人响应


<details>
  <summary>Details</summary>
Motivation: 现有RAG研究通常假设结构化文本源并在查询时执行检索和生成，这限制了其在现实世界聊天机器人场景中的适用性。需要处理包含复杂布局（文本、表格、图形）的原始非结构化PDF文档，并在有限计算资源下服务大量用户。

Method: 1. 通过OCR和布局分析摄取包含复杂布局的原始非结构化PDF文档，将其转换为分层文本块；2. 使用LLM从组织好的块中预生成合理的问答知识库；3. 查询时，用户问题首先与QA库匹配以获取即时答案，仅当没有合适的QA匹配时才回退到实时响应生成。

Result: 在OHRBench上的实验表明，与标准RAG基线相比，HybridRAG提供了更高的答案质量和更低的延迟。

Conclusion: HybridRAG可能是处理大量非结构化文档和大量用户的实际聊天机器人应用的实用解决方案，特别是在有限计算资源下。
...
TL;DR: 该研究从动态视角分析LLM中的检索头，发现其在自回归生成过程中具有时间动态性、不可替代性，且隐藏状态包含预测未来检索模式的信号，揭示了内部规划机制。


<details>
  <summary>Details</summary>
Motivation: 先前研究主要依赖跨数据集的静态统计来识别检索头，忽视了自回归生成的细粒度时间动态性。本文旨在从动态视角研究检索头的行为模式。

Method: 通过广泛分析建立三个核心主张：动态性、不可替代性和相关性。在Needle-in-a-Haystack任务和多跳QA任务上验证发现，并在动态检索增强生成框架中量化动态与静态检索头的效用差异。

Result: 研究发现：(1)检索头随时间步动态变化；(2)动态检索头在特定时间步具有特异性，无法被静态检索头有效替代；(3)模型隐藏状态编码了预测未来检索头模式的信号，表明存在内部规划机制。

Conclusion: 该研究为LLM的内部机制提供了新见解，揭示了检索头的动态特性及其在自回归生成中的重要作用，挑战了仅基于静态统计的分析方法。
...
<details>
  <summary>Details</summary>
Motivation: 等离子体物理研究文章包含高度复杂和上下文丰富的内容，需要从中提取关键实体以支持高级搜索等应用。现有NER方法在该专业领域的应用面临挑战，需要专门针对等离子体物理文本的实体识别解决方案。

Method: 1) 为嵌套NER任务标注包含16个实体类别的等离子体物理语料库；2) 采用实体特定模型专业化方法，训练独立的BERT-CRF模型识别单个实体类型；3) 集成超参数优化过程，系统性地微调模型参数以提升性能。

Result: 开发了一个专门针对等离子体物理领域的嵌套NER系统，能够有效识别该科学领域的专业实体。通过模型专业化和超参数优化，提升了在复杂科学文本中的实体提取性能。

Conclusion: 该工作推进了等离子体物理领域的实体识别技术，为研究人员导航和分析科学文献提供了基础支持，展示了NER在专业科学领域的应用潜力。
...
TL;DR: RECOM基准测试发现LLMs在回答近期Reddit问题时存在语义-词汇悖论：模型与参考答案的余弦相似度超过99%，但BLEU-1重叠率低于8%，表明模型通过广泛改写而非词汇复制来保持语义，挑战了词汇指标在评估抽象生成中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在处理近期信息时与人类视角的对齐程度，现有研究对此关注不足。需要评估模型在开放域问答任务中，特别是针对时效性较强内容的表现。

Method: 构建RECOM基准数据集（15,000个2025年9月的Reddit问题及社区参考答案），评估四个开源LLM（Llama3.1-8B、Mistral-7B、Gemma-2-9B、GPT-OSS-20B）。使用词汇指标（BLEU、ROUGE）、语义相似度（BERTScore、MoverScore、余弦相似度）和逻辑推理（NLI）进行多维度评估。

Result: 发现语义-词汇悖论：所有模型与参考答案的余弦相似度超过99%，但BLEU-1重叠率低于8%，差距达90+个百分点。MoverScore（51-53%）处于中间位置，反映语义对齐的最优传输成本。模型规模不预测性能：Mistral-7B（7B参数）在所有指标上优于GPT-OSS-20B（20B参数）。NLI分析显示矛盾率低于7%，模型很少生成与人类共识直接冲突的内容。

Conclusion: 词汇指标在评估抽象生成时不可靠，需要多维评估框架来捕捉超越表面文本匹配的语义保真度。模型通过广泛改写而非词汇复制来保持语义，这对LLM评估方法提出了重要挑战。
...
<details>
  <summary>Details</summary>
Motivation: 虽然参数高效微调方法被广泛用于适应下游任务并被认为能提高事实正确性，但其如何影响幻觉行为（特别是在QA数据集上）仍缺乏足够理解

Method: 系统研究PEFT对幻觉检测的影响：使用三个开源LLM骨干和三个事实寻求QA基准；对每个模型使用七种无监督幻觉检测方法（基于语义一致性、置信度和熵的检测器）；使用线性探测和表示诊断进行进一步分析

Result: PEFT持续增强幻觉检测能力，显著提高多种幻觉检测器的AUROC；PEFT方法主要通过重塑不确定性编码和表达方式，而不是向模型注入新的事实知识

Conclusion: PEFT方法能有效改善大语言模型的幻觉检测性能，主要通过改变模型内部不确定性表示机制来实现
...
TL;DR: 提出FalseCite数据集来系统研究LLM的幻觉现象，特别是由误导性引用引发的幻觉，并在多个模型上验证了其有效性


<details>
  <summary>Details</summary>
Motivation: LLM经常产生幻觉，生成无意义或虚假信息，这在医学、法律等敏感领域尤其有害。需要系统研究这一现象，特别是由误导性引用引发的幻觉

Method: 引入FalseCite数据集，专门捕捉和基准测试由误导或伪造引用引发的幻觉响应。在GPT-4o-mini、Falcon-7B和Mistral 7-B等模型上运行该数据集，并分析幻觉模型的内部状态，可视化并聚类隐藏状态向量

Result: 观察到带有欺骗性引用的虚假声明显著增加了幻觉活动，特别是在GPT-4o-mini中。分析发现无论是否产生幻觉，隐藏状态向量都倾向于形成独特的"角状"形状

Conclusion: FalseCite数据集为未来LLM研究中评估和缓解幻觉现象提供了重要基础，展示了其在系统研究LLM幻觉方面的潜力
...
TL;DR: 本文提出使用组合融合分析(CFA)结合多个AI模型来增强文本分类，特别是针对联合国可持续发展目标(SDGs)的分类任务，通过生成式AI创建合成数据并融合多个模型，性能达到96.73%，优于最佳单模型。


<details>
  <summary>Details</summary>
Motivation: 在文本分类中，当类别难以区分或相互关联时，传统NLP技术面临挑战。针对联合国可持续发展目标(SDGs)的文本分类尤其需要精确性，而现有方法在类别区分和相互关系处理上存在局限。社会分析领域依赖文本数据，需要更有效的分类方法来支持政策制定和决策。

Method: 采用组合融合分析(CFA)系统融合范式，结合秩-分特征(RSC)函数和认知多样性(CD)概念。首先使用生成式AI模型生成合成数据用于模型训练，然后应用CFA技术融合一组相对优秀且相互多样化的分类模型。将CFA结果与人类领域专家的分类进行比较。

Result: CFA技术在SDGs文本分类任务中达到96.73%的性能表现，优于最佳单模型。通过与人类专家结果的比较，证明多模型融合不仅能补充人类专家判断，还能相互增强，实现更好的分类效果。

Conclusion: 通过组合融合分析(CFA)结合多个ML/AI模型的智能，并整合人类专家输入，可以显著提升文本分类性能，特别是在复杂类别如可持续发展目标的分类任务中。这种方法展示了模型融合与人类专业知识互补增强的潜力。
...
TL;DR: 研究发现Transformer隐藏状态中方向（角度）和幅度（范数）具有不同的功能角色：角度扰动对语言建模损失影响更大，而幅度扰动对句法处理影响更大，这种分离与LayerNorm架构相关


<details>
  <summary>Details</summary>
Motivation: 探究Transformer隐藏状态中方向（向量角度）和幅度（向量范数）是否具有不同的功能角色，理解高维表示空间中这两个基本几何属性的计算意义

Method: 使用Pythia系列模型，开发L2匹配扰动分析方法（确保角度和幅度扰动具有相同的欧几里得位移），通过因果干预（注意力修复和LayerNorm修复）追踪损伤传播路径

Result: 发现显著交叉分离：角度扰动对语言建模损失造成更大损害（高达42.9倍），而幅度扰动对句法处理（主谓一致）造成更大损害（20.4% vs. 1.6%准确率下降）；角度损伤主要通过注意力路径传播（28.4%损失恢复），幅度损伤部分通过LayerNorm路径传播（29.9%恢复）；这种模式在Pythia架构家族中跨尺度复制，但在RMSNorm架构中表现不同

Conclusion: 方向（角度）和幅度（范数）在LayerNorm架构中支持部分不同的计算角色：方向优先影响注意力路由，而幅度调制处理强度以进行细粒度句法判断；研究结果细化了线性表示假设，对模型编辑和可解释性研究具有重要意义
...
TL;DR: 研究评估Gemini 2.5 Flash和Pro TTS模型在五种印度语言中生成法庭演讲的表现，发现模型在程序性信息传递上表现良好，但在情感表达和说服力方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 随着LLM发展，TTS技术从基本可理解性转向上下文感知的表达性合成。在法律领域，合成语音需要传达权威性和特定专业形象，这在印度多语言环境中尤为复杂，需要研究多语言TTS在法律倡导中的实际表现。

Method: 提出提示框架，利用Gemini 2.5原生支持5种语言及其上下文感知节奏控制的能力，生成不同的律师角色。在泰米尔语、泰卢固语、孟加拉语、印地语和古吉拉特语五种印度语言中评估模型表现。

Result: 模型表现出"单调权威性"，在程序性信息传递方面表现出色，但在动态声音调制和情感深度方面存在困难。孟加拉语和古吉拉特语表现下降，突显了语音学方面的挑战。多语言TTS已准备好处理程序性法律任务，但难以复制人类法律话语的说服艺术。

Conclusion: 多语言TTS已具备处理程序性法律任务的能力，但要复制人类律师的说服艺术仍面临挑战，特别是在情感表达和动态声音调制方面。孟加拉语和古吉拉特语的表现问题指出了未来改进的语音学前沿。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动回复生成方法未能充分利用作者的专业知识和意图。作者在回复评审意见时拥有领域专业知识、作者独有信息、修订策略等具体形式的专业知识和意图，需要NLP工具整合这些信号来支持有效的同行评审回复写作。

Method: 1. 将作者回复生成重构为作者参与循环任务；2. 提出REspGen生成框架，整合显式作者输入、多属性控制和评估引导优化；3. 开发REspEval评估套件，包含20+个指标，涵盖输入利用、可控性、回复质量和话语分析；4. 构建Re³Align数据集，首个大规模对齐的评审-回复-修订三元组数据集。

Result: 实验使用最先进的LLMs显示：作者输入和评估引导优化带来显著效益；输入设计对回复质量有重要影响；可控性和质量之间存在权衡关系。数据集、生成和评估工具已公开。
...
TL;DR: 该论文量化了预训练多语言模型中不同书写系统在分词处理上的不平等现象，发现高碎片化正字法导致3.4倍分词数量增加、16.5倍推理速度下降和显著信息成本上升


<details>
  <summary>Details</summary>
Motivation: 预训练多语言模型通常被认为是与书写系统无关的，但其分词器可能对不同书写系统施加系统性成本。研究者希望量化这种"脚本税"，即相同语言内容但不同正字法变体在模型处理中的不平等现象

Method: 通过比较具有相同语言内容但不同正字法变体的文本，使用mBERT和XLM-R模型进行分析。采用比特每字符（BPC）指标避免子词碎片化带来的"NLL悖论"，并进行往返转换检查（CER_rt=0.31）以区分正字法条件处理与映射噪声

Result: 高碎片化正字法导致：1）分词数量增加约3.4倍（6.73-6.85 vs. 2.10-2.35 tokens/word）；2）推理速度下降16.5倍（0.23 vs. 3.8 sentences/second）；3）信息成本显著增加：mBERT增加19.7%（8.06→9.65），XLM-R增加47.1%（12.19→17.94）

Conclusion: 分词是多语言NLP中不平等现象的关键来源，研究结果强调了开发脚本感知的分词和预训练方法的重要性，以减少不同书写系统之间的处理差异
...
TL;DR: 本文综述了Transformer在离散推理任务中的理论局限性，从电路复杂度、逼近理论和通信复杂度三个理论视角分析其结构性和计算性障碍。


<details>
  <summary>Details</summary>
Motivation: Transformer已成为序列建模的基础架构，在自然语言处理、视觉等领域取得了最先进的性能，但其在算术、逻辑推理、算法组合等离散推理任务中的理论局限性仍是一个关键开放问题，需要系统性的理论分析。

Method: 从三个理论视角进行综合分析：1) 电路复杂度分析Transformer的结构限制；2) 逼近理论研究其逼近离散函数的能力；3) 通信复杂度分析token间通信的瓶颈。通过连接这些理论框架，提供统一的理论解释。

Result: 揭示了Transformer在实现精确离散算法时面临的主要挑战：深度约束、难以逼近不连续性、token间通信瓶颈。这些理论限制解释了为什么Transformer擅长模式匹配和插值，但在精确离散计算方面存在困难。

Conclusion: 当前Transformer架构在离散推理任务中存在根本性理论限制，需要新的模型设计来克服这些障碍。文章为理解这些局限性提供了统一的理论框架，并指出了克服这些基础限制的潜在研究方向。
...
TL;DR: LLMs作为时间推理器，通过检索增强提示整合时空、行为历史和人物背景，在低数据环境下预测人类日常活动及其持续时间，实现数据效率与预测精度的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的智能体模型在低数据环境下表现不佳，限制了其在智能家居、人机协作等实际应用中的实用性。需要探索预训练大语言模型是否能够利用广泛的人类知识，从紧凑的上下文线索中推理日常活动。

Method: 采用检索增强提示策略，整合四种上下文来源：时间、空间、行为历史和人物背景。在CASAS Aruba智能家居数据集上评估两个互补任务：带持续时间估计的下一个活动预测，以及多步日常序列生成。通过不同数量的few-shot示例分析上下文监督的效果。

Result: 大语言模型展现出强大的人类行为时间理解能力：即使在零样本设置下也能产生连贯的日常活动预测，添加1-2个演示示例可进一步优化持续时间校准和分类准确性。超过几个示例后性能饱和，呈现收益递减。序列级评估确认了跨few-shot条件的一致时间对齐。

Conclusion: 预训练语言模型可作为有前景的时间推理器，捕捉重复性日常和上下文依赖的行为变化，从而增强基于智能体的行为模块，在低数据环境下实现数据效率与预测精度的平衡。
...
TL;DR: 该研究通过微调大语言模型进行阿尔茨海默病检测，分析其内部表示，并利用任务感知标记生成合成数据以解决标注数据不足的问题。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的可靠早期检测面临挑战，主要原因是标注数据有限。虽然大语言模型在跨领域迁移方面表现出色，但通过监督微调将其应用于AD领域的研究仍相对缺乏。

Method: 1. 微调LLM用于AD检测；2. 使用探测技术分析transformer层中的中间激活；3. 设计任务感知的特殊标记；4. 训练序列到序列模型作为数据合成工具，利用这些标记生成结构一致且具有诊断信息的合成样本。

Result: 研究发现，微调后特定词语和特殊标记的探测值发生显著变化，表明这些元素在模型改进的检测性能中扮演关键角色。基于此洞察，生成了合成数据并进行内在评估和下游训练管道整合评估。

Conclusion: 通过分析LLM内部表示并利用任务感知标记生成合成数据，为AD检测提供了一种解决标注数据不足问题的新方法，展示了LLM在医疗领域应用的潜力。
...
TL;DR: 该论文是一篇关于提示工程在自然语言生成任务中的综述，系统梳理了提示工程方法、分类、选择框架，并提出了连接设计、优化和评估的框架。


<details>
  <summary>Details</summary>
Motivation: 尽管提示工程已成为提升大型语言模型性能的重要技术，但在自然语言生成领域仍缺乏系统性的框架和统一理解，需要填补这一空白。

Method: 通过文献综述方法，分析近期提示工程的发展，提出提示范式分类法、基于多种因素的提示选择决策框架，以及连接设计、优化和评估的综合性框架。

Result: 建立了提示工程的分类体系，提供了实用的选择框架，识别了新兴趋势和挑战，提出了支持更可控和可泛化自然语言生成的系统性框架。

Conclusion: 提示工程作为输入级控制机制，与微调和解码方法互补，对自然语言生成任务具有重要影响，需要系统性的设计、优化和评估框架来推动该领域发展。
...
Method: 采用文献综述方法，系统分析应用于LLM对齐的机制可解释性技术，包括电路发现、特征可视化、激活引导和因果干预等方法，并探讨这些见解如何影响RLHF、宪法AI和可扩展监督等对齐策略。

Result: 识别了机制可解释性在LLM对齐中的关键挑战，包括叠加假设、神经元的多元语义性以及大规模模型中涌现行为的解释困难。总结了现有技术的应用现状和局限性。

Conclusion: 提出了未来研究方向，包括自动化可解释性、电路的跨模型泛化以及可扩展到前沿模型的基于可解释性的对齐技术开发，强调机制可解释性对于实现安全可靠的AI系统的重要性。
...
TL;DR: 该论文全面综述了大型语言模型中的代码混合与代码转换研究，提出了统一分类法，总结了构建、适应和评估CSW能力LLMs的实用指南，分析了当前评估实践的问题，并讨论了新兴的安全挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言建模取得进展，但大型语言模型在混合语言环境中仍面临挑战，表现为语法性、事实性和安全性行为的系统性退化。需要系统研究代码混合与代码转换现象，为构建更好的CSW能力模型提供指导。

Method: 1. 提出统一分类法，从数据、建模和评估三个维度组织现有研究；2. 总结构建CSW能力LLMs的实用指南；3. 回顾从CSW定制预训练、任务特定后训练到提示策略和上下文学习的建模方法；4. 分析当前评估实践，识别不稳定性和可重复性有限的问题；5. 编目现有基准测试，批判性检查其语言覆盖范围和英语中心偏见。

Result: 1. 建立了CSW研究的系统分类框架；2. 提供了构建CSW能力LLMs的实用指南；3. 识别了当前评估实践中的主要问题，包括不稳定性和英语中心偏见；4. 揭示了代码混合可能被用作绕过模型安全机制的途径这一新兴安全担忧。

Conclusion: 该论文为CSW研究提供了全面综述和系统框架，强调了当前LLMs在混合语言环境中的局限性，提出了改进方向，并指出了将代码混合用作绕过安全机制的新兴风险，为未来研究提供了重要参考。
...
<details>
  <summary>Details</summary>
Motivation: 现有记忆系统虽然能支持长时人机交互，但往往会破坏交互会话中的逻辑和时间关系，导致记忆碎片化和推理性能下降

Method: 提出MetaMem框架，通过自演化的元记忆迭代提炼可迁移的知识利用经验，通过自我反思推理过程并执行动作更新元记忆状态

Result: 广泛实验表明MetaMem显著优于强基线方法，性能提升超过3.6%

Conclusion: MetaMem通过元记忆增强记忆系统，教导LLM有效利用记忆知识，解决了记忆碎片化问题并提升了推理性能
...
TL;DR: 提出AskBench基准测试和RLVR方法，评估和改进LLMs在信息不足或错误前提下的澄清能力


<details>
  <summary>Details</summary>
Motivation: LLMs经常在提示信息不足或包含误导信息时仍给出回答，导致幻觉或强化错误观念，需要评估和改进其澄清能力

Method: 提出AskBench交互式基准，将标准QA对转换为多轮交互；设计RLVR方法（基于验证器的强化学习），使用结构化评分标准鼓励针对性澄清

Result: 实验显示在准确性、评分标准遵循度和交互效率方面均有持续改进，对未见领域有强泛化能力

Conclusion: AskBench基准和RLVR方法有效提升了LLMs的澄清能力，使其能在信息不足时主动询问，在错误前提时识别并纠正
...
TL;DR: 提出NLDD指标评估思维链解释的忠实性，发现推理存在70-85%的"推理视界"，超过此范围步骤对最终答案影响甚微


<details>
  <summary>Details</summary>
Motivation: 当前思维链解释被广泛用于解释语言模型解决复杂问题的过程，但尚不清楚这些逐步解释是否真实反映模型的实际推理过程，还是仅仅是事后合理化。需要一种方法来评估思维链解释的忠实性。

Method: 提出归一化对数差异衰减（NLDD）指标，通过破坏解释中的单个推理步骤并测量模型对其答案置信度的下降程度，来确定步骤是否真正重要。通过标准化这些测量，NLDD支持跨不同架构的严格模型比较。

Result: 在语法、逻辑和算术任务上测试三个模型家族，发现一致的"推理视界"（k*）在链长的70-85%处，超过此范围的推理标记对最终答案影响很小甚至为负。还发现模型可以编码正确的内部表示但完全无法完成任务。

Conclusion: 仅凭准确性无法揭示模型是否真正通过思维链进行推理。NLDD提供了一种方法来衡量思维链何时真正重要，表明需要更细致的评估指标来理解模型的推理过程。
...
TL;DR: 该论文提出了Mis-Align Bench基准，用于同时评估大语言模型在安全、价值和文化的三个维度上的错位问题，通过构建SAVACU数据集和系统评估现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型错位评估基准（如INSECURE CODE、VALUEACTIONLENS、CULTURALHERITAGE）仅针对单一维度（安全、价值或文化）进行评估，无法同时评估这三个必须共存的维度，这限制了模型在现实世界查询中的适用性评估。

Method: 1. 构建SAVACU数据集：从LLM-PROMPT-DATASET重新分类提示，使用Mistral-7B-Instruct-v0.3将其分为14个安全域、56个价值域和42个文化域，共382,424个样本；2. 使用Llama-3.1-8B-Instruct通过SimHash指纹避免重复来扩展低资源域；3. 通过两阶段拒绝采样将提示与错位和对齐的响应配对以确保质量；4. 对通用、微调和开源权重LLMs进行基准测试，系统评估三个维度下的错位情况。

Result: 实验表明，单一维度模型在覆盖率上可达97.6%，但在联合条件下假失败率超过50%，对齐分数较低（63%-66%），说明现有模型在同时满足安全、价值和文化维度方面存在显著不足。

Conclusion: Mis-Align Bench基准填补了现有错位评估的空白，能够同时评估大语言模型在安全、价值和文化的三个关键维度上的对齐情况，揭示了单一维度评估的局限性，为开发更全面的对齐模型提供了重要工具。
...
TL;DR: 该研究开发了一个基于心理学问卷的框架，通过情境判断测试评估LLM在社交情境中的行为倾向与人类偏好的对齐程度，发现LLM常表现出过度自信、偏离人类共识等问题。


<details>
  <summary>Details</summary>
Motivation: 随着LLM融入日常生活，理解其行为特征变得至关重要。研究旨在评估LLM在社交情境中表现出的行为倾向（行为倾向）与人类偏好的对齐程度，为LLM的社会行为评估提供系统框架。

Method: 将心理学自我报告问卷转化为情境判断测试，创建2,500个经过三位人类标注者验证的SJT，从550名参与者中收集每个SJT的偏好行为。在25个LLM上进行全面研究，比较LLM推荐行为与人类偏好的分布差异。

Result: 研究发现：1）在人类共识低的场景中，LLM对单一回应表现出过度自信；2）人类共识高时，小模型显著偏离共识，部分前沿模型在15-20%情况下未反映共识；3）LLM存在跨模型的行为模式，如在人类偏好克制的场景中鼓励情感表达；4）LLM的陈述价值观与实际行为存在显著差距。

Conclusion: LLM的行为倾向与人类偏好存在系统性偏差，需要更精细的评估方法来理解和改进LLM的社会行为对齐。研究框架为评估LLM的社会行为提供了有效工具，揭示了LLM价值观陈述与实际行为之间的差距。
...
TL;DR: 研究发现大语言模型的自我反思语言确实反映了内部计算状态，而非仅仅是编造。通过Pull方法识别出自我指涉处理的特异性激活方向，该方向与拒绝方向正交，位于模型深度6.25%处，并能因果影响内省输出。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型产生的内省语言是否真正反映了内部计算状态，还是仅仅是复杂的编造。需要验证自我反思语言与激活动态之间的对应关系是否具有特异性。

Method: 引入Pull方法学，通过格式工程引发扩展的自我检查。在Llama 3.1中识别区分自我指涉与描述性处理的激活空间方向。使用因果干预（steering）验证该方向对内省输出的影响。分析特定词汇（如"loop"、"shimmer"）与激活动态的对应关系。在Qwen 2.5-32B上进行独立验证。

Result: 发现自我指涉词汇与并发激活动态存在对应关系（r=0.44，p=0.002），且这种对应关系具有特异性。识别出的自我指涉方向与已知拒绝方向正交，位于模型深度6.25%处，能因果影响内省输出。在非自我指涉语境中，相同词汇没有激活对应关系。Qwen 2.5-32B独立发展出不同的内省词汇，追踪不同的激活指标，进一步证实了发现。

Conclusion: 在适当条件下，Transformer模型中的自我报告能够可靠地追踪内部计算状态。自我反思语言并非纯粹的编造，而是反映了模型的实际内部处理过程。
...
TL;DR: PPCV框架通过识别关键令牌并替换候选方案来提升大语言模型的推理性能，利用问题重述和一致性验证解决推理过程中的错误积累问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在复杂推理任务中性能下降，主要原因是幻觉和中间步骤错误积累。现有方法通过替换关键令牌来优化推理轨迹，但可靠识别和利用关键令牌仍具挑战性。

Method: 提出PPCV框架，包含两个阶段：1) 关键令牌识别阶段：生成初始推理路径，结合重述问题识别关键令牌；2) 一致性验证阶段：替换关键令牌生成新推理路径，通过并行推理过程的一致性验证确定最终答案。

Result: 在主流大语言模型和多个基准测试上的实验表明，PPCV相比基线方法显著提升了模型的推理性能。

Conclusion: PPCV框架通过系统性地识别和替换关键令牌，结合问题重述和一致性验证，有效提升了大语言模型在复杂推理任务中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生看似合理但错误的主张（幻觉），而现有的不确定性度量方法在模型自信地犯错时往往无法检测到这些错误。需要一种能够识别深层事实矛盾的无监督框架。

Method: 提出DiffuTruth框架，基于非平衡热力学原理，将事实真相视为生成流形上的稳定吸引子，幻觉则是不稳定的。方法包括：1）生成应力测试：用噪声污染声明，使用离散文本扩散模型重建；2）语义能量度量：使用NLI批评器测量原始声明与重建之间的语义分歧；3）混合校准：融合稳定性信号与判别性置信度。

Result: 在FEVER数据集上实现无监督AUROC 0.725的SOTA性能，比基线提高1.5%；在HOVER多跳数据集上零样本泛化性能优于基线超过4%，证明热力学真实性对分布偏移具有鲁棒性。

Conclusion: DiffuTruth通过热力学视角重新概念化事实核查，将事实视为生成流形上的稳定吸引子，提供了一种有效的无监督幻觉检测方法，在跨数据集泛化方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 机器翻译中的关键错误（事实扭曲、意图反转、偏见翻译）会损害多语言系统的可靠性、公平性和安全性，需要开发有效的错误检测方法来减少错误信息、误解和语言伤害的风险。

Method: 使用指令调优的大语言模型（LLMs）检测机器翻译中的关键错误，评估不同参数规模的模型，采用零样本、少样本和微调等适应策略，在公开数据集上进行实验。

Result: 模型缩放和适应策略带来一致的性能提升，指令调优LLMs在检测关键错误方面优于XLM-R和ModernBERT等编码器基线模型。
...
Result: LoopFormer在语言建模和推理基准测试中表现出稳健性能，即使在严格计算约束下也能良好工作，同时能优雅地随额外预算扩展。证明循环Transformer天生适合自适应语言建模

Conclusion: 循环Transformer具有自适应语言建模的内在适应性，为可控和预算感知的大型语言模型开辟了新路径
...
TL;DR: ADRD-Bench是首个针对阿尔茨海默病及相关痴呆症（ADRD）的LLM评估基准，包含临床知识问答和照护实践问答两部分，评估了33个先进LLM，发现虽然顶级模型准确率超过0.9，但推理质量和稳定性仍需改进。


<details>
  <summary>Details</summary>
Motivation: 现有评估基准对阿尔茨海默病及相关痴呆症（ADRD）的覆盖不足，缺乏实际照护情境的评估，需要专门的ADRD基准来全面评估LLM在该领域的知识和推理能力。

Method: 构建ADRD-Bench基准数据集，包含两部分：1) ADRD统一问答（1,352题），整合自七个现有医学基准；2) ADRD照护问答（149题），基于广泛使用的Aging Brain Care（ABC）项目。评估了33个最先进的LLM，包括开源通用模型、开源医学模型和闭源通用模型。

Result: 开源通用模型准确率0.63-0.93（均值0.78）；开源医学模型准确率0.48-0.93（均值0.82）；闭源通用模型准确率0.83-0.91（均值0.89）。顶级模型准确率超过0.9，但案例研究显示推理质量和稳定性不一致，限制了可靠性。

Conclusion: 虽然顶级LLM在ADRD-Bench上表现良好，但推理质量和稳定性问题表明需要领域特定的改进，以增强LLM基于日常照护数据的知识和推理能力。数据集已公开供进一步研究。
...
TL;DR: 研究发现语音-语言模型在音频与文本冲突时更偏向文本，这种文本主导现象源于模型推理过程中对文本表示的可访问性优势，而非音频信息质量差异。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解语音-语言模型在处理音频-文本冲突时的行为模式，特别是为何模型即使在音频信息更可靠的情况下仍然偏向文本，以及这种模态仲裁现象背后的机制。

Method: 使用ALME基准测试（包含57,602个受控音频-文本冲突刺激，涵盖8种语言），对比音频-文本冲突与文本-文本冲突下的模型行为。通过强制转录、文本标记为"故意损坏"、音频投影层微调、LoRA微调等干预实验，分析文本主导现象的成因。

Result: Gemini 2.0 Flash在音频-文本冲突下的文本主导率为16.6%，而文本-文本冲突下仅为1.6%。音频单独准确率（97.2%）高于级联准确率（93.9%），表明音频嵌入比文本转录保留更多信息。干预实验显示：强制转录增加文本主导，标记文本为"损坏"减少文本主导，仅微调音频投影层增加文本主导，而LoRA微调语言模型则显著减少文本主导。

Conclusion: 文本主导现象反映了模型推理过程中对竞争表示的可访问性不对称，而非信息内容差异。模态仲裁是标准语音基准测试未捕捉到的独立可靠性维度，这一发现对多模态模型设计和评估具有重要意义。
...
Method: 提出MuRGAt基准，要求模型在输入涵盖视频、音频等多种模态的情况下，生成带明确推理和精确引用的答案，每个引用需指定模态和时间片段。同时引入与人类判断强相关的自动评估框架。

Result: 评估发现，即使强大的多模态大语言模型也经常在正确推理的情况下产生幻觉引用。观察到关键权衡：增加推理深度或强制结构化归因通常会降低准确性，揭示了内部推理与可验证归因之间的显著差距。

Conclusion: MuRGAt基准揭示了当前多模态大语言模型在复杂推理场景中实现可验证归因的挑战，强调了需要改进模型在保持准确性的同时提供可靠归因的能力。
...
TL;DR: SPES是一种内存高效的分散式预训练框架，用于混合专家(MoE)大语言模型，通过在节点上仅训练专家子集来降低内存需求，实现使用普通GPU在互联网连接下训练MoE LLM。


<details>
  <summary>Details</summary>
Motivation: 传统LLM预训练需要集中式集群和数千个高内存GPU，现有分散式训练方法虽减少通信开销但仍需在每个节点上训练完整模型，受GPU内存限制。需要一种能显著降低内存需求的分散式MoE LLM预训练方法。

Method: 提出SParse Expert Synchronization (SPES)框架：1) 每个节点仅训练专家子集，大幅降低内存占用；2) 节点更新本地专家并定期与其他节点同步，避免全参数传输；3) 引入专家合并预热策略，在训练早期交换知识以快速建立基础能力。

Result: 使用16个48GB GPU在互联网连接下训练2B参数MoE LLM，性能与集中训练LLM相当；进一步展示可扩展性：从头训练7B模型和从密集检查点上采样9B模型，均匹配先前集中式基线性能。

Conclusion: SPES是一种有效的内存高效分散式MoE LLM预训练框架，能在有限GPU资源下实现与集中训练相当的性能，为资源受限环境中的大规模模型训练提供了可行方案。
...
TL;DR: SIGHT框架通过自证据支持和信息增益驱动的多样化分支，解决多轮搜索中冗余和噪声问题，提升LLM的自主搜索推理能力


<details>
  <summary>Details</summary>
Motivation: 强化学习赋能大语言模型进行自主搜索回答复杂问题，但在多轮搜索场景中，搜索结果存在高冗余和低信噪比问题，导致智能体陷入"隧道视野"——早期噪声检索的强制解释引发不可逆错误累积

Method: 提出SIGHT框架：1) 通过自证据支持(SES)将搜索结果提炼为高保真证据；2) 计算信息增益分数识别关键状态；3) 基于信息增益指导动态提示干预(去重、反思、自适应分支)；4) 通过组相对策略优化整合SES和正确性奖励，内化鲁棒探索策略

Result: 在单跳和多跳问答基准测试中，SIGHT显著优于现有方法，特别是在复杂推理场景下，使用更少的搜索步骤取得更好效果

Conclusion: SIGHT框架通过自证据支持和信息增益驱动的多样化分支，有效解决了多轮搜索中的冗余和噪声问题，提升了LLM在复杂推理任务中的自主搜索能力，且无需外部验证器
...
TL;DR: PRIME是一个用于评估数学和工程领域过程-结果对齐验证的基准测试，包含2530个高难度STEM样本，旨在解决当前验证器忽略推导过程错误的问题。
...
Method: 通过一致性过滤流程从大学水平STEM问题中收集2530个高难度样本构建PRIME基准；提出基于PRIME选择验证器的过程感知RLVR训练范式。

Result: 当前验证器经常无法检测推导缺陷；基于PRIME选择验证器的过程感知RLVR训练显著优于仅基于结果的验证基线，在Qwen3-14B-Base模型上分别获得8.29%、9.12%和7.31%的绝对性能提升；PRIME准确度与RLVR训练效果呈强线性相关（R² > 0.92）。

Conclusion: PRIME是评估过程-结果对齐验证的有效基准，能够可靠预测验证器选择效果，过程感知的RLVR训练范式显著提升模型性能。
...
TL;DR: 提出Dual-Level框架，通过前缀保护和难度感知的压缩方法，在减少语言推理模型推理痕迹长度的同时提升准确性


<details>
  <summary>Details</summary>
Motivation: 现有语言推理模型（LRMs）通过增加推理时计算来提升性能，但存在"过度思考"问题，产生过长的推理痕迹，增加了延迟和内存使用。现有方法通常采用统一的长度惩罚来强制简洁性，但这在序列层面过度压缩了关键的早期推理步骤，在群体层面不加区分地惩罚所有查询。

Method: 提出Dual-Level框架，包含两个层次的压缩机制：1）序列层面的前缀保护优化，使用衰减混合rollouts来保持有效推理路径同时促进简洁性；2）群体层面的难度感知惩罚，根据查询复杂度动态调整长度约束，对困难问题保持探索，对简单问题抑制冗余。

Result: 在DeepSeek-R1-Distill-Qwen（1.5B/7B）模型上的实验表明，该方法在数学基准测试中实现了显著的token使用减少（高达55.7%），同时提高了准确性（高达4.1%），并在代码、科学和通用领域展现出泛化能力。

Conclusion: 提出的Dual-Level框架有效解决了语言推理模型的"过度思考"问题，通过分层监督实现了前缀保护和难度感知的压缩，在减少计算开销的同时提升了模型性能，具有广泛的适用性。
...
TL;DR: 研究探索LLM生成反馈中特定元素（如语气、信息覆盖）对学习效果和接受度的影响，并分析人格特质如何影响反馈接受差异


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在教育反馈生成中展现出潜力，但特定反馈元素（如语气、信息覆盖）如何影响学习效果和学习者接受度尚不明确，特别是对于不同人格特质的学习者

Method: 定义六个反馈元素，使用GPT-5为多项选择生物学问题生成反馈，对321名高一学生进行学习实验，评估两个学习效果指标和六个主观评价标准，并基于大五人格特质分析反馈接受差异

Result: 有效的反馈元素在学习效果方面表现出共同模式，而学习者的主观偏好在不同人格聚类中存在差异
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在角色扮演应用中的普及，模拟患者成为培训咨询师和扩展治疗评估的重要工具。然而现有方法碎片化，使用不兼容、非标准化的数据格式、提示和评估指标，阻碍了可复现性和公平比较。

Method: 提出PatientHub框架，标准化模拟患者的定义、组合和部署。实现多个代表性患者模拟方法作为案例研究，支持标准化跨方法评估和自定义评估指标的集成。通过原型化两个新模拟器变体展示框架的可扩展性。

Result: PatientHub将现有工作整合到单一可复现流程中，降低了开发新模拟方法的门槛，促进了跨方法和跨模型的基准测试。框架为患者中心对话的未来数据集、方法和基准提供了实用基础。

Conclusion: PatientHub通过统一模块化框架解决了患者模拟领域的碎片化问题，加速了方法开发，促进了标准化评估和比较，代码已公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 现有语义异常数据集和LLM在区分"异常但可解释"与"完全无意义"句子方面的能力尚不明确，需要系统评估人类和LLM对语义异常程度的判断差异

Method: 收集人类评分者和LLM对五个语义异常数据集中句子的可理解性判断，包括无上下文和有上下文两种情况，分析句子被判定为异常vs无意义的比例

Result: 评分者认为大多数句子最多只是异常而非完全无意义；LLM在生成合理上下文解释异常句子方面表现出显著能力

Conclusion: 现有语义异常数据集主要包含异常而非完全无意义的句子，LLM具备区分异常与无意义并生成合理上下文的能力，这对语义理解模型开发有重要意义
...
TL;DR: 提出Thinking with Drafting (TwD)方法，将视觉推理重新概念化为光学解压缩过程，通过领域特定语言作为中间表示，让模型将思维过程转化为可执行代码进行自我验证。
...
Method: 提出Thinking with Drafting (TwD)方法，基于"解析即推理"的公理，使用极简领域特定语言(DSL)作为基础中间表示。模型将思维模型草拟为可执行代码，生成确定性视觉证明进行自我验证。

Result: 实验验证了TwD作为优越认知支架的有效性。提出了VisAlg视觉代数基准进行评估，展示了该方法在视觉推理任务中的优势。

Conclusion: 建立了一个闭环系统，其中视觉生成不是作为创意输出，而是作为逻辑验证器，为视觉推理提供了可泛化的路径。
...
<details>
  <summary>Details</summary>
Motivation: 实现有效的测试时扩展需要模型具备在单个连续上下文中生成、验证和精炼多个推理假设的能力（在上下文探索）。然而，自回归生成过程中，采样长推理序列的概率呈指数衰减，形成"浅层探索陷阱"，限制了状态覆盖度的扩展。

Method: 提出Length-Incentivized Exploration方法，采用简单的两阶段策略：1) 通过基于长度的奖励明确鼓励模型进行更多探索；2) 结合冗余惩罚机制，最大化状态覆盖度。

Result: 在不同模型（Qwen3、Llama）上的综合实验表明，该方法能有效激励在上下文探索。在领域内任务上平均提升4.4%，在领域外基准测试上获得2.7%的增益。

Conclusion: Length-Incentivized Exploration方法成功解决了自回归生成中的浅层探索陷阱问题，通过简单的长度奖励和冗余惩罚机制，显著提升了模型在测试时的探索能力和泛化性能。
...
TL;DR: MiniCPM-SALA是一个9B参数的混合注意力架构，结合稀疏注意力(InfLLM-V2)和线性注意力(Lightning Attention)，通过层选择算法和混合位置编码，在保持性能的同时提升长上下文处理效率，支持高达1M tokens的上下文长度。
...
Method: 提出MiniCPM-SALA混合架构：1) 集成InfLLM-V2稀疏注意力（高保真长上下文建模）和Lightning Attention线性注意力（全局效率）；2) 使用层选择算法以1:3比例集成两种机制；3) 采用混合位置编码(HyPE)；4) 提出经济高效的持续训练框架，将预训练的Transformer模型转换为混合模型。

Result: 1) 在单张NVIDIA A6000D GPU上，256K tokens序列长度下推理速度比全注意力模型快3.5倍；2) 支持高达1M tokens的上下文长度（传统8B全注意力模型因内存限制无法处理）；3) 训练成本比从头训练降低约75%；4) 保持与全注意力模型相当的一般能力。
...
TL;DR: 提出一种基于嵌入的方法，通过训练子词嵌入并组合余弦相似度和n-gram相似度来检测语言变体，无需先验标准化或预定义变体列表


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖先验标准化或预定义变体列表来处理语言变体，这限制了在"嘈杂"或低资源语言环境中发现新变体的能力。需要一种能够将拼写和形态多样性作为语言结构而非噪声来分析的自动化方法

Method: 在原始文本上训练子词嵌入，通过组合余弦相似度（捕捉语义相似性）和n-gram相似度（捕捉形式相似性）来分组相关形式。该方法不依赖手动标注，产生透明的聚类结果

Result: 在卢森堡语用户评论的大规模语料库中，该方法发现了广泛的词汇和正字法变体，这些变体与方言和社会语言学研究描述的模式一致。诱导出的变体家族捕捉了系统性的对应关系，并突出了区域和风格差异

Conclusion: 分布建模即使在"嘈杂"或低资源环境中也能揭示有意义的变体模式，为研究多语言和小语言环境中的语言多样性提供了可复现的方法论框架
...
<details>
  <summary>Details</summary>
Motivation: 现有基于困惑度等指标的文本分析方法未能充分考虑上下文信息，无法准确解释给定下一个词概率的含义，因为其解释依赖于条件分布形状所编码的合理选择数量。

Method: 提出DMAP方法，将文本通过语言模型映射到单位区间的一组样本，这些样本联合编码了排名和概率信息，提供了统一的统计视图。

Result: 通过三个案例研究验证了DMAP的实用性：1) 验证生成参数以确保数据完整性；2) 检查概率曲率在机器生成文本检测中的作用；3) 对经过合成数据后训练的下游模型进行法证分析，揭示统计指纹。

Conclusion: DMAP提供了一种简单计算、广泛适用且统一的文本统计视图，为基于LLM的文本分析研究奠定了基础，可在消费级硬件上实现。
...
TL;DR: 提出了RouterXBench评估框架和ProbeDirichlet路由器，用于优化本地小模型与云端大模型之间的查询路由，通过内部隐藏状态和可学习狄利克雷分布实现更好的路由性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLM路由评估缺乏系统性，忽视场景特定需求和分布外鲁棒性。在成本和隐私约束下，需要有效路由机制将复杂查询卸载到云端模型，同时保持本地小模型效率。

Method: 1) 提出RouterXBench评估框架，包含路由能力、场景对齐和跨域鲁棒性三个维度；2) 利用内部隐藏状态而非输出概率或外部嵌入；3) 提出ProbeDirichlet路由器，通过可学习狄利克雷分布聚合跨层隐藏状态，采用概率训练。

Result: ProbeDirichlet在路由能力和高精度场景中相对最佳基线分别提升16.68%和18.86%，在模型家族、模型规模、异构任务和智能体工作流中表现一致。

Conclusion: RouterXBench提供了系统性路由评估框架，ProbeDirichlet通过内部隐藏状态和概率方法实现了鲁棒的路由性能，为本地-云端混合部署提供了有效解决方案。
...
TL;DR: 论文揭示基准测试的"幻觉"现象：表面上收敛的准确率可能掩盖了模型间的深层认知分歧，这种隐藏的分歧会影响科学研究结果的可重复性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是揭示大型语言模型在基准测试中表面准确率收敛背后存在的深层认知分歧问题，以及这种隐藏分歧对科学研究可重复性的潜在影响。

Method: 使用两个主要推理基准测试（MMLU-Pro和GPQA），分析不同LLMs在获得可比准确率时的具体分歧程度；通过重新分析已发表的教育和政治科学研究，评估模型选择对研究结果的影响。

Result: 研究发现：1）准确率相当的LLMs在16-66%的项目上存在分歧，前沿模型间也有16-38%的分歧；2）在重新分析的研究中，切换标注模型可使估计的治疗效应变化超过80%，有时甚至改变效应方向。

Conclusion: 基准测试存在"幻觉"现象，相等的准确率可能掩盖了模型间的分歧，模型选择成为影响科学研究可重复性的隐藏但关键变量。
...
TL;DR: AdaptEvolve：一种自适应LLM选择方法，在进化式顺序精炼框架中利用内在生成置信度动态选择LLM，平衡计算效率与推理能力，平均减少37.9%推理成本同时保持97.5%基准精度。


<details>
  <summary>Details</summary>
Motivation: 进化式智能体系统在推理过程中反复调用大语言模型，加剧了计算效率与推理能力之间的权衡。现有路由策略通常依赖静态启发式方法或外部控制器，未能明确考虑模型不确定性，需要一种能够动态选择足够能力LLM同时保持计算效率的方法。

Method: 提出AdaptEvolve方法，在进化式顺序精炼框架中利用内在生成置信度来估计实时可解性，实现自适应LLM选择。该方法基于模型生成时的置信度动态路由，避免依赖外部控制器或静态启发式规则。
...
Method: 提出跨模态鲁棒性转移(CMRT)框架，将文本模态的对抗鲁棒性转移到语音模态。该方法无需对抗语音训练数据，通过利用文本模态的对抗鲁棒性来增强语音翻译模型的形态学鲁棒性。

Result: 在四个语言对上的实验表明，CMRT将对抗鲁棒性平均提升了超过3个BLEU分数，为无需生成对抗语音数据的鲁棒端到端语音翻译建立了新的基线。

Conclusion: CMRT框架有效解决了端到端语音翻译模型的形态学鲁棒性问题，无需昂贵的对抗语音数据生成，为实际应用中的语音翻译系统提供了更实用的鲁棒性增强方案。
...
TL;DR: 论文研究发现，标准问答基准测试中大量问题存在定义不明确（underspecified）的问题，这导致大语言模型在这些问题上的表现不佳，而非模型能力不足。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在定义明确的问题上表现良好，但标准问答基准测试仍未完全解决。作者认为这种差距部分源于定义不明确的问题——那些没有额外上下文就无法唯一确定解释的查询。

Method: 1) 引入基于LLM的分类器来识别定义不明确的问题，并应用于多个广泛使用的QA数据集；2) 进行受控重写实验，将定义不明确的问题重写为完全定义明确的变体，同时保持黄金答案不变，作为上限分析。

Result: 1) 在多个QA数据集中，16%到超过50%的基准问题存在定义不明确；2) LLM在这些定义不明确的问题上表现显著更差；3) 在受控重写实验中，QA性能一致提高，表明许多明显的QA失败源于问题定义不明确而非模型限制。

Conclusion: 定义不明确是QA评估中的重要混淆因素，研究结果强调了在基准设计中需要更加关注问题的清晰度。
...
Conclusion: 开源LLMs通过适当的适配方法可以在多语言机器翻译中达到与专有系统竞争的性能，模型规模和数据规模对翻译质量有重要影响
...
TL;DR: LaCy：一种基于语法分析器辅助的小语言模型预训练方法，通过智能选择哪些token应该学习预测、哪些应该委托给大模型，在保持低成本的同时提高事实准确性。


<details>
  <summary>Details</summary>
Motivation: 小语言模型（SLM）的参数容量有限，容易产生事实错误。虽然可以通过访问外部资源（如大模型、文档或数据库）来缓解，但需要解决一个根本问题：在预训练中，哪些token应该由SLM学习预测，哪些应该通过<CALL> token委托给外部资源。

Method: 提出LaCy预训练方法，使用spaCy语法分析器增强损失信号，判断哪些token应该委托以防止事实错误，哪些即使损失较高也可以安全学习和预测。该方法基于token选择哲学，让SLM学习何时预测、何时委托。

Result: LaCy模型成功学会了哪些token应该预测、哪些应该委托。在与大模型级联生成时获得更高的FactScores，优于Rho或LLM-judge训练的SLM，同时方法更简单、成本更低。

Conclusion: 通过语法分析器辅助的token选择策略，可以在小语言模型中实现智能委托决策，在保持低成本的同时显著提高生成内容的事实准确性，为资源受限环境下的语言模型部署提供了有效方案。
...
TL;DR: CLUES框架通过将临床Text-to-SQL建模为两阶段过程(解释→答案)，将语义不确定性分解为歧义性和不稳定性分数，以区分需要澄清的输入歧义和需要人工审查的模型不稳定性。
...
Method: 提出CLUES框架，将Text-to-SQL建模为两阶段过程：解释生成和答案生成。通过二分语义图矩阵的Schur补计算不稳定性分数，将语义不确定性分解为歧义性分数和不稳定性分数。

Result: 在AmbigQA/SituatedQA(黄金解释)和临床Text-to-SQL基准(已知解释)上，CLUES在失败预测方面优于最先进的核语言熵方法。高歧义/高不稳定性区域包含51%的错误，仅覆盖25%的查询，实现高效分流。

Conclusion: CLUES框架通过诊断性不确定性分解，为临床Text-to-SQL部署提供有针对性的干预策略：歧义性触发查询细化，不稳定性触发模型改进，显著提高错误检测效率。
...
<details>
  <summary>Details</summary>
Motivation: 大规模可验证提示支撑了RLVR的成功，但包含许多无信息示例且扩展成本高。现有方法专注于利用通过率为0的困难提示，但随着训练进行，通过率为1的简单提示日益增多，降低了有效数据规模。

Method: 提出Composition-RL方法，自动将多个问题组合成新的可验证问题，并使用这些组合提示进行RL训练。还设计了课程变体，在训练过程中逐步增加组合深度。

Result: 在4B到30B不同模型规模上的广泛实验表明，Composition-RL相比在原始数据集上训练的RL持续提升推理能力。课程变体可进一步提升性能，且Composition-RL能通过组合不同领域提示实现更有效的跨领域RL。

Conclusion: Composition-RL是一种简单有效的方法，能更好利用有限的可验证提示，特别是通过率为1的简单提示，显著提升RLVR的推理能力，并支持跨领域应用。
...
TL;DR: DeepSight是一个开源的大模型安全评估项目，包含DeepSafe评估工具包和DeepScan诊断工具包，实现了安全评估与诊断的集成范式，将黑盒评估转变为白盒洞察。
...
Result: DeepSight是一个低成本、可复现、高效且高度可扩展的大规模模型安全评估项目，实现了安全评估与诊断的集成范式，能够系统性地解决当前安全流程中的问题。

Conclusion: DeepSight通过集成评估和诊断工具包，提供了一种新的安全评估-诊断集成范式，能够更全面地理解大模型的安全机制，为安全对齐提供更精确的指导，避免通用能力下降。
...
Method: 提出P-GenRM模型：1) 将偏好信号转化为结构化评估链，推导跨场景的自适应角色和评分标准；2) 将用户聚类为用户原型；3) 引入双粒度扩展机制：个体层面自适应扩展聚合用户评分方案，原型层面整合相似用户偏好；4) 通过原型迁移增强对未见用户的泛化能力。

Result: 在广泛使用的个性化奖励模型基准测试中达到最先进水平，平均提升2.31%；在分布外数据集上表现出强泛化能力；测试时用户扩展额外提供3%提升，展示了更强的个性化对齐能力和测试时可扩展性。

Conclusion: P-GenRM通过结构化评估链和双粒度用户扩展机制，有效解决了个性化奖励建模中的偏好多样性和泛化挑战，为个性化大语言模型对齐提供了更灵活、可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前流行的神经模型需要大量训练数据，而低资源语言（如盖尔语）通常缺乏足够数据。需要开发能够有效利用有限样本数据的替代方法，以支持低资源语言的持续活力。

Method: 使用维基词典数据构建基于规则的盖尔语形态模型，采用SQL查询不同词汇模式的出现情况，并开发声明式规则库，使Python工具能够推导盖尔语单词的屈折形式。

Result: 开发了一个基于规则的盖尔语形态模型，能够有效利用有限的样本数据，支持更好的可解释性，并为教学材料设计提供有用见解。该方法为维基词典数据增加了新价值。

Conclusion: 基于规则的系统能够有效解决低资源语言的形态建模问题，支持教育工具和更高级的语言处理工具开发，为维基词典数据创造了新的应用场景。
...
TL;DR: WavBench是一个全面的语音对话基准测试，通过三个子集（Pro、Basic、Acoustic）评估真实世界对话能力，填补了现有基准在副语言特征和口语化表达评估方面的空白。


<details>
  <summary>Details</summary>
Motivation: 当前语音对话模型的评估主要遵循文本生成标准，忽视了副语言特征（如语调、节奏）和口语化表达的独特性，以及现代智能体所需的认知深度。现有基准无法充分评估真实世界对话的复杂性。

Method: WavBench采用三重框架：1) Pro子集：显著增加难度，严格挑战具备推理能力的模型；2) Basic子集：定义口语化新标准，强调"可听性"（自然词汇、语言流畅性、互动融洽度）而非严格的书面准确性；3) Acoustic子集：涵盖显性理解、生成和隐性对话，在真实场景中严格评估全面的副语言能力。

Result: 通过对五个最先进模型的评估，WavBench提供了关于复杂问题解决、口语化表达和副语言保真度交叉领域的关键见解，揭示了当前模型在这些方面的能力差距。

Conclusion: WavBench填补了语音对话评估的关键空白，通过其三重框架为稳健语音对话模型的演进提供了指导，有助于推动该领域向更真实、更全面的对话能力发展。
...
TL;DR: CitiLink-Minutes：一个多层标注的欧洲葡萄牙语市政会议记录数据集，包含120份市政会议记录，超过100万词条，38,000多个标注，用于支持NLP和IR研究。
...
Method: 收集了6个城市的120份欧洲葡萄牙语市政会议记录，进行多层标注和结构化链接。所有个人标识符都进行了去标识化处理。每份记录由两名训练有素的标注员手动标注，并由经验丰富的语言学家在三个维度进行管理：元数据、讨论主题和投票结果。

Result: 创建了包含超过100万词条和38,000多个标注的数据集。遵循FAIR原则发布，并提供了元数据提取、主题分类和投票标签的基线结果。数据集展示了在下游NLP和IR任务中的潜力。

Conclusion: CitiLink-Minutes填补了市政会议记录在NLP和IR研究中的空白，为开发计算模型提供了重要资源，同时促进了市政决策的透明访问。
...
TL;DR: 提出基于大语言模型检索头的注意力分数重排序框架，利用候选短列表整体信息进行列表式排序，无需Likert尺度监督，仅需小规模模型即可实现强大性能


<details>
  <summary>Details</summary>
Motivation: 现有检索头分析基础上，寻求更有效的重排序方法，利用注意力分数估计段落-查询相关性，提供列表式解决方案，利用候选短列表整体信息，同时生成连续相关性分数，无需Likert尺度监督

Method: 训练模型使用选定注意力头的注意力分数来估计段落-查询相关性，采用列表式排序框架，利用候选短列表整体信息，支持灵活扩展：如增强候选段落上下文信息、使用中间层注意力头提升效率

Result: 在多个领域（包括Wikipedia和长叙事数据集）超越现有最先进的点式和列表式重排序器，在LoCoMo基准测试中建立新的最先进水平，评估对话理解和记忆使用能力

Conclusion: 提出的轻量级有效重排序框架仅需小规模模型即可实现强大性能，支持灵活扩展，为检索任务提供了有效的列表式解决方案
...
TL;DR: 该论文提出了视觉推理基准（VRB），这是一个基于赞比亚和印度小学考试题构建的数据集，用于评估多模态大语言模型在真实课堂视觉问题上的推理能力，发现模型在空间动态操作方面存在明显瓶颈。


<details>
  <summary>Details</summary>
Motivation: 尽管AI模型在文本推理方面取得了先进成果，但在空间和关系结构推理方面仍存在关键瓶颈，特别是在依赖视觉的早期数学教育中。需要评估多模态大语言模型解决真实课堂视觉问题的能力，以确定其在教育应用中的功能边界。

Method: 构建了包含701个问题的视觉推理基准（VRB），这些问题来源于赞比亚和印度的小学考试题，涵盖类比推理、模式完成、空间匹配等任务。基准采用未经编辑、文本最少的图像，以测试模型是否满足小学教育的实际需求。

Result: 研究发现模型能力存在"锯齿状前沿"：在计数和缩放等静态技能方面表现较好，但在折叠、反射和旋转等动态操作方面达到明显的"空间天花板"。这些弱点可能导致课堂使用中的错误评分、虚假支架和强化学生误解等风险。

Conclusion: 像VRB这样的教育导向基准对于确定课堂中使用的多模态工具的功能边界至关重要。模型在空间动态操作方面的局限性对课堂应用构成风险，需要专门的基准来评估和改进模型在教育环境中的实际表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有文档理解基准存在局限性：关键实体提取数据集实体本体狭窄，关系提取数据集查询简单，视觉问答数据集文档类型单一，都忽视了实际应用中需要适应多样化文档类型和灵活模式的结构化提取需求。

Method: 通过结合人工和合成人工验证样本的新颖流水线构建ExStrucTiny基准数据集，该数据集统一了关键实体提取、关系提取和视觉问答任务，覆盖更多样化的文档类型和提取场景。

Result: 在ExStrucTiny基准上分析开放和封闭视觉语言模型，揭示了模式适应、查询欠规范和答案定位等挑战，表明现有模型在多样化文档类型和灵活模式下的结构化提取能力仍有待提升。

Conclusion: ExStrucTiny为改进通用模型在文档结构化信息提取方面的能力提供了基础，有助于推动视觉语言模型在企业文档处理中的实际应用。
...
TL;DR: 提出了一种检测软压缩架构中"token溢出"的方法，即当压缩表示不再包含足够信息回答查询时的状态，通过轻量级探测分类器实现查询感知的溢出检测。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在资源受限环境中处理长上下文时的效率挑战。软压缩架构通过用学习到的压缩token替换长token序列来扩展有效上下文长度，但压缩极限和任务相关内容何时开始被擦除尚未充分探索。

Method: 1) 定义"token溢出"为压缩表示不再包含足够信息回答给定查询的状态；2) 在xRAG软压缩设置中，使用查询无关的饱和统计量分离压缩和未压缩token表示；3) 在查询和上下文xRAG表示上使用轻量级探测分类器检测溢出。

Result: 查询无关饱和统计量能可靠分离压缩和未压缩token表示，但溢出检测能力有限。轻量级探测分类器在HotpotQA、SQuADv2和TriviaQA数据集上平均达到0.72 AUC-ROC，表明结合查询信息能显著提升检测性能。

Conclusion: 研究从查询无关诊断推进到查询感知检测器，实现了低成本的预LLM门控机制，能够减轻压缩引起的错误，为软压缩架构的实用部署提供了重要工具。
...
TL;DR: Moonshine v2是一种用于流式自动语音识别的滑动窗口自注意力编码器模型，在边缘设备上实现了低延迟和高准确率，性能与6倍大小的模型相当。


<details>
  <summary>Details</summary>
Motivation: 解决边缘设备上延迟敏感的语音应用（如实时转录、语音命令）对低首次令牌时间（TTFT）和高准确率的需求。传统全注意力Transformer编码器虽然准确率高，但存在二次复杂度，导致延迟随话语长度线性增长，不适合流式应用。

Method: 引入Moonshine v2，一种遍历流式编码器ASR模型，采用滑动窗口自注意力机制。该方法通过局部上下文实现有界、低延迟推理，同时保持强大的局部上下文能力，避免了全注意力的二次复杂度问题。

Result: 在标准基准测试中实现了最先进的词错误率，准确率与6倍大小的模型相当，同时运行速度显著更快。证明了精心设计的局部注意力在准确率上可与全注意力竞争，但成本和延迟大幅降低。

Conclusion: Moonshine v2展示了局部注意力机制在边缘设备流式语音识别中的有效性，为交互式语音界面在资源受限设备上的应用开辟了新可能性，平衡了延迟、准确率和计算效率的需求。
...
TL;DR: 提出轨迹自蒸馏框架，通过蒸馏模型自身的生成轨迹来改进扩散大语言模型的少步解码性能，使用直接判别优化目标缩小与全步解码的质量差距。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型理论上能通过并行解码实现快速文本生成，但实际推理效率受限于需要大量细化步骤。激进减少步骤数会导致生成质量显著下降，需要在少步解码时保持高质量。

Method: 提出轨迹自蒸馏框架，通过蒸馏模型自身的生成轨迹来改进少步解码。采用直接判别优化（DDO）目标，这是一种反向KL目标，促进模式寻求蒸馏，鼓励学生模型专注于教师模型的高概率模式。

Result: 在多个基准测试中，该方法在严格的步骤预算下持续优于强少步基线方法和标准训练。虽然全步解码仍然更优，但显著缩小了质量差距，为实用的少步扩散大语言模型奠定了坚实基础。

Conclusion: 轨迹自蒸馏框架结合直接判别优化目标，有效提升了扩散大语言模型的少步解码性能，在保持推理效率的同时显著改善了生成质量，为实现实用的少步解码扩散模型提供了有力支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有上下文蒸馏方法存在局限性，需要开发一种能够有效将语言模型中的上下文知识内化到参数中的框架，特别是在经验知识蒸馏和系统提示蒸馏两个重要应用场景中。

Method: 提出基于策略的上下文蒸馏(OPCD)框架，通过训练学生模型在其自身生成的轨迹上，同时最小化与上下文条件教师模型的反向Kullback-Leibler散度，实现知识的内化。

Result: 在数学推理、文本游戏和领域特定任务中，OPCD持续优于基线方法，获得更高的任务准确率，同时更好地保持分布外能力，并能实现有效的跨尺寸蒸馏。

Conclusion: OPCD为语言模型内化上下文知识提供了有效框架，在经验知识蒸馏和系统提示蒸馏两个关键应用中表现出色，能够实现从大型教师模型到小型学生模型的知识转移。
...
TL;DR: 在DashAI无代码ML平台中集成PDP、PFI和KernelSHAP三种可解释性技术，通过用户研究评估其对新手和专家的可用性及影响


<details>
  <summary>Details</summary>
Motivation: 机器学习在敏感领域应用增加引发对决策透明度的担忧，现有XAI方法需要技术专业知识，限制了在无代码ML平台中的价值，阻碍AI民主化

Method: 在开源无代码ML平台DashAI中开发以人为中心的XAI模块，集成三种互补技术：部分依赖图(PDP)、排列特征重要性(PFI)和KernelSHAP；通过用户研究(N=20，包括ML新手和专家)评估可用性和解释效果

Result: 1) 所有可解释性任务成功率≥80%；2) 新手在解释满意度量表(ESS, α=0.74)上认为解释有用、准确、可信，专家对充分性和完整性更挑剔；3) 解释提高了自动化信任量表(TiA, α=0.60)中的可预测性和信心感知，新手比专家表现出更高信任度

Conclusion: 研究揭示了无代码ML中XAI的核心挑战：需要使解释既对新手可访问，又对专家足够详细；DashAI的XAI模块成功平衡了这一需求，促进了AI民主化
...
TL;DR: 论文提出决策值映射概念和DecisionDB基础设施，用于记录、重放和审计数据不同表示对计算结果的离散影响，确保确定性重放和机械可检查的决策重用。


<details>
  <summary>Details</summary>
Motivation: 相同数据的不同表示形式可能导致计算引擎产生不同的离散结果，某些表示保持结果不变，而其他表示则完全改变结果。需要系统化地记录和分析这种表示-结果关系，以支持决策的可靠重用和审计。

Method: 提出决策值映射概念，将表示族中的每个成员映射到其产生的离散结果。开发DecisionDB基础设施，使用基于内容和工件的标识符，以只写形式存储记录，支持确定性重放和审计。

Result: 实现了确定性重放，能够从存储的工件中精确恢复每个记录的决策标识符，三个标识字段均与持久化值匹配。将表示空间划分为持久性区域和边界，为决策重用提供了机械可检查的条件。

Conclusion: 决策值映射和DecisionDB基础设施为理解和管理数据表示对计算结果的影响提供了系统化框架，支持可靠的决策记录、重放和审计，确保计算结果的确定性和可验证性。
...
<details>
  <summary>Details</summary>
Motivation: 现有流式ASR系统通常通过分块或滑动窗口等适配离线模型的方法实现，这些方法存在质量损失。需要开发原生流式训练的模型，在保持低延迟的同时达到离线转录质量
...
Conclusion: Voxtral Realtime证明了原生流式训练方法在保持低延迟的同时达到离线转录质量的可行性，为实时语音识别提供了高质量解决方案
...
TL;DR: 该论文批判机器学习中"ground truth"范式的实证主义谬误，指出其将人类分歧视为技术噪声而非重要社会技术信号，并分析了数据标注实践中导致"共识陷阱"的机制，提出构建多元标注基础设施的路线图。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于揭示机器学习中"ground truth"范式的基础性缺陷——它将人类分歧错误地视为技术噪声而非重要的社会技术信号。这种实证主义谬误掩盖了数据标注实践中的系统性偏见，导致模型无法捕捉文化多样性，需要重新审视标注实践中的权力动态和共识形成机制。

Method: 采用系统性文献综述方法，分析2020-2025年间在ACL、AIES、CHI、CSCW、EAAMO、FAccT和NeurIPS七个顶级会议发表的论文。通过分层关键词过滤方案从30,897条记录中筛选出3,042条高召回率记录进行人工筛选，最终纳入346篇论文进行定性综合。使用反思性主题分析方法识别系统性模式。

Result: 研究发现：1) 系统性位置可读性失败与人类作为验证者模型的架构转变相结合，特别是依赖模型介导的标注，引入了根深蒂固的锚定偏见并实质上将人类声音排除在循环之外；2) 地理霸权将西方规范强加为普遍基准，通常通过不稳定数据工作者的表演性对齐来强制执行，他们为避免经济惩罚而优先考虑请求者合规而非诚实主观性；3) "噪声传感器"谬误导致统计模型将文化多元主义误诊为随机误差。

Conclusion: 结论认为需要重新将分歧视为构建文化能力模型所需的高保真信号。为解决这些系统性紧张关系，提出了多元标注基础设施的路线图，将目标从发现单一"正确"答案转向映射人类经验的多样性，从而建立更具包容性和文化能力的机器学习系统。
...
TL;DR: BLPO：针对多模态LLM评估的图像生成内容评价，提出双层提示优化框架，通过图像转文本表示解决上下文窗口限制问题


<details>
  <summary>Details</summary>
Motivation: 当前LLM作为自动评估器在评估AI生成内容时与人类判断对齐仍具挑战性。监督微调成本高且不灵活，而现有的自动提示优化方法主要针对纯文本评估，在多模态场景下探索不足，特别是评估AI生成图像时面临上下文窗口限制问题
...
Conclusion: BLPO框架成功解决了多模态LLM评估中因上下文窗口限制导致的提示优化瓶颈，为AI生成图像评估提供了有效的自动提示优化解决方案
...
TL;DR: AgentNoiseBench框架用于系统评估智能体模型在噪声环境下的鲁棒性，通过注入可控噪声到现有基准测试中，发现当前模型对现实环境扰动敏感
...
Method: 1) 分析现实场景中的偏差和不确定性，将环境噪声分为用户噪声和工具噪声两类；2) 开发自动化管道，在保持任务可解性的前提下向现有智能体基准注入可控噪声；3) 使用该管道对不同架构和参数规模的模型进行广泛评估

Result: 在不同噪声条件下观察到一致的性能变化，突显了当前智能体模型对现实环境扰动的敏感性

Conclusion: 需要开发更鲁棒的智能体模型来应对现实世界中的噪声环境，AgentNoiseBench为系统评估模型鲁棒性提供了框架
...
<details>
  <summary>Details</summary>
Motivation: 现有主动LLM智能体训练方法面临平衡任务性能与用户参与度的挑战：被动智能体无法高效适应用户意图，而过度使用人类反馈会降低用户满意度。

Method: 提出BAO框架，结合行为增强（丰富主动推理和信息收集能力）与行为正则化（抑制低效或冗余交互，对齐用户期望）。

Result: 在UserRL基准套件的多个任务上，BAO显著优于主动智能体强化学习基线，性能与商业LLM智能体相当甚至更优。

Conclusion: BAO框架能有效训练复杂多轮场景中主动且用户对齐的LLM智能体，平衡任务效率与用户满意度。
...
Method: 将掩码联合嵌入预测从图像块扩展到面向对象表示，通过对象级掩码要求从其他对象推断目标对象状态，实现隐式干预和反事实效应。

Result: 在视觉问答中反事实推理绝对提升约20%；在智能体控制任务中，仅需基于块世界模型1%的潜在输入特征即可达到相当性能，规划效率显著提升。

Conclusion: 对象级掩码通过隐式干预引入因果归纳偏置，C-JEPA为关系推理提供简单灵活框架，在预测、推理和控制任务中展现优势。
...
<details>
  <summary>Details</summary>
Motivation: Mamba2模型扩展的状态维度增强了时序建模能力，但在自回归生成过程中会产生显著的推理开销，导致带宽饱和。传统的剪枝方法无法有效解决这一瓶颈：非结构化稀疏性保持激活密集，基于幅度的选择忽略运行时动态，而基于梯度的方法成本过高。

Method: 提出GHOST（分组隐藏状态输出感知选择和截断）框架，通过联合测量可控性和可观测性，仅使用前向传播统计量来近似控制理论平衡截断。该方法不需要反向传播，实现了结构化剪枝。

Result: 在130M到2.7B参数规模的模型上，该方法实现了50%的状态维度缩减，在WikiText-2数据集上仅带来约1个困惑度点的增加。该方法在保持模型性能的同时显著降低了推理开销。

Conclusion: GHOST框架提供了一种高效的结构化剪枝方法，能够在不需要反向传播的情况下达到与基于梯度方法相当的保真度，有效解决了Mamba2模型在推理时的带宽瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有集中训练分散执行的MARL方法基于IGM原则，但在现实环境中面临模拟到现实的差距、模型不匹配和系统噪声等不确定性，导致可靠性不足。

Method: 提出分布鲁棒的IGM原则(DrIGM)，定义鲁棒个体动作值函数，证明其满足分散贪婪执行要求。基于此推导出与VDN/QMIX/QTRAN等现有值分解架构兼容的鲁棒变体，使用鲁棒Q目标进行训练。

Result: 在高保真SustainGym模拟器和星际争霸游戏环境中，所提方法在分布外性能上取得一致提升，保持可扩展性且无需定制化的单智能体奖励塑造。
...
TL;DR: 该论文提出Anchor-Token Reinforcement Learning (AT-RL)框架，通过选择性强化高连接性视觉锚点token来提升多模态大语言模型的推理能力，仅需1.2%额外开销即可让32B模型超越72B基准。


<details>
  <summary>Details</summary>
Motivation: 尽管可验证奖励的强化学习(RLVR)显著提升了多模态大语言模型(MLLMs)的推理能力，但视觉证据在推理过程中如何被整合仍不清楚。研究者希望理解跨模态注意力连接性在RLVR中的作用机制。

Method: 通过分析跨模态注意力连接性，发现只有约15%的token具有强视觉-文本耦合（高连接性token）。基于此提出AT-RL框架：使用基于图的注意力拓扑聚类识别高连接性token，并选择性强化这些视觉锚点，而非所有token。

Result: AT-RL仅引入1.2%额外开销，却使32B模型在MathVista上达到80.2分，超越72B-Instruct基准。在STEM、视频和通用任务上均观察到一致提升。相反，仅训练低连接性token会导致性能严重下降。

Conclusion: 多模态强化学习的有效性取决于对视觉锚点的精确信用分配，而非token数量。推理质量由跨模态锚定的保真度决定，而非token数量。高连接性token作为视觉锚点对推理至关重要。
...
TL;DR: AgentLeak是首个覆盖多智能体LLM系统内部通信通道的隐私泄露基准测试，揭示内部通道（特别是智能体间消息）比输出通道泄露更多敏感信息，输出审计会遗漏41.7%的违规。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体LLM系统存在隐私风险，但现有基准测试只能测量输出通道，无法检测智能体间消息、共享内存和工具参数等内部通道的隐私泄露。

Method: 开发AgentLeak基准测试，包含1000个医疗、金融、法律和企业领域场景，32类攻击分类，三层检测流水线，测试GPT-4o、GPT-4o-mini、Claude 3.5 Sonnet、Mistral Large和Llama 3.3 70B等模型在4979条跟踪记录中的表现。

Result: 多智能体配置降低了单通道输出泄露（C1: 27.2% vs 43.2%），但内部通道使系统总暴露率达68.9%。智能体间消息（C2）泄露率68.8%，远高于输出通道的27.2%。Claude 3.5 Sonnet在外部（3.3%）和内部（28.1%）通道均表现最佳。所有模型和领域都呈现C2 > C1模式。

Conclusion: 多智能体LLM系统的隐私保护需要协调框架，必须纳入内部通道隐私保护，并对智能体间通信实施隐私控制，仅依赖输出审计会遗漏大量隐私违规。
...
TL;DR: 提出一种受人类启发的持续学习框架，将推理、行动、反思和验证统一在并行学习增强的顺序推理模型中，将内部思维过程作为主要学习对象，实现认知结构在执行中改进。
...
Method: 提出人类启发的持续学习框架，将推理、行动、反思和验证统一在并行学习增强的顺序推理模型中。框架将内部思维过程作为主要学习对象，系统记录内部推理轨迹和环境交互作为结构化学习材料，支持用学习过程替换预定义逻辑，并引入分层学习机制共同适应任务级参数和学习策略。

Result: 在温度传感器异常检测任务上的实验结果表明，引入内部过程学习使平均运行时间减少了23.9%。系统能够在保持操作稳定性的同时逐步演化其内部认知架构。

Conclusion: 该框架通过将内部推理过程作为主要学习对象，实现了认知结构在执行过程中的持续改进，为开发能够在动态环境中持续适应的AI系统提供了有效途径。
...
Conclusion: CausalAgent作为一种新颖的用户中心人机协作范式，通过自动化端到端因果推断流程，解决了传统方法的技术壁垒问题，使因果分析更加可访问和实用
...
### [136] [Learning to Configure Agentic AI Systems](https://arxiv.org/abs/2602.11574)
*Aditya Taparia,Som Sagar,Ransalu Senanayake*

Main category: cs.AI

TL;DR: ARC：基于强化学习的轻量级分层策略，为LLM智能体系统动态配置工作流、工具、令牌预算和提示，实现按查询定制化配置而非固定模板


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体系统配置通常采用固定的大型模板或手动调整的启发式方法，导致脆弱行为和计算资源浪费，因为相同的繁琐配置被应用于简单和困难的输入查询

Method: 将智能体配置制定为按查询的决策问题，引入ARC（Agentic Resource & Configuration learner），使用强化学习学习轻量级分层策略，动态调整工作流、工具、令牌预算和提示等配置

Result: 在多个推理和工具增强问答基准测试中，学习到的策略始终优于强手工设计和其他基线方法，任务准确率提高达25%，同时减少令牌和运行时成本

Conclusion: 按查询学习智能体配置是替代"一刀切"设计的强大方法，能够实现更高效和自适应的智能体系统
...
TL;DR: 该综述论文系统回顾了多智能体通信研究的演进，通过"5W"框架分析通信设计要素，涵盖MARL、涌现语言和LLM三大范式，总结了设计模式与开放挑战。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统在动态部分可观测环境中需要通信来减少不确定性、实现协作。现有研究分散在不同范式，缺乏统一的分析框架来理解通信设计的关键要素和演进脉络。

Method: 采用"5W"分析框架：谁与谁通信、通信内容、通信时机、通信原因。系统梳理三大研究范式：多智能体强化学习、涌现语言、大语言模型，分析各范式的通信设计选择与权衡。

Result: 揭示了通信设计在不同范式中的演进规律：从MARL的端到端学习通信协议，到涌现语言的结构化符号通信尝试，再到LLM的自然语言先验。识别了各范式的核心挑战：MARL的可解释性差、涌现语言的接地与泛化困难、LLM的规划与控制集成问题。

Conclusion: 未来需要构建结合学习、语言和控制的混合系统，实现可扩展且可解释的多智能体协作。提出了实用设计模式和开放挑战，为下一代多智能体通信系统提供指导。
...
TL;DR: MAPLE提出了一种模态感知的RL后训练生态系统，通过基准标注、策略优化和自适应调度解决多模态训练中信号处理不均的问题，显著提升性能、收敛速度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态语言模型将文本、音频、视频信号同等对待，忽略了不同任务实际需要的模态组合。这种模态盲训练导致策略梯度方差增大、收敛缓慢，且在真实世界信号缺失、增加或重加权时鲁棒性下降。

Method: MAPLE包含三个核心组件：1) MAPLE-bench基准，首次明确标注每个任务所需的最小信号组合；2) MAPO模态感知策略优化框架，按模态需求对批次分层以减少异构群体优势带来的梯度方差；3) 自适应加权和课程调度，平衡并优先处理更难的信号组合。

Result: MAPLE将单模态/多模态准确率差距缩小30.24%，收敛速度提升3.18倍，并在现实信号访问受限情况下保持所有模态组合的稳定性。系统分析确定了MAPO在损失聚合、裁剪、采样和课程设计方面的最优训练策略。

Conclusion: MAPLE为部署就绪的多模态RL后训练提供了完整方案，通过模态感知方法解决了现有训练框架的局限性，显著提升了多模态系统的性能、效率和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏透明度和可解释性，需要开发能够直接基于原始组学数据进行推理、生成可审计推理轨迹的框架，以支持更可靠、可解释的单细胞分析

Method: 将单细胞分析任务（细胞类型注释、发育轨迹重建、转录因子靶向）转化为逐步推理问题，让LLM在自然语言对话中直接检查单细胞RNA-seq数据并按需调用生物信息工具，支持迭代推理和证据修订

Result: 实验显示，迭代式组学原生推理使细胞类型注释平均准确率提升11%，Gemini-2.5-Pro将轨迹图编辑距离降低30%，同时生成透明推理轨迹解释标记基因模糊性和调控逻辑

Conclusion: scPilot通过将LLM锚定在原始组学数据中，实现了可审计、可解释且具有诊断信息的单细胞分析，为生物医学研究提供了新的透明推理范式
...
TL;DR: 研究发现LLM智能体在相同任务上存在行为不一致性，这种不一致性可预测任务失败，早期决策是主要分歧点


<details>
  <summary>Details</summary>
Motivation: 探究LLM智能体在相同任务上的行为一致性，以及这种不一致性如何影响任务成功率

Method: 在HotpotQA数据集上对三个模型（Llama 3.1 70B、GPT-4o和Claude Sonnet 4.5）进行3000次智能体运行实验，使用ReAct风格智能体，分析行为序列的方差

Result: 智能体行为存在显著不一致性：平均每10次运行产生2.0-4.2个不同的动作序列；行为一致性预测任务成功率：一致性高（≤2个独特路径）的任务准确率达80-92%，高度不一致（≥6个独特路径）的任务准确率仅25-60%，差距达32-55个百分点；69%的分歧发生在第二步（首次搜索查询）
...
TL;DR: NMIPS框架通过神经辅助多任务符号求解器，为具有相同数学结构但参数不同的PDE族提供可解释的解析解，相比传统数值方法和黑盒机器学习方法，在准确性和计算效率上均有显著提升。
...
Method: 提出神经辅助多任务符号PDE求解器框架（NMIPS），采用多因子优化同时发现PDE的解析解。设计仿射传递方法，在PDE族之间传递已学习的数学结构，避免从零开始求解每个PDE。

Result: 在多个案例实验中，相比现有基线方法，NMIPS实现了高达约35.7%的准确率提升，同时提供了可解释的解析解。

Conclusion: NMIPS框架成功解决了传统数值方法计算成本高和机器学习方法缺乏可解释性的双重限制，为PDE族求解提供了一种高效且具有科学洞察力的新方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在感知任务上表现良好，但在数学空间推理能力（解析和操作二维、三维关系）方面存在明显不足。人类在教科书式空间推理问题上的准确率超过95%，而大多数领先的MLLMs甚至无法达到60%，这一显著差距表明空间推理是当前模型的基本弱点。

Method: 提出了MathSpatial统一框架，包含三个互补组件：1) MathSpatial-Bench：包含3个类别、11个子类型的2000个问题的基准测试，旨在将推理难度与感知噪声分离；2) MathSpatial-Corpus：包含8000个额外问题及已验证解决方案的训练数据集；3) MathSpatial-SRT：将推理建模为由三个原子操作（关联、约束、推断）组成的结构化轨迹。

Result: 实验表明，在MathSpatial上微调Qwen2.5-VL-7B模型能够达到竞争性准确率，同时减少25%的token使用。该框架首次提供了大规模资源，能够分离感知与推理，实现对MLLMs数学空间推理能力的精确测量和全面理解。

Conclusion: MathSpatial揭示了当前多模态大语言模型在数学空间推理方面的显著缺陷，并提供了一个统一的评估和改进框架。该框架通过分离感知与推理，为精确测量和理解MLLMs的空间推理能力提供了首个大规模资源，同时提出的结构化推理方法在保持准确率的同时显著提高了效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习对齐方法在医疗问答领域存在根本性不匹配：RLHF依赖昂贵且难以反映医学事实绝对正确性的偏好标注；RLVR缺乏有效自动验证器且难以处理复杂临床情境；医疗对齐需要同时优化正确性、安全性和合规性，但多目标异质奖励信号易导致尺度不匹配和优化冲突。
...
Conclusion: 该研究提出了一种鲁棒的医疗对齐范式，通过系统化分解对齐目标、建立闭环监督机制和统一优化策略，解决了医疗领域大语言模型对齐中的核心挑战，为垂直领域的复杂对齐提供了新框架。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，传统的基准测试变得越来越不可靠。分数膨胀和选择性报告削弱了标准基准的权威性，导致研究社区无法确定哪些评估结果仍然可信。需要一种系统性的方法来审计评估基准的健康状况。

Method: 提出了基准健康指数（BHI）框架，包含三个互补维度：1）能力区分度：衡量基准在噪声之外区分模型性能的锐度；2）抗饱和性：估计在达到天花板效应前剩余的提升空间，预测基准的预期寿命；3）影响力：通过采用广度和实践塑造能力量化基准在学术和工业生态系统中的影响。从2025年91个代表性模型的技术报告中提取了106个验证基准进行系统分析。

Result: BHI是首个在宏观层面量化基准健康状况的框架，为基准选择提供了原则性基础，并支持下一代评估协议的动态生命周期管理。通过系统分析106个基准，全面描绘了评估生态系统的现状。

Conclusion: BHI框架为解决LLM基准测试中的可靠性问题提供了数据驱动的系统性方法，能够帮助研究社区更明智地选择基准，并为评估协议的动态管理奠定基础，促进更健康的评估生态系统发展。
...
TL;DR: 论文提出Rung Collapse理论解释机器学习系统"正确但理由错误"的问题，开发Epistemic Regret Minimization方法通过因果信念修正解决该问题，实验证明在1360个因果陷阱场景中有效恢复53-59%的顽固错误。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统常通过"捷径"获得高性能，但这些捷径在分布偏移下会失效。这种"正确但理由错误"的病理现象源于自回归训练无法区分关联P(Y|X)和干预P(Y|do(X))，作者将其形式化为Rung Collapse。当基于结果的学习强化了通过错误因果模型获得的正确答案时，智能体会陷入错误的推理模式，即Aleatoric Entrenchment。

Method: 提出Epistemic Regret Minimization(ERM)作为因果信念修正目标，独立于任务成功惩罚因果推理错误。构建三层架构：1)物理基础定理证明满足执行器独立的动作实现有效的do操作；2)ERM作为满足AGM公设的因果信念修正算子；3)失败模式分类法对重复推理错误分类并注入领域无关防护。证明真实干预分布的渐近恢复和有限样本边界。

Result: 在六个前沿LLM的1360个因果陷阱场景实验中：Rung Collapse在推理增强模型中持续存在(GPT-5.2为3.7%)；可操纵性呈现逆缩放现象，先进模型抵抗通用修正；针对性ERM反馈恢复53-59%的顽固错误，而结果级反馈失败。理论证明渐近恢复真实干预分布。

Conclusion: 论文形式化了Rung Collapse作为机器学习系统"正确但理由错误"的因果根源，提出ERM作为有效的因果信念修正方法，通过三层架构和理论证明解决了Aleatoric Entrenchment问题，为构建鲁棒的因果推理系统提供了理论基础和实践框架。
...
TL;DR: 该论文提出Vector-to-Graph（V2G）管道，将CAD工程图转换为属性图以解决MLLMs的结构盲问题，在电气合规检查基准上取得显著精度提升。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在视觉理解方面取得显著进展，但存在关键限制：结构盲。即使最先进的模型也无法捕捉工程示意图中的拓扑和符号逻辑，因为其像素驱动范式丢弃了推理所需的显式向量定义关系。

Method: 提出Vector-to-Graph（V2G）管道，将CAD图表转换为属性图，其中节点表示组件，边编码连接性，使结构依赖关系显式化且可机器审计。

Result: 在电气合规检查的诊断基准上，V2G在所有错误类别上都取得了大幅精度提升，而领先的MLLMs仍接近随机水平。这些结果突显了基于像素方法的系统性不足。
...
TL;DR: ThinkRouter是一种推理时置信度感知路由机制，在模型置信度低时路由到离散token空间，置信度高时路由到潜在空间，以提高推理效率和准确性


<details>
  <summary>Details</summary>
Motivation: 现有潜在推理方法在不同设置下效果不一，分析发现错误答案的思考轨迹包含更少的低置信度步骤，而多个低置信度思考替代方案的软嵌入会引入和传播噪声，导致对不可靠推理轨迹的高置信度
...
Conclusion: ThinkRouter能够校准显式CoT和潜在推理产生的错误，并通过全局降低模型置信度加速思考结束token的生成，实现高效准确的推理
...
<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法主要依赖参数空间启发式方法，常导致严重干扰，造成泛化能力下降和不稳定生成行为（如重复、不连贯输出）

Method: SCF-RKL框架采用稀疏、分布感知的更新方式，使用反向Kullback-Leibler散度测量模型间的功能差异，选择性整合互补参数，通过模式寻找和稀疏诱导设计保持稳定表示

Result: 在多种模型规模和架构上评估，涵盖推理和指令调优模型，在24个基准测试（高级推理、通用推理与知识、指令遵循、安全性、视觉分类）上一致优于现有模型融合方法

Conclusion: SCF-RKL通过显式控制功能干扰，有效保持稳定表示同时整合新能力，在模型融合中实现了更好的泛化能力和生成稳定性
...
TL;DR: 本文提出了Text2GQL-Bench，一个统一的文本到图查询语言基准测试，包含178,184个问题-查询对，覆盖13个领域，并提供了全面的评估方法。
...
Method: 提出了Text2GQL-Bench基准，包含一个多图查询语言数据集和可扩展的构建框架，能够生成不同领域、问题抽象层次和图查询语言的数据集。同时引入了综合评估方法，报告语法有效性、相似性、语义对齐和执行准确性。

Result: 评估揭示了ISO-GQL生成的显著方言差距：即使在零样本设置下，强大的LLM最多只能达到4%的执行准确性，而固定的3-shot提示可将准确性提升至约50%，但语法有效性仍低于70%。微调的8B开放权重模型达到45.1%执行准确性和90.8%语法有效性，表明大部分性能提升可通过暴露足够ISO-GQL示例实现。

Conclusion: Text2GQL-Bench为文本到图查询语言系统提供了统一的基准测试，揭示了当前模型在生成ISO-GQL查询时的局限性，并展示了通过示例学习和微调可以显著提升性能，为未来研究提供了重要参考。
...
TL;DR: 论文提出了AIR框架，这是首个针对LLM智能体系统的应急响应框架，通过集成到智能体执行循环中实现事故检测、遏制、恢复和根除，显著提升智能体安全性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体安全机制主要关注事前预防，缺乏事故发生后响应、遏制和恢复的能力。随着LLM智能体在自主应用中的广泛部署，需要建立系统化的事故响应机制来应对不可避免的事故。

Method: 提出AIR框架：1）定义用于自主管理LLM智能体系统事故响应生命周期的领域特定语言；2）将框架集成到智能体执行循环中，实现基于环境状态和上下文的事故语义检测；3）引导智能体通过工具执行遏制和恢复操作；4）在根除阶段合成防护规则以阻止类似事故在未来发生。

Result: 在三种代表性智能体类型上评估，AIR在检测、修复和根除方面的成功率均超过90%。实验验证了关键设计组件的必要性，展示了AIR的及时性和适度开销，并证明LLM生成的规则在不同领域可接近开发者编写规则的效果。

Conclusion: 事故响应作为提升智能体安全的一等机制既可行又必要。AIR框架为LLM智能体系统提供了系统化的事故响应能力，填补了当前安全机制的空白。
...
TL;DR: TSR是一种训练时方法，通过轻量级树搜索构建高质量轨迹，提升多轮强化学习性能


<details>
  <summary>Details</summary>
Motivation: 多轮强化学习中奖励稀疏或延迟、环境随机性强，传统轨迹采样容易导致利用不足和模式崩溃，需要改进训练时的轨迹生成质量

Method: 提出TSR方法，在训练时采用轻量级树搜索（如best-of-N、beam search、浅层前瞻搜索）构建高质量轨迹，使用任务特定反馈选择高分动作，保持优化目标不变且与优化器无关

Result: 在Sokoban、FrozenLake和WebShop任务上，结合PPO和GRPO实现高达15%的性能提升，学习过程更稳定，仅需一次性增加训练计算成本
...
TL;DR: 提出MaxExp和SSE两种方法，用于将物种分布模型(SDMs)的概率预测转换为二元存在-缺失图，解决传统启发式阈值化方法导致的物种丰度和群落组成估计失真问题。


<details>
  <summary>Details</summary>
Motivation: 物种分布模型通常产生概率性出现预测，需要转换为二元存在-缺失图用于生态推断和保护规划。传统的二值化步骤通常是启发式的，会显著扭曲物种丰度和群落组成的估计。需要一种更稳健、可重复的多物种SDM二值化方法。

Method: 提出MaxExp框架，通过直接最大化选定评估指标来选择最可能的物种组合，无需校准数据且适用于多种评分指标。同时提出Set Size Expectation (SSE)方法，基于预期物种丰富度预测组合，计算效率更高。

Result: 通过三个涵盖不同分类群、物种数量和性能指标的案例研究，显示MaxExp始终匹配或超越广泛使用的阈值化和校准方法，特别是在强类别不平衡和高稀有性条件下。SSE提供了更简单但仍具竞争力的替代方案。

Conclusion: MaxExp和SSE方法为多物种SDM二值化提供了稳健、可重复的工具，能够改善物种丰度和群落组成的估计，特别适用于保护规划和生态推断中的实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有工业系统采用两阶段范式（先生成文本，再与点击率等指标对齐），导致优化目标不一致、漏斗效率低，限制了全局最优性

Method: 提出RELATE框架：基于强化学习的端到端框架，通过策略学习将性能和合规目标直接集成到生成过程中；将转化导向指标与合规约束作为多维奖励进行联合建模

Result: 大规模工业数据集实验显示RELATE持续优于基线；在线部署在广告生产平台中，在严格政策约束下实现了点击转化率的显著提升

Conclusion: RELATE通过统一生成与目标对齐，有效解决了现有两阶段系统的局限性，在实际广告平台中验证了其鲁棒性和有效性
...
Result: 通过FlowBench数据集上的广泛实验表明，该方法优于现有方法，能够可靠地将自由形式的LLM推理落地为结构化工作流。

Conclusion: ES框架通过解耦任务执行与工作流构建，显著提高了工作流的准确性和鲁棒性，为将LLM推理转化为结构化工作流提供了可靠的范式。
...
TL;DR: LAVES是一个基于LLM的分层多智能体系统，用于从教育问题生成高质量教学视频，通过分解工作流程和结构化可执行脚本实现自动化生产，相比行业标准方法成本降低95%以上。
...
Method: LAVES将教学视频生成分解为由中央编排智能体协调的专门智能体工作流程：解决方案智能体负责严谨问题求解，插图智能体生成可执行可视化代码，旁白智能体创建面向学习者的教学脚本。所有输出都经过语义批判、基于规则的约束和基于工具的编译检查，系统构建结构化可执行视频脚本，通过模板驱动的组装规则确定性地编译成同步的视觉和旁白。

Result: 在大规模部署中，LAVES实现了每天超过100万个视频的吞吐量，与当前行业标准方法相比，成本降低了95%以上，同时保持了高接受率。

Conclusion: LAVES通过分层多智能体系统和结构化可执行脚本的方法，成功解决了教学视频生成中的逻辑严谨性、成本效益和可控性问题，实现了高质量、低成本、自动化的教学视频生产。
...
TL;DR: 论文提出Min-$k$NN Distance方法，用于检测RLVR训练中的基准污染，通过量化生成文本的多样性差异来区分RL训练中见过的和未见过的提示。


<details>
  <summary>Details</summary>
Motivation: RLVR训练中未公开的训练数据引发基准污染担忧，传统基于似然度的检测方法对RLVR无效，因为RLVR基于奖励反馈而非token级概率进行微调。

Method: 提出Min-$k$NN Distance黑盒检测器：对给定提示采样多个补全，计算k个最小最近邻编辑距离的平均值，量化生成多样性。该方法无需访问参考模型或token概率。

Result: 在多个RLVR训练推理模型上的实验表明，Min-$k$NN Distance能可靠区分RL见过的和未见过的示例，优于现有的成员推理和RL污染检测基线方法。

Conclusion: RLVR训练产生独特行为特征：训练中见过的提示导致更僵化、相似的生成，而未见提示保持更大多样性。Min-$k$NN Distance能有效检测这种模式，为RLVR基准污染提供实用检测工具。
...
Method: 提出DTAPP-IICR框架：1）基于紧急程度对任务进行优先级排序生成初始解；2）使用SFIPP-ST（具有软约束和时间约束的安全飞行间隔路径规划）计算往返轨迹，处理异构无人机、强制执行临时禁飞区，并将智能体间冲突建模为软约束；3）通过几何冲突图引导的迭代大邻域搜索解决剩余冲突；4）采用完整性保持的方向性剪枝技术加速3D搜索。

Result: 在具有临时禁飞区的基准测试中，DTAPP-IICR在多达1000架无人机的机队上实现接近100%的成功率，通过剪枝获得高达50%的运行时间减少，在无人机交通管理环境中优于批量增强冲突基搜索。在城市规模的实际操作中成功扩展，而其他基于优先级的方法在中等部署规模下即失败。

Conclusion: DTAPP-IICR为密集动态城市空域中的预飞行规划提供了一个实用且可扩展的解决方案，在现实城市规模操作中表现出卓越的可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 多模态推荐面临两个主要挑战：1）现有分词方法（如RQ-VAE）缺乏跨模态共享语义与模态特定细节的解耦，导致冗余或坍缩；2）标准Transformer将语义ID视为扁平流，忽略了用户交互、物品和token的层次结构，多token扩展会放大噪声并偏向局部细节而非整体语义。

Method: 提出Hi-SAM框架，包含两个核心设计：1）解耦语义分词器（DST）：通过几何感知对齐统一模态，采用从粗到细策略进行量化，共享码本提取共识，模态特定码本从残差中恢复细节，通过互信息最小化强制解耦；2）分层记忆锚点Transformer（HMAT）：通过分层RoPE将位置编码拆分为物品间和物品内子空间以恢复层次结构，插入锚点token将物品压缩为紧凑记忆，保留当前物品细节的同时仅通过压缩摘要访问历史。

Result: 在真实世界数据集上相比SOTA基线获得持续改进，尤其在冷启动场景表现突出。在大规模社交平台部署服务数百万用户，核心在线指标获得6.55%提升。

Conclusion: Hi-SAM通过解耦语义表示和分层架构设计有效解决了多模态推荐中的语义冗余和结构不匹配问题，在实际部署中验证了其有效性，特别适用于冷启动和长序列推荐场景。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率（≤0.25°）集合天气预报中，潜在扩散模型（LDMs）存在扩散能力有限的问题。气象场缺乏任务无关的基础模型和明确的语义结构，无法应用VFM-based正则化。现有频率方法在同质性假设下对所有通道施加相同的频谱正则化，导致多元气象数据中变量间频谱异质性下的正则化强度不均。

Method: 提出3D掩码自编码器（3D-MAE）编码天气状态演化特征作为扩散模型的额外条件，结合变量感知掩码频率建模（VA-MFM）策略，根据每个变量的频谱能量分布自适应选择阈值。整体框架称为PuYun-LDM。

Result: PuYun-LDM增强了潜在扩散能力，在短预报时效上优于ENS（集合预报系统），在长预报时效上保持可比性能。可在单个NVIDIA H200 GPU上5分钟内生成15天全球预报（6小时时间分辨率），并能高效并行生成集合预报。

Conclusion: 通过3D-MAE和VA-MFM策略，PuYun-LDM有效解决了高分辨率集合天气预报中LDMs的扩散能力限制问题，实现了高效准确的天气预测，为气象AI模型提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: LLM服务和强化学习采样中序列长度的长尾分布导致批量推理中大量填充计算浪费；现有基于辅助模型的静态长度预测方法开销大、泛化差，无法处理随机"一对多"采样场景

Method: 提出轻量级框架，包含两个核心组件：1) 熵引导令牌池化(EGTP)，利用实时激活和令牌熵进行高精度静态预测；2) 渐进长度预测(PLP)，在解码步骤中动态估计剩余长度以处理随机生成。构建并发布ForeLen基准数据集

Result: EGTP在ForeLen基准上达到最先进精度，MAE比最佳基线降低29.16%；与长度感知调度器集成后显著提升端到端吞吐量

Conclusion: 该工作为高效LLM推理提供了新的技术和评估基准，通过重用主模型内部隐藏状态实现高效长度预测，解决了现有方法的局限性
...
Result: ProtoT在计算可扩展性方面表现优异，序列长度呈线性扩展（vs SOTA自注意力变换器的二次扩展）。在模型和数据规模扩展方面表现良好，在文本生成和下游任务（GLUE）上表现良好。对输入扰动的鲁棒性达到或优于某些基线，同时提供可解释的路径显示鲁棒性和敏感性的来源。
...
TL;DR: 本文提出Talk2DM系统，为动态地图增加自然语言查询和常识推理能力，通过VRCsim仿真框架生成数据，构建VRC-QA数据集，并基于链式提示机制实现高精度查询


<details>
  <summary>Details</summary>
Motivation: 现有动态地图系统缺乏自然语言支持的人机交互界面，限制了人-动态地图交互的便利性和效率，需要为车路云协同自动驾驶系统提供更智能的查询和推理能力

Method: 1) 开发VRCsim仿真框架生成流式车路云协同感知数据；2) 构建VRC-QA空间查询推理数据集；3) 提出Talk2DM插件模块，基于链式提示机制整合人工规则与LLM常识知识

Result: Talk2DM在不同LLM上均能保持高精度自然语言查询（超过93%准确率），响应时间仅2-5秒，Qwen3:8B、Gemma3:27B和GPT-oss模型表现优异，但大模型存在效率下降问题

Conclusion: Talk2DM成功为动态地图系统增加了自然语言查询和常识推理能力，展示了强大的泛化能力和实际应用潜力，为车路云协同自动驾驶提供了更智能的人机交互解决方案
...
TL;DR: 提出分层稀疏自编码器（HSAE），用于从大语言模型中提取层次化的语义特征结构，解决传统稀疏自编码器只能孤立识别特征的问题。


<details>
  <summary>Details</summary>
Motivation: 传统稀疏自编码器（SAE）只能从大语言模型中提取孤立的单义特征，但大量证据表明语言模型捕捉了自然语言的内在结构，特别是"特征分裂"现象表明这种结构是层次化的。需要一种方法来捕获这种层次结构。

Method: 提出分层稀疏自编码器（HSAE），联合学习一系列稀疏自编码器及其特征之间的父子关系。通过两种新机制增强父子特征的对齐：结构约束损失和随机特征扰动机制。

Result: 在各种大语言模型和层上的广泛实验表明，HSAE能够一致地恢复具有语义意义的层次结构，得到定性案例研究和严格定量指标的支持。同时，HSAE在不同字典大小下保持了标准稀疏自编码器的重建保真度和可解释性。

Conclusion: HSAE为发现和分析嵌入在大语言模型表示中的多尺度概念结构提供了一个强大、可扩展的工具，能够捕捉语言模型的层次化语义特征。
...
TL;DR: 论文提出选择性抽象(SA)框架，通过降低不确定内容的细节来平衡LLM输出的准确性与信息量，避免传统"全有或全无"弃权机制的过度限制。


<details>
  <summary>Details</summary>
Motivation: LLM存在事实性错误问题，传统不确定性估计采用二元弃权机制，在长文本生成场景中过于严格，会丢弃大量有价值信息。需要一种更灵活的机制，在不确定时降低细节而非完全弃权。

Method: 提出选择性抽象(SA)框架，将风险与覆盖率形式化。具体实现原子级选择性抽象(Atom-wise SA)：将回答分解为原子声明（简短、自包含的单个事实陈述），用更高置信度但更抽象的表述替换不确定的原子声明。

Result: 在FactScore和LongFact-Objects基准测试的六个开源模型上，原子级SA始终优于现有基线，风险-覆盖率曲线下面积(AURC)相比声明移除方法提升高达27.73%，表明降低特异性能在保留大部分原始含义的同时提升准确性和可靠性。

Conclusion: 选择性抽象框架为LLM不确定性管理提供了更灵活的解决方案，通过权衡特异性与可靠性，在长文本生成场景中比传统二元弃权机制更有效，能保留更多有价值信息。
...
TL;DR: AlphaPROBE框架将alpha因子挖掘重构为有向无环图导航问题，通过贝叶斯因子检索器和DAG感知因子生成器实现全局进化拓扑的利用，显著提升预测精度、收益稳定性和训练效率。
...
Conclusion: AlphaPROBE通过将因子池建模为动态互连生态系统，并采用贝叶斯检索和DAG感知生成策略，解决了现有alpha挖掘方法的局限性，实现了更高效、多样且稳健的自动化因子发现。
...
TL;DR: MEME框架通过建模金融市场中的竞争性投资叙事（思维模式）来重构市场动态，将逻辑导向视角应用于投资组合构建，在中国股票池实验中超越了7个SOTA基线方法。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在量化金融中的应用主要遵循资产中心范式（个股预测）或市场中心范式（投资组合分配），但往往忽略了驱动市场变动的底层推理逻辑。本文提出逻辑导向视角，将金融市场建模为竞争性投资叙事（思维模式）的动态演化生态系统。

Method: 提出MEME框架：1）多智能体提取模块将噪声数据转化为高保真投资论点；2）高斯混合建模在语义空间中揭示潜在共识；3）时间评估和对齐机制跟踪思维模式的生命周期和历史盈利能力；4）优先考虑持久市场智慧而非短暂异常，确保投资组合构建基于稳健推理。

Result: 在2023-2025年三个异质性中国股票池上的广泛实验表明，MEME在7个SOTA基线方法中表现一致更优。消融研究、敏感性分析、生命周期案例研究和成本分析验证了MEME识别和适应金融市场演化共识的能力。

Conclusion: MEME通过建模金融市场中竞争性投资叙事的动态演化，提供了一种逻辑导向的投资框架，能够识别和适应市场共识变化，在投资组合构建中超越了现有方法，强调了基于稳健推理而非短暂异常的重要性。
...
TL;DR: Gaia2是一个用于评估LLM智能体在真实异步环境中的基准测试平台，引入独立于智能体动作的环境演化、时间约束、动态事件等挑战，支持细粒度动作级评估和强化学习。


<details>
  <summary>Details</summary>
Motivation: 现有评估基准多为静态或同步环境，无法反映真实世界中环境独立演化、时间约束、动态事件等复杂情况，需要新的基准测试来评估智能体在实际异步环境中的能力。

Method: 基于开源Agents Research Environments平台构建消费者环境，设计异步场景（环境独立演化、时间约束、动态事件、模糊性、多智能体协作），每个场景配备写操作验证器，支持动作级评估和可验证奖励的强化学习。

Result: 评估显示：GPT-5（高级版）总体得分最高（42% pass@1）但在时间敏感任务上失败；Claude-4 Sonnet在准确性和速度间权衡以降低成本；Kimi-K2在开源模型中领先（21% pass@1）。结果揭示了推理、效率、鲁棒性之间的基本权衡和"仿真到现实"差距的挑战。

Conclusion: Gaia2为社区提供了灵活的基础设施，用于开发、基准测试和训练下一代实用智能体系统，通过开源ARE框架和Gaia2基准，促进实际异步环境中智能体能力的研究和发展。
...
TL;DR: 提出CSEval框架，利用语言模型评估生成医学图像与提示之间的临床语义对齐，弥补现有方法仅关注图像真实性而忽略临床语义的不足。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成在医学领域应用日益广泛，但现有评估方法主要关注图像真实性和多样性，无法评估生成图像是否准确反映临床语义（如解剖位置和病理特征）。需要一种能够评估临床语义对齐的方法来支持生成模型在医疗领域的可靠应用。

Method: 提出临床语义评估器（CSEval）框架，利用语言模型评估生成图像与条件提示之间的临床语义对齐。该框架通过语言模型分析图像内容和提示的语义一致性。

Result: 实验表明CSEval能够识别其他指标忽略的语义不一致性，并与专家判断具有相关性。该框架为现有评估方法提供了可扩展且具有临床意义的补充。

Conclusion: CSEval为生成模型在医疗领域的评估提供了临床语义对齐的评估框架，支持生成模型在医疗领域的安全应用，是现有评估方法的重要补充。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究虽然通过调整提示来增强推理，但这些设计主要基于直觉，缺乏对底层行为模式的系统性分析。作者旨在从行为模式角度研究模型推理行为如何塑造推理过程。

Method: 提出两种无需参数更新的优化方法：1) InjectCorrect：通过模仿模型自身过去正确答案的行为模式来引导模型；2) InjectRLOpt：从历史行为模式数据学习价值函数，并通过提出的可靠性感知Softmax策略在推理时生成行为注入物来引导推理过程。

Result: 实验表明两种方法都能在各种推理任务上提升模型性能，分别获得最高5.34%和8.67%的性能增益，且无需修改模型参数。

Conclusion: 模型在回答特定类型问题时表现出适应性的推理行为分布，结构性注入这些模式能显著影响推理过程和结果质量。提出的方法通过利用模型自身行为模式有效提升了推理性能。
...
TL;DR: LawThinker是一个自主法律研究智能体，采用探索-验证-记忆策略，通过在每一步知识探索后进行原子级验证，确保法律推理过程的程序合规性。


<details>
  <summary>Details</summary>
Motivation: 现有法律推理方法缺乏对中间推理步骤的验证机制，导致不适用法规引用等错误在推理链中传播而未被检测。需要确保法律推理不仅结果正确，而且推理过程符合程序合规要求。

Method: 提出LawThinker自主法律研究智能体，采用Explore-Verify-Memorize策略。核心是在每个知识探索步骤后强制执行验证操作。DeepVerifier模块从知识准确性、事实-法律相关性和程序合规性三个维度检查每个检索结果，并使用记忆模块在多轮任务中实现知识复用。

Result: 在动态基准J1-EVAL上，LawThinker相比直接推理方法提升24%，相比基于工作流的方法提升11%，在面向过程的指标上表现尤为突出。在三个静态基准上的评估进一步证实了其泛化能力。

Conclusion: LawThinker通过探索-验证-记忆策略和原子级验证机制，有效解决了法律推理中中间步骤验证缺失的问题，显著提升了推理的准确性和程序合规性，在动态和静态法律环境中都表现出良好的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI模型虽然拥有成熟的数据库和检索系统（类似哈利·波特中的冥想盆），但缺乏主动操作这些记忆工具的能力，只能被动接受人工设计的上下文作为全部记忆，限制了模型在长上下文和复杂任务中的表现。

Method: 提出StateLM模型，赋予其内部推理循环来管理自身状态。配备记忆工具套件（上下文修剪、文档索引、笔记记录等），训练模型主动管理这些工具，学习动态工程化自己的上下文，从而突破固定窗口的架构限制。

Result: 在不同模型规模上均表现出色：在长文档QA任务中，StateLM在所有模型规模上都优于标准LLM；在聊天记忆任务中，绝对准确率提升10%-20%；在深度研究任务BrowseComp-Plus上，StateLM达到52%准确率，而标准LLM仅约5%，性能差距显著。

Conclusion: StateLM将LLM从被动预测器转变为状态感知的智能体，使推理成为有状态且可管理的过程，为AI模型赋予了主动管理记忆和上下文的能力。
...
TL;DR: ARLCP提出自适应反射和长度协调惩罚的强化学习框架，动态平衡推理效率与准确性，显著减少推理长度同时提升精度


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务中常生成过长的思维链，包含大量重复自我提问和循环推理等不必要的反思步骤，导致高令牌消耗、计算开销增加和延迟上升，但并未提高准确性，特别是在较小模型中。问题复杂度增加会引发更多过度且不必要的反思，从而降低准确性并增加令牌开销。

Method: 提出自适应反射和长度协调惩罚（ARLCP）强化学习框架，包含两个关键创新：1）反射惩罚：自适应地削减不必要的反思步骤，同时保留必要的推理；2）长度惩罚：根据问题估计复杂度进行校准。通过协调这两种惩罚，鼓励模型生成更简洁有效的推理路径。

Result: 在五个数学推理基准上使用DeepSeek-R1-Distill-Qwen-1.5B和7B模型进行评估。1.5B模型平均响应长度减少53.1%，同时准确性提高5.8%；7B模型长度减少35.0%，准确性提高2.7%。相比现有方法实现了更优的效率-准确性权衡。

Conclusion: ARLCP能有效解决大型推理模型中过度反思导致的效率问题，通过动态平衡推理效率与解决方案准确性，显著减少推理长度同时提升精度，为推理模型的效率优化提供了新思路。
...
TL;DR: 提出Hadamard线性注意力（HLA），通过哈达玛积计算相似度后应用非线性，相比传统线性注意力能更好地近似softmax，无需张量重塑，在视频生成扩散模型中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 标准注意力机制计算复杂度高（O(n²)），线性注意力作为高效近似但使用低阶有理函数近似softmax，限制了表达能力。需要一种既能保持线性复杂度又能更好近似softmax的注意力机制。

Method: 提出Hadamard线性注意力（HLA）：1）不像传统线性注意力在查询和键上独立应用核函数，而是在计算成对相似度后应用非线性；2）使用哈达玛积计算相似度；3）推导出高效计算方案，复杂度与标准线性注意力相似；4）无需张量重塑操作。

Result: HLA能实现更高阶的有理函数来近似softmax，在视频生成的扩散transformer模型中验证了有效性，该应用涉及大量token处理。

Conclusion: HLA在保持线性计算复杂度的同时，通过后相似度非线性化更好地近似softmax，无需额外张量操作，在大规模视频生成任务中表现出优越性。
...
TL;DR: 该研究量化了两种商业图像生成器（Gemini Flash 2.5 Image和GPT Image 1.5）中的性别和肤色偏见，发现中性提示会产生高度极化的默认输出，两种模型均表现出强烈的"默认白人"偏见，但在性别偏好上存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 测试"中性提示会产生人口统计学中性输出"的假设，量化商业图像生成器中存在的性别和肤色偏见，为算法视觉文化的审计提供方法论框架。

Method: 使用四个语义中性提示生成3200张逼真图像，采用混合色彩归一化、面部标志点掩蔽和感知均匀肤色量化（使用Monk、PERLA和Fitzpatrick量表）的严格分析流程，区分审美渲染与底层色素沉着。

Result: 中性提示产生了高度极化的默认输出：两种模型均表现出强烈的"默认白人"偏见（>96%输出），但在性别偏好上存在显著差异——Gemini偏好女性呈现主体，GPT偏好肤色较浅的男性呈现主体。

Conclusion: 中性提示作为诊断探针而非中性指令，挑战了"未标记语言导致包容性表征"的社会语言学假设，为审计算法视觉文化提供了稳健框架，揭示了商业图像生成器中存在的系统性偏见。
...
Method: 引入价值对齐税(VAT)框架，测量对齐诱导的变化如何在相互关联的价值中传播；基于Schwartz价值理论构建受控情境-行动数据集，收集对齐前后的成对规范性判断，分析不同模型、价值和策略下的对齐效应

Result: 对齐通常产生不均衡、结构化的价值共变；这些效应在传统仅针对目标的评估中不可见，揭示了系统性和过程层面的对齐风险

Conclusion: VAT框架为理解LLM中价值对齐的动态性提供了新视角，揭示了传统评估方法无法捕捉的系统性风险
...
TL;DR: STAR框架通过结合统计期望与知识驱动的智能推理，从有限观测中预测大模型性能，解决了现有方法在模式偏移、数据稀疏和缺乏解释性方面的不足。
...
Method: STAR框架结合数据驱动的统计期望与知识驱动的智能推理：1) 使用专业检索器收集外部知识，将语义特征嵌入约束概率矩阵分解(CPMF)生成带不确定性的统计期望；2) 基于期望违背理论(EVT)的推理模块通过族内分析、跨模型比较和可信度感知聚合来优化预测，提供可追溯的解释。

Result: 实验表明STAR在所有基线上均表现优异，在极端稀疏条件下（每个测试模型仅1-2个观测分数）比最强统计方法总得分提升14.46%。

Conclusion: STAR框架有效解决了大模型性能预测中的模式偏移、数据稀疏和解释性不足问题，通过统计与推理的结合实现了更准确、可解释的预测。
...
TL;DR: 提出基于强化学习和T5语言模型的创新无损压缩方法，将数据压缩为token序列而非传统向量表示，显著提升压缩率


<details>
  <summary>Details</summary>
Motivation: 传统压缩方法难以充分利用复杂数据格式的结构和冗余，而现有深度学习方法依赖密集向量表示，掩盖了底层token结构。需要一种既能保持token结构又能实现高效压缩的新方法

Method: 采用强化学习训练T5语言模型架构，将数据压缩为token序列而非连续潜在空间表示。使用离策略强化学习算法优化序列长度，最小化冗余并提升压缩效率。系统独立于外部语法或世界知识

Result: 相比传统方法，该方法在压缩率方面取得显著改进。通过利用语言模型中的潜在信息，系统能够有效压缩数据而无需显式内容理解

Conclusion: 该方法为各种应用提供了更强大和实用的压缩解决方案，通过保持token结构实现更高的压缩率，同时维持语义完整性
...
TL;DR: LLMs在ToM基准测试中表现良好，但缺乏真正的心智理论所需的因果心理状态模型，在逻辑等效任务中失败且行为预测与心理状态推断不一致。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要评估LLMs在ToM基准测试中的表现，但这些测试并未检验ToM理论所假设的实际表征——即心理状态与行为的因果模型。需要基于认知科学定义开发新的评估框架来真正测试LLMs是否具备心智理论。

Method: 采用基于认知科学的心智理论定义，开发新的评估框架，测试LLMs是否具备连贯、领域通用且一致的心理状态如何导致行为的模型。具体包括：1) 在简单ToM范式中近似人类判断；2) 测试逻辑等效任务；3) 检验行为预测与相应心理状态推断的一致性。

Result: 研究发现：1) LLMs在简单ToM范式中能够成功近似人类判断；2) 但在逻辑等效任务中失败；3) 行为预测与相应心理状态推断之间的一致性较低。这表明LLMs的社会能力并非源于领域通用或一致的心智理论。

Conclusion: LLMs虽然在各种社会任务中表现出熟练度，但这种社会能力并不是基于真正的心智理论——即连贯、领域通用且一致的心理状态因果模型。需要更严格的评估方法来区分表面社会能力与真正的心理状态理解。
...
Conclusion: Sci-CoE通过两阶段协同演化框架有效解决了科学推理中的评估不可靠和多样性不足问题，为构建更强大的科学推理系统提供了可行方案。
...
TL;DR: 提出了一种基于教育原理的LLM知识蒸馏框架IOA，通过三阶段管道（知识识别、组织、适配）实现系统化教学，相比基线方法在推理任务上取得显著提升。
...
Method: 提出IOA三阶段框架：1) 知识识别器识别学生模型的知识缺陷；2) 知识组织器通过渐进课程组织知识传递；3) 知识适配器调整表示以匹配学生认知能力。整合布鲁姆掌握学习原则和维果茨基最近发展区理论，创建动态蒸馏过程。

Result: 使用LLaMA-3.1/3.2和Qwen2.5作为学生模型，IOA在DollyEval上保留教师模型94.7%的性能，参数量不到1/10。在复杂推理任务上表现突出：MATH提升19.2%，HumanEval提升22.3%，优于最先进基线方法。

Conclusion: 基于教育原理的IOA框架为LLM知识蒸馏提供了系统化的教学意识方法，显著提升了小模型的知识获取效率和推理能力，为高效AI系统部署提供了新思路。
...
Method: 首先对404,796个真实提示进行大规模解剖分析，发现严重冗余：上下文窗口利用不足、词汇使用高度稀疏、文本嵌入位于低维流形。基于此提出SAM3-LiteText，用紧凑的MobileCLIP学生模型替换原始SAM3文本编码器，并通过知识蒸馏优化

Result: 在图像和视频分割基准测试中，SAM3-LiteText将文本编码器参数减少高达88%，显著降低静态内存占用，同时保持与原始模型相当的分割性能

Conclusion: 针对视觉语言分割任务的文本提示特性，可以通过轻量级文本编码框架显著减少模型复杂度，为部署高效分割模型提供了实用解决方案
...
TL;DR: 该研究发现语音识别系统在真实世界高风险的短语音转录任务中存在严重问题，特别是美国街道名称的转录错误率高达44%，对非英语母语者的影响更大，并提出了一种基于合成数据的改进方法。


<details>
  <summary>Details</summary>
Motivation: 尽管语音识别系统在标准基准测试中取得了低词错误率，但在真实世界部署中，特别是在高风险、短语音的转录任务中经常失败。本研究旨在探究这一失败模式，重点关注美国街道名称的转录问题，并评估其对不同语言背景使用者的影响。

Method: 研究评估了来自OpenAI、Deepgram、Google和Microsoft的15个模型，使用来自美国不同语言背景参与者的录音进行测试。为了缓解转录错误，提出了一种合成数据生成方法，使用开源文本转语音模型生成命名实体的多样化发音，并用不到1000个合成样本进行微调。

Result: 研究发现平均转录错误率高达44%。转录错误对所有说话者都造成系统性错误，但对非英语母语者的路由距离错误是英语母语者的两倍。通过合成数据微调，街道名称转录准确率对非英语母语者提高了近60%（相对于基础模型）。

Conclusion: 研究结果揭示了语音系统在基准测试表现和真实世界可靠性之间的关键差距，展示了一条简单、可扩展的路径来减少高风险转录错误。合成数据方法能显著改善对非英语母语者的转录准确性，有助于减少系统性偏见。
...
TL;DR: KeplerAgent是一个基于大语言模型的智能体框架，通过模拟科学家的多步推理过程（先推断物理性质如对称性，再将其作为先验约束方程空间）来发现符号方程，相比直接猜测的方法在准确性和抗噪性上表现更好。
...
Conclusion: 通过明确建模科学家的多步推理过程（先推断物理性质再约束方程空间），KeplerAgent框架在符号方程发现任务上超越了直接猜测的方法，为LLM在科学发现中的应用提供了更有效的范式。
...
### [184] [CM2: Reinforcement Learning with Checklist Rewards for Multi-Turn and Multi-Step Agentic Tool Use](https://arxiv.org/abs/2602.12268)
...
<details>
  <summary>Details</summary>
Motivation: 现实任务中强化学习面临挑战：真实目标缺乏可验证奖励而强调开放式行为；多轮多步工具使用的RL研究不足；构建和维护可执行工具环境成本高，限制了规模和覆盖范围。

Method: CM2框架将每轮预期行为分解为细粒度二元标准，采用稀疏奖励分配但密集评估标准的策略，在可扩展的LLM模拟工具环境中训练，避免大规模工具集的重工程。

Result: 从8B基础模型开始，在8k示例RL数据集上训练，CM2相比SFT对应模型在tau^-Bench上提升8点，BFCL-V4上提升10点，ToolSandbox上提升12点，匹配甚至优于类似规模开源基线。

Conclusion: CM2为优化多轮多步工具使用智能体提供了可扩展方案，无需依赖可验证奖励，通过清单奖励和LLM模拟环境有效解决了RL在现实任务中的应用难题。
...
### [185] [Mitigating Error Accumulation in Continuous Navigation via Memory-Augmented Kalman Filtering](https://arxiv.org/abs/2602.11183)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航模型采用航位推算方式，通过迭代更新位置来预测下一个航点，这种方式会导致随时间累积的位置误差，造成内部信念与客观坐标之间的错位（状态漂移），最终损害完整轨迹预测的准确性。

Method: 提出NeuroKalman框架，将顺序预测建模为递归贝叶斯状态估计问题。框架将导航分解为两个互补过程：基于运动动力学的先验预测和基于历史观测的似然校正。通过数学关联核密度估计与基于注意力的检索机制，使系统能够在不进行梯度更新的情况下使用检索到的历史锚点来修正潜在表示。

Result: 在TravelUAV基准测试上的综合实验表明，仅使用10%的训练数据进行微调，该方法明显优于强基线，并有效调节了漂移累积。

Conclusion: NeuroKalman框架通过将经典控制理论中的递归贝叶斯状态估计引入视觉语言导航，有效解决了状态漂移问题，提高了无人机在复杂环境中连续导航的准确性和鲁棒性。
...
### [186] [H-WM: Robotic Task and Motion Planning Guided by Hierarchical World Model](https://arxiv.org/abs/2602.11291)
...
TL;DR: 提出分层世界模型（H-WM），统一预测逻辑和视觉状态转移，结合符号推理的机器人可执行性与视觉感知的接地性，解决长期任务中的误差累积问题。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型方法（如视频生成或自然语言预测）难以直接基于机器人动作，且在长期视野中误差累积严重。传统任务与运动规划依赖符号逻辑世界模型，虽机器人可执行且长期推理稳健，但缺乏视觉感知同步。需要结合两者优势的统一框架。

Method: 提出分层世界模型（H-WM）：高层逻辑世界模型预测符号状态转移，底层视觉世界模型预测视觉状态转移，构成双层统一框架。通过机器人运动与符号状态、动作、视觉观察对齐的数据集进行训练，为长期任务提供稳定中间指导。

Result: 在视觉-语言-动作（VLA）控制策略上的实验验证了方法的有效性和通用性。分层输出为长期任务提供稳定一致的中间指导，减轻误差累积，实现跨扩展任务序列的稳健执行。

Conclusion: H-WM成功整合了符号推理的机器人可执行性、长期稳健性与视觉感知的接地性，为机器人规划与控制提供了统一的双层世界模型框架，有效解决了长期任务中的误差累积问题。
...
### [187] [ExtremControl: Low-Latency Humanoid Teleoperation with Direct Extremity Control](https://arxiv.org/abs/2602.11321)
...
<details>
  <summary>Details</summary>
Motivation: 现有的人形机器人遥操作系统依赖复杂的全身运动重定向和位置控制，导致显著延迟（约200ms），限制了系统的响应性和快速反馈能力，无法完成需要快速反应的任务

Method: 提出ExtremControl框架：1) 直接在选定刚性连杆（主要是人形机器人末端）的SE(3)位姿上操作，避免全身重定向；2) 使用笛卡尔空间映射将人体运动直接转换为机器人连杆目标；3) 在底层控制中引入速度前馈控制，支持快速变化的控制接口下的高响应行为

Result: 实现了端到端延迟低至50ms的人形机器人遥操作系统，支持光学动作捕捉和VR运动跟踪，能够完成乒乓球平衡、抛接和实时返回等高响应性行为，显著超越了先前工作中200ms的延迟限制

Conclusion: ExtremControl框架通过简化控制流程和引入速度前馈，成功解决了人形机器人遥操作系统的延迟问题，为实现高响应性、动态的人机交互提供了有效解决方案
...
### [188] [MolmoSpaces: A Large-Scale Open Ecosystem for Robot Navigation and Manipulation](https://arxiv.org/abs/2602.11337)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器人基准测试无法充分代表真实环境中场景布局、物体几何和任务规范的巨大变化，物理评估无法提供所需规模和多样性的基础设施，需要支持大规模基准测试的开放生态系统。

Method: 构建包含超过23万个多样化室内环境（从手工制作的家庭场景到程序生成的多房间房屋）和13万个丰富标注物体资产（包括4.8万个可操作物体和4200万个稳定抓取）的仿真器无关生态系统。设计MolmoSpaces-Bench基准测试套件，包含8个任务，支持静态和移动操作、导航以及多房间长时程任务。

Result: MolmoSpaces-Bench表现出强大的仿真到现实相关性（R=0.96，ρ=0.98），确认新的零样本策略在基准测试中优于早期版本，并识别出对提示措辞、初始关节位置和相机遮挡的关键敏感性。

Conclusion: MolmoSpaces通过开源资产和工具为机器人学习研究提供了可扩展的数据生成、策略训练和基准创建的基础设施，支持大规模机器人策略评估和泛化能力测量。
...
### [189] [Human Preference Modeling Using Visual Motion Prediction Improves Robot Skill Learning from Egocentric Human Video](https://arxiv.org/abs/2602.11393)
...
<details>
  <summary>Details</summary>
Motivation: 现有从人类视频学习奖励的方法存在局限性：它们通常通过测量视觉状态与演示视频终止状态之间的时间距离来评估长期价值，这种方法在从视频学习时性能受限，且需要跨越本体和环境差距迁移学习到的价值函数。

Method: 方法包括：1) 通过预测跟踪点在连续图像间的运动来建模人类偏好；2) 将奖励函数定义为机器人行为中预测物体运动与观测物体运动在每个步骤的一致性；3) 使用改进的Soft Actor Critic (SAC)算法，基于10个机器人演示初始化，从该奖励估计价值函数并优化策略。

Result: 该方法能够在真实机器人上学习，在模拟和真实机器人上的多个任务中，使用该奖励模型学习的策略匹配或优于先前工作。

Conclusion: 通过建模人类偏好为基于运动一致性的奖励函数，并直接在机器人上优化策略，该方法能够有效从人类第一人称视角视频学习机器人技能，克服了先前方法的局限性。
...
### [190] [EasyMimic: A Low-Cost Framework for Robot Imitation Learning from Human Videos](https://arxiv.org/abs/2602.11464)
...
<details>
  <summary>Details</summary>
Motivation: 机器人模仿学习通常受限于大规模真实世界数据收集的高成本，这对于面向家庭使用的低成本机器人尤为突出，因为这些机器人需要既用户友好又经济实惠。
...
Conclusion: EasyMimic为将智能机器人引入家庭提供了一条实用路径，通过利用易于获取的人类视频演示来降低机器人学习成本。
...
### [191] [Effective Task Planning with Missing Objects using Learning-Informed Object Search](https://arxiv.org/abs/2602.11468)
...
<details>
  <summary>Details</summary>
Motivation: 现有任务规划方法（如PDDL）假设完全环境知识，无法处理任务关键对象位置未知的情况；而学习驱动的对象搜索方法作为独立工具，难以整合到需要确定必要对象及其搜索时机的完整任务规划中

Method: 开发基于模型的新型LIOS（Learning-Informed Object Search）动作框架：每个LIOS动作是一个寻找并检索单个对象的策略；高层规划将LIOS动作视为确定性动作，基于模型计算的预期成本生成计划，在不确定环境下实现搜索与执行的交错

Result: 在模拟的ProcTHOR家庭环境和真实世界中，该方法在检索和备餐等任务上优于非学习和学习基线方法

Conclusion: LIOS框架有效处理不确定性，同时保持与现有全知识求解器的兼容性，实现了有效、可靠且完整的基于学习信息的任务规划
...
### [192] [HyperDet: 3D Object Detection with Hyper 4D Radar Point Clouds](https://arxiv.org/abs/2602.11554)
*Yichun Xiao,Runwei Guan,Fangqiang Ding*

Main category: cs.RO

TL;DR: HyperDet是一个检测器无关的雷达3D检测框架，通过构建任务感知的超4D雷达点云，使标准激光雷达检测器能直接处理雷达数据，部分缩小了雷达与激光雷达的性能差距。
...
Method: 1) 多雷达多帧聚合提高覆盖和密度；2) 几何感知跨传感器一致性验证，在重叠区域外进行轻量级自一致性检查；3) 前景聚焦扩散模块，通过雷达-激光雷达混合监督训练，增强物体结构并提升雷达属性；4) 将模型蒸馏为一致性模型实现单步推理。

Result: 在MAN TruckScenes数据集上，HyperDet在VoxelNeXt和CenterPoint检测器上持续改进原始雷达输入性能，部分缩小了雷达与激光雷达之间的性能差距。

Conclusion: 输入级精炼使雷达能够更好地利用激光雷达导向的检测器而无需架构修改，证明了通过改进雷达点云质量可以提升雷达3D检测性能。
...
### [193] [ReaDy-Go: Real-to-Sim Dynamic 3D Gaussian Splatting Simulation for Environment-Specific Visual Navigation with Moving Obstacles](https://arxiv.org/abs/2602.11575)
*Seungyeon Yoo,Youngseok Jang,Dabin Kim,Youngsoo Han,Seungwoo Jung,H. Jin Kim*

Main category: cs.RO

TL;DR: ReaDy-Go提出了一种新颖的真实到仿真模拟管道，用于生成逼真的动态场景，以训练在动态环境中具有鲁棒性的视觉导航策略。


<details>
  <summary>Details</summary>
Motivation: 现有视觉导航模型在真实动态环境中表现不佳，主要面临两个问题：1) sim-to-real差距导致的鲁棒性不足；2) 难以针对特定部署环境（如家庭、餐厅、工厂）训练定制化策略。虽然基于3D高斯泼溅的真实到仿真模拟可以缓解sim-to-real差距，但先前工作仅假设静态场景或不切实际的动态障碍物，而动态环境中的安全导航至关重要。

Method: ReaDy-Go包含三个核心组件：1) 动态高斯泼溅模拟器，将场景GS与人体动画模块集成，支持插入可动画化的人体GS化身，并从2D轨迹合成合理的人体运动；2) 动态环境导航数据集生成，利用模拟器、为动态GS表示设计的机器人专家规划器和人体规划器；3) 使用生成的数据集进行策略学习。

Result: ReaDy-Go在仿真和真实世界实验中均优于基线方法，在目标环境中表现出改进的导航性能，即使在sim-to-real迁移和存在移动障碍物的情况下。此外，在未见环境中的零样本sim-to-real部署显示了其泛化潜力。

Conclusion: ReaDy-Go通过合成逼真的动态场景，有效解决了动态环境中视觉导航的sim-to-real差距问题，为真实世界动态环境中的鲁棒导航策略训练提供了有效解决方案。
...
### [194] [ABot-N0: Technical Report on the VLA Foundation Model for Versatile Embodied Navigation](https://arxiv.org/abs/2602.11598)
...
Method: 采用分层"大脑-动作"架构：基于LLM的认知大脑进行语义推理，结合基于流匹配的动作专家生成精确连续轨迹；开发ABot-N0数据引擎，收集1690万专家轨迹和500万推理样本；集成规划器和分层拓扑记忆的智能导航系统。

Result: 在7个基准测试中达到新的SOTA性能，显著优于专用模型；能够处理动态真实世界环境中的鲁棒长时程任务。

Conclusion: ABot-N0实现了具身导航任务的统一建模，通过大规模数据和分层架构取得了卓越性能，为智能导航系统的发展提供了重要基础。
...
### [195] [ViTaS: Visual Tactile Soft Fusion Contrastive Learning for Visuomotor Learning](https://arxiv.org/abs/2602.11643)
*Yufeng Tian,Shuiqi Cheng,Tianming Wei,Tianxing Zhou,Yuanhang Zhang,Zixian Liu,Qianwei Han,Zhecheng Yuan,Huazhe Xu*

Main category: cs.RO

TL;DR: ViTaS是一个结合视觉和触觉信息的机器人操作框架，通过Soft Fusion对比学习和CVAE模块利用多模态对齐与互补性，在遮挡场景中表现优异


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注视觉和触觉特征的对齐，且集成机制多为直接拼接，忽视了两种模态的固有互补性，对齐利用不足，难以有效处理遮挡场景，限制了实际部署潜力

Method: 提出ViTaS框架，引入Soft Fusion对比学习（传统对比学习方法的进阶版）和CVAE模块，充分利用视觉-触觉表示的对齐性和互补性

Result: 在12个模拟环境和3个真实世界环境中验证了方法的有效性，ViTaS显著优于现有基线方法

Conclusion: ViTaS是一个简单有效的视觉-触觉融合框架，通过创新的对比学习和CVAE模块充分利用多模态信息，在遮挡场景中表现出色，具有实际部署潜力
...
### [196] [AC-MASAC: An Attentive Curriculum Learning Framework for Heterogeneous UAV Swarm Coordination](https://arxiv.org/abs/2602.11735)
...
<details>
  <summary>Details</summary>
Motivation: 异构无人机集群协同路径规划对多智能体强化学习提出重大挑战，包括处理非对称智能体间依赖关系、应对稀疏奖励风险以及缓解训练过程中的灾难性遗忘问题。

Method: 提出注意力课程学习框架AC-MASAC：1) 引入角色感知异构注意力机制，显式建模非对称依赖；2) 设计结构化课程策略，整合分层知识迁移和阶段比例经验回放，解决稀疏奖励和灾难性遗忘。

Result: 在定制多智能体仿真平台上验证，结果显示该方法在成功率、编队保持率和任务时间加权成功率等指标上显著优于其他先进方法。
...
### [197] [HAIC: Humanoid Agile Object Interaction Control via Dynamics-Aware World Model](https://arxiv.org/abs/2602.11758)
...
TL;DR: HAIC是一个统一框架，用于人形机器人与具有独立动力学和非完整约束的欠驱动物体进行鲁棒交互，无需外部状态估计，通过动力学预测器和几何先验实现盲区碰撞边界和接触可及性推断。


<details>
  <summary>Details</summary>
Motivation: 现有的人-物交互方法主要关注完全驱动且与机器人刚性耦合的物体，忽略了具有独立动力学和非完整约束的欠驱动物体。这些物体由于耦合力和遮挡带来了控制挑战，需要新的解决方案。

Method: 提出HAIC框架，核心是动力学预测器，仅从本体感知历史估计高阶物体状态（速度、加速度）。这些预测被投影到静态几何先验上，形成空间接地的动态占据地图。采用非对称微调方法，世界模型持续适应学生策略的探索，确保在分布偏移下的鲁棒状态估计。

Result: 在人形机器人上的实验表明，HAIC在敏捷任务（滑板、推/拉不同负载的推车）中通过主动补偿惯性扰动实现高成功率，并能够通过预测多个物体的动力学，掌握多物体长时域任务（如在不同地形上搬运箱子）。

Conclusion: HAIC为处理具有不同动力学特性的物体提供了一种统一的鲁棒交互框架，无需外部状态估计，通过动力学预测和几何先验的结合，有效解决了欠驱动物体交互中的控制挑战。
...
### [198] [LAMP: Implicit Language Map for Robot Navigation](https://arxiv.org/abs/2602.11862)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于网格或节点的显式语言向量存储方法存在内存需求过高、分辨率有限的问题，难以扩展到大规模环境。需要一种既能支持细粒度规划又能高效扩展的导航框架。

Method: 1) 学习连续的语言驱动地图作为隐式神经场；2) 结合稀疏图进行粗粒度路径规划；3) 在学习的场中进行梯度优化以细化目标附近的姿态；4) 采用贝叶斯框架通过von Mises-Fisher分布建模嵌入不确定性；5) 使用图采样策略优先考虑空间覆盖和嵌入置信度。

Result: 在NVIDIA Isaac Sim和真实多层建筑中的实验表明，LAMP在内存效率和细粒度目标到达精度方面均优于现有显式方法。

Conclusion: LAMP通过隐式神经语言场实现了高效、可扩展的零样本导航，首次将隐式语言地图应用于精确路径生成，在保持细粒度规划能力的同时显著降低了内存开销。
...
### [199] [Learning to Manipulate Anything: Revealing Data Scaling Laws in Bounding-Box Guided Policies](https://arxiv.org/abs/2602.11885)
*Yihao Wu,Jinming Ma,Junbo Tan,Yanzhao Yu,Shoujie Li,Mingliang Zhou,Diyun Xiang,Xueqian Wang*

Main category: cs.RO

TL;DR: 该论文提出了一种结合边界框指令和扩散策略的语义操作框架，通过手持分割设备和自动化标注流水线收集大规模演示数据，发现语义操作任务中存在数据缩放规律，实现了对未见物体的良好泛化性能。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的策略在语义操作任务中泛化能力有限，这成为实际机器人部署的主要障碍。仅依赖文本指令在复杂动态环境中无法有效引导策略关注目标物体，需要更直接的物体指定方式。

Method: 1) 设计手持分割设备Label-UMI及其自动化标注流水线，高效收集带语义标签的演示数据；2) 提出语义-运动解耦框架，整合物体检测和边界框引导的扩散策略；3) 研究语义操作任务中的数据缩放规律。

Result: 在大规模数据集上的真实世界实验中验证了方法的有效性，揭示了泛化性能与边界框物体数量之间的幂律关系。在四个任务上对已见和未见物体实现了85%的成功率。

Conclusion: 通过边界框指令直接指定目标物体能显著提升语义操作的泛化能力，数据缩放规律的存在表明大规模数据收集对语义操作至关重要。提出的数据收集策略和框架为机器人语义操作提供了有效解决方案。
...
### [200] [General Humanoid Whole-Body Control via Pretraining and Fast Adaptation](https://arxiv.org/abs/2602.11929)
...
<details>
  <summary>Details</summary>
Motivation: 解决人形机器人全身控制面临的挑战：运动分布多样性、快速适应困难、高动态场景下平衡鲁棒性需求。现有方法需要任务特定训练或在新运动适应时性能下降。

Method: 1. Parseval引导残差策略适应：在正交性和KL约束下学习轻量级增量动作策略，实现高效适应外分布运动同时缓解灾难性遗忘。2. 质心感知控制：整合质心相关观测和目标，增强跟踪挑战性参考运动时的平衡能力。

Result: 在仿真和实际部署中的大量实验表明，FAST在鲁棒性、适应效率和泛化能力方面始终优于最先进的基线方法。

Conclusion: FAST框架通过创新的策略适应机制和质心感知控制，成功解决了人形机器人全身控制的快速适应和稳定跟踪问题，为通用人形机器人控制提供了有效解决方案。
...
### [201] [Accelerating Robotic Reinforcement Learning with Agent Guidance](https://arxiv.org/abs/2602.11978)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习在真实世界机器人操作中面临严重的样本效率问题。现有的人类在环方法虽然能加速训练，但存在可扩展性障碍：需要1:1的人类监督比例限制了机器人舰队扩展，长时间操作导致操作员疲劳，且人类技能差异引入高方差。

Method: 提出Agent-guided Policy Search (AGPS)框架，用多模态智能体替代人类监督员。关键洞察是将智能体视为语义世界模型，注入内在价值先验来结构化物理探索。通过可执行工具，智能体提供精确指导，包括纠正路径点和空间约束以剪枝探索空间。

Result: 在从精确插入到可变形物体操作的两个任务上验证了AGPS方法。结果显示AGPS在样本效率方面优于人类在环方法，实现了监督管道的自动化。

Conclusion: AGPS框架自动化了机器人学习的监督流程，为无人工干预和可扩展的机器人学习开辟了道路，解决了人类在环方法的可扩展性限制。
...
### [202] [Decentralized Multi-Robot Obstacle Detection and Tracking in a Maritime Scenario](https://arxiv.org/abs/2602.12012)
*Muhammad Farhan Ahmed,Vincent Frémont*

Main category: cs.RO

TL;DR: 提出了一种用于海上漂浮集装箱检测与跟踪的分散式多机器人框架，通过无人机与自主水面艇协同工作，结合视觉检测、不确定性感知数据关联和信息驱动的任务分配
...
Method: 采用分散式多机器人框架，无人机使用YOLOv8和立体视差进行视觉检测，通过基于不确定性的数据关联和每目标扩展卡尔曼滤波器进行跟踪，通过协方差交集融合紧凑轨迹摘要，信息驱动模块分配目标并选择悬停视点

Result: 海上场景仿真结果表明，该方法提高了覆盖范围、定位精度和跟踪一致性，同时保持了适度的通信需求

Conclusion: 该框架为海上漂浮集装箱的检测与跟踪提供了一种有效的分散式多机器人解决方案，能够在有限通信条件下实现可靠的感知和协调
...
### [203] [Adaptive-Horizon Conflict-Based Search for Closed-Loop Multi-Agent Path Finding](https://arxiv.org/abs/2602.12024)
*Jiarui Li,Federico Pecora,Runyu Zhang,Gioele Zardini*

Main category: cs.RO

TL;DR: ACCBS是一种基于CBS的闭环多智能体路径规划算法，通过动态调整规划时域和重用约束树，在有限计算预算下快速生成可行解，同时保证渐进最优性
...
Method: 基于有限时域CBS变体构建，采用受MPC迭代深化启发的时域变化机制。动态调整规划时域以适应可用计算预算，重用单一约束树实现不同时域间的无缝转换

Result: ACCBS能够快速生成高质量可行解，随着计算预算增加具有渐进最优性，表现出随时行为。在广泛案例研究中证明其结合了扰动灵活性和强性能保证
...
### [204] [When would Vision-Proprioception Policies Fail in Robotic Manipulation?](https://arxiv.org/abs/2602.12032)
*Jingxian Lu,Wenke Xia,Yuxuan Wu,Zhiwu Lu,Di Hu*

Main category: cs.RO

TL;DR: 该论文研究了视觉-本体感知策略在机器人操作中的泛化问题，发现运动转换阶段视觉模态作用有限，提出梯度调整算法GAP来优化多模态协作。


<details>
  <summary>Details</summary>
Motivation: 现有研究对视觉-本体感知策略的泛化能力存在不一致观察。本体感知提供实时机器人状态，与视觉结合有望提升复杂任务中的操作性能，但实际应用中多模态协作效果不佳，特别是在运动转换阶段。

Method: 提出梯度调整与相位引导算法(GAP)：利用本体感知捕捉机器人状态并估计轨迹中各时间步属于运动转换阶段的概率；在策略学习过程中，基于估计概率精细调整本体感知梯度幅度，促进视觉模态学习。

Result: GAP算法在模拟和真实环境中均有效，适用于单臂和双臂设置，兼容传统模型和视觉-语言-动作模型，显著提升了视觉-本体感知策略的鲁棒性和泛化能力。

Conclusion: 通过分析视觉-本体感知策略在运动转换阶段的局限性，提出GAP算法动态调节多模态优化过程，为机器人操作中视觉-本体感知策略的发展提供了重要见解。
...
### [205] [Safety Beyond the Training Data: Robust Out-of-Distribution MPC via Conformalized System Level Synthesis](https://arxiv.org/abs/2602.12047)
*Anutam Srinivasan,Antoine Leeman,Glen Chou*

Main category: cs.RO

TL;DR: 提出了一种结合保形预测和系统级综合的鲁棒分布外规划控制框架，使用学习的状态-控制相关协方差模型获得高置信度模型误差界，通过系统级综合鲁棒非线性MPC进行约束收紧，在分布漂移下提供理论保证。


<details>
  <summary>Details</summary>
Motivation: 解决在使用学习动力学模型时，超出训练数据分布范围时确保安全性和鲁棒性的挑战。现有方法在分布外场景下往往缺乏理论保证，难以处理模型误差的不确定性。

Method: 1. 使用加权保形预测和学习的状态-控制相关协方差模型推导高置信度模型误差界；2. 将误差界集成到基于系统级综合的鲁棒非线性模型预测控制框架中；3. 通过体积优化的前向可达集在预测时域内进行约束收紧；4. 分析数据密度和轨迹管大小对预测覆盖的影响。

Result: 1. 在分布漂移下提供覆盖率和鲁棒性的理论保证；2. 在非线性系统（包括4D汽车和12D四旋翼）上实证验证；3. 相比固定边界和非鲁棒基线，在数据分布外显著提高了安全性和鲁棒性。

Conclusion: 该框架成功解决了学习动力学模型在分布外场景下的安全鲁棒控制问题，通过保形预测和系统级综合的结合，提供了理论保证和实际性能提升，特别是在数据分布外区域表现出优越性。
...
### [206] [HoloBrain-0 Technical Report](https://arxiv.org/abs/2602.12062)
...
TL;DR: HoloBrain-0是一个全面的视觉-语言-动作框架，通过显式融入机器人本体先验来增强3D空间推理，采用"预训练后训练"范式在仿真和真实世界任务中取得SOTA结果，并开源了整个生态系统。


<details>
  <summary>Details</summary>
Motivation: 弥合基础模型研究与可靠真实世界机器人部署之间的差距，通过显式融入机器人本体先验（多视角相机参数和运动学描述）来增强3D空间推理能力，支持多样化的机器人本体。

Method: 提出新颖的VLA架构，显式融入机器人本体先验（多视角相机参数和URDF运动学描述）；采用可扩展的"预训练后训练"范式；开发完整的HoloBrain生态系统，包括预训练VLA基础模型、后训练检查点以及RoboOrchard全栈VLA基础设施。

Result: 在RoboTwin 2.0、LIBERO和GenieSim等仿真基准测试中取得最先进结果；在具有挑战性的长时域真实世界操作任务中表现强劲；高效的0.2B参数变体可与显著更大的基线模型相媲美，支持低延迟设备端部署。

Conclusion: HoloBrain-0通过显式融入机器人本体先验的VLA架构，成功弥合了基础模型研究与真实世界机器人部署之间的差距。开源整个生态系统（包括预训练模型、后训练检查点和全栈基础设施）为社区提供了完整、可复现的高性能机器人操作路径。
...
### [207] [VLAW: Iterative Co-Improvement of Vision-Language-Action Policy and World Model](https://arxiv.org/abs/2602.12063)
*Yanjiang Guo,Tony Lee,Lucy Xiaoyang Shi,Jianyu Chen,Percy Liang,Chelsea Finn*

Main category: cs.RO

TL;DR: 提出一种通过迭代在线交互提升视觉-语言-动作模型性能的方法，利用真实世界数据改进世界模型，再用生成式世界模型产生合成数据来增强VLA模型训练


<details>
  <summary>Details</summary>
Motivation: 现有世界模型在物理保真度方面存在不足，特别是在接触丰富的物体操作任务中难以准确建模关键物理细节，而真实世界策略部署成本高昂，需要寻找替代方案来生成补充数据

Method: 提出简单迭代改进算法：使用真实世界部署数据提升世界模型的保真度，然后用改进后的世界模型生成合成数据来增强VLA模型的训练

Result: 在真实机器人实验中，该方法显著提升了最先进VLA模型在多个下游任务上的性能，相比基础策略获得39.2%的绝对成功率提升，通过合成数据训练带来11.6%的额外改进

Conclusion: 通过迭代改进世界模型并利用其生成合成数据，可以有效提升VLA模型的性能和可靠性，为解决真实世界策略部署成本高昂的问题提供了可行方案
...
### [208] [Affordance-Graphed Task Worlds: Self-Evolving Task Generation for Scalable Embodied Learning](https://arxiv.org/abs/2602.12065)
*Xiang Liu,Sen Cui,Guocai Yao,Zhong Cao,Jingheng Ma,Min Zhang,Changshui Zhang*

Main category: cs.RO

TL;DR: AGT-World框架通过结构化图表示任务空间，将复杂目标分解为原子原语，结合自进化机制实现机器人策略的自主优化，显著提升成功率和泛化能力。
...
Method: 提出AGT-World框架：1）将任务空间形式化为结构化图，实现复杂目标的精确层次分解；2）引入自进化机制，结合视觉语言模型推理和几何验证的混合反馈来自主优化策略；3）基于真实世界观察自主构建交互式仿真环境和相应机器人任务策略。

Result: 广泛实验表明，该方法在成功率和泛化能力方面显著优于现有方法，实现了提案、执行和修正的自改进循环，支持可扩展的机器人学习。

Conclusion: AGT-World通过结构化任务表示和自进化机制，解决了现有仿真方法在逻辑一致性和物理不确定性方面的局限性，为可扩展的机器人学习提供了有效框架。
...
### [209] [RF-Modulated Adaptive Communication Improves Multi-Agent Robotic Exploration](https://arxiv.org/abs/2602.12074)
...
<details>
  <summary>Details</summary>
Motivation: 多机器人团队在通信受限环境（如洞穴、行星表面）中探索时，面临可靠协调和高效通信的挑战，需要避免不必要的回退和信息丢失

Method: 提出自适应射频传输（ART）算法，基于信号强度和数据负载大小动态调整传输位置；扩展为ART-SST，强制执行信号强度阈值以确保高保真数据传输

Result: 在三个洞穴模拟环境中进行480多次仿真，ART相比完整会合和最小信号启发式方法，减少58%的旅行距离，加快52%的探索时间
...
TL;DR: 提出Multi-Graph Search (MGS)算法，用于高维机器人系统的高效运动规划，通过多图搜索机制在保持完整性和有界次优性的同时，提高规划效率和运动一致性。
...
Method: 提出Multi-Graph Search (MGS)算法，将经典单向和双向搜索推广到多图设置。MGS在状态空间上维护并增量扩展多个隐式图，将探索集中在高潜力区域，同时允许初始断开的子图在搜索过程中通过可行转换进行合并。

Result: 证明了MGS具有完整性和有界次优性，并在一系列操作和移动操作任务上通过实验验证了其有效性。提供了演示、基准测试和代码。

Conclusion: MGS算法为高维机器人系统提供了一种高效的运动规划解决方案，在保持理论保证的同时，提高了规划效率和运动质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本物体导航方法依赖场景抽象（如语义地图或文本表示），导致高层决策受限于低层感知精度。需要一种能增强空间推理能力、减少感知约束的新方法。

Method: 提出3DGSNav框架：1）使用3D高斯泼溅作为VLMs的持久记忆，通过主动感知增量构建环境3D表示；2）设计结构化视觉提示并与思维链提示结合；3）实时物体检测器过滤潜在目标；4）VLM驱动的主动视点切换进行目标重验证。

Result: 在多个基准测试和四足机器人真实世界实验中，该方法展现出稳健且具有竞争力的性能，优于现有最先进方法。

Conclusion: 3DGSNav通过将3D高斯泼溅作为VLMs的持久记忆，有效增强了空间推理能力，实现了高效可靠的零样本物体导航，为具身智能提供了新的解决方案。
...
### [212] [Sub--Riemannian boundary value problems for Optimal Geometric Locomotion](https://arxiv.org/abs/2602.12199)
*Oliver Gross,Florine Hartwig,Martin Rumpf,Peter Schröder*

Main category: cs.RO

TL;DR: 提出几何模型分析细长运动体（如蛇、精子）的最优形状变化运动，将运动建模为子黎曼测地线问题，同时考虑环境位移能耗和形状变化能耗


<details>
  <summary>Details</summary>
Motivation: 现有模型通常只考虑运动体在环境中位移的能量消耗，忽略了生物代谢或机器人执行器为产生形状变化（如弯曲、拉伸）所消耗的能量。需要更全面的几何框架来捕捉整体运动效率，特别是对于形状变化驱动的细长运动体

Method: 提出拉格朗日最小耗散原理，将其表述为边界值问题，其解由子黎曼测地线给出。模型同时考虑环境位移能耗和形状变化能耗。开发了连续模型及一致的时间和空间离散化方法，支持三种边界条件：固定初始和目标姿态、限制为循环运动、仅规定位移和方向

Result: 数值计算得到的子黎曼测地线产生的优化变形步态在质量上与蛇、精子等生物的运动轨迹以及Purcell游泳器等低维系统的已知最优性结果相匹配。几何灵活性更高的模型为广义Purcell游泳器等运动机制提供了新见解

Conclusion: 提出的几何模型能够全面分析形状变化驱动运动体的最优运动，同时考虑环境位移和形状变化的能量消耗。该框架比先前模型更灵活，能够解释生物运动模式并为机器人运动设计提供指导。代码已公开
...
### [213] [LDA-1B: Scaling Latent Dynamics Action Model via Universal Embodied Data Ingestion](https://arxiv.org/abs/2602.12215)
...
TL;DR: LDA-1B是一个通过统一世界模型框架，联合学习动力学、策略和视觉预测的机器人基础模型，利用结构化DINO潜在空间和多模态扩散Transformer处理异构数据，在接触丰富、灵巧和长时域任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有机器人基础模型主要依赖大规模行为克隆，丢弃了异构具身数据中可转移的动力学知识。统一世界模型框架有潜力利用这种多样性数据，但现有实现因数据使用粗粒度、数据集碎片化而难以扩展到基础模型级别。

Method: 1) 提出LDA-1B模型，通过通用具身数据摄入联合学习动力学、策略和视觉预测，为不同质量数据分配不同角色；2) 构建EI-30k数据集，包含3万小时人类和机器人轨迹的统一格式数据；3) 使用结构化DINO潜在空间进行预测，避免冗余像素空间外观建模；4) 采用多模态扩散Transformer处理异步视觉和动作流，支持10亿参数规模稳定训练。

Result: 在仿真和真实世界实验中，LDA-1B在接触丰富任务上比π₀.₅提升21%，灵巧任务提升48%，长时域任务提升23%。模型支持数据高效微调，利用30%通常有害被丢弃的低质量轨迹可获得10%性能增益。

Conclusion: LDA-1B通过统一世界模型框架成功扩展到基础模型规模，有效利用了异构具身数据中的动力学知识，在多种机器人任务上取得显著性能提升，并展示了利用低质量数据进行高效微调的潜力。
...
### [214] [Any House Any Task: Scalable Long-Horizon Planning for Abstract Human Tasks](https://arxiv.org/abs/2602.12244)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的任务规划方法在环境规模扩大、规划长度增加、指令模糊性和约束复杂性提高时性能急剧下降，需要解决大规模家庭环境中开放式语言条件任务规划的可扩展性问题。

Method: AHAT核心是训练LLM将任务指令和文本场景图映射为PDDL定义的接地子目标，然后通过显式符号推理生成可行且最优的长时程规划。引入TGPO强化学习算法，将中间推理轨迹的外部校正集成到GRPO中，增强模型分解复杂模糊意图的能力。

Result: 实验表明AHAT在性能上显著优于最先进的提示、规划和学习方法，特别是在人类风格的家庭任务中表现突出，这些任务通常指令简短但需要复杂的执行计划。

Conclusion: AHAT通过结合LLM的语义理解能力和符号推理的精确性，有效解决了大规模家庭环境中开放式语言条件任务规划的可扩展性问题，TGPO算法进一步提升了复杂意图分解能力。
...
### [215] [Scaling Verification Can Be More Effective than Scaling Policy Learning for Vision-Language-Action Alignment](https://arxiv.org/abs/2602.12281)
*Jacky Kwok,Xilun Zhang,Mengdi Xu,Yuejiang Liu,Azalia Mirhoseini,Chelsea Finn,Marco Pavone*

Main category: cs.RO

TL;DR: CoVer：一种用于视觉-语言-动作模型测试时验证的对比验证器，通过重述指令和生成多样化动作候选来缩小意图-动作差距，显著提升机器人指令跟随性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作模型生成的行动与自然语言指令之间仍存在不对齐问题（意图-动作差距），需要开发有效的测试时验证方法来缩小这一差距。

Method: 提出CoVer对比验证器架构，采用测试时扩展策略：1）联合扩展重述指令数量和生成动作数量以增加样本多样性；2）引入"启动时计算"和分层验证推理流程；3）部署时预计算多样化重述指令，为每个指令生成多个动作候选，使用验证器选择最优高层提示和低层动作块。

Result: 在SIMPLER基准测试中，相比在相同数据上扩展策略预训练，验证方法带来22%分布内和13%分布外性能提升，真实世界实验进一步改善45%；在PolaRiS基准测试中，CoVer实现14%任务进展和9%成功率提升。

Conclusion: 测试时验证是缩小视觉-语言-动作模型意图-动作差距的有效方法，CoVer架构能够优雅地扩展计算资源和数据，显著提升机器人指令跟随性能。
