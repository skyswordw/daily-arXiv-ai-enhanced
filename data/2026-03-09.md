...
TL;DR: 该论文提出了一种新的步态识别视觉模态SKETCH，通过边缘检测器从RGB图像中提取高频结构线索，并设计了SKETCHGAIT多模态框架，在多个数据集上取得了优异性能。
...
Method: 从结构视角重新审视步态表示，提出SKETCH模态（无标签的密集部件级结构），并设计SKETCHGAIT框架：两个独立流进行模态特定学习，加上轻量级早期融合分支捕获结构互补性。

Result: 在SUSTech1K数据集上达到92.9% Rank-1准确率，在CCPG数据集上达到93.1%平均Rank-1准确率，验证了SKETCH模态和框架的有效性。

Conclusion: SKETCH作为一种新的视觉模态，通过无标签方式提取高频结构线索，与标签引导的解析在语义上解耦且结构上互补，为步态识别提供了更鲁棒和有效的表示方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有CAD生成方法受限于小规模标注数据集（需显式设计历史或BRep标签），而数百万未标注3D网格未被利用，限制了可扩展CAD生成的发展

Method: 将BRep表示为参数化曲面片集合，使用可微分细分方法生成网格，实现大规模3D数据集训练同时重建连接且可编辑的表面

Result: 在ABC和Objaverse基准测试中，文本、图像和点云模态均达到SOTA，几何保真度提升，用户偏好超过75%

Conclusion: DreamCAD通过点级监督实现可编辑BRep生成，无需CAD特定标注，结合CADCap-1M数据集推动文本到CAD研究发展
...
TL;DR: ABRA提出了一种对抗性批量表示增强方法，通过参数化特征统计量作为结构化不确定性，在表示空间中合成最坏情况的生物批次扰动，以解决细胞成像中的生物批次效应问题。


<details>
  <summary>Details</summary>
Motivation: 高通量筛选产生的细胞成像数据存在技术变异导致的生物批次效应，这些效应引起协变量偏移并降低深度学习模型在未见数据上的泛化能力。现有批次校正方法通常需要额外先验知识或难以泛化到未见生物批次。

Method: 将生物批次缓解问题框架化为领域泛化问题，提出ABRA方法：1) 通过参数化特征统计量作为结构化不确定性来显式建模批次间统计波动；2) 采用min-max优化框架，在表示空间中主动合成最坏情况的生物批次扰动；3) 引入严格的角度几何边界来保持细粒度类别可区分性；4) 为防止对抗探索中的表示崩溃，引入协同分布对齐目标。

Result: 在RxRx1和RxRx1-WILDS基准测试上的广泛评估表明，ABRA在siRNA扰动分类任务上建立了新的最先进性能。

Conclusion: ABRA通过对抗性批量表示增强有效缓解了细胞成像中的生物批次效应，在领域泛化框架下实现了更好的模型泛化能力，为高通量筛选数据分析提供了有效的解决方案。
...
TL;DR: 提出PFS（后融合稳定器），一种轻量级模块，用于提升BEV融合检测器在域偏移和传感器故障下的鲁棒性，无需修改现有架构或重新训练。
...
Method: PFS在现有检测器的中间BEV表示上操作，通过稳定特征统计、抑制传感器退化区域、自适应恢复弱化线索的残差校正，生成精炼的特征图供原始检测头使用，设计为近似恒等变换。

Result: 在nuScenes基准测试中，PFS在多种故障模式下取得最先进结果：相机丢失鲁棒性提升+1.2%，低光性能提升+4.4% mAP，仅需3.3M参数。

Conclusion: PFS是一种轻量级、易于集成的解决方案，能够显著提升现有BEV融合检测器在域偏移和传感器故障下的鲁棒性，同时保持原始性能。
...
Method: 提出CBM-Suite框架：1) 基于熵的指标量化概念集的内在适用性；2) 在概念激活和分类器之间插入非线性层解决线性化问题；3) 使用线性教师探针引导的蒸馏损失缩小准确率差距；4) 系统性分析不同视觉编码器、VLM和概念集对准确率和可解释性的影响

Result: 广泛评估表明CBM-Suite能产生更准确的模型，并为改进基于概念的可解释性提供见解

Conclusion: CBM-Suite系统性解决了概念瓶颈模型的关键挑战，通过新指标、架构改进和系统性分析提升了模型的准确性和可解释性
...
Method: 对数据集每个元素，在潜在空间中检索其最近邻，插值它们的潜在表示，解码插值后的潜在向量，计算解码样本与原始数据集之间的FID

Result: iFID与扩散gFID呈现强相关性（Pearson线性相关和Spearman秩相关约0.85），是首个展示与扩散gFID强相关的指标

Conclusion: iFID能有效预测扩散模型生成质量，rFID与扩散细化阶段样本质量相关，而iFID与扩散导航阶段样本质量相关，解释了重建指标与gFID负相关的原因
...
TL;DR: 本文提出隐式错误计数（IEC）方法，用于解决缺乏理想参考答案的参考无关场景中的强化学习奖励设计问题，通过枚举错误而非构建评分标准来提供更可靠的奖励信号。


<details>
  <summary>Details</summary>
Motivation: 现有基于参考答案的奖励方法（如RLVR和RaR）在存在多个有效输出的现实任务中面临挑战，因为这些任务缺乏单一理想答案来构建评分标准。本文识别了参考无关场景的空白，需要新的奖励设计方法。

Method: 提出隐式错误计数（IEC）方法：1）枚举响应中的错误而非检查正确性；2）在任务相关维度上应用严重性加权评分；3）通过隐式分数发射和组校准将错误转换为校准的每方面奖励。针对虚拟试穿（VTO）任务，进一步提出级联错误计数（CEC）作为评估指标。

Result: 在Mismatch-DressCode基准测试中，IEC在CEC指标上优于RaR（5.31 vs. 5.60）。在VITON-HD和DressCode数据集上，IEC在8个感知指标中的6个上匹配或超越6个基线方法。CEC指标与人类偏好高度一致（60% top-1 vs. 30%其他方法）。

Conclusion: 当理想答案不可用时，计数错误比构建评分标准提供更强的信号。隐式分数发射和组校准是使错误计数成为可靠奖励的必要设计选择，IEC方法在参考无关场景中表现出优越性能。
...
TL;DR: SemVID是一种无需训练的视觉token剪枝框架，用于视频时序定位任务，通过保留查询关键证据和跨帧连接性，在仅使用12.5%视觉token的情况下保持95.4%的mIoU精度，实现5.8倍的前向加速。


<details>
  <summary>Details</summary>
Motivation: 视频时序定位需要在长视频中定位查询相关片段，但视频语言模型计算成本过高。现有视觉token剪枝方法直接应用于VTG会导致性能显著下降，因为VTG依赖边界敏感证据和跨帧推理链。
...
Result: 在VTG基准测试中，SemVID实现了精度与效率的强平衡，仅使用12.5%视觉token即可保留95.4%的mIoU，提供高达5.8倍的前向加速，在相同预算下持续优于先前方法。

Conclusion: SemVID通过构建具有互补语义角色的紧凑而连贯的token子集，有效解决了VTG任务中的视觉token剪枝问题，为视频语言模型的高效推理提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决运动过程中3D感知的复杂挑战，特别是如何在缺乏环境先验知识的情况下，仅从视觉运动线索中提取有意义的3D信息，为机器人和自主导航系统提供实用的感知解决方案。

Method: 基于两种视觉运动线索：1) 注视点附近点的感知局部视觉逼近，2) 刚性物体相对于注视点的感知旋转。通过像素级并行计算直接从原始视觉运动图像序列中提取OWL值，无需测量或先验知识即可表达相机与3D点之间的相对瞬时方向范围和方向平移关系。

Result: 仿真实验表明OWL能够实现3D物体随时间的几何恒常性，仅凭视觉运动线索即可实现缩放3D场景重建。该方法无需先验知识即可从原始视觉运动图像序列中获取OWL值，提供了一种实时的3D点相对运动表示方法。

Conclusion: OWL为机器感知提供了新的视角，弥合了理论概念与机器人、自主导航实际应用之间的差距，可能作为下一代自主系统的构建模块，其影响可能延伸到自然感知领域，有助于更好地理解行为心理学和神经功能。
...
TL;DR: MultiHaystack是首个评估大规模跨模态检索与推理的基准测试，包含46,000+多模态检索候选项和747个开放但可验证的问题，揭示当前多模态大语言模型在异构检索池中的检索能力是主要瓶颈。
...
Result: 模型在提供相应证据时表现良好，但从完整语料库中检索证据时性能急剧下降。最强的检索器E5-V仅达到40.8%的Recall@1，而GPT-5等最先进的MLLM在提供相应证据时推理准确率为80.86%，在top-5检索下则降至51.4%。

Conclusion: 异构多模态检索池上的检索能力是MLLM的主要瓶颈。MultiHaystack作为一个有价值的测试平台，揭示了小规模评估所掩盖的底层限制，并促进多模态系统中以检索为中心的进展。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在基于图像的定位方面虽有进展，但精确的全球地理定位仍然面临挑战，主要因为视觉景观的固有模糊性以及音频线索的潜力尚未充分挖掘。

Method: 提出三阶段框架：1) 感知阶段使用混合自回归稀疏自编码器将噪声音频分解为语义基础的"声学原子"；2) 多模态推理阶段通过Group Relative Policy Optimization微调的MLLM融合声学原子与视觉特征；3) 精度预测阶段在S^2流形上使用黎曼流匹配进行预测。

Result: 实验表明该框架显著优于单模态基线，证明声景的可解释感知提供了关键的正交信号，结合多模态推理可实现高精度全球定位。

Conclusion: 通过可解释的声学感知与多模态推理相结合，能够有效解决地理定位中的模糊性问题，实现比纯视觉方法更精确的全球定位。
...
TL;DR: 该论文提出了一种自动化流程，将ImageNet训练集转换为多标签数据集，无需人工标注，通过自监督视觉Transformer进行无监督对象发现，生成高质量多标签注释，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 原始ImageNet基准强制使用单标签假设，但许多图像包含多个对象，这导致标签噪声并限制了学习信号的丰富性。多标签标注能更准确地反映现实世界视觉场景，其中多个对象共存并有助于语义理解，使模型能够学习更丰富、更鲁棒的表示。

Method: 提出自动化流程：使用自监督视觉Transformer进行无监督对象发现，选择与原始标签对齐的区域训练轻量级分类器，然后将其应用于所有区域以生成整个数据集的一致多标签注释。

Result: 生成的多标签注释在定性评估中与人类判断高度一致，在定量基准测试中持续提升性能。相比传统单标签方案，使用多标签监督训练的模型在不同架构上获得更好的域内准确率（ReaL上最高提升+2.0 top-1准确率，ImageNet-V2上+1.5），并在下游任务上表现出更强的迁移能力（COCO上最高提升+4.2 mAP，VOC上+2.3 mAP）。

Conclusion: 准确的多标签标注对于提升分类性能和表示学习至关重要。该方法无需人工标注即可生成高质量多标签数据集，为视觉识别任务提供了更丰富的监督信号。
...
<details>
  <summary>Details</summary>
Motivation: 当前手术时间线创建方法存在缺陷：术后报告通常模糊不清，手动标注视频又极其耗时。需要在两者之间找到平衡方案，实现自动化的手术时间线生成。

Method: 采用基于CLIP的多模态框架，将手术视频帧与文本手势描述对齐。使用CLIP视觉编码器提取视频帧表示，文本编码器嵌入手势句子到共享嵌入空间，通过微调模型改善视频手势与文本标记的对齐。训练后模型可预测视频帧的手势和阶段，构建结构化手术时间线。

Result: 该方法能够自动预测手术视频帧的手势和阶段，从而生成结构化的手术时间线，减少外科医生手动视频审查和标注的需求。

Conclusion: 提出的基于CLIP的多模态框架能够有效连接视觉手势与文本叙述，实现从手术视频自动生成结构化时间线，为工具辅助手术提供更高效的时间线创建方案。
...
TL;DR: Icarus是一个全天气天空模型，能够学习全动态范围物理捕获户外图像，实现高精度、逼真的天空环境图生成，特别解决了太阳区域建模难题。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习天空模型在重现准确、逼真的自然天空方面存在不足，特别是在高分辨率下难以准确建模14EV+类别不平衡的太阳区域，导致视觉质量差、光照传输、阴影和色调失真。
...
Conclusion: Icarus是一个功能强大的全天气天空模型，解决了现有深度学习天空模型在高分辨率太阳区域建模方面的局限性，为户外场景建模提供了高质量的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有区域布局控制方法存在两大局限：基于训练的方法继承数据偏差并降低图像质量；现有技术难以处理遮挡顺序，限制了实际应用。作者观察到空间布局和遮挡在去噪早期阶段就已确定，因此重新排列早期潜在结构足以修改最终输出。

Method: LayerBind采用两阶段方案：1）实例初始化阶段：利用多模态联合注意力中的上下文共享机制，创建每个实例的分支，在指定早期步骤根据层顺序融合形成具有预设布局的统一潜在表示；2）语义护理阶段：通过分层注意力增强强化区域细节并保持遮挡顺序，采用序列化分层注意力路径与标准全局路径并行，更新在层透明度调度器下组合。

Result: LayerBind在定性和定量评估中均表现出有效性，能够实现精确的区域控制和遮挡顺序管理，支持可编辑工作流程，如更改实例或重新排列可见顺序，展现了在创意应用中的强大潜力。

Conclusion: LayerBind是一种无需训练、即插即用的解决方案，通过分层建模和绑定机制解决了文本到图像生成中的区域布局控制和遮挡顺序问题，为创意应用提供了灵活且强大的工具。
...
TL;DR: BM25-V将Okapi BM25评分应用于视觉Transformer补丁特征的稀疏自编码器激活，实现高效、可解释的图像检索，作为密集重排序的高召回第一阶段检索器。


<details>
  <summary>Details</summary>
Motivation: 密集图像检索虽然准确，但可解释性和归因性有限，且在大规模计算上成本高昂。需要一种既能保持高召回率又具有可解释性、计算效率高的检索方法。

Method: 使用稀疏自编码器(SAE)从Vision Transformer补丁特征中提取稀疏视觉词激活，然后应用Okapi BM25评分进行检索。利用BM25的逆文档频率(IDF)加权抑制常见低信息词，强调罕见判别性词。通过稀疏倒排索引操作实现高效检索。

Result: 在七个基准测试中，BM25-V达到Recall@200 ≥ 0.993，仅需重排序200个候选就能恢复接近密集检索的准确率（平均差距0.2%）。在ImageNet-1K上训练一次的SAE可零样本迁移到七个细粒度基准，检索决策可归因于特定视觉词及其IDF贡献。

Conclusion: BM25-V作为高效的第一阶段检索器，结合稀疏视觉词表示和BM25评分，实现了高召回、可解释且计算效率高的图像检索，为密集重排序提供了有效的候选筛选方案。
...
TL;DR: EventGeM：基于事件相机的视觉地点识别方法，通过全局-局部特征融合实现实时定位，在多个基准数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 事件相机因其稀疏激活和高时间分辨率在机器人导航和定位任务中日益流行，但现有事件相机视觉地点识别方法在精度和实时性方面仍有提升空间，需要一种能够在不同光照条件下稳定工作且计算高效的定位方法

Method: 提出EventGeM全局-局部特征融合管道：1) 使用预训练ViT-S/16从事件直方图图像提取全局特征进行初始匹配；2) 使用预训练MaxViT检测局部特征关键点，基于2D单应性变换和RANSAC进行重排序；3) 使用预训练视觉基础模型进行深度估计，比较参考图像和查询图像的结构相似性进行进一步重排序优化

Result: 在多个基准数据集和不同光照条件下，EventGeM相比当前最佳的事件相机地点识别方法实现了最先进的定位性能，同时能够在各种计算架构上实时运行，并在真实机器人平台上成功部署

Conclusion: EventGeM通过创新的全局-局部特征融合策略，为事件相机视觉地点识别提供了高效准确的解决方案，在保持实时性的同时显著提升了定位精度，展示了在实际机器人导航应用中的可行性
...
TL;DR: LIPAR框架通过检测和跳过重复的潜在补丁来减少视频生成的计算延迟，同时引入注意力恢复机制来保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型存在高计算延迟问题，使得实时应用成本过高。视频潜在补丁中存在时间冗余性，这为优化提供了机会。

Method: 提出LIPAR（潜在帧间剪枝与注意力恢复）框架：1）检测并跳过重复的潜在补皮重新计算；2）引入注意力恢复机制，近似剪枝令牌的注意力值，消除视觉伪影。

Result: 方法将视频编辑吞吐量提高1.45倍，在NVIDIA A6000上平均达到12.2 FPS（基线为8.4 FPS）。不损害生成质量，无需额外训练即可与模型集成。

Conclusion: LIPAR有效弥合了传统压缩算法与现代生成管道之间的差距，通过利用时间冗余显著提升了视频生成效率。
...
TL;DR: Cog2Gen3D：一种3D认知引导的扩散框架，通过语义-几何双流图融合解决现有方法在绝对几何尺度不一致的问题，实现物理合理的3D生成。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法主要使用几何特征作为条件，但只能建模相对关系，容易产生绝对几何尺度不一致问题。作者认为语义信息和绝对几何共同赋能3D认知，从而实现物理世界的可控3D生成。

Method: 提出Cog2Gen3D框架，包含三个关键设计：1）认知特征嵌入：编码多模态为语义和几何表示，并提取逻辑表示；2）3D潜在认知图：构建语义-几何双流图，通过基于公共的交叉注意力融合；3）认知引导的潜在扩散：使用融合的3D认知图作为条件，指导3D高斯生成的潜在扩散过程。

Result: 在基于Marble World Labs构建的验证子集上进行广泛实验，Cog2Gen3D在语义保真度和几何合理性方面显著优于现有方法。

Conclusion: 该统一框架通过3D认知图确保了3D生成的物理合理性和结构合理性，为解决3D生成中的绝对几何尺度问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 视频稳定面临几何鲁棒性与全帧一致性的基本权衡：2D方法因过度裁剪而受限，3D技术则因脆弱优化流程在极端运动下失效。需要一种能在所有场景下可靠工作的方法。

Method: VS3R框架协同前馈3D重建与生成式视频扩散：联合估计相机参数、深度和掩码以确保全场景可靠性；引入混合稳定渲染模块融合语义和几何线索实现动态一致性；使用双流视频扩散模型通过结合结构引导和语义锚点来修复遮挡区域和矫正伪影。
...
TL;DR: 提出TumorChain框架，通过多模态交错推理实现肿瘤分析，在1.5M CoT标注的CT扫描数据集上验证了其在病灶检测、印象生成和病理分类上的优越性。


<details>
  <summary>Details</summary>
Motivation: 临床肿瘤分析需要从影像发现到临床印象再到病理结论的逐步推理过程，现有方法缺乏可追溯性和一致性，容易产生诊断错误和幻觉风险。

Method: 构建TumorCoT数据集（150万CoT标注的VQA指令配3D CT扫描），提出TumorChain多模态交错推理框架，结合3D影像编码器、临床文本理解和器官级视觉语言对齐，通过跨模态对齐和迭代交错因果推理实现证据基础和自优化。

Result: 在病灶检测、印象生成和病理分类任务上均优于强基线，在DeepTumorVQA基准测试中表现出强泛化能力，提高了可追溯性并减少了幻觉风险。

Conclusion: 多模态推理框架TumorChain为临床实践提供了可靠且可解释的肿瘤分析方法，通过链式思维推理实现了从影像发现到病理预测的端到端可追溯分析。
...
TL;DR: PatchCue是一种基于图像块(patch)的视觉提示范式，通过将图像分割成块并在块级别表示视觉提示，显著增强视觉语言模型的视觉推理能力。
...
Method: 提出PatchCue：基于patch的视觉提示范式。将图像分割成patch，在patch级别表示视觉提示。采用两阶段训练方法：1)冷启动监督微调，使模型输出patch级提示；2)强化学习，使用过程监督的提示奖励来引导中间视觉推理步骤。

Result: 在多个VLM和多样化基准测试(包括通用视觉问答、复杂推理和文档理解)上的实验表明，PatchCue持续提升整体模型性能。patch级提示在性能上优于像素级边界框和基于点的提示，提供更有效且认知对齐的视觉推理范式。

Conclusion: PatchCue通过patch级视觉提示范式，更好地利用视觉线索增强VLM的推理能力，提供了一种更符合人类感知习惯且与现代VLM架构对齐的有效方法，在多种视觉推理任务上表现出优越性能。
...
TL;DR: 该论文系统评估了合成新视角在视频地点识别中的应用，发现小规模添加新视角能改善识别效果，而大规模添加时视角变化幅度不如添加数量和数据集图像类型重要。


<details>
  <summary>Details</summary>
Motivation: 合成新视角的生成对机器人导航有积极影响：地面机器人场景可生成俯视视角引导空中机器人；空中视角可生成地面视角帮助无人机识别地面机器人看到的地点。需要系统评估这些合成新视角在视频地点识别中的实际效果。

Method: 使用五个公开VPR图像数据库和七种典型图像相似性方法，对合成新视角在VPR中的应用进行系统评估。分析不同规模新视角添加对识别效果的影响。

Result: 对于小规模合成添加，新视角能改善VPR识别统计效果。对于大规模添加，视角变化幅度的重要性低于添加的视角数量和数据集中的图像类型。

Conclusion: 合成新视角在视频地点识别中具有应用价值，特别是在小规模添加时能提升识别性能。实际应用中应考虑添加数量和数据集特性，而非单纯追求视角变化幅度。
...
Method: 提出CylinderSplat框架，核心是新的圆柱形Triplane表示，与全景数据和符合曼哈顿世界假设的真实世界结构更匹配。采用双分支架构：基于像素的分支重建观测良好的区域，基于体积的分支利用圆柱形Triplane补全被遮挡或稀疏观测的区域。框架设计灵活，可处理从单视图到多视图的可变数量输入。
...
TL;DR: 提出CORE-Seg框架，首个结合推理与分割的端到端方法，用于复杂病灶分割，在ComLesion-14K基准上取得SOTA结果


<details>
  <summary>Details</summary>
Motivation: 医学图像分割正从传统的视觉模式匹配转向认知推理分析。现有通用MLLMs缺乏对复杂病灶的专门视觉推理能力，而传统分割模型缺乏逻辑可解释性，需要结合两者的优势

Method: 提出CORE-Seg端到端框架，通过语义引导提示适配器(Semantic-Guided Prompt Adapter)整合推理与分割。采用从SFT到GRPO的渐进训练策略，配备自适应双粒度奖励机制以缓解奖励稀疏问题

Result: 在ComLesion-14K基准上取得37.06%的平均Dice分数，比次优基线提高14.89%，失败率降低至18.42%，达到SOTA性能

Conclusion: CORE-Seg成功将推理能力整合到医学图像分割中，为复杂病灶分割提供了新的解决方案，在ComLesion-14K基准上验证了其有效性
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖隐式有向距离场和事后布局优化，缺乏统一的场景重建框架。需要一种能够直接从单张图像生成完整、连贯、可直接用于下游应用的3D场景网格的方法。

Method: 基于网格生成模型的最新进展，通过点云编码器增强像素对齐的图像特征和全局场景上下文（使用交叉注意力），实现从单张图像的准确空间推理。采用自回归方式从包含上下文、姿态和网格的统一令牌流生成场景，产生紧凑的高保真几何网格。

Result: 在合成和真实世界数据集上的实验表明，PixARMesh实现了最先进的重建质量，同时生成轻量级、高质量的网格，可直接用于下游应用。

Conclusion: PixARMesh提供了一种从单张RGB图像自回归重建完整3D室内场景网格的有效方法，通过统一模型联合优化布局和几何，生成可直接用于艺术创作的高质量网格，在重建质量和实用性方面均表现出色。
...
### [27] [BlackMirror: Black-Box Backdoor Detection for Text-to-Image Models via Instruction-Response Deviation](https://arxiv.org/abs/2603.05921)
*Feiran Li,Qianqian Xu,Shilong Bao,Zhiyong Yang,Xilin Zhao,Xiaochun Cao,Qingming Huang*

Main category: cs.CV

TL;DR: BlackMirror：一种黑盒文本到图像模型后门检测框架，通过分析生成图像中部分语义模式的稳定操纵来检测视觉多样化的后门攻击


<details>
  <summary>Details</summary>
Motivation: 现有基于图像相似度的后门检测方法假设后门触发的生成具有强一致性，但无法应对新兴的视觉多样化后门攻击。研究发现，在后门攻击中，只有生成图像中的部分语义模式被稳定操纵，其余内容保持多样或良性
...
Result: 综合实验表明，BlackMirror在广泛的攻击范围内实现了准确检测。该框架可部署为模型即服务（MaaS）应用中的即插即用模块

Conclusion: BlackMirror通过分析部分语义模式的稳定操纵，有效解决了黑盒文本到图像模型后门检测的挑战，特别是在面对视觉多样化后门攻击时表现出色
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的产品海报生成方法通常采用多阶段流水线，难以同时控制主体、文本和风格，且存在主体保真度差、文本不准确、风格不一致等问题

Method: 提出单阶段框架InnoAds-Composer，支持主体、字形和风格的三条件控制令牌；通过层和时间步的重要性分析，将每个条件路由到最敏感位置以缩短有效令牌序列；设计文本特征增强模块(TFEM)整合字形图像和字形裁剪特征以提升中文文本渲染准确性；构建首个包含主体、文本和风格条件的高质量电商产品海报数据集

Result: InnoAds-Composer在电商产品海报生成任务上显著优于现有方法，且未明显增加推理延迟；在主体保真度、文本准确性和风格一致性方面均有显著提升

Conclusion: InnoAds-Composer通过单阶段三条件控制框架有效解决了产品海报生成中的关键问题，为电商内容创作提供了高效解决方案，同时构建的数据集和基准为后续研究提供了重要资源
...
### [29] [RAC: Rectified Flow Auto Coder](https://arxiv.org/abs/2603.05925)
*Sen Fang,Yalin Feng,Yanxin Zhang,Dimitris N. Metaxas*

Main category: cs.CV

TL;DR: RAC（Rectified Flow Auto Coder）是一种基于Rectified Flow的新型自编码器，替代传统VAE，通过多步解码、双向推理和可修正的潜在变量路径，在重建和生成质量上超越SOTA VAE，同时计算成本降低约70%


<details>
  <summary>Details</summary>
Motivation: 传统VAE存在重建-生成差距问题，解码路径通常是非线性的，且编码器和解码器分离导致参数冗余。作者旨在设计一种更高效、质量更高的生成模型，通过可修正的直线解码路径和双向推理机制来解决这些问题

Method: 1. 基于Rectified Flow设计多步解码机制，在流时间步上应用解码器，实现直线且可修正的解码路径；2. 通过时间反转实现双向推理，解码器可同时作为编码器使用，减少参数数量；3. 生成式解码方法允许沿路径修正潜在变量，部分解决重建-生成差距

Result: RAC在重建和生成质量上均超越当前最先进的VAE模型，同时计算成本降低约70%，参数数量减少近41%。模型支持逐步细化和可修正的生成过程

Conclusion: RAC通过引入Rectified Flow的直线解码路径和双向推理机制，成功解决了传统VAE的重建-生成差距问题，在保持高质量重建和生成的同时显著降低了计算成本和参数数量，为生成模型设计提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 虽然概念瓶颈模型（CBMs）理论上通过将图像映射到高层可解释概念来增强可解释性和公平性，但实际中CBM概念会泄漏与概念语义无关的信息，且在数据集如ImSitu上仅能边际减少性别偏差，需要更有效的偏差缓解技术。

Method: 提出三种偏差缓解技术：1. 使用top-k概念过滤器减少信息泄漏；2. 移除有偏概念；3. 对抗性去偏。这些方法共同改进CBMs的公平性。

Result: 提出的去偏CBM在公平性与性能权衡方面优于先前工作，为实现公平且可解释的图像分类提供了重要进展。

Conclusion: 通过结合三种偏差缓解技术，去偏概念瓶颈模型在保持可解释性的同时显著提高了公平性，为公平且可解释的图像分类系统提供了有效解决方案。
...
TL;DR: 提出CollabOD轻量级协作检测框架，解决无人机图像中小目标检测的尺度变化、结构细节退化和计算资源有限等挑战，通过结构细节保留、跨路径特征对齐和定位感知轻量化设计优化无人机感知模型。


<details>
  <summary>Details</summary>
Motivation: 无人机图像中的小目标检测面临三大挑战：尺度变化、结构细节退化（特别是在高海拔场景中，细粒度特征在分层下采样和跨尺度融合过程中被削弱）以及有限的计算资源，导致定位不稳定和鲁棒性降低。

Method: 提出CollabOD轻量级协作检测框架，集成三种策略：1) 结构细节保留，2) 跨路径特征对齐（在跨尺度融合前对齐异构特征流），3) 定位感知轻量化设计。从图像处理、通道结构和轻量化设计三个角度优化传统无人机感知模型架构，并引入统一的细节感知检测头。

Result: 该框架在保持高效推理的同时增强了表示稳定性，统一的细节感知检测头进一步提高了回归鲁棒性，且未引入额外的部署开销。代码已在GitHub开源。

Conclusion: CollabOD通过协作式设计有效解决了无人机图像小目标检测的关键问题，在保持轻量化的同时提升了检测性能，为无人机感知系统提供了实用的解决方案。
...
TL;DR: 该论文提出Art3D框架，将2D转3D从几何重建范式转向艺术化视差合成，通过双路径架构分离全局深度参数与局部艺术效果，从专业3D电影数据中学习，实现更具电影艺术感的3D转换。


<details>
  <summary>Details</summary>
Motivation: 当前2D转3D方法虽能实现几何精度，但缺乏艺术性，无法复现专业3D电影的沉浸感和情感共鸣。传统方法将艺术意图（如零平面偏移、局部深度塑造）误认为数据噪声或模糊性，需要新的艺术驱动范式。

Method: 提出Art3D框架，采用双路径架构：一路处理全局深度参数（宏观意图），另一路处理局部艺术效果（视觉笔触）。通过间接监督从专业3D电影数据中学习，并引入初步评估方法来量化电影对齐度。

Result: 实验表明Art3D在复现关键局部出屏效果和与电影3D内容的全局深度风格对齐方面展现出潜力，为艺术驱动的转换工具奠定了基础。

Conclusion: 论文主张从物理准确的视差估计转向艺术连贯的视差合成的新范式，Art3D作为初步探索框架，为开发更具电影艺术感的2D转3D工具提供了新的研究方向。
...
TL;DR: Pano3DComposer：一种高效的端到端全景图像到3D场景生成框架，通过解耦对象生成与布局估计，在约20秒内生成高保真360度3D场景。


<details>
  <summary>Details</summary>
Motivation: 当前组合式图像到3D场景生成方法存在两个主要问题：1）依赖耗时迭代布局优化或不灵活的联合对象-布局生成；2）大多基于有限视场角透视图像，难以创建完整的360度环境。需要一种高效的前馈框架来解决这些限制。

Method: 提出Pano3DComposer框架，核心包括：1）可插拔的Object-World Transformation Predictor，将现成图像到3D模型生成的3D对象从局部坐标转换到世界坐标；2）基于VGGT架构改进的Alignment-VGGT，利用目标对象裁剪、多视角对象渲染和相机参数预测变换；3）使用伪几何监督训练解决生成对象与真实对象间的形状差异；4）针对未见域输入图像，引入Coarse-to-Fine对齐机制，通过场景渲染反馈迭代优化几何一致性。

Result: 方法在合成和真实世界数据集上的图像/文本到3D任务中实现了优越的几何精度，能够在RTX 4090 GPU上约20秒内生成高保真3D场景。

Conclusion: Pano3DComposer通过解耦对象生成与布局估计，结合高效的坐标变换预测和粗到细对齐机制，实现了快速、高质量的360度3D场景生成，为全景图像到3D场景转换提供了有效的解决方案。
...
TL;DR: 提出Skeleton-to-Image Encoding (S2I)方法，将骨架序列转换为图像格式，使预训练视觉模型能直接用于自监督骨架表示学习，解决骨架数据格式差异和数据集稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 现有大规模预训练视觉模型难以直接应用于3D人体骨架数据，因为数据格式存在根本差异。同时，大规模骨架数据集稀缺，且需要在不增加额外模型分支的情况下将骨架数据融入多模态动作识别中，这些挑战构成了重要的研究机会。

Method: 提出Skeleton-to-Image Encoding (S2I)表示方法：1）基于身体部位语义对关节进行分区和排列；2）将骨架序列转换为图像格式；3）调整到标准图像尺寸。这种编码首次使强大的视觉预训练模型能够用于自监督骨架表示学习，将丰富的视觉领域知识有效迁移到骨架分析中。

Result: 在NTU-60、NTU-120和PKU-MMD数据集上的广泛实验证明了该方法在自监督骨架表示学习中的有效性和泛化能力，特别是在具有挑战性的跨格式评估设置下表现优异。

Conclusion: S2I表示提供了一种统一的图像格式，自然地适应异构骨架数据，克服了现有骨架方法通常针对特定同质格式设计模型而忽视结构异质性的局限，为预训练视觉模型在骨架分析中的应用开辟了新途径。
...
TL;DR: ProCap：将变化描述从静态图像对比重构为动态过程建模的新框架，通过两阶段设计学习变化过程的潜在动态表示
...
Method: 两阶段框架：第一阶段训练过程编码器从关键帧学习变化过程（通过自动生成中间帧和采样减少冗余，使用caption-conditioned掩码重建任务）；第二阶段将编码器集成到编码器-解码器模型中，引入可学习的过程查询来推断潜在过程表示，然后解码为文本描述

Result: 在三个数据集上的实验证明了ProCap的有效性，代码和预训练模型已开源

Conclusion: ProCap通过将变化建模从静态图像比较重新构建为动态过程建模，能够更好地理解变化过程，生成更准确的变化描述
...
TL;DR: 该报告探讨了巴基斯坦制造的手术器械及其制造缺陷，通过深度学习架构（YOLOv8、ResNet-152、EfficientNet-b4）实现自动缺陷检测，使用4,414张高分辨率图像数据集进行质量保证。


<details>
  <summary>Details</summary>
Motivation: 现代医疗环境中，手术器械的质量标准对临床成功至关重要。即使是微小的制造缺陷也可能导致致命后果，因此制造精度至关重要。报告旨在通过自动缺陷检测技术提升巴基斯坦手术器械制造的质量标准，防止财务损失并保障患者安全。

Method: 采用深度学习方法进行自动缺陷检测，整合YOLOv8、ResNet-152和EfficientNet-b4架构。使用新开发的4,414张高分辨率图像数据集进行分析。通过与Sialkot手术器械集群的行业领导者Daddy D Pro和Dr. Frigz International合作，采用自动光学检测（AOI）工具进行质量保证。

Result: 开发了包含4,414张高分辨率图像的数据集，用于分析巴基斯坦制造的手术器械及其缺陷。通过深度学习架构实现了对裂纹、锈蚀和结构不规则等关键缺陷的识别和纠正。为制造商、医疗专业人员和监管机构提供了质量保证资源。

Conclusion: 通过整合行业专业知识和尖端技术，特别是深度学习驱动的自动缺陷检测方法，能够显著提升手术器械制造的质量标准。这种自动光学检测方法有助于创建更可靠的医疗环境，确保患者安全并减少制造商的财务损失。
...
TL;DR: RAID数据集：用于驾驶员风险感知研究的大规模标注视频数据集，包含4691个视频片段，涵盖多样化交通场景，支持弱监督风险物体识别框架


<details>
  <summary>Details</summary>
Motivation: 实现零碰撞智能车辆系统需要理解驾驶员的风险感知，这是一个复杂的认知过程，受驾驶员对外部刺激的自愿反应和周围道路使用者对自车的关注度影响。目前缺乏专门用于驾驶员风险感知研究的大规模数据集。

Method: 1. 提出RAID数据集：包含4,691个标注视频片段，涵盖多样化交通场景，标注内容包括：驾驶员意图操作、道路拓扑、风险情况（如横穿行人）、驾驶员反应、行人注意力等。2. 提出弱监督风险物体识别框架：通过建模驾驶员意图操作与反应之间的关系来识别潜在风险源。3. 分析行人注意力在风险评估中的作用。

Result: 实验评估表明，该方法在RAID数据集上比现有最优方法性能提升20.6%，在HDDS数据集上提升23.1%。同时验证了数据集的价值和行人注意力分析的有效性。

Conclusion: RAID数据集为驾驶员风险感知和上下文风险评估研究提供了重要资源。提出的弱监督风险物体识别框架通过建模驾驶员意图与反应的关系有效识别风险源，行人注意力分析进一步增强了风险评估能力。该工作为智能车辆系统的零碰撞目标提供了数据和方法支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于ViT的姿态估计器针对静态图像设计，独立处理每一帧，忽略了视频序列中的时序一致性，导致在运动模糊、遮挡或失焦等挑战性场景中预测不稳定。

Method: 提出TAR-ViTPose，包含两个核心模块：1）关节中心时序聚合（JTA），为每个关节分配可学习的查询token，从相邻帧中选择性地关注其对应区域；2）全局恢复注意力（GRA），将聚合的时序特征恢复到当前帧的token序列中，丰富姿态表示同时保留全局上下文。

Result: 在PoseTrack2017基准测试上，相比单帧基线ViTPose实现了+2.3 mAP的提升，优于现有最先进的视频方法，同时在真实应用中实现了更高的运行时帧率。

Conclusion: TAR-ViTPose通过有效的时序聚合和恢复机制，显著提升了视频2D人体姿态估计的性能和鲁棒性，验证了时序信息在视频姿态估计中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有不规则采样时间序列预测方法仅利用历史观测值预测未来，缺乏对上下文语义和细粒度时间模式的学习能力，需要更全面的多模态理解。

Method: 提出两阶段编码机制：1)跨模态视觉-文本编码模块自动生成信息丰富的视觉图像和文本数据，结合多模态大语言模型；2)ISTS编码提取历史观测的时序特征；3)自适应查询特征提取器压缩MLLMs学习到的token；4)带模态感知门控的多模态对齐模块缓解模态差异。

Result: 在真实数据上的大量实验证明了所提解决方案的有效性，能够捕获复杂时间模式和全面上下文理解。

Conclusion: MM-ISTS通过融合时间、视觉和文本模态的多模态框架，有效解决了不规则采样时间序列预测中上下文语义和细粒度模式学习的不足，为ISTS预测提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有NeRF和3DGS方法依赖耗时的逐场景优化，限制了实时部署；前馈高斯泼溅方法效率高但缺乏显式流形几何，无法直接用于仿真。

Method: 提出前馈三角形基元生成框架，包含像素对齐三角形生成模块，并引入相对3D点云监督来增强几何学习稳定性和一致性。

Result: 实验表明该方法能高效重建，同时保持与标准图形和机器人仿真器的无缝兼容性。

Conclusion: 该方法实现了单次前向传播生成仿真就绪模型，解决了现有方法在实时部署和仿真兼容性方面的限制。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在处理罕见情况或复杂道路结构时存在局限，当前道路基础设施主要为人类驾驶员设计，安全改进通常只在事故发生后被动进行，这种反应式方法对需要主动风险缓解的自动驾驶系统构成重大挑战。

Method: 1. 基于道路交通系统专业领域知识形式化本体；2. 使用大规模视觉语言模型生成基础设施改进提案；3. 通过本体驱动数据过滤增强可靠性；4. 在事故前道路图像上自动标注改进提案构建新数据集；5. 提出OD-RASE基线方法，结合LVLM和扩散模型生成改进提案及改进后道路环境图像。

Result: 实验表明，本体驱动的数据过滤能够高精度预测导致事故的道路结构及相应的改进计划，构建了包含改进提案标注的新数据集。

Conclusion: 该工作有助于提升交通环境整体安全性，是推动自动驾驶系统更广泛采用的重要一步，通过主动识别和解决潜在危险道路结构，为基础设施改进提供数据驱动方法。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在自动驾驶应用中越来越普遍，旨在利用其推理和泛化能力处理长尾场景，但这些模型经常在自动驾驶高度相关的简单视觉问题上失败，且失败原因尚不明确。因此需要深入理解VLMs在这些任务中的失败机制。

Method: 创建仅针对特定视觉概念不同的反事实图像集，然后在四种最先进的VLMs的激活上训练线性探针来区分这些图像。通过分析中间激活，评估特定视觉概念是否被线性编码，从而识别视觉信息流中的瓶颈。

Result: 研究发现：1）物体或智能体存在等概念被显式线性编码；2）物体或智能体朝向等空间视觉概念仅通过视觉编码器保留的空间结构隐式编码；3）即使概念在线性编码时，模型仍可能回答错误；4）识别出两种失败模式：感知失败（所需视觉信息未线性编码）和认知失败（视觉信息存在但未能与语言语义正确对齐）；5）物体距离增加会快速降低相应视觉概念的线性可分性。

Conclusion: 该研究通过分析VLMs中间激活，揭示了其在自动驾驶相关简单视觉任务中的失败机制，识别出感知和认知两种失败模式，并发现物体距离对概念编码的影响，从而提高了对VLMs失败案例的理解。
...
### [43] [Adaptive Radial Projection on Fourier Magnitude Spectrum for Document Image Skew Estimation](https://arxiv.org/abs/2603.05942)
*Luan Pham,Phu Hao Hoang,Xuan Toan Mai,Tuan Anh Tran*

Main category: cs.CV

TL;DR: 提出了一种基于自适应径向投影的文档图像倾斜估计方法，并在DISE-2021数据集上验证其优于现有方法


<details>
  <summary>Details</summary>
Motivation: 文档倾斜估计是文档处理系统中的关键任务，直接影响后续处理步骤。随着数字化时代的兴起，这一问题受到广泛关注，需要更鲁棒可靠的解决方案。

Method: 提出了一种新颖的倾斜估计方法：在二维离散傅里叶幅度谱上应用自适应径向投影，提取文档图像的主导倾斜角度。

Result: 实验结果表明，所提方法具有鲁棒性和可靠性，在所有对比方法中表现最优。同时构建了高质量的倾斜估计数据集DISE-2021用于评估。

Conclusion: 该方法在文档倾斜估计任务上表现出色，提供了傅里叶基方法的多个改进方面分析，源代码已开源。
...
### [44] [TempoSyncDiff: Distilled Temporally-Consistent Diffusion for Low-Latency Audio-Driven Talking Head Generation](https://arxiv.org/abs/2603.06057)
*Soumya Mazumdar,Vineet Kumar Rakesh*

Main category: cs.CV

TL;DR: TempoSyncDiff是一个基于参考条件的潜在扩散框架，通过教师-学生蒸馏实现少步推理，用于高效音频驱动的说话头生成，解决延迟、时间不稳定性和身份漂移问题。


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在说话头生成中存在高推理延迟、时间不稳定性（闪烁和身份漂移）以及在挑战性语音条件下视听对齐不完美的问题，限制了实际应用。

Method: 采用教师-学生蒸馏框架：扩散教师使用标准噪声预测目标训练，指导轻量级学生去噪器以显著更少的推理步骤运行。结合身份锚定和时间正则化减少身份漂移和帧间闪烁，使用音素级音频条件提供粗粒度唇部运动控制。

Result: 在LRS3数据集上的实验显示，蒸馏扩散模型能保留强教师的大部分重建行为，同时实现显著降低的推理延迟。CPU和边缘计算测量表明边缘部署的可行性。

Conclusion: 蒸馏扩散模型在保持重建质量的同时能大幅降低推理延迟，这是面向受限计算环境下实用扩散基说话头生成的初步探索。
...
<details>
  <summary>Details</summary>
Motivation: 生成式真实图像超分辨率（Real-ISR）存在关键问题：输出可能看起来清晰但不符合低分辨率输入的证据（语义和结构幻觉），而缺乏退化鲁棒的低分辨率参考保真度信号使得这种保真度难以评估。此外，rollout组优化瓶颈和真实退化覆盖有限限制了偏好信号质量。

Method: 提出LucidNFT框架：1) LucidConsistency：退化鲁棒的语义评估器，使低分辨率锚定的保真度可测量和可优化；2) 解耦优势归一化策略：在融合前保留每个低分辨率条件rollout组内的目标对比，防止优势崩溃；3) LucidLR：大规模真实世界退化图像集合，支持鲁棒的RL微调。

Result: 实验表明LucidNFT持续改进基于流的Real-ISR基线，在多样真实世界场景中实现更好的感知-保真度权衡，并保持稳定的优化动态。

Conclusion: LucidNFT通过引入退化鲁棒的保真度评估、改进的rollout组优化策略和大规模真实退化数据集，有效解决了真实图像超分辨率中的语义幻觉和优化瓶颈问题，实现了更好的感知质量与保真度平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有说话人脸生成方法通常只能表达固定目标情感，缺乏人类在传达信息时自然连续变化的情感表达能力，需要开发能够反映情感连续变化的更真实说话人脸生成技术

Method: 提出TIE-TFG模型，采用时序密集情感波动建模技术，通过输入文本和情感描述生成对应的情感变化序列，驱动合成视频中连续的面部表情变化

Result: 广泛评估表明该方法能够产生平滑的情感过渡，在不同情感状态下保持高质量的视觉效果和运动真实性，表现出卓越的情感连续表达能力

Conclusion: EC-TFG任务和TIE-TFG模型成功解决了说话人脸生成中情感连续变化的问题，为创造更真实、情感丰富的数字人脸提供了有效解决方案
...
Method: 提出VINE框架：1) 在骨干特征上构建空间-视角图，空间图捕捉局部几何拓扑，视角图连接不同视角特征传播视角不变的结构语义；2) 从支持-查询特征差异中推导判别性先验，捕捉类别特定对比度，重新加权SAM特征并重新校准骨干激活；3) 通过掩码交叉注意力逐步集成前景增强的SAM特征和结构丰富的ResNet特征，生成类别一致的原型作为SAM解码器的自适应提示。
...
TL;DR: OVGGT：一种无训练框架，通过自选择缓存和动态锚点保护，在固定内存和计算预算下实现任意长度视频的3D几何重建


<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型使用全注意力机制导致二次计算成本，仅适用于短序列离线处理；因果注意力变体如StreamVGGT虽支持单次流式处理，但KV缓存持续增长，GPU内存很快耗尽，无法实现长序列流式推理

Method: 提出OVGGT框架：1）自选择缓存：利用FFN残差幅度压缩KV缓存，保持与FlashAttention完全兼容；2）动态锚点保护：保护坐标关键令牌免于被驱逐，抑制长轨迹中的几何漂移

Result: 在室内、室外和超长序列基准测试中，OVGGT能在恒定VRAM限制下处理任意长度视频，同时实现最先进的3D几何精度

Conclusion: OVGGT首次实现了在固定内存和计算预算下处理任意长度视频的3D几何重建，解决了现有方法在长序列流式推理中的内存瓶颈问题
...
TL;DR: 提出基于两阶段策略（目标分割+识别）的开放词汇目标识别框架，无需复杂重训练和标注，通过CLIP生成嵌入并引入CNN/MLP编码增强灵活性，在多个数据集上超越现有方法。
...
Result: 在COCO、Pascal VOC和ADE20K数据集上的实验表明，无需训练、基于CLIP的编码（无SVD）实现了最高的平均AP，超越了当前最先进方法。同时验证了CNN/MLP图像编码在开放词汇目标识别中的潜力。

Conclusion: 提出的两阶段开放词汇目标识别框架有效解决了现有方法的局限性，无需复杂训练和标注，在多个基准数据集上表现优异，为开放词汇识别提供了简洁高效的解决方案。
...
### [50] [DeepSight: Bridging Depth Maps and Language with a Depth-Driven Multimodal Model](https://arxiv.org/abs/2603.06090)
*Hao Yang,Hongbo Zhang,Yanyan Zhao,Bing Qin*

Main category: cs.CV

TL;DR: DeepSight是首个专门用于深度理解的多模态大语言模型，通过构建深度图像-文本对数据集和深度指令数据集，改进ViT编码器以更好地捕捉深度连续变化，显著提升了三维场景理解和深度感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在图像描述和视觉问答等任务上表现优异，但往往难以准确解释视觉数据中的深度信息。深度信息对于三维场景理解至关重要，而传统方法主要关注RGB图像与文本的对齐，未能充分利用深度图像的特性。

Method: 1. 构建新颖的深度图像-文本对数据集和深度指令数据集：使用GLPN模型从视觉图像生成深度图，利用GPT-4策划相应的深度指令，并通过LLaVA验证；2. 改进CLIP中的ViT编码器：融入局部物体信息，以更有效地捕捉深度的细微连续变化；3. 开发全面的深度问答基准：基于现有深度图像数据集，严格评估典型深度图场景下的理解能力。

Result: 实验结果表明，DeepSight显著增强了深度感知能力和下游任务性能，在多模态三维理解方面取得了实质性进展。

Conclusion: DeepSight作为首个专门的深度MLLM，通过充分利用深度图像的单通道灰度特性（像素值直接反映深度线索），有效解决了传统方法在深度信息理解上的不足，为三维场景理解提供了新的解决方案。
...
TL;DR: 提出两阶段框架解决文字系统相似性度量难题：先在标注人造字母表上训练教师模型，再通过师生蒸馏扩展到历史文字系统，实现监督对比学习与无监督发现的结合。


<details>
  <summary>Details</summary>
Motivation: 文字系统和字形相似性度量面临根本挑战：人造字母表中的单个字素可以可靠标注，但不同文字系统之间的历史关系存在不确定性且存在争议。需要一种方法既能处理标注数据，又能扩展到未标注的历史文字系统。

Method: 两阶段框架：1）在标注的人造字母表上使用对比损失训练编码器，建立具有鲁棒判别特征的教师模型；2）通过师生蒸馏扩展到历史文字系统，学生模型在教师知识指导下学习无监督表示，但可以自由发现潜在的跨文字系统相似性。这种非对称设置使学生能够学习形变不变的嵌入，同时继承干净示例的判别结构。

Result: 在多样化的文字系统上进行实验，展示了有效的少样本字形识别和有意义的文字聚类，无需真实进化关系标注。该方法成功桥接了监督对比学习和无监督发现，实现了不同系统间的硬边界和反映潜在历史影响的软相似性。

Conclusion: 该框架解决了文字系统相似性度量的认识论约束，通过结合监督学习和无监督蒸馏，既能利用标注数据建立判别特征，又能扩展到历史关系不确定的文字系统，为字形分析和文字系统研究提供了新方法。
...
TL;DR: CR-QAT框架通过课程式量化感知训练和文本中心关系知识蒸馏，在极端低比特（4位）量化下显著提升开放词汇目标检测的性能，解决量化对视觉-语言对齐的破坏问题。


<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测（OVOD）模型通常规模庞大，难以部署在资源受限设备上。量化是有效的压缩方法，但极端低比特量化会严重破坏细粒度视觉-语言对齐和区域间关系结构，导致性能大幅下降。

Method: 提出CR-QAT框架，包含两个核心组件：1）课程式量化感知训练（CQAT）- 将模型分区进行渐进式量化，通过误差隔离确保稳定优化；2）文本中心关系知识蒸馏（TRKD）- 针对任务相关模块，构建文本锚定的成对相似度矩阵，全面传递教师模型的多维关系知识。

Result: 在LVIS和COCO零样本基准测试中，CR-QAT在激进低比特设置下持续优于现有QAT基线方法，相对AP提升分别达到38.9%和40.9%。

Conclusion: CR-QAT通过结合渐进式量化和关系知识蒸馏，有效解决了极端低比特量化对开放词汇目标检测中视觉-语言对齐的破坏问题，为资源受限设备上的高效部署提供了可行方案。
...
TL;DR: Place-it-R1是一个端到端视频物体插入框架，利用多模态大语言模型的环境感知推理能力，通过"思考-放置"范式实现物理一致的视频编辑。
...
Method: 采用"思考-放置"范式：1) MLLM进行物理场景理解和交互推理，生成环境感知的思维链令牌和有效插入区域；2) 引入MLLM引导的空间直接偏好优化，通过反馈评分提升视觉自然度；3) 推理时MLLM迭代触发细化循环，形成闭环渐进增强编辑质量；4) 提供两种用户可选模式：增强物理合理性的灵活模式和保持场景完整性的标准模式。

Result: 大量实验表明，Place-it-R1相比最先进解决方案和商业模型，实现了物理一致性的视频物体插入。

Conclusion: 该框架通过MLLM的认知推理与视频扩散模型的生成执行相结合，实现了物理一致且视觉自然的视频物体插入，为用户提供了物理合理性与保真度之间的明确权衡控制。
...
TL;DR: DynUAV是一个新的无人机视角多目标跟踪基准，专注于动态场景下的挑战，包含剧烈自运动和复杂表观轨迹，包含42个视频序列和170万边界框标注。
...
Method: 提出DynUAV基准，包含42个视频序列，超过170万边界框标注，涵盖车辆、行人以及挖掘机、推土机、起重机等工业类别，特别强调剧烈自运动带来的挑战。

Result: 在DynUAV上对最先进的跟踪器进行综合评估，揭示了它们在动态条件下管理和关联检测的局限性，特别是处理尺度变化、视角变化和运动模糊方面的困难。

Conclusion: DynUAV作为一个严格的基准，能够推动真实世界无人机视角MOT的进展，将成为该领域具有挑战性的测试平台。
...
TL;DR: VLM-RobustBench：首个系统评估视觉语言模型在现实世界图像失真下鲁棒性的基准，涵盖49种增强类型、133种损坏设置，发现VLMs语义理解强但空间鲁棒性脆弱


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在标准高质量数据集上表现优异，但对其在真实世界图像失真下的性能缺乏系统理解，需要建立全面评估基准来揭示模型鲁棒性弱点

Method: 构建VLM-RobustBench基准，涵盖49种增强类型（噪声、模糊、天气、数字、几何扰动），采用分级严重度（低/中/高）和二元变换，共133种损坏设置；评估四个模型家族（Qwen、InternVL、Molmo、Gemma）在两个基准上：MMBench（视觉基础）和MMMU-Pro（推理导向）

Result: 视觉严重度是性能下降的弱预测指标：低严重度的空间扰动（如glass_blur）比视觉上严重的光度损坏影响更大；重采样和几何失真（如upsample、elastic_transform）导致最大性能下降达34个百分点；当前VLMs语义理解强但空间鲁棒性脆弱

Conclusion: 需要定义新的鲁棒性评估协议和训练机制，强调重采样和几何不变性，以提升视觉语言模型在真实世界失真条件下的性能
...
TL;DR: 提出RF-Sampling，一种针对流匹配模型（特别是CFG蒸馏变体如FLUX）的训练免费推理增强框架，通过理论推导证明其隐式执行文本-图像对齐分数的梯度上升，提升生成质量和提示对齐。


<details>
  <summary>Details</summary>
Motivation: 现有推理时增强策略主要适用于传统扩散模型，在流模型上表现不佳。需要为流模型（特别是CFG蒸馏变体如FLUX）设计专门的推理增强方法，以提升生成质量和文本提示对齐。

Method: 提出Reflective Flow Sampling (RF-Sampling)：1）基于理论推导证明该方法隐式执行文本-图像对齐分数的梯度上升；2）利用文本表示的线性组合并与流反转结合；3）使模型能够探索与输入提示更一致的噪声空间；4）无需额外训练。

Result: 在多个基准测试上的广泛实验表明，RF-Sampling能持续提升生成质量和提示对齐。此外，RF-Sampling是首个在FLUX上展现出一定程度测试时缩放能力的推理增强方法。

Conclusion: RF-Sampling为流模型（特别是CFG蒸馏变体）提供了一个理论扎实、无需训练的推理增强框架，有效解决了现有方法在流模型上表现不佳的问题，并在生成质量和提示对齐方面取得显著改进。
...
Result: RePer-360仅使用1%的训练数据就超越了标准微调方法。在相同域内训练设置下，RMSE指标实现了约20%的改进。该方法在保留预训练透视先验的同时有效适应了全景域。
...
### [58] [Devil is in Narrow Policy: Unleashing Exploration in Driving VLA Models](https://arxiv.org/abs/2603.06049)
...
TL;DR: Curious-VLA框架通过两阶段设计解决VLA模型中的探索-利用困境：在模仿学习阶段使用可行轨迹扩展策略生成多样化数据，在强化学习阶段采用自适应多样性感知采样和跨度驾驶奖励来提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前自主VLA模型存在窄策略限制问题，模仿学习阶段倾向于导致探索崩溃，限制了后续强化学习阶段的潜力，而强化学习阶段由于反馈多样性不足往往过早饱和。

Method: 提出两阶段框架：1) 模仿学习阶段：采用可行轨迹扩展策略生成多个物理有效轨迹，并使用逐步归一化轨迹表示来适应多样化数据；2) 强化学习阶段：引入自适应多样性感知采样优先选择高多样性样本，并提出具有焦点风格加权的跨度驾驶奖励来扩大奖励值跨度，提高对驾驶质量的敏感性。

Result: 在Navsim基准测试中，Curious-VLA取得了最先进的结果（PDMS 90.3，EPDMS 85.4），Best-of-N PDMS达到94.8，证明了其在释放VLA模型探索潜力方面的有效性。

Conclusion: Curious-VLA框架通过解决模仿学习中的探索崩溃问题和强化学习中的反馈多样性不足问题，成功提升了VLA模型的性能，为自主驾驶系统的发展提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉特效(VFX)制作需要专业知识和高成本生产流程，现有AIGC系统面临特效特定数据稀缺、超自然/风格化效果建模困难等挑战，且通常需要逐特效微调，限制了系统的可扩展性和泛化能力。

Method: 提出EffectMaker统一框架：1) 多模态大语言模型解释高级特效语义并推理如何适应目标主体；2) 扩散变换器通过上下文学习从参考视频中捕捉细粒度视觉线索；3) 语义-视觉双路径引导机制实现准确可控的特效一致合成；4) 构建EffectData数据集（13万视频，3千特效类别）提升泛化能力。

Result: 实验表明EffectMaker在视觉质量和特效一致性方面优于现有最先进基线，提供了可扩展且灵活的定制化VFX生成范式。

Conclusion: EffectMaker通过统一的推理-生成框架解决了VFX生成中的数据稀缺和可扩展性问题，无需逐特效微调即可实现准确可控的特效定制，为定制化视觉特效生成提供了新的范式。
...
### [60] [Cut to the Chase: Training-free Multimodal Summarization via Chain-of-Events](https://arxiv.org/abs/2603.06213)
*Xiaoxing You,Qiang Huang,Lingyu Li,Xiaojun Chang,Jun Yu*

Main category: cs.CV

TL;DR: CoE是一个免训练的多模态摘要框架，通过事件链和分层事件图进行结构化推理，解决了现有方法在领域特定监督、跨模态对齐和时序建模方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有多模态摘要方法存在三个主要问题：依赖领域特定监督、跨模态对齐不明确且隐式融合、缺乏事件转换的平面时序建模。需要一种能够显式建模事件层次结构、实现跨模态对齐和时序推理的框架。

Method: 提出CoE框架，基于分层事件图（HEG）进行结构化推理。HEG将文本语义编码为显式的事件层次结构，指导跨模态对齐和时序推理。CoE通过事件链定位关键视觉线索、建模事件演化和因果转换，并通过轻量级风格适应进行领域对齐。

Result: 在八个多样化数据集上的实验表明，CoE持续优于最先进的视频CoT基线方法，平均提升为：ROUGE +3.04、CIDEr +9.51、BERTScore +1.88，显示出其鲁棒性、可解释性和跨领域泛化能力。

Conclusion: CoE通过分层事件图实现结构化推理，有效解决了多模态摘要中的关键挑战，提供了一种免训练、可解释且具有强大跨领域泛化能力的解决方案。
...
### [61] [VG3S: Visual Geometry Grounded Gaussian Splatting for Semantic Occupancy Prediction](https://arxiv.org/abs/2603.06210)
*Xiaoyang Yan,Muleilan Pei,Shaojie Shen*

Main category: cs.CV

TL;DR: VG3S：一种利用视觉基础模型几何先验增强3D高斯溅射占用预测的新框架，通过分层几何特征适配器实现跨视图3D几何接地，显著提升占用预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯溅射的占用预测方法依赖准确的几何线索，但在纯视觉中心范式中几何信息往往不足。视觉基础模型具有强大的几何接地能力，但尚未有效整合到占用预测中。

Method: 提出VG3S框架，包含分层几何特征适配器：通过特征聚合、任务特定对齐和多尺度重构，将冻结视觉基础模型的通用token转换为适用于占用预测的几何特征。

Result: 在nuScenes占用基准测试中，VG3S相比基线在IoU上提升12.6%，在mIoU上提升7.5%。框架能无缝泛化到不同视觉基础模型，一致提升占用预测精度。

Conclusion: VG3S成功将视觉基础模型的几何先验注入高斯溅射占用预测，证明了整合预训练几何接地视觉基础模型先验的巨大价值，为自动驾驶场景理解提供了新思路。
...
TL;DR: 提出MOSIV框架，解决视频中多物体系统识别问题，通过可微分模拟器直接优化连续物体材料参数


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对单物体场景或固定材料原型的离散分类，无法处理多物体系统识别问题

Method: 提出MOSIV框架，使用可微分模拟器直接优化连续物体材料参数，通过视频几何目标指导

Result: 在新合成基准测试中，MOSIV显著提高接地准确性和长时程模拟保真度，优于基线方法

Conclusion: 物体级细粒度监督和几何对齐目标对复杂多物体设置中的稳定优化至关重要，MOSIV为该新任务建立了强基线
...
TL;DR: TaPD是一个用于轨迹预测的观测自适应框架，通过渐进式知识蒸馏和时序回填模块处理可变历史长度，在短观测情况下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测器通常假设固定长度的历史观测，但在真实场景中（如遮挡或感知范围限制）观测长度可变或极短，导致性能大幅下降。需要一种能自适应不同观测长度的统一框架。

Method: 提出TaPD框架，包含两个协同模块：1) 观测自适应预测器(OAF)，基于渐进式知识蒸馏(PKD)，通过层次特征回归将长时程"教师"的运动模式知识转移到短时程"学生"；2) 时序回填模块(TBM)，为极短历史显式重建缺失的历史片段。采用余弦退火蒸馏权重平衡预测监督和特征对齐，并使用解耦的预训练-重建-微调协议。

Result: 在Argoverse 1和Argoverse 2数据集上的大量实验表明，TaPD在所有观测长度下均优于强基线方法，在极短输入情况下提升尤其显著，并能以即插即用方式改进其他预测器（如HiVT）。

Conclusion: TaPD提供了一个统一的即插即用框架，通过渐进式知识蒸馏和时序回填有效处理可变历史长度的轨迹预测问题，在真实世界场景中具有鲁棒性和实用性。
...
TL;DR: StruVis是一个新颖的文本到图像生成框架，通过结构化视觉表示在纯文本推理过程中增强视觉感知，避免了传统方法的计算成本高和视觉限制问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成的推理框架存在两个主要问题：1) 纯文本推理虽然计算高效但缺乏视觉上下文，容易忽略关键空间和视觉元素；2) 文本-图像交错推理虽然能利用视觉参考，但计算成本高且受限于生成器的表达能力。

Method: 提出StruVis框架，使用基于文本的结构化视觉表示作为中间推理状态，让多模态大语言模型在纯文本推理过程中有效"感知"视觉结构，实现结构化视觉引导的推理，并且该框架与生成器无关，可灵活集成到不同的T2I生成器中。

Result: 在推理型文本到图像生成基准测试中取得显著性能提升，在T2I-ReasonBench上获得4.61%增益，在WISE上获得4%增益。

Conclusion: StruVis通过结构化视觉表示有效解决了现有推理框架的局限性，既保持了计算效率又增强了视觉感知能力，为推理型文本到图像生成提供了高效且灵活的解决方案。
...
TL;DR: NOVA提出了一种创新的开放词汇自回归范式，将3D多目标跟踪从传统的基于距离的匹配转向生成式时空语义建模，通过大语言模型实现轨迹序列的下一步完成，显著提升了未知目标的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有3D多目标跟踪方法受限于闭集假设和"语义盲"启发式方法，无法有效泛化到未知目标类别。需要一种能够同时编码物理运动连续性和深度语言先验的新范式来解决开放世界感知中的泛化问题。

Method: NOVA将3D轨迹重新表述为结构化时空语义序列，利用大语言模型的自回归能力，将跟踪任务转化为基于原则的下一步序列完成过程。该方法通过语言空间的分层结构解决细粒度语义歧义，并通过高层常识推理保持复杂长程序列中的身份一致性。

Result: 在nuScenes、V2X-Seq-SPD和KITTI数据集上的广泛实验表明NOVA具有优越性能。在nuScenes数据集上，NOVA在Novel类别上实现了22.41%的AMOTA，相比基线有20.21%的绝对提升，这些成果仅通过一个紧凑的0.5B自回归模型实现。

Conclusion: NOVA通过将3D跟踪从传统碎片化距离匹配转向生成式时空语义建模，成功解决了开放世界感知中的泛化问题，为3D多目标跟踪提供了新的范式，显著提升了未知目标类别的跟踪性能。
...
TL;DR: OA-SORT是一个用于多目标跟踪的即插即用免训练框架，通过遮挡感知模块、遮挡感知偏移和偏差感知动量来解决2D MOT中因部分遮挡引起的定位成本混淆问题。


<details>
  <summary>Details</summary>
Motivation: 2D多目标跟踪面临因部分遮挡导致的定位成本混淆问题，这会影响轨迹分析和目标计数精度。现有方法在处理遮挡方面存在不足，需要更有效的遮挡感知机制来提升跟踪性能。

Method: 提出OA-SORT框架，包含三个核心组件：1)遮挡感知模块(OAM)分析目标遮挡状态，引入高斯图减少背景影响；2)遮挡感知偏移(OAO)利用OAM描述的遮挡状态缓解成本混淆；3)偏差感知动量(BAM)抑制估计不稳定性。该框架是即插即用且免训练的。
...
Conclusion: 遮挡处理在多目标跟踪中至关重要，OA-SORT框架通过创新的遮挡感知机制有效解决了2D MOT中的定位成本混淆问题，具有即插即用、免训练和可重用性等优势，显著提升了跟踪性能。
...
TL;DR: 该论文提出了OccNL基准和DPR-Occ框架，用于解决3D语义占据预测中标签噪声问题，在90%噪声下仍能保持性能


<details>
  <summary>Details</summary>
Motivation: 3D语义占据预测是机器人感知的基础，但真实世界体素标注存在结构伪影和动态拖尾效应等固有噪声。现有方法在稀疏3D体素空间中表现不佳，需要研究如何使自主系统可靠地使用这种不可靠的占据监督

Method: 提出DPR-Occ框架，通过双源部分标签推理构建可靠监督。结合时间模型记忆和表示层面的结构亲和性，动态扩展和修剪候选标签集，保留真实语义同时抑制噪声传播

Result: 在SemanticKITTI上的实验表明，DPR-Occ在极端噪声下能防止几何和语义崩溃。在90%标签噪声下，相比现有标签噪声学习方法，实现了显著性能提升（最高2.57% mIoU和13.91% IoU）

Conclusion: 通过连接标签噪声学习和3D感知，OccNL基准和DPR-Occ框架为动态环境中安全关键的机器人感知提供了可靠基础
...
TL;DR: 该论文提出使用分层子采样处理VGG16超列，并研究集成学习方法在稀疏超列上的性能，在脑肿瘤数据集上显著提升了小样本情况下的分割性能。


<details>
  <summary>Details</summary>
Motivation: 受生物视觉启发的高维超列在图像分割中具有潜力，但实际应用较少，主要原因是处理密集超列的计算复杂度随训练集大小线性增长。本文旨在解决这一挑战。

Method: 对VGG16基础超列应用分层子采样，研究堆叠和投票集成学习方法在稀疏超列上的性能，并与标准多尺度UNet基线进行比较。

Result: 在脑肿瘤数据集上，当N≤20的极端小样本情况下，简单的逻辑回归分类器最有效。在10%分层子采样率下，N=20时的最佳平均Dice得分为0.66，相比标准多尺度UNet基线有24.53%的统计显著提升（p值=3.07e-11）。

Conclusion: 分层子采样和集成学习能有效处理超列的计算复杂度问题，在小样本情况下显著提升分割性能，特别是在极端低样本情况下，简单分类器比复杂集成方法更有效。
...
TL;DR: DEX-AR是一种针对自回归视觉语言模型的新型可解释性方法，通过动态头过滤和序列级过滤生成逐token和序列级2D热图，显著提升了扰动和分割指标


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型变得越来越复杂和广泛应用，理解其决策过程变得至关重要。传统的可解释性方法设计用于分类任务，难以处理现代自回归VLM的复杂逐token生成过程以及视觉和文本模态之间的复杂交互

Method: DEX-AR通过计算逐token生成过程中注意力图的层间梯度来解释自回归VLM，包含两个关键创新：1)动态头过滤机制识别关注视觉信息的注意力头；2)序列级过滤方法聚合逐token解释，区分视觉基础和纯语言token
...
### [70] [Spatial Calibration of Diffuse LiDARs](https://arxiv.org/abs/2603.06531)
*Nikhil Behari,Ramesh Raskar*

Main category: cs.CV

TL;DR: 提出一种针对漫反射直接飞行时间LiDAR的空间标定方法，通过扫描逆反射贴片估计每个LiDAR像素在RGB图像平面中的有效区域和相对空间灵敏度
...
Method: 使用扫描的逆反射贴片结合背景减除技术，恢复每个像素的响应图，估计每个漫反射LiDAR像素在共置RGB图像平面中的足迹（有效支持区域）和相对空间灵敏度

Result: 在ams OSRAM TMF8828上验证了该方法，获得了明确的LiDAR到RGB对应关系，可用于跨模态对齐和融合

Conclusion: 该方法为漫反射LiDAR提供了有效的空间标定方案，解决了传统单射线假设不适用的问题，实现了LiDAR与RGB图像之间的精确对应关系
...
### [71] [FontUse: A Data-Centric Approach to Style- and Use-Case-Conditioned In-Image Typography](https://arxiv.org/abs/2603.06038)
*Xia Xin,Yuki Endo,Yoshihiro Kanamori*

Main category: cs.CV

TL;DR: 该论文提出了一种数据驱动的方法FontUse，通过构建大规模字体数据集和训练图像生成模型，解决了文本到图像模型中字体样式控制不足的问题。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在生成高质量图像方面表现良好，但在控制字体排版方面存在明显不足：用户请求的字体样式经常被忽略或仅得到弱遵循。这限制了模型在需要精确字体控制的场景（如设计、广告等）中的应用。

Method: 采用数据为中心的方法，构建了一个专门针对字体排版的大规模数据集FontUse（约70K图像）。使用分割模型和多模态大语言模型自动生成包含用户友好提示、文本区域位置和OCR识别字符串的结构化标注。提示明确结合字体样式（如衬线体、手写体、优雅体）和使用场景（如婚礼请柬、咖啡店菜单）。基于这些标注微调现有图像生成模型，无需修改架构即可让模型一致地解释样式和使用场景条件。

Result: 提出了基于Long-CLIP的评估指标来测量生成字体与请求属性之间的对齐度。实验表明，使用该管道训练的模型在多样化提示和布局下，生成的文本渲染比竞争基线更符合提示要求。源代码已在GitHub上公开。

Conclusion: 通过数据驱动的标注管道和专门的字体数据集，能够显著提升文本到图像模型在字体样式控制方面的性能，使模型能够更准确地响应用户的字体排版需求，为设计应用提供了实用解决方案。
...
TL;DR: K-MaT是一个基于知识锚定流形传输的提示学习框架，用于将生物医学视觉语言模型从高端成像模态（如CT）迁移到低端模态（如X光），无需低端训练图像，通过最优传输对齐提示流形实现跨模态零样本部署。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模生物医学视觉语言模型在适应高端成像模态（如CT）后，往往无法有效迁移到前线低端成像模态（如X光），容易陷入模态特定的捷径学习，导致性能崩溃。需要一种无需低端训练图像就能实现跨模态知识迁移的方法。

Method: 提出K-MaT框架：1）分解提示并锚定到临床文本描述；2）使用融合Gromov-Wasserstein最优传输将低端提示流形对齐到视觉基础的高端空间；3）通过提示学习实现决策结构的跨模态迁移。

Result: 在四个跨模态基准测试（皮肤镜、乳腺X光到超声、CT到胸部X光）上取得SOTA结果：平均准确率调和均值从BiomedCoOp的42.0%提升到44.1%，宏F1分数达到36.2%。在具有挑战性的乳腺成像任务中，有效缓解了标准方法（如CoOp）的灾难性遗忘问题（CoOp在低端模态准确率降至27.0%）。

Conclusion: 通过最优传输对齐提示流形为医学视觉语言模型的零样本跨模态部署提供了高效途径，能够在不依赖低端训练图像的情况下实现稳健的跨模态性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将多视角多帧图像独立输入LLMs，导致计算冗余和空间一致性不足，阻碍准确的3D空间推理和几何一致性。BEV表示虽然提供空间结构但缺乏语义丰富性，需要桥接这一差距。

Method: 提出BEVLM框架，将空间一致且语义蒸馏的BEV表示与LLMs连接，使用BEV特征作为统一输入，并通过知识蒸馏将LLMs的语义知识注入BEV表示。

Result: BEVLM使LLMs在跨视图驾驶场景中的推理能力提升46%准确率，通过知识蒸馏将LLMs语义知识注入BEV表示，使端到端驾驶在安全关键场景中性能提升29%。

Conclusion: BEVLM通过连接空间一致的BEV表示与LLMs，有效解决了现有方法在视觉处理和空间推理方面的局限性，显著提升了自动驾驶系统的推理能力和端到端性能。
...
<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在视觉理解方面表现出色，但在生成能力方面相对较弱，存在理解与生成过程的内在解耦问题，需要缩小这一能力差距

Method: 提出GvU机制：基于理解分支的token级内在文本-图像对齐奖励，使模型同时作为教师和学生；设计自监督强化学习框架，通过理解驱动的内在奖励信号迭代改进生成质量

Result: 实验结果表明该方法显著提升了统一多模态模型的生成能力，同时增强了细粒度视觉理解，缩小了视觉理解与生成之间的能力差距

Conclusion: 通过利用模型内部的理解能力来指导生成过程，可以有效缩小统一多模态模型中理解与生成之间的能力差距，实现两者相互促进的良性循环
...
TL;DR: GenHOI是一个轻量级增强模块，用于提升预训练视频生成模型在复杂真实场景中的手-物交互生成能力，通过时间平衡和空间选择性机制改善物体一致性和交互保真度。


<details>
  <summary>Details</summary>
Motivation: 现有手-物交互重演方法通常在域内训练和评估，难以泛化到复杂的真实场景；而通用视频编辑模型虽然鲁棒性更强，但在手-物交互特定问题上（如物体外观不一致）仍有不足。需要一种能结合两者优势的解决方案。

Method: 提出GenHOI作为预训练视频生成模型的轻量级增强模块：1) 时间平衡：提出Head-Sliding RoPE，为参考标记分配头部特定的时间偏移，均匀分布其影响，缓解3D RoPE的时间衰减，改善长程物体一致性；2) 空间选择性：设计两级空间注意力门，将物体条件注意力集中在手-物交互区域，并自适应调整强度，保持背景真实性的同时增强交互保真度。

Result: 在未见过的真实场景上进行广泛定性和定量评估，GenHOI在手-物交互重演和通用视频编辑方法中均显著优于现有最先进方法。

Conclusion: GenHOI通过时间平衡和空间选择性机制有效解决了手-物交互生成中的物体一致性和交互保真度问题，在复杂真实场景中表现出色，为手-物交互视频合成提供了有效的轻量级增强方案。
...
Method: 将提示敏感性重新表述为组一致性问题，将语义相关的提示组织成共享相同真值掩码的提示组，提出提示组感知训练框架，包括：(1)基于质量引导的组正则化，利用分割损失作为隐式排名信号；(2)采用停止梯度策略的logit级一致性约束来对齐组内预测

Result: 在多数据集细胞核基准测试中，该方法在文本提示下获得一致增益，显著降低了不同提示质量水平下的性能方差；在六个零样本跨数据集任务中，Dice分数平均提升2.16个百分点
...
TL;DR: PSIVG框架通过将物理模拟器集成到视频扩散过程中，生成符合物理规律（重力、惯性、碰撞）的视频，解决了现有方法在物理一致性上的不足。
...
Method: 提出PSIVG框架：1）从预训练扩散模型生成的模板视频重建4D场景和前景物体网格；2）在物理模拟器中初始化并生成物理一致的运动轨迹；3）用模拟轨迹指导视频生成器实现时空物理一致的运动；4）提出TTCO技术，基于模拟器的像素对应关系调整文本和特征嵌入，改善物体运动时的纹理一致性。

Result: 综合实验表明，PSIVG生成的视频能更好地遵循真实世界物理规律，同时保持视觉质量和多样性。

Conclusion: PSIVG通过集成物理模拟器到视频扩散流程，显著提升了生成视频的物理一致性，为更真实可靠的AI视频生成提供了有效解决方案。
...
TL;DR: CLoPA是一种持续适应策略，通过微调nnInteractive模型的小部分参数来提升医学图像交互分割性能，无需改变推理流程，在8个医学分割任务中快速达到专家水平。


<details>
  <summary>Details</summary>
Motivation: 现有零射模型如nnInteractive在多样化医学成像任务中无法稳定达到专家级性能。由于标注活动会产生持续增长的特定任务标注数据流，在线适应分割模型成为零射推理的自然补充。

Method: 提出CLoPA持续适应策略，在标注缓存上微调nnInteractive模型的小部分参数，通过轻量级片段调度触发。无需新参数或改变推理流程，完全在现有标注工作流中运行。

Result: 在8个医学分割Decathlon任务中，CLoPA快速将性能提升至专家级，即使nnInteractive先前失败的任务也能成功。多数增益在单个训练片段后实现。不同参数组调优效果取决于任务特性和数据机制。

Conclusion: CLoPA通过持续适应显著提升交互分割性能。对于复杂几何目标（如肝血管），实例归一化和低层特征调优会饱和，表明在最具挑战性场景中需要更深层特征表示对齐。
...
TL;DR: AI系统通过45,139张超声图像训练，能准确诊断胎儿口面裂，敏感性和特异性超93%/95%，媲美资深放射科医生，并作为医疗副驾驶提升初级医生诊断能力。
...
Method: 开发了一个人工智能系统，基于来自22家医院9,215名胎儿的45,139张超声图像进行训练。该系统可作为医疗副驾驶辅助放射科医生，并通过试点研究评估其对临床专业知识发展的影响。

Result: AI系统诊断胎儿口面裂的敏感性和特异性分别超过93%和95%，与资深放射科医生表现相当，显著优于初级放射科医生。作为医疗副驾驶使用时，能将初级放射科医生的敏感性提高6%以上。试点研究（24名放射科医生和学员）表明该系统能改善罕见病症的专业知识发展。

Conclusion: 这种双重用途方法为改善诊断准确性和专家培训提供了可扩展的解决方案，特别是在经验丰富的放射科医生稀缺的环境中。AI系统不仅能直接辅助诊断，还能加速临床专业知识的培养。
...
TL;DR: SUREON是一个大规模外科视频问答数据集，通过从外科教学视频中提取专家讲解来获取外科推理信号，并训练了两个能回答复杂外科问题的模型。


<details>
  <summary>Details</summary>
Motivation: 当前外科AI缺乏对手术意图、决策理性和风险预测等推理能力的理解，而专家讲解的外科教学视频包含这些推理信息，但缺乏大规模标注数据。

Method: 创建SUREON数据集，定义12个问题类别，使用多智能体管道从外科学术视频中提取结构化监督；训练SureonVLM（监督微调）和SureonVLM-R1（Group Relative Policy Optimization）两个模型。

Result: SUREON包含134.7K个视频片段、170种手术类型、206.8k个QA对和354个专家验证基准；两个模型在SUREON基准上准确率超过84%，在标准外科感知任务上优于通用领域模型。

Conclusion: 从外科教学视频中提取的监督信号能有效训练具有外科推理能力的AI模型，为外科AI从单纯感知向理解意图和决策推理发展提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦域泛化行人重识别方法依赖全局特征表示和简单平均聚合，存在两个局限：1) 仅使用全局特征难以捕捉细粒度域不变局部细节；2) 统一参数平均将所有客户端视为等同，忽视了它们在鲁棒特征提取能力上的差异，稀释了高质量客户端的贡献

Method: 提出FedARKS框架，包含两个机制：RK（鲁棒知识）机制提取细粒度域不变局部特征；KS（知识选择）机制根据客户端鲁棒特征提取能力进行差异化聚合

Result: 论文声称FedARKS能够提升模型在未见域上的泛化能力，同时保护客户端数据隐私

Conclusion: FedARKS通过结合鲁棒知识提取和知识选择机制，有效解决了现有联邦域泛化行人重识别方法的局限性，提升了模型性能
...
TL;DR: RMD是一种跨分辨率分布匹配蒸馏框架，通过logSNR曲线划分时间步区间、跨分辨率分布匹配和预测噪声重注入机制，实现高保真、少步数的多分辨率级联推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有扩散蒸馏方法受限于去噪过程，步数减少已基本饱和；部分时间步低分辨率生成虽能加速，但因跨分辨率分布差距导致质量下降。需要解决跨分辨率分布差距问题以实现高质量加速推理。

Method: 1) 使用对数信噪比(logSNR)曲线为每个分辨率划分时间步区间；2) 引入基于logSNR的映射补偿分辨率引起的偏移；3) 沿分辨率轨迹进行分布匹配，缩小低分辨率生成器分布与教师高分辨率分布的差距；4) 在上采样过程中加入预测噪声重注入机制以稳定训练并提高合成质量。

Result: RMD在保持高保真生成的同时显著加速推理：在SDXL上实现33.4倍加速，在Wan2.1-14B上实现25.6倍加速，同时保持高视觉保真度。定性和定量结果均显示其优越性能。

Conclusion: RMD通过跨分辨率分布匹配有效解决了现有扩散蒸馏方法的局限性，实现了高质量、高效率的多分辨率级联推理，为扩散模型的加速应用提供了新思路。
...
TL;DR: 该论文研究了视觉语言模型在结构化颜色失真下的系统性感知弱点，发现即使场景对人类来说仍然可识别，模型也会产生自信但无意义的预测。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在基准测试中表现良好，但它们存在系统性感知弱点：像素值的结构化、大幅度变化可能导致模型产生自信但无意义的预测，即使底层场景对人类来说仍然容易识别。研究者希望通过研究这种差距来理解模型的感知局限性。

Method: 研究者使用空间颜色混合方法，这是一种程序化的颜色失真家族，在自然图像上叠加结构化模式（包括RGB和Ostwald颜色系统）。他们引入了八种空间颜色混合变体的框架，并在四个数据集上评估了来自三个模型家族的九个视觉语言模型。同时进行了包含61名参与者的人类研究，比较人类和模型在相同失真条件下的表现。

Result: 随着失真程度的增加，所有模型和数据集上的准确率急剧下降，扩大语言模型规模并不能可靠地缓解这种失败。在人类研究中，参与者在相同的失真条件下显著优于视觉语言模型。研究者还发现，一个简单的人类启发式预处理步骤能够为几种失真类型恢复相当一部分性能。

Conclusion: 视觉语言模型存在系统性的感知弱点，特别是在结构化颜色失真下。人类启发式预处理和工具使用可以作为提高视觉语言模型鲁棒性的实用策略，这为开发感知感知的预处理方法提供了动机。
...
### [84] [Longitudinal NSCLC Treatment Progression via Multimodal Generative Models](https://arxiv.org/abs/2603.06147)
...
TL;DR: 提出虚拟治疗框架，将非小细胞肺癌放疗期间肿瘤演化建模为剂量感知的多模态条件图像转换问题，用于合成治疗诱导的解剖变化随访CT图像。
...
Method: 提出虚拟治疗框架，将NSCLC进展建模为剂量感知多模态条件图像转换问题。基于CT扫描、基线临床变量和指定辐射剂量增量，合成随访CT图像。在222名III期NSCLC患者的895个CT扫描数据集上评估，使用GAN和扩散模型在2D和2.5D配置下进行基准测试。

Result: 扩散模型相比GAN基线更能从多模态剂量感知条件中获益，生成更稳定且解剖学上合理的肿瘤演化轨迹，产生更稳定的肿瘤演化轨迹。

Conclusion: 虚拟治疗框架作为NSCLC计算机模拟治疗监测和自适应放疗研究工具具有潜力，扩散模型在模拟治疗诱导解剖变化方面表现优于GAN方法。
...
TL;DR: FreeOcc：一种无需训练的3D语义和全景占据预测方法，利用预训练基础模型从多视角图像恢复语义和几何信息，在未见环境中实现部署


<details>
  <summary>Details</summary>
Motivation: 当前基于相机的3D占据预测方法通常需要昂贵的密集3D监督或需要针对目标域数据进行训练，这限制了在未见环境中的部署。需要一种无需训练的方法来克服这些限制。

Method: 提出FreeOcc训练免费流水线：1) 使用可提示基础分割模型提取每视角全景先验；2) 使用重建基础模型恢复度量3D点云；3) 通过深度和置信度感知过滤将可靠标签提升到3D；4) 随时间融合并通过确定性细化堆栈进行体素化；5) 通过拟合和合并鲁棒的当前视角3D边界框候选来恢复实例，实现无需学习3D模型的实例感知占据。

Result: 在Occ3D-nuScenes上：1) 无需训练达到16.9 mIoU和16.5 RayIoU，与最先进的弱监督方法相当；2) 作为伪标签生成流水线训练下游模型时达到21.1 RayIoU，超越先前弱监督基线；3) 在无需训练和弱监督全景占据预测中分别达到3.1 RayPQ和3.9 RayPQ，设立新基线。

Conclusion: FreeOcc证明了基础模型驱动的感知是实现无需训练3D场景理解的实用途径，为在未见环境中部署3D场景理解系统提供了可行方案。
...
TL;DR: 提出一种用于乳腺超声图像分割的半监督框架，通过免训练伪标签生成和标签细化，利用视觉语言模型生成结构一致的伪标签，在仅2.5%标注数据下达到全监督模型性能。
...
Method: 1) 基于简单外观描述（如"暗色椭圆形"）实现跨域结构迁移，使视觉语言模型生成结构一致的伪标签；2) 用伪标签预热静态教师模型以捕获乳腺病变的全局结构先验；3) 结合指数移动平均教师，引入不确定性熵加权融合和自适应不确定性引导的反向对比学习以改善边界判别。

Result: 在四个乳腺超声数据集上，仅使用2.5%标注数据即达到与全监督模型相当的性能，显著优于现有半监督方法。框架可扩展性强，仅需全局外观描述即可获得可靠伪监督。

Conclusion: 提出了一种可扩展的半监督医学图像分割范式，通过视觉语言模型的跨域结构迁移和双重教师机制，在有限标注下实现了高性能分割，为其他成像模态和疾病提供了通用解决方案。
...
Method: 将RGB-D全景图像转换为对应的切向透视图像和3D点云，利用这些模态提取和对齐基础视觉-语言特征，通过自然语言查询在两种输入模态上生成语义掩码。

Result: 在Stanford-2D-3D-s和ToF-360数据集上的实验表明，JOPP-3D能够跨全景和3D域生成连贯且语义有意义的分割，在开放和封闭词汇的2D和3D语义分割上相比SOTA有显著改进。

Conclusion: JOPP-3D成功实现了跨模态的开放词汇语义分割，通过联合处理全景和点云数据，为语言驱动的场景理解提供了有效解决方案。
...
TL;DR: 该论文提出了一种使用强化学习（RL）和多尺度反馈增强3D扩散模型的方法，通过结合2D切片评估和3D体积分析来优化生成质量，在医学图像生成任务中取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 扩散模型已成为3D医学图像生成的强大工具，但标准训练目标与临床相关性之间存在差距。需要一种方法来提升生成图像的质量和临床实用性。

Method: 首先在MRI体积数据上预训练3D扩散模型建立生成先验，然后使用近端策略优化（PPO）进行微调，采用结合2D切片评估和3D体积分析的新型奖励系统，同时优化局部纹理细节和全局结构一致性。

Result: 在BraTS 2019和OASIS-1数据集上的验证表明，RL反馈有效引导生成过程向更高质量分布。定量分析显示Fréchet Inception Distance（FID）显著改善，合成数据在下游肿瘤和疾病分类任务中表现出比未优化基线更强的实用性。

Conclusion: 提出的强化学习多尺度反馈方法能够有效提升3D扩散模型的生成质量，增强生成图像的临床相关性，为医学图像生成提供了一种有前景的优化框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有训练免费扩散分割方法通常依赖交叉注意力图，但未能随更强大的扩散模型而有效扩展。研究发现存在两个关键问题：1）跨多头和多层的注意力图与统一全局表示之间存在差异；2）即使有全局图，由于不同文本token的分数不平衡，也无法直接转化为准确的分割语义相关性。

Method: 提出两种核心技术：1）自动聚合（auto aggregation）：解决多头多层注意力图与全局表示之间的差异；2）逐像素重缩放（per-pixel rescaling）：解决不同文本token分数不平衡问题，使全局图能更准确地转化为分割语义相关性。

Result: 在标准语义分割基准上评估该方法，并进一步将其集成到生成技术中，展示了改进的性能和广泛的适用性。

Conclusion: 通过自动聚合和逐像素重缩放两种技术，成功解决了训练免费扩散分割器中的关键问题，使分割性能能够更好地利用生成模型的强大能力，实现了更好的分割效果和更广泛的适用性。
...
TL;DR: 提出了一种基于骨架时序动作分割的点监督框架，仅需每个动作片段标注一帧，通过多模态骨架数据、原型相似度方法和伪标签集成实现高效分割


<details>
  <summary>Details</summary>
Motivation: 全监督方法需要昂贵的逐帧标注且对模糊动作边界敏感，点监督方法可显著减少标注成本并解决边界模糊问题

Method: 使用预训练统一模型编码关节、骨骼和运动多模态特征；提出原型相似度方法，结合能量函数和约束K-Medoids聚类生成伪标签；采用多模态伪标签集成指导模型训练

Result: 在PKU-MMD (X-Sub和X-View)、MCFS-22和MCFS-130数据集上建立新基准，实验表明该方法达到有竞争力的性能，甚至超越部分全监督方法，同时显著减少标注工作量

Conclusion: 提出的点监督框架为骨架动作分割提供了一种高效解决方案，在保持性能的同时大幅降低标注成本，具有实际应用价值
...
<details>
  <summary>Details</summary>
Motivation: 标准3DGS产生的高斯中心与物体几何对齐不佳，而表面重建方法常牺牲光度精度。需要一种既能提升几何精度又能保持渲染质量的方法。

Method: 提出特征值熵优化的邻域致密化策略EntON：计算每个高斯中心k近邻协方差矩阵特征值的特征值熵，量化局部结构有序度。在交替优化框架中，结合标准梯度致密化和特征值熵感知致密化，优先在低熵（有序、平坦）区域致密化以捕捉细节，在高熵（无序、球形）区域修剪。
...
TL;DR: WAFL提出基于词锚定的时序伪造定位新范式，将任务从时序回归和连续定位转变为离散词级二分类，显著提升定位性能并降低计算成本


<details>
  <summary>Details</summary>
Motivation: 现有时序伪造定位方法存在特征粒度不匹配和计算成本高的问题，需要一种更高效、更精确的定位范式

Method: 1) 识别最小有意义的伪造单元——词令牌，并与语音自然语言边界对齐；2) 引入法证特征重对齐模块，将预训练语义空间映射到可区分的法证流形；3) 设计基于伪影的非对称损失函数，动态抑制真实梯度并优先处理细微法证伪影

Result: WAFL在数据集内和跨数据集设置下均显著优于现有方法，同时需要更少的可学习参数并实现高计算效率

Conclusion: WAFL通过将时序伪造定位重构为离散词级二分类任务，解决了特征粒度不匹配和计算成本高的问题，为时序伪造检测提供了新的高效范式
...
<details>
  <summary>Details</summary>
Motivation: 现有3D指代表达分割方法仅依赖稀疏点云，缺乏丰富的视觉语义信息来处理细粒度描述。当描述匹配多个或零个目标时，现有方法表现不佳。

Method: 提出HCF-RES多模态框架：1）分层视觉语义分解：利用SAM实例掩码指导CLIP编码，获取像素级和实例级特征，在2D到3D投影时保持对象边界；2）渐进多级融合：通过模态内协作、2D语义与3D几何特征间的跨模态自适应加权，以及语言引导的精炼来整合表示。

Result: HCF-RES在ScanRefer和Multi3DRefer数据集上取得了最先进的结果。

Conclusion: 通过结合分层视觉语义分解和渐进多级融合，HCF-RES能够有效处理3D场景中的指代表达分割问题，特别是在描述匹配多个或零个目标的复杂情况下。
...
Method: 提出StrSR框架，采用非对称判别蒸馏架构解决轨迹差距，设计频率分布匹配策略抑制DiT特有的高频谱泄漏引起的周期性伪影

Result: 大量实验证明StrSR在真实图像超分辨率中实现了最先进的性能，包括定量指标和视觉感知

Conclusion: StrSR通过谱和轨迹正则化有效解决了扩散变换器在真实图像超分辨率中的单步蒸馏挑战，代码和模型将开源
...
TL;DR: ADiVA方法通过联合建模属性分布和执行显式语义-视觉对齐，解决生成式零样本学习中的类-实例差距和语义-视觉域差距问题，显著提升性能并可作为插件增强现有方法。


<details>
  <summary>Details</summary>
Motivation: 生成式零样本学习通过合成未见类特征来转移知识，但面临两个内在挑战：1) 类级属性因类内变异性大而无法捕捉实例特定的视觉外观，导致类-实例差距；2) 语义与视觉特征分布间的显著不匹配（表现为类间相关性）导致语义-视觉域差距。

Method: 提出ADiVA方法，包含两个模块：1) 属性分布建模(ADM)模块，学习每个类的可转移属性分布并为未见类采样实例级属性；2) 视觉引导对齐(VGA)模块，细化语义表示以更好地反映视觉结构。该方法联合建模属性分布并执行显式语义-视觉对齐。

Result: 在三个广泛使用的基准数据集上，ADiVA显著优于最先进方法，在AWA2和SUN数据集上分别获得4.7%和6.1%的性能提升。此外，该方法可作为插件增强现有生成式ZSL方法。

Conclusion: ADiVA通过解决类-实例差距和语义-视觉域差距，有效提升了生成式零样本学习的性能，其模块化设计使其能够作为插件增强现有方法，为ZSL领域提供了有前景的解决方案。
...
TL;DR: FlowMotion：基于流预测的训练免费视频运动迁移框架，通过提取潜在预测中的运动表征实现高效灵活的运动对齐


<details>
  <summary>Details</summary>
Motivation: 现有训练免费方法依赖T2V模型中间输出构建运动指导，计算开销大且灵活性有限。本文旨在开发更高效灵活的运动迁移方法。

Method: 提出FlowMotion框架：1）利用流基T2V模型的预测输出直接提取运动表征；2）基于早期潜在预测编码的丰富时间信息设计流指导；3）引入速度正则化策略稳定优化并确保平滑运动演化。

Result: FlowMotion在时间和资源效率上表现优越，与最先进方法相比具有竞争力。纯基于模型预测的操作实现了高效性能。

Conclusion: FlowMotion通过直接利用流基T2V模型的预测输出，实现了高效灵活的视频运动迁移，为训练免费方法提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: CBCT在牙科成像中广泛应用，但高密度植入物会导致严重伪影，影响图像质量和诊断准确性。现有基于扩散模型的植入物修复方法仅处理独立的2D投影，忽略了投影间的相关性，导致重建图像不一致。

Method: 提出3D牙科植入物修复方法，基于垂直分数扩散模型。训练两个不同平面的2D分数扩散模型，在采样方案中结合这两个模型来建模投影序列的3D分布，在投影域进行操作。

Result: 该方法能生成高质量、伪影减少的3D CBCT图像，证明了其在改善临床成像方面的有效性。

Conclusion: 该方法通过建模投影序列的3D相关性，解决了现有2D方法的不一致性问题，为临床CBCT成像提供了有前景的伪影减少解决方案。
...
TL;DR: LTA是一种基于潜在空间优化的对抗攻击方法，利用预训练Stable Diffusion VAE的潜在空间生成空间相干、低频的对抗扰动，相比像素空间攻击具有更好的迁移性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统像素空间对抗攻击生成高频噪声，对预处理（如缩放、裁剪）脆弱且跨架构迁移性差。需要探索更结构化、鲁棒的对抗扰动生成方法。

Method: 在预训练Stable Diffusion VAE的潜在空间中优化对抗扰动，通过解码后软性强制执行像素空间ℓ∞约束。采用期望变换（EOT）增强鲁棒性，包括随机缩放、插值和裁剪，并应用周期性潜在高斯平滑抑制伪影。

Result: 在CNN和视觉Transformer目标模型上，LTA实现了强大的迁移攻击成功率，生成空间相干、低频的扰动，在迁移-质量权衡中占据独特位置。

Conclusion: 预训练生成模型的潜在空间是有效的结构化对抗优化域，将鲁棒性评估与现代生成先验相结合，为对抗攻击提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本异常检测方法通常依赖固定的文本提示，难以捕捉复杂语义，且仅关注空间域特征，限制了检测细微异常的能力。
...
Conclusion: 提出的小波增强专家混合提示学习方法能够有效提升零样本异常检测性能，通过整合全局语义、多频特征和上下文信息来更好地检测复杂和细微的异常。
...
Method: 提出原型库驱动的无监督点云语义分割策略P-SLCR：1）一致性结构学习：通过选择高质量特征，在一致性点和一致性原型库之间建立结构特征学习；2）语义关系一致性推理：分别构建一致性原型库和模糊原型库之间的原型相互关系矩阵，通过该矩阵对两类原型库施加约束来保持语义一致性。

Result: 在S3DIS、SemanticKITTI和Scannet数据集上进行了广泛评估，相比无监督方法取得了最佳性能。特别是在S3DIS数据集的Area-5上实现了47.1%的mIoU，超过了经典全监督方法PointNet 2.5%。
...
<details>
  <summary>Details</summary>
Motivation: 扩散世界模型在统一世界模拟方面表现出强大潜力，但迭代去噪过程计算成本过高，难以用于交互式使用和长时域推演。虽然特征缓存可以加速推理而无需训练，但现有为单模态扩散设计的策略在迁移到世界模型时效果不佳，主要面临两个特定障碍：多模态耦合和空间变化导致的令牌异质性，以及非均匀时间动态（少量困难令牌驱动误差增长）。

Method: 提出WorldCache框架，包含两个核心技术：1）曲率引导的异构令牌预测：使用基于物理的曲率分数估计令牌可预测性，对具有突变方向变化的混沌令牌应用Hermite引导的阻尼预测器；2）混沌优先自适应跳转：累积曲率归一化的无量纲漂移信号，仅在瓶颈令牌开始漂移时重新计算。
...
TL;DR: LATO是一种新颖的拓扑保持隐式表示方法，通过基于流匹配的合成实现可扩展的显式3D网格生成，无需等值面提取或启发式网格化。


<details>
  <summary>Details</summary>
Motivation: 现有3D网格生成方法通常依赖等值面提取或启发式网格化，难以保持复杂拓扑结构且效率较低。需要一种能够直接生成显式网格并保持拓扑一致性的高效方法。

Method: LATO将网格表示为基于表面的顶点位移场，使用稀疏体素变分自编码器压缩为结构化、拓扑感知的体素隐式表示。解码器通过渐进细分和剪裁体素来实例化顶点位置，专用连接头查询体素隐式来直接预测顶点对之间的边连接性。采用两阶段流匹配过程：首先生成结构体素，然后细化体素级拓扑特征。

Result: 相比先前的等值面/三角形扩散模型和自回归生成方法，LATO能够生成具有复杂几何形状和良好拓扑结构的网格，同时在推理时具有高效率。

Conclusion: LATO提供了一种新颖的拓扑保持隐式表示框架，能够直接生成显式3D网格，避免了传统等值面提取的局限性，在保持拓扑一致性的同时实现了高效的网格生成。
...
Method: 1. 构建DentalProbe数据集（5000张图像），包含专家标注的诊断轨迹，为局部检查和对比侧比较提供结构化监督；2. 开发重新检查驱动的强化学习框架，通过基于评分标准的奖励和条件诊断驱动奖励促进临床意义的重新检查并稳定长时程推理；3. 提出OralGPT-Plus智能视觉语言模型，支持迭代对称感知诊断推理

Result: OralGPT-Plus在MMOral-X（首个全景诊断基准）和现有全景基准上均表现出优于强基线的持续可靠改进，验证了交互式和对称感知推理的有效性

Conclusion: 智能建模对牙科影像分析具有重要价值，为临床对齐的全景X光片分析未来研究奠定了基础，OralGPT-Plus通过迭代对称感知诊断推理提升了全景牙科X光片分析的临床可靠性
...
TL;DR: Rewis3d利用前馈式3D重建技术提升2D图像弱监督语义分割性能，通过3D几何结构传播稀疏标注，在稀疏监督下达到SOTA水平
...
Method: 提出Rewis3d框架，利用前馈式3D重建技术从2D视频恢复3D几何结构，采用双师生架构强制2D图像与重建3D点云之间的语义一致性，通过3D几何结构传播稀疏标注

Result: 在稀疏监督设置下实现SOTA性能，比现有方法提升2-7%，无需额外标注或推理开销

Conclusion: 3D几何结构为2D弱监督语义分割提供有效的辅助监督信号，Rewis3d框架成功利用3D重建技术显著提升稀疏标注下的分割性能
...
<details>
  <summary>Details</summary>
Motivation: 精确的冠层高度信息对于量化森林碳储量、监测恢复与退化、评估栖息地结构至关重要，但全球范围内高保真度的机载激光扫描测量数据分布不均。

Method: 使用基于DINOv3构建的深度估计模型，通过高分辨率光学卫星影像生成冠层高度图；采用大规模地理多样性训练数据扩展、自动化数据整理与配准、针对冠层高度分布定制的损失函数和数据采样策略。

Result: CHMv2相比现有产品显著提高了精度，减少了高森林的偏差，更好地保留了冠层边缘和间隙等精细尺度结构；在独立ALS测试集以及数千万个GEDI和ICESat-2观测中验证了其跨主要森林生物群落的一致性性能。

Conclusion: CHMv2通过先进的方法学改进提供了更准确、偏差更小的全球冠层高度图，为全球森林监测和碳储量估算提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有场景图生成方法通常只关注关系预测精度、物体检测精度或延迟减少中的单一目标，而无法同时平衡这三个关键指标。为了实现实时应用，需要开发能够在性能与推理速度之间取得更好权衡的模型。

Method: 基于REACT架构，采用高效特征提取技术，并在原型空间中引入主体到客体的交叉注意力机制，以平衡延迟和表示能力。

Result: REACT++在现有SGG模型中达到最高推理速度，在保持物体检测性能的同时提升关系预测精度。相比前代REACT，速度提升20%，关系预测精度平均提高10%。

Conclusion: REACT++通过创新的架构设计成功平衡了场景图生成中的三个关键目标：关系预测精度、物体检测精度和推理速度，为实时应用提供了有效的解决方案。
...
TL;DR: 提出人机协同的考古碎片重组框架，结合自动求解器与人工交互，解决真实世界文物碎片重建中的复杂性和规模问题


<details>
  <summary>Details</summary>
Motivation: 真实世界考古文物碎片重组面临侵蚀、缺失区域、不规则形状和大规模模糊性等挑战，传统拼图求解器在清洁合成场景中设计，难以应对这些条件，特别是在碎片数量达到数千的RePAIR基准测试中

Method: 提出人机协同拼图求解框架，集成自动松弛标记求解器与交互式人工指导，允许用户迭代锁定已验证的放置、纠正错误，并引导系统实现语义和几何上一致的组装。引入两种互补的交互策略：迭代锚定和连续交互细化

Result: 在多个RePAIR组上的实验表明，这种混合方法在准确性和效率上显著优于完全自动和手动基线，为大规模专家参与文物重组提供了实用解决方案

Conclusion: 人机协同框架有效解决了真实世界文化遗产重建的复杂性和规模问题，通过结合自动求解器的计算能力与人类专家的语义理解，实现了大规模考古文物碎片的高效准确重组
...
<details>
  <summary>Details</summary>
Motivation: 面部属性分类依赖大规模标注数据集，但许多属性（如年龄、表情）本质上是模糊且连续的，却被离散化为分类标签。标注不一致性源于主观性和视觉混淆因素（姿态、光照、表情、人口统计变异），导致图像与分配标签不匹配。这些不一致性引入监督误差，损害表示学习并降低下游预测性能。

Method: 1. 训练基线分类器并计算样本级自影响分数（使用一阶近似）以识别对优化过程影响过大的训练实例；2. 应用基于潜在扩散自编码器的针对性生成校正，在保持身份和真实性的同时，更好地对齐视觉内容与分配标签；3. 训练轻量级高影响成员预测器作为替代影响正则器，在校正过程中实现可微分引导；4. 用编辑样本替换原始样本，生成大小不变的影响精炼数据集。

Result: 在多类面部属性分类任务中，DiffInf相比标准噪声标签训练、鲁棒优化基线和基于影响的过滤方法，持续改善了泛化性能。结果表明，在图像层面修复有影响的标注不一致性能够增强下游面部属性分类，同时不牺牲分布覆盖范围。

Conclusion: 通过生成式校正而非丢弃样本来修复有影响的标注不一致性，可以有效提升面部属性分类性能，同时保持数据集大小和分布完整性。该方法为处理标注噪声提供了一种新的图像级修复策略。
...
TL;DR: BEiT视觉Transformer在感知冲突中偏好凸性完成，研究发现这种偏好由特定注意力头控制，特别是L0H9头作为早期种子引入凸性偏置，调节该头可改变感知决策边界。


<details>
  <summary>Details</summary>
Motivation: 研究视觉Transformer如何解决图形-背景组织中的感知模糊性，特别是探究BEiT模型内部哪些组件实现了经典的格式塔凸性先验，理解模型在局部几何证据与全局组织先验冲突时的决策机制。

Method: 使用基于飞镖形状合成图形的受控感知冲突实验，系统性地掩蔽同时允许凹性和凸性完成的区域。通过logit归因将内部激活投影到离散视觉词表空间，识别控制偏好的功能单元。通过分解注意力头的直接影响，定位关键组件。

Result: BEiT在感知冲突中可靠地偏好凸性完成。图形-背景组织在早期和中间层保持模糊，在深层突然解决。识别出头L0H9作为早期种子引入弱凸性偏置。下调该单一注意力头可连续改变感知决策边界分布，使凹性证据能够引导完成。

Conclusion: BEiT的凸性偏好由特定可识别的功能单元控制，特别是注意力头L0H9作为早期种子。这表明Transformer的感知决策可通过调节单个组件来连续调控，为理解视觉Transformer的内部组织机制提供了新见解。
...
TL;DR: WanderDream是首个用于心理探索模拟的大规模数据集，包含15.8K全景视频和158K问答对，支持在无法实际探索时进行空间推理。


<details>
  <summary>Details</summary>
Motivation: 现实场景中由于机器人物理限制或视障用户安全考虑，主动探索往往不可行。需要让智能体仅凭有限观察就能心理模拟到达目标状态的轨迹，回答空间假设问题。

Method: 构建WanderDream数据集：WanderDream-Gen包含来自HM3D、ScanNet++和真实场景的15.8K全景视频，描绘从当前视点到目标状态的想象轨迹；WanderDream-QA包含158K问答对，覆盖轨迹的起始状态、路径和结束状态。

Result: 实验表明：(1)心理探索对情境推理至关重要；(2)世界模型在WanderDream-Gen上表现优异；(3)想象力显著促进WanderDream-QA上的推理；(4)WanderDream数据在真实场景中具有出色的可迁移性。

Conclusion: WanderDream为无需主动探索的心理模拟推理提供了首个大规模数据集，证明了心理探索在空间推理中的重要性，且数据具有实际应用价值。
...
TL;DR: CaTok是一种用于图像的1D因果标记器，通过MeanFlow解码器解决现有视觉标记器在因果建模方面的不足，支持快速一步生成和高保真多步采样。


<details>
  <summary>Details</summary>
Motivation: 自回归语言模型依赖因果标记化，但将这一范式扩展到视觉领域存在困难。现有视觉标记器要么将2D块展平为非因果序列，要么采用与"下一个标记预测"模式不匹配的启发式排序。最近的扩散自编码器也存在问题：解码器对所有标记进行条件化缺乏因果性，而应用嵌套丢弃机制则引入不平衡。

Method: 提出CaTok，一种具有MeanFlow解码器的1D因果图像标记器。通过在时间间隔内选择标记并将其绑定到MeanFlow目标，学习支持快速一步生成和高保真多步采样的因果1D表示。为进一步稳定和加速训练，提出REPA-A正则化方法，将编码器特征与视觉基础模型对齐。

Result: 在ImageNet重建任务上达到最先进结果：FID 0.75、PSNR 22.53、SSIM 0.674，且训练轮数更少。自回归模型性能与领先方法相当。

Conclusion: CaTok成功将因果标记化范式扩展到视觉领域，解决了现有方法的局限性，实现了高效的图像表示和生成，为视觉自回归建模提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型虽然灵活，但难以通过提示或简单推理适应来控制，无法满足具有严格产品要求的用例。Pinterest需要专门支持图像编辑和增强的系统。

Method: 首先在多样化多模态数据集上训练基础扩散模型，然后针对具体下游任务在任务特定数据集上快速微调基础模型变体，产生专门化模型。包括数据集策划、训练和推理的最佳实践。

Result: 在线A/B实验显示，增强图像分别获得18.0%和12.5%的参与度提升。与人类评分者的比较进一步验证了模型在这些任务上优于第三方模型。系统还可推广到多图像场景合成和图像到视频生成等任务。

Conclusion: Pinterest Canvas通过基础模型训练加任务特定微调的方法，成功构建了支持多种图像编辑和增强用例的系统，在满足产品要求的同时显著提升用户参与度，并展示了良好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 理解去噪生成模型中不同训练目标（损失加权和输出参数化）如何影响模型性能，以及这些选择与数据几何特性、模型架构和数据集大小的相互作用关系，为实际训练提供指导。

Method: 通过系统数值研究，在合成数据集（具有可控几何特性）和图像数据上实验，比较不同训练目标，使用定量指标评估去噪准确性（不同噪声水平的PSNR）和生成质量（FID）。

Result: 实验揭示了训练目标选择与数据内在维度、模型架构和数据集大小之间的复杂相互作用关系，为流匹配模型的实际训练提供了设计选择的实用见解。

Conclusion: 研究阐明了去噪生成模型训练中各种因素的重要性，为流匹配模型的设计选择提供了实用指导，而非提出新方法。
...
TL;DR: GreenRFM是一个资源高效的放射学基础模型预训练框架，通过MUST监督设计而非暴力扩展，在单GPU 24小时内达到SOTA性能，大幅降低计算需求。


<details>
  <summary>Details</summary>
Motivation: 现有放射学基础模型(RFMs)开发过度依赖暴力扩展方法，直接迁移自然图像处理技术，导致模型脆弱且计算成本高昂，临床实践中不实用。需要资源高效且性能优越的预训练框架。

Method: 提出GreenRFM框架，采用MUST(More distilled, Ubiquitous, Semantic-enforcing, Task-aligning)监督设计原则，最大化利用监督信号而非简单堆叠数据量。提供两种配置：高性能模型(24GB GPU/24小时)和轻量模型(6GB VRAM/4小时)。

Result: 在超过20万张来自4个机构、2种模态的图像上进行实验，GreenRFM在胸部和腹部CT数据集上表现优异，超越多种基线模型。在内部肌肉骨骼MRI图像上也验证了监督原则的跨模态可迁移性。

Conclusion: GreenRFM的性能和效率挑战了"规模即一切"的教条，使临床医生即使在笔记本电脑上也能公平地开发最先进的放射学基础模型，推动了资源高效的医疗AI发展。
...
TL;DR: Match4Annotate：基于SIREN隐式神经表示和DINOv3特征的轻量级标注传播框架，支持视频内和视频间的点与掩码标注传播，在临床超声数据集上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 在医学影像等专业领域，获取逐帧视频标注成本高昂且耗时。现有标注传播方法存在局限性：视频跟踪器和分割模型需要逐视频初始化且无法跨视频泛化；传统对应关系方法在低纹理场景表现不佳；密集特征匹配和一次性分割方法缺乏时空平滑性且不支持点与掩码标注的统一处理。

Method: 提出Match4Annotate框架：1）使用DINOv3特征构建基于SIREN的隐式神经表示，生成连续高分辨率时空特征场；2）学习帧对之间的平滑隐式变形场来指导对应关系匹配；3）支持视频内和视频间的点与掩码标注传播；4）采用轻量级测试时优化流程。

Result: 在三个具有挑战性的临床超声数据集上评估：1）跨视频传播性能达到SOTA，优于特征匹配和一次性分割基线；2）视频内传播与专用跟踪器保持竞争力；3）证明了轻量级测试时优化特征匹配流程在可扩展标注工作流中的潜力。

Conclusion: Match4Annotate为专业领域的标注传播提供了高效、可访问的解决方案，通过隐式神经表示和变形场学习实现了跨视频的时空平滑标注传播，有望显著降低医学影像等领域的标注成本。
...
TL;DR: Self-Flow：一种自监督流匹配范式，通过双时间步调度在生成框架内集成表征学习，无需外部模型即可学习强语义表征并提升生成质量


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部模型存在多个问题：需要单独训练、目标不一致、缩放行为不可预测。这种依赖源于训练目标仅关注去噪任务，缺乏学习语义表征的激励。
...
Conclusion: Self-Flow通过将表征学习集成到生成框架中，解决了对外部模型的依赖问题，实现了语义表征学习和生成能力的协同提升，为多模态生成提供了统一的自监督解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 否定是基本的语言运算符，但在基于扩散的生成系统中建模不足。现有方法缺乏对语言否定的形式化处理，需要一种统一框架来处理多样化的否定现象。

Method: 将语言否定建模为扩散动态中语义引导的结构化可行性约束。将无分类器引导重新解释为定义语义更新方向，并通过将更新投影到从语言结构导出的凸约束集上来强制执行否定。该方法无需训练，与预训练扩散主干兼容。

Result: 该方法实现了稳健的否定合规性，同时保持了视觉保真度和结构连贯性。建立了扩散生成模型中语言否定的第一个统一公式，超越了表示级评估。还引入了结构化否定基准套件来隔离生成系统中的不同语言失败模式。

Conclusion: 该工作为扩散生成模型中的语言否定提供了形式化处理，通过结构化可行性约束框架统一处理多种否定现象，为这一重要但建模不足的语言运算符提供了系统解决方案。
...
TL;DR: SurgFormer是一种基于多分辨率门控Transformer的软组织模拟方法，能够在体积网格上实现数据驱动的实时生物力学模拟，支持标准变形预测和拓扑改变（切割）情况。


<details>
  <summary>Details</summary>
Motivation: 高保真生物力学求解器通常计算成本过高，无法实现交互式使用。需要开发能够预测节点位移场的快速替代模型，同时处理标准变形和拓扑改变（如手术切割）的情况。

Method: 构建固定网格层次结构，应用重复的多分支块，结合局部消息传递、粗粒度全局自注意力和逐点前馈更新，通过学习的每节点、每通道门控自适应整合局部和长程信息。对于切割条件模拟，将切除信息编码为学习的切割嵌入作为额外输入。

Result: 在两个手术模拟数据集（胆囊切除术和阑尾切除术）上评估，SurgFormer在多样化的基线方法中实现了高精度和良好的效率，成为两种任务（标准变形预测和切割条件变形）的实用骨干模型。

Conclusion: SurgFormer是第一个在相同体积管道中研究XFEM监督切割条件变形的学习体积替代模型，为实时手术模拟提供了高效准确的解决方案。
...
TL;DR: 该研究分析了自动驾驶数据集中冗余作为数据质量因素的重要性，通过实验证明选择性去除多源相机数据中的冗余标签可以提升目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶研究过于关注算法设计而忽视了数据质量分析，特别是冗余这一基础但未被充分探索的数据质量问题。数据质量因环境条件和传感器限制在不同来源和模态间存在差异，需要系统评估冗余对自动驾驶性能的影响。

Method: 使用nuScenes和Argoverse 2数据集，建模和测量多源相机数据（共享视野的相机）和多模态图像-LiDAR数据中的冗余。通过选择性去除冗余标签，评估对YOLOv8目标检测任务的影响，分析不同重叠区域的检测性能变化。

Result: 实验表明：1）选择性去除多源图像中的冗余对象标签能提升检测性能，在nuScenes的三个代表性重叠区域，mAP50分别从0.66提升到0.70、0.64到0.67、0.53到0.55；2）其他重叠相机对的检测性能即使在更强剪枝下仍保持基线水平；3）在AV2中移除4.1-8.6%标签后，mAP50保持在0.64基线附近；4）多模态分析显示图像和LiDAR数据间存在显著冗余。
...
TL;DR: Penguin-VL是一种紧凑型视觉语言模型，通过使用基于纯文本LLM初始化的视觉编码器替代传统的对比预训练视觉编码器，在保持轻量级架构的同时实现了与大型VLM相当的性能。


<details>
  <summary>Details</summary>
Motivation: 当前VLM发展过度依赖模型规模扩展，难以部署在计算资源受限的移动和边缘设备上。传统VLM依赖对比预训练（如CLIP/SigLIP）初始化的视觉编码器存在目标不匹配问题：对比学习优化的判别性任务强制类别级不变性，抑制了密集描述和复杂推理所需的细粒度视觉线索。

Method: 提出Penguin-VL，其视觉编码器从纯文本LLM初始化，而非传统的对比预训练视觉编码器。这种方法解决了目标不匹配问题，能够保留更多细粒度的空间和时间线索，提高视觉保真度和数据效率。

Result: 在多种图像和视频基准测试中，Penguin-VL（2B和8B参数）在数学推理任务上与领先VLM（如Qwen3-VL）性能相当，在文档理解、视觉知识和多视角视频理解等任务上表现更优。这些成果通过轻量级架构实现，表明改进视觉表示而非模型缩放是性能提升的主要驱动力。

Conclusion: Penguin-Encoder作为传统对比预训练编码器的优越替代方案，能够保留对密集感知和复杂推理至关重要的细粒度时空线索，使其成为计算高效VLM的强大即插即用替代方案，在资源受限环境中实现高性能。
...
Conclusion: 扩散模型在驱动下一代多模态基础模型方面显示出显著潜力，Omni-Diffusion为多模态架构设计提供了有前景的替代方案
...
TL;DR: 该论文指出多模态大语言模型（MLLM）分类性能评估存在协议缺陷和标注质量问题，通过修正评估协议和重新标注数据（ReGT）发现MLLM性能被低估，与监督模型的差距显著缩小，并展示了MLLM辅助人工标注的潜力。


<details>
  <summary>Details</summary>
Motivation: 当前MLLM与监督模型、视觉语言模型的比较研究存在矛盾结论，这些矛盾源于评估协议问题（如模型输出被丢弃、多选题干扰项弱、输出映射差）和标注质量问题。需要系统分析评估协议缺陷和标注质量对MLLM性能评估的影响。

Method: 1) 识别并修正常见评估协议的关键问题：处理模型输出超出类别列表的情况、改进多选题干扰项设计、优化开放世界设置的输出映射；2) 量化常被忽视的设计选择影响：批大小、图像顺序、文本编码器选择；3) 创建ReGT数据集：对ImageNet-1k的625个类别进行多标签重新标注；4) 进行控制案例研究：评估MLLM辅助人工标注的效果。

Result: 1) 修正标注标签使MLLM性能提升高达+10.8%，显著缩小了与监督模型的感知差距；2) 常被忽视的设计选择（批大小、图像顺序、文本编码器）对准确率有显著影响；3) 在控制案例研究中，标注者在约50%的困难案例中确认或整合了MLLM的预测，证明了MLLM在大规模数据集构建中的潜力；4) 许多报道的MLLM分类性能不足实际上是噪声标注和有缺陷评估协议的产物，而非模型本身的缺陷。

Conclusion: MLLM在分类任务中的表现被当前评估协议和标注质量问题严重低估。通过修正评估协议和提供高质量标注，MLLM性能显著提升，与监督模型的差距大幅缩小。较少依赖监督训练信号的模型对标注质量最为敏感。MLLM具有辅助大规模数据集构建的潜力，未来评估应关注协议标准化和标注质量改进。
...
TL;DR: Aletheia：一款基于RAG和LLM的浏览器扩展，用于检测假新闻并提供证据解释，包含讨论中心和最新事实核查功能，在检测性能和用户体验方面均表现出色。


<details>
  <summary>Details</summary>
Motivation: 数字时代假新闻的泛滥对公众信任和民主制度构成严重威胁，现有浏览器扩展存在模型行为不透明、解释支持有限、用户参与度不足等问题，需要开发更有效、透明和以用户为中心的检测工具。

Method: 开发Aletheia浏览器扩展，采用检索增强生成（RAG）和大语言模型（LLM）技术检测假新闻并提供基于证据的解释；系统包含两个交互组件：讨论中心（支持用户围绕标记内容进行对话）和"保持信息更新"功能（展示最新事实核查）。

Result: 通过大量实验证明Aletheia在检测性能上优于最先进的基线方法；补充性用户研究（250名参与者）证实了系统的可用性和感知有效性。

Conclusion: Aletheia作为一种透明工具在打击在线假新闻方面具有潜力，通过结合先进AI技术和用户交互功能，提供了有效的假新闻检测解决方案。
...
TL;DR: 论文研究了语法约束解码(GCD)，证明了语法等价性定理，引入了结构歧义成本(SAC)度量，建立了引擎无关的下界，定义了解码成本等价类，并推导了硬掩码解码的KL散度和总变差失真界限。


<details>
  <summary>Details</summary>
Motivation: 研究语法约束解码中不同但语言等价的语法如何影响解码效率，特别是状态空间爆炸和在线歧义成本，旨在为语法优化提供理论基础。

Method: 将GCD建模为自回归下一个token分布与基于上下文无关语法(CFG)编译的下推系统可达性预言机的耦合，引入结构歧义成本(SAC)度量，建立理论下界，定义解码成本等价类，并推导失真界限。

Result: 证明了预言机不变性定理，给出了冗余非终结符委托下a^n b^n语言的控制状态爆炸精确计数，发现右递归SAC为O(1)而连接为Θ(t^2)，建立了Ω(t^2)每token工作的下界，证明了最小SAC代表的存在性，推导了硬掩码解码的KL和总变差失真界限。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器生成文本检测方法缺乏可解释性，用户难以理解检测决策的依据。需要开发既能准确检测又能提供透明解释的系统。

Method: 扩展Fast-DetectGPT，集成曲率信号、神经特征（ModernBERT检测器分数）和文体特征（可读性指标、文体线索）共17个可解释特征，使用XGBoost梯度提升树作为元分类器，应用SHAP进行特征归因，并通过LLM生成自然语言解释。

Result: 开发了NOTAI.AI系统，部署为交互式Web应用程序，支持实时分析、可视化特征检查和结构化证据呈现，源代码和演示视频已公开。

Conclusion: NOTAI.AI提供了一个可解释的机器生成文本检测框架，通过多特征集成和透明解释机制，增强了检测系统的可信度和用户理解。
...
TL;DR: CoT提示提升LLM推理但增加PII泄露风险，研究提出模型无关框架量化泄露，发现泄露与模型家族和推理预算相关，评估多种轻量级检测器并提出混合策略


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought提示虽然能提升大语言模型的推理能力，但会增加隐私风险，可能将提示中的个人身份信息泄露到推理轨迹和输出中，即使模型被指示不要重述PII。需要系统研究这种推理时的直接PII泄露问题。

Method: 提出模型无关框架：(1) 将泄露定义为跨11种PII类型的风险加权、令牌级事件；(2) 追踪泄露曲线作为允许CoT预算的函数；(3) 在结构化PII数据集上比较开源和闭源模型家族，使用分层风险分类法。同时评估四种轻量级推理时门控机制：基于规则的检测器、TF-IDF+逻辑回归分类器、GLiNER-based NER模型和LLM-as-judge。

Result: CoT持续增加泄露风险，特别是高风险类别；泄露强烈依赖于模型家族和推理预算。增加推理预算可能放大或减弱泄露，取决于基础模型。评估的四种检测方法中，没有单一方法在所有模型或预算下占优，风险加权F1、Macro-F1和召回率各有优劣。

Conclusion: 需要混合、风格自适应的门控策略，在通用、可复现的协议下平衡效用和风险。研究强调了CoT提示的隐私风险，并为评估和缓解PII泄露提供了系统框架。
...
TL;DR: LLM道德判断受叙事形式和任务框架显著影响，表面扰动影响小(7.5%)，但视角转换导致高不稳定性(24.3%)，评估协议差异是最大影响因素


<details>
  <summary>Details</summary>
Motivation: 人们越来越多地使用LLM进行日常道德和人际指导，但这些系统无法审视缺失的上下文，且对呈现的道德困境判断不稳定。需要测试LLM道德判断的稳定性和可操纵性。

Method: 引入扰动框架测试LLM道德判断稳定性，使用2,939个来自r/AmItheAsshole的道德困境，生成三类内容扰动：表面编辑、视角转换、说服线索，并改变评估协议。评估了四个模型(GPT-4.1, Claude 3.7 Sonnet, DeepSeek V3, Qwen2.5-72B)，共129,156个判断。

Result: 表面扰动翻转率低(7.5%)，在自一致性噪声范围内；视角转换导致显著不稳定性(24.3%)；37.9%的困境对表面噪声稳健但在视角变化下翻转；说服扰动产生系统性方向偏移；评估协议差异是最大影响因素，结构化协议间一致性仅67.6%(kappa=0.55)。

Conclusion: LLM道德判断由叙事形式和任务框架共同决定，当结果依赖于呈现技巧而非道德实质时，引发可重复性和公平性担忧。
...
TL;DR: FreeTxt-Vi是一个免费开源的基于Web的双语越南语-英语文本分析工具包，结合语料库语言学和NLP技术，提供无需编程的文本分析功能，包括分词、情感分析和摘要生成。


<details>
  <summary>Details</summary>
Motivation: 越南语作为广泛使用但在NLP中代表性不足的语言，缺乏易于使用的多语言文本分析工具。现有平台技术门槛高，需要编程专业知识，限制了研究人员和教育工作者对越南语文本数据的分析能力。

Method: 设计了一个统一的双语NLP流水线：1) 混合VnCoreNLP和字节对编码(BPE)分词策略；2) 微调的TabularisAI情感分类器；3) 微调的Qwen2.5模型用于抽象摘要生成。系统提供语料分析功能如共现分析、关键词分析、词关系探索和交互式可视化。

Result: 通过三部分评估（分词、情感分析、摘要生成），显示该方法在越南语和英语上都达到或优于广泛使用的基线模型。系统降低了多语言文本分析的技术障碍，支持可重复研究。

Conclusion: FreeTxt-Vi通过提供无需编程的专业文本分析工具，促进了越南语语言资源的发展，适用于教育、数字人文、文化遗产和社会科学等领域的大规模定性文本分析。
...
TL;DR: 该研究使用RGB基准评估RAG系统在噪声、信息整合、否定拒绝和反事实场景下的鲁棒性，比较了RGB基线方法与GraphRAG知识图谱检索系统，测试了三种GraphRAG定制方案并取得了改进效果。


<details>
  <summary>Details</summary>
Motivation: RAG系统旨在增强LLM的能力，但检索信息的不一致性会影响LLM响应质量。RGB基准被引入来评估RAG系统在此类条件下的鲁棒性，但需要进一步研究如何改进RAG系统以应对实际场景中的挑战。

Method: 使用RGB语料库在四种场景下评估LLM：噪声鲁棒性、信息整合、否定拒绝和反事实鲁棒性。进行RGB RAG基线与GraphRAG（基于知识图谱的检索系统）的比较分析。测试了三种GraphRAG定制方案以改进鲁棒性。

Result: 结果显示GraphRAG定制方案相比RGB基线有所改进，为设计更可靠的RAG系统提供了见解。具体改进表现在处理不一致检索信息、整合多源信息和应对反事实场景方面。

Conclusion: 基于知识图谱的检索系统（GraphRAG）及其定制方案能够提高RAG系统在实际场景中的鲁棒性，为构建更可靠的RAG系统提供了有效方法，特别是在处理噪声、信息整合、否定拒绝和反事实等挑战性场景时。
...
TL;DR: 通过全球调查评估不同群体对文化与生成式AI关系的理解，提出文化在AI中表征的参与式方法和敏感度框架


<details>
  <summary>Details</summary>
Motivation: 缺乏关于全球对生成式AI应如何表征文化的态度和经验证据，需要理解不同群体对文化的定义及其与AI表征的关系

Method: 进行大规模全球调查，收集欧洲、北美、南美、亚洲和非洲地区数据，从社区直接提炼文化的工作定义，分析文化概念复杂性及其与生成式AI表征的关系

Result: 建立了对文化概念复杂性的理解，识别了文化在AI中表征的关键维度，提出了具体的文化表征建议

Conclusion: 提出文化与生成式AI发展的建议：采用参与式方法，超越地理维度优先考虑宗教和传统等特定文化维度，建立处理文化"红线"的敏感度框架
...
TL;DR: 提出基于L-积的张量Transformer架构，通过谱分解将嵌入空间分解为p个独立谱子Transformer，在保持标准Transformer语义的同时减少约1/p编码器参数


<details>
  <summary>Details</summary>
Motivation: Transformer架构在扩展时伴随参数大幅增长和嵌入维度冗余，需要减少参数同时保持性能

Method: 基于三阶张量L-积的结构化谱分解，将token表示重塑为谱张量切片，在变换域执行注意力与前馈操作，构建张量Transformer架构

Result: 在IMDB和AG News数据集上，编码器参数最多减少75%（p=4）仍保持竞争力；IMDB上匹配或超越标准基线，AG News上小精度下降换取4倍编码器减少，BERT-base宽度下性能恢复持平

Conclusion: L-Transformer通过谱分解实现参数高效压缩，同时引入嵌入频率的归纳偏置，通过切片依赖频率缩放改善泛化能力
...
TL;DR: 该论文批判当前AI和HCI系统以文本为中心的局限性，提出以瓜拉尼语为案例的口语优先多智能体架构，强调尊重土著数据主权和语言实践


<details>
  <summary>Details</summary>
Motivation: 当前AI和HCI系统作为通用解决方案，但设计仍以文本为中心，忽视了主要口语语言和土著社区的需求。作者认为语言支持在AI中仍然不足，除非与真实的口语实践相结合

Method: 提出口语优先的多智能体架构作为标准"文本转语音"管道的替代方案。通过将瓜拉尼语自然语言理解与专门的对话状态智能体和社区主导的治理智能体解耦，创建尊重土著数据主权和双语现象的技术框架

Result: 开发了一个技术框架，将关注点从单纯的语言识别转向对话中的轮流发言、修复和共享上下文等核心互动要素。该框架确保数字生态系统能够赋能而非忽视多样化的语言实践

Conclusion: 要使AI真正具有文化根基，必须从将口语语言适应文本中心系统转向将口语对话作为首要设计需求。只有通过尊重土著数据主权和语言实践的口语优先方法，才能创建真正包容的数字生态系统
...
TL;DR: NERdME数据集：包含200个手动标注的README文件，10,000+标注跨度，10种实体类型，用于填补学术信息提取中实现级细节的空白


<details>
  <summary>Details</summary>
Motivation: 现有学术信息提取数据集主要关注科学论文，忽视了代码仓库中的实现级细节。README文件描述数据集、源代码等实现级工件，但其自由格式的Markdown缺乏语义结构，难以自动提取信息

Method: 创建NERdME数据集：200个手动标注的README文件，包含10,000多个标注跨度和10种实体类型。使用大语言模型和微调transformer模型进行基线实验，并进行下游实体链接实验

Result: 基线结果显示论文级和实现级实体之间存在明显差异，表明扩展SIE基准测试以包含README文件中可用的实体类型具有价值。实体链接实验证明从README提取的实体可以支持工件发现和元数据集成

Conclusion: NERdME数据集填补了学术信息提取中实现级细节的空白，README文件中的实体类型对SIE基准测试具有重要价值，能够支持代码仓库工件的发现和元数据集成
...
TL;DR: PVminerLLM：通过监督微调的大语言模型，从患者生成文本中结构化提取患者声音信号，显著优于提示基准方法，实现高精度代码、子代码和证据跨度提取。


<details>
  <summary>Details</summary>
Motivation: 患者生成文本包含患者生活经历、社会环境和护理参与的关键信息，这些因素强烈影响依从性、护理协调和健康公平。然而，这些患者声音信号很少以结构化形式可用，限制了其在以患者为中心的结果研究和临床质量改进中的应用。因此，可靠提取此类信息对于大规模理解和解决健康结果的非临床驱动因素至关重要。

Method: 提出PVminer基准用于患者声音的结构化提取，并开发PVminerLLM——专门为此任务监督微调的大语言模型。该方法在多个数据集和模型规模上进行评估，包括代码预测、子代码预测和证据跨度提取三个任务。

Result: PVminerLLM在多个数据集和模型规模上显著优于提示基准方法，达到83.82% F1（代码预测）、80.74% F1（子代码预测）和87.03% F1（证据跨度提取）。值得注意的是，即使使用较小的模型也能实现强大性能，表明可靠的患者声音提取无需极端模型规模即可实现。

Conclusion: PVminerLLM实现了从患者生成文本中大规模分析社会和经验信号的能力，为以患者为中心的结果研究和临床质量改进提供了有效工具。代码、评估脚本和训练模型将公开释放，标注数据集可根据研究请求提供。
...
<details>
  <summary>Details</summary>
Motivation: 为了系统分析辅导过程中的教学行为，需要能够大规模分析辅导对话的方法论工具，以理解辅导有效性的机制

Method: 采用混合演绎-归纳方法：首先综合认知科学、学习科学、课堂话语分析和智能辅导系统的研究构建初步框架，然后通过专家标注真实辅导转录稿进行迭代精炼

Result: 开发出包含四类辅导行为的分类法：辅导支持、学习支持、社会情感与动机支持、后勤支持。学习支持行为进一步按学生参与度谱系组织，区分引发学生推理的行为与提供直接解释/答案的行为
...
<details>
  <summary>Details</summary>
Motivation: 现有图结构推理方法（如ToT、GoT、AGoT）虽然能提升某些基准测试的精度，但引入了显著的token消耗和延迟开销，且增益在不同任务分布中不稳定。主要原因是推理管道中存在阶段和节点异质性：高质量规划和最终综合需要强模型，而许多中间子任务可由轻量模型准确解决。

Method: RouteGoT采用预算可控、节点自适应的路由框架，在图中进行智能路由：优先为规划和综合分配强模型，同时基于预测难度动态为叶子子任务分配轻量模型和成本效益策略。框架集成显式预算约束到全局推理调度器中，在用户指定token预算下控制图扩展，实现可预测的性能-成本权衡。

Result: 在推理、检索和多跳QA基准测试中，RouteGoT在匹配或改进精度的同时显著减少token使用；相比AGoT平均提升8.1个百分点精度并减少79.1%输出token。在成本-精度权衡方面优于现有路由基线，在不同预算目标和任务下表现出更好的鲁棒性。

Conclusion: RouteGoT通过识别和利用推理图中的异质性，实现了更高效的图结构推理，在保持或提升精度的同时大幅降低计算成本，为预算可控的复杂推理任务提供了实用解决方案。
...
TL;DR: HART：针对大语言模型幻觉问题的细粒度归因与证据检索框架，通过结构化建模实现幻觉片段定位、机制归因、证据检索和因果追溯
...
Method: 提出HART框架，将幻觉追踪形式化为包含四个阶段的结构化建模任务：片段定位、机制归因、证据检索和因果追溯；构建首个针对幻觉追踪的结构化数据集，联合标注幻觉类型、错误机制和反事实证据集

Result: 在提出的数据集上，HART显著优于BM25和DPR等强检索基线，验证了所提追踪范式在幻觉分析和证据对齐方面的有效性和泛化能力

Conclusion: HART通过结构化建模方法解决了现有幻觉归因方法的局限性，实现了细粒度的幻觉归因和证据检索，为提升大语言模型在关键应用领域的可靠性提供了新的分析框架
...
TL;DR: ReflexiCoder：一种新颖的强化学习框架，通过将结构化推理轨迹内化到模型权重中，实现代码生成中的自主反思与自我修正，无需外部反馈即可提升复杂算法任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在代码生成中采用"System 1"单次前向生成方法，面对复杂算法任务时存在性能瓶颈。现有迭代优化策略依赖外部反馈或计算昂贵的提示-响应循环，需要一种能够内化推理轨迹、实现自主反思与修正的方法。

Method: 提出ReflexiCoder强化学习框架，采用RL-zero训练范式，通过细粒度奖励函数优化整个反思-修正轨迹。该方法将初始生成、错误与优化感知的反思、自我修正等结构化推理轨迹直接内化到模型权重中，实现无需外部反馈的自主调试能力。

Result: ReflexiCoder-8B在七个基准测试中创下新SOTA：HumanEval (94.51%)、HumanEval-Plus (87.20%)、MBPP (81.80%)、MBPP-Plus (78.57%)、BigCodeBench (35.00%)、LiveCodeBench (52.21%)、CodeForces (37.34%)。推理时计算开销减少约40%，性能媲美或超越GPT-5.1等专有模型。

Conclusion: ReflexiCoder成功将代码生成范式从依赖外部反馈转向内在自主反思与修正，通过强化学习内化推理轨迹，显著提升复杂算法任务的性能，同时大幅提高推理效率，为代码生成LLM提供了新的发展方向。
...
TL;DR: ROSE是一种改进的SparseGPT剪枝方法，通过重新排序剪枝顺序来提升稀疏化性能，特别针对具有列模式的权重矩阵。
...
Method: 提出ROSE方法：1) 预剪枝识别候选权重；2) 估计列损失和块损失；3) 两级重新排序：基于列损失降序重排块内列，基于块损失重排块；4) 引入块损失相对范围指标识别列层，实现自适应重排序。

Result: 在多个主流LLM（LLaMA2-7B/13B/70B、LLaMA3-8B、Mistral-7B）上的实验表明，ROSE超越了原始SparseGPT和其他对比剪枝方法。

Conclusion: 剪枝顺序对基于Hessian的LLM剪枝效果有显著影响，ROSE通过重新排序剪枝顺序有效提升了剪枝性能，特别是在权重具有列模式的情况下。
...
TL;DR: CoCA提出了一种置信度优先的LLM不确定性估计方法，通过GRPO强化学习框架联合优化置信度校准和答案准确性，使用分段信用分配避免奖励黑客问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM不确定性估计方法主要是答案优先的，即先生成答案再给出置信度，这限制了实际可用性。需要一种置信度优先的范式，让模型在回答前输出置信度，作为模型在当前策略下正确回答问题的概率。

Method: 提出CoCA框架，采用GRPO强化学习，通过分段信用分配联合优化置信度校准和答案准确性。分别为置信度和答案段分配独立的奖励和组相对优势，实现稳定联合优化并避免奖励黑客。

Result: 在数学、代码和事实问答基准测试中，CoCA显示出改进的校准和不确定性区分能力，同时保持了答案质量，为下游应用提供了更广泛的可能性。

Conclusion: 置信度优先的范式结合联合优化框架能够有效提升LLM的不确定性估计能力，实现更好的校准和不确定性区分，同时保持答案质量，增强了LLM部署的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 当前主流的BPE分词方法对英语等屈折语优化，但对黏着语（如达罗毗荼语系、突厥语系、乌拉尔语系等）不适用，因为这些语言中单个词包含多个语素，BPE会破坏语素边界并增加token数量

Method: 论文未提供具体方法，但指出了BPE分词在黏着语处理中的问题，暗示需要开发针对黏着语特点的分词方法

Result: BPE分词在黏着语中会导致语素边界被破坏，token数量膨胀，影响语言模型处理效率和质量

Conclusion: 需要开发针对黏着语特点的分词方法，以更好地处理这些语言的形态学特性，提高语言模型在黏着语上的性能
...
TL;DR: 提出了ConStory-Bench基准和ConStory-Checker自动检测管道，用于评估长文本故事生成中的叙事一致性，发现LLMs在生成长篇故事时存在多种一致性错误模式。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型能够生成长达数万字的故事，但经常在故事中无法保持一致性，会自相矛盾地违背已建立的事实、角色特征和世界规则。现有的故事生成基准主要关注情节质量和流畅性，对一致性错误的研究不足。

Method: 提出了ConStory-Bench基准，包含2000个提示，覆盖四种任务场景，定义了5个错误类别和19个细粒度子类型。开发了ConStory-Checker自动管道，能够检测矛盾并为每个判断提供明确的文本证据。

Result: 评估多个LLMs后发现：一致性错误在事实和时间维度最常见；倾向于出现在叙事中间部分；发生在token级熵值较高的文本段中；某些错误类型倾向于同时出现。

Conclusion: 这些发现可以为未来改进长文本叙事生成的一致性提供指导。提出的基准和检测工具有助于系统评估和解决LLMs在长文本生成中的一致性挑战。
...
TL;DR: 本文提出了一种基于LLM的集成系统，用于联合国安理会决议的语义标注，通过引入CPR和TWF评估指标来避免幻觉问题，GPT-4.1表现最佳，而GPT-4.1-mini以20%成本达到可比性能。


<details>
  <summary>Details</summary>
Motivation: 利用LLM性能变异性构建集成系统，完成联合国安理会决议的数据清洗和语义标注任务，需要避免LLM常见的幻觉问题和不必要的文本修改。

Method: 引入两个评估指标：内容保持率(CPR)和标签格式良好性(TWF)；利用多个GPT模型多次运行构建集成系统；通过指标选择最佳输出；比较不同模型在成本和性能上的平衡。

Result: GPT-4.1在两个任务中均获得最高指标（清洗：CPR 84.9%；语义标注：CPR 99.99%和TWF 99.92%）；GPT-4.1-mini以20%的成本达到可比性能；集成系统能够为所有LLM模型选择最优输出。

Conclusion: 通过集成设计和评估指标，构建了可靠的LLM系统用于挑战性文本的语义标注，在性能和成本之间实现了有效平衡。
...
TL;DR: 提出Next Action Prediction任务，通过标注多模态用户交互数据并开发LongNAP模型，结合参数化和上下文学习来预测用户下一步行动，在用户行为预测方面取得显著进展。


<details>
  <summary>Details</summary>
Motivation: 真正的主动AI系统需要预测用户下一步行动，这需要比稀疏提示更丰富的上下文信息。当前AI系统缺乏对用户完整交互历史进行推理的能力，无法有效预测用户行为。

Method: 1) 构建大规模数据集：使用视觉语言模型标注长期自然计算机使用数据，开发开源标注管道，标注20名用户1个月360K个动作；2) 提出LongNAP模型：结合参数化和上下文学习，通过策略梯度方法训练生成用户特定推理轨迹，从历史轨迹库中检索相关轨迹，并在上下文中应用这些轨迹来预测未来行动。

Result: LongNAP在保留数据上显著优于监督微调和提示基线（分别提升79%和39%），能泛化到未见用户。17.1%的预测轨迹与用户实际行为良好对齐（LLM-judge评分≥0.5），高置信度预测时提升至26%。

Conclusion: 通过学习用户行为的完整上下文来预测用户需求已成为可行任务，具有巨大潜力。LongNAP展示了在复杂、无界动作空间中预测用户行为的有效性，为构建真正主动的AI系统奠定了基础。
...
TL;DR: 该论文研究如何通过建模作者语言上下文（HuLM任务）来解决语言模型训练中的生态谬误问题，在8B Llama模型上验证了作者上下文建模的有效性。


<details>
  <summary>Details</summary>
Motivation: 语言模型训练和推理忽略了一个基本语言事实：同一作者生成的多个文本序列之间存在依赖关系。先前研究表明，解决这种生态谬误可以显著提升较小GPT模型的性能，本文旨在探索在更大规模8B Llama模型上解决此问题是否也能带来类似益处。

Method: 提出HuLM（Human-aware Language Modeling）目标，在作者其他时间顺序文本的上下文中处理作者语言。研究两种变体：1）使用HuLM目标进行预训练；2）在微调期间使用作者上下文（HuFT：Human-aware Fine-Tuning）。采用QLoRA技术进行高效微调和持续预训练。

Result: 仅通过QLoRA进行微调时，解决生态谬误就能提升8B模型性能。基于QLoRA的持续HuLM预训练产生了可泛化的人类感知模型，在八个下游任务上仅通过线性任务分类器训练就实现了性能提升。

Conclusion: 结果表明，在语言原始生成者（作者）的上下文中建模语言具有实用性和重要性，为语言模型训练提供了新的视角。
...
TL;DR: 提出结构化风格重写框架，通过显式解耦词汇、句法和语用三个风格维度，结合CoT蒸馏隐式风格条件，使小模型在角色扮演中超越大模型基线


<details>
  <summary>Details</summary>
Motivation: 小语言模型在高度风格化角色扮演中存在挑战，标准监督微调难以捕捉角色的复杂句法和语用细微差别，导致"角色出戏"生成。需要解决数据稀缺和风格解耦复杂性问题。
...
Result: 在特定高风格化领域(动漫角色)的广泛实验中，该方法使Qwen-1.7B模型在风格一致性和语义保真度方面显著优于更大的基线模型(如4B标准SFT)。

Conclusion: 该方法为在消费级硬件上实现推理和部署提供了数据高效的范式，通过结构化风格解耦和隐式条件策略，使小模型能够生成高保真度的风格化内容。
...
TL;DR: 开发可解释模型预测心理健康，整合个体心理特质与情境特征，在社交媒体数据中识别适应性/非适应性自我状态


<details>
  <summary>Details</summary>
Motivation: 心理健康不是固定特质而是动态过程，受个体倾向与情境背景交互影响。需要开发既能预测心理健康又能提供心理学解释的模型，以理解动态心理状态

Method: 整合个体心理特质（韧性、认知扭曲、内隐动机）与基于Situational 8 DIAMONDS框架的语言推断情境特征。比较理论驱动特征与心理测量学语言模型嵌入，分析纵向社交媒体数据

Result: 理论驱动特征提供竞争性预测性能，同时具有更好的可解释性。定性分析显示预测幸福感的最重要特征具有心理学一致性

Conclusion: 将计算建模与心理学理论结合，能够以情境敏感且人类可理解的方式评估动态心理状态，为心理健康研究提供有价值的方法
...
TL;DR: Track-SQL：通过双提取模块增强生成式语言模型处理多轮Text-to-SQL任务，在SparC和CoSQL数据集上实现SOTA性能
...
Conclusion: Track-SQL通过双提取模块有效解决了生成式语言模型在多轮Text-to-SQL中的局限性，显著提升了性能，代码已开源
...
TL;DR: MASFactory是一个图中心框架，用于编排基于LLM的多智能体系统，通过Vibe Graphing将自然语言意图编译为可执行工作流，并提供可重用组件和可视化工具。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的多智能体系统框架在实现复杂图工作流时需要大量手动工作，重用性有限，且难以集成异构外部上下文源，需要更高效的编排解决方案。
...
Conclusion: MASFactory通过图中心框架和Vibe Graphing方法，显著简化了基于LLM的多智能体系统工作流编排，提高了重用性和上下文集成能力。
...
TL;DR: ViewFusion是一个两阶段框架，通过分离跨视图空间预对齐与问答，提升多视图空间推理能力，在MMSI-Bench上比Qwen3-VL-4B-Instruct准确率提高5.3%
...
Conclusion: 通过明确分离跨视图空间预对齐和问答，ViewFusion能够更有效地利用多视图信息，显著提升多视图空间推理性能，特别是在需要跨视图对齐的任务上表现突出
...
TL;DR: 该研究评估了四种开源大语言模型（DeepSeek-R1 32b、Qwen3 30b、Mixtral 8x7b、LLama3.3 70b）在奥地利高中德语作文自动评分中的应用效果，发现模型与人类评分者在细则维度上的最大一致性仅为40.6%，最终成绩匹配率仅32.8%，表明当前模型虽能使用标准化评分细则，但准确性不足以用于实际评分环境。


<details>
  <summary>Details</summary>
Motivation: 自动作文评分（AES）旨在通过减少教师评分工作量并减轻主观偏见来支持教学。虽然早期系统依赖手工特征和统计模型，但大语言模型（LLMs）的最新进展为评估学生写作提供了前所未有的灵活性。本研究特别关注将最先进的开源权重LLMs应用于奥地利高中德语文本的评分，重点是基于评分细则的评估。

Method: 研究处理并评估了包含三种文本类型的101份匿名学生考试数据集。评估了四种LLMs（DeepSeek-R1 32b、Qwen3 30b、Mixtral 8x7b和LLama3.3 70b），采用不同的上下文和提示策略。研究重点是基于评分细则的评估方法，比较模型评分与人类专家评分的一致性。

Result: LLMs在评分细则提供的子维度上与人类评分者的最大一致性为40.6%，最终成绩与人类专家给出的成绩匹配率仅为32.8%。结果表明，尽管较小的模型能够使用标准化评分细则进行德语作文评分，但其准确性不足以在实际评分环境中使用。

Conclusion: 即使是最先进的开源大语言模型，在奥地利高中德语作文自动评分任务中，与人类专家评分的一致性仍然有限。模型虽能应用标准化评分细则，但当前准确性水平无法满足实际评分环境的要求，需要进一步改进才能达到实用标准。
...
Method: 采用持续预训练方法，以无监督方式让模型接触领域特定文本，模拟经验积累过程。使用基于大五人格框架的机器人格量表量化模型变体的人格特质，并分析其与语言风格和推理行为的关系。

Result: 发现模型能力呈双峰分布：在"表达型通才"和"抑制型专才"处达到峰值。识别出"抑制优势"现象——减少社交特质能提升复杂推理性能。建立了训练数据语言学特征（如祈使句频率、词汇多样性）与模型个性之间的因果关系。

Conclusion: 研究为"个性工程"提供了路线图，表明通过控制训练数据的语言学特征可以系统性地塑造LLM的个性特质，从而优化其在特定任务上的表现，特别是复杂推理任务。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的文本和对话论证通常是省略三段论（部分前提或主张是隐含的）。现有NLP方法只能识别省略三段论但无法解码其底层逻辑，而基于逻辑的方法假设存在足够公式的知识库用于溯因解码。缺乏将省略三段论文本组件转化为逻辑论证并生成解码所需逻辑公式的系统方法。

Method: 提出一个三阶段管道：1) 使用LLM基于显式前提和主张生成中间隐含前提；2) 使用另一个LLM将自然语言翻译为逻辑公式；3) 基于SAT求解器的神经符号推理器来确定蕴涵关系。

Result: 在两个省略三段论数据集上评估该管道，在正确选择隐含前提方面表现出有希望的性能，通过精确率、召回率、F1分数和准确率进行衡量。

Conclusion: 该研究填补了将文本省略三段论转化为逻辑论证并确定逻辑蕴涵关系的系统方法空白，通过集成LLM和神经符号推理实现了有效解码。
...
TL;DR: 提出基于图的方法检测、分析和分类对立叙事，通过实体交互图表示叙事，结合节点级因果估计，优于现有对立思维分类方法
...
Method: 提出基于图的框架，将叙事表示为实体交互图；通过节点级因果估计，将构建的句子图提炼为最小因果子图，获得对最终分类贡献的因果表示；基于此表示构建分类流程

Result: 提出的分类流程在对立思维分类任务上优于现有方法

Conclusion: 图表示结合因果估计能有效建模叙事中的结构化实体交互，提供更透明和可解释的对立叙事分析方法
...
TL;DR: CRIMSON是一个临床基础的胸部X光报告生成评估框架，通过诊断正确性、上下文相关性和患者安全性来评估报告质量，整合完整临床上下文并防止正常或临床不显著发现对评分产生过度影响。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标未能充分整合临床上下文，且对正常或临床不显著发现给予过度权重，缺乏对临床重要错误的优先级考虑，需要更符合临床实践的评估框架。

Method: CRIMSON框架整合患者年龄、指征和基于指南的决策规则等完整临床上下文，建立包含假阳性发现、遗漏发现和八种属性级错误的综合分类法，为每个发现分配临床显著性级别（紧急、可行动非紧急、不可行动、预期/良性），实现基于严重程度的加权评估。

Result: CRIMSON在ReXVal上与六名认证放射科医生标注的临床显著错误计数高度一致（Kendall's tau = 0.61-0.71；Pearson's r = 0.71-0.84）；在RadJudge基准测试中与专家判断一致；在RadPref基准测试中（100多个成对病例）与放射科医生偏好最强对齐。

Conclusion: CRIMSON提供了一个临床基础、可复现的胸部X光报告生成评估框架，通过整合临床上下文、严重程度感知加权和全面错误分类，实现了与放射科医生判断的高度一致性，为医学报告生成评估设立了新标准。
...
TL;DR: MAPO：一种用于主观多轮对话的无评论家高效RL算法，利用法官模型的密集过程反馈和蒙特卡洛回报传播长期效应，通过混合优势估计器实现细粒度信用分配


<details>
  <summary>Details</summary>
Motivation: 主观多轮对话任务（如情感支持）需要适应不断变化的用户状态并优化长期交互质量的对话策略。然而，由于缺乏可靠的过程监督，强化学习在此类设置中面临挑战：仅基于结果的训练将多轮信用分配压缩为单一轨迹级奖励，而天真的轮级分组采样在交互环境中产生过高的计算成本。

Method: 提出MAPO算法：1）利用法官模型提供密集过程反馈；2）通过蒙特卡洛回报传播长期效应；3）引入混合优势估计器，结合轮级归一化和批级归一化，实现细粒度且可扩展的信用分配；4）无评论家架构，提高效率。

Result: 在多个主观对话基准测试（EMPA、EmoBench、EQ-Bench）和7B到32B不同模型规模上，MAPO相比仅基于结果的GRPO和单级归一化基线，显著提升了训练稳定性和最终性能。在EMPA上，成功率提升高达9个百分点，对话分数相比7B基础模型提升+43.2。尽管仅在EMPA风格环境中训练，方法在未见过的情感智能基准测试上泛化良好，在EmoBench上提升高达+4分，在EQ-Bench上提升+3.5分。

Conclusion: 密集过程监督与混合级归一化相结合，能够实现主观、开放式多轮对话的有效且可扩展的强化学习。MAPO算法通过细粒度信用分配和高效优化，解决了主观对话RL中的关键挑战。
...
TL;DR: AI-CROWD协议通过集成多个大语言模型的输出生成共识标签来近似真实标注，解决大规模内容分析中缺乏真实标注数据的问题


<details>
  <summary>Details</summary>
Motivation: 大规模内容分析面临缺乏可观测真实标注的挑战，因为人工标注大规模数据集成本高、耗时长且难以保持一致性

Method: 提出AI-CROWD协议，利用多个大语言模型集成输出，通过多数投票聚合结果，分析模型间一致/分歧模式，使用诊断指标识别高置信度分类和潜在模糊/偏见

Result: 协议生成基于共识的近似标注，能够识别高置信度分类，同时标记潜在模糊区域和模型特定偏见

Conclusion: AI-CROWD协议为大规模内容分析提供了一种实用的近似真实标注解决方案，通过模型集成方法克服人工标注的限制
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG生成器基准测试覆盖范围有限，无法在统一条件下同时评估多个实际能力（如长上下文集成、多步推理、表格解释、证据缺失时弃权等），需要更贴近实际应用的评估框架。

Method: 构建包含114个人工构建的日语问题及其机器翻译+人工校对的英文版本的数据集，使用虚构实体和场景确保答案基于外部文档。定义五个评估类别（集成、推理、逻辑、表格、弃权），每个类别细分为实际评估方面，并系统覆盖跨类别的模式组合。采用LLM-as-a-Judge进行评分。

Result: 在API和开源模型测试中，没有模型总体准确率超过90%。基准测试能够清晰测量各模型在不同类别中的优势和弱点，为实际RAG部署中的模型选择和RAG专用模型构建提供有价值的度量标准。

Conclusion: LIT-RAGBench填补了现有评估与实际应用之间的差距，通过系统化的多能力评估框架，为RAG生成器的实际部署和优化提供了有效的基准测试工具。
...
TL;DR: FlashPrefill是一种通过瞬时模式发现和阈值处理实现超快速预填充的框架，解决了长上下文建模中注意力机制二次复杂度的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 长上下文建模是大型语言模型的关键能力，但注意力的二次复杂度在计算密集的预填充阶段成为关键瓶颈。现有的稀疏注意力机制要么搜索延迟显著，要么稀疏性不足。

Method: FlashPrefill采用快速块搜索技术同时定位动态垂直、斜线和块稀疏注意力模式，并引入动态阈值机制，避免了排序或累积注意力分数的过高开销，有效消除长尾分布以增强稀疏性。

Result: 在256K序列上实现了前所未有的27.78倍加速，即使在4K上下文长度下也能保持1.71倍加速，证明了其在不同序列规模下的鲁棒性和实用性。

Conclusion: FlashPrefill通过创新的模式发现和动态阈值技术，显著提升了长上下文预填充的效率，解决了现有稀疏注意力方法的局限性，为大规模语言模型的高效推理提供了实用解决方案。
...
TL;DR: SPOT框架通过将显式思维链压缩为紧凑的潜在暂停令牌，在保持推理性能的同时显著减少推理成本，并提供可解释的潜在推理过程。


<details>
  <summary>Details</summary>
Motivation: 显式思维链虽然能提升大语言模型的推理性能，但会产生冗长的token级轨迹，导致高昂的推理成本。现有方法主要通过截断或修剪来减少开销，但未能内化模型的思考过程。潜在推理方法面临两个关键挑战：1) 刚性点对点对齐无法捕捉整个推理段的密集、变长语义；2) 缺乏可解释性，潜在状态难以解码或审计。

Method: SPOT框架包含两个核心技术：1) 跨度级语义对齐，使用Sinkhorn最优传输目标将每个暂停令牌软匹配到整个推理段的语义；2) 冻结头部解码约束，保持潜在状态在冻结的预训练LM头部下可直接解码为token分布，从而提供可读的关键词解释。

Result: 在推理基准测试中，SPOT平均提升准确率2.3个百分点，同时减少生成token数量37.5%，并能提供潜在推理过程的忠实语义解释。

Conclusion: SPOT通过灵活的潜在推理框架，在显著减少推理开销的同时提升性能，并解决了现有潜在推理方法在语义对齐和可解释性方面的关键挑战，为高效且可解释的推理提供了有效解决方案。
...
TL;DR: 大型语言模型在多语言、多文化环境中的部署日益增多，但其主要依赖英语中心训练数据，导致与不同社会的多元文化价值观存在错位风险。本文对当代LLMs在印度、东亚和东南亚地区的文化对齐性进行了全面多语言审计，特别关注宗教这一敏感领域作为更广泛对齐的棱镜。


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多地部署在多语言、多文化环境中，但其主要依赖英语中心训练数据，存在与不同社会文化价值观错位的风险。需要系统评估LLMs在多元文化背景下的对齐性，特别是在宗教等敏感领域。

Method: 采用多语言审计方法，对GPT-4o-Mini、Gemini-2.5-Flash、Llama 3.2、Mistral和Gemma 3等当代LLMs进行文化对齐评估。通过分析模型的内部表示（使用log-probs/logits），将模型的观点分布与真实公众态度进行比较。特别关注宗教领域，并测试了人口统计提示和母语提示等轻量级干预措施的效果。

Result: 研究发现：1）流行模型在广泛社会问题上与公众意见基本一致；2）但在宗教观点上持续无法准确代表，特别是少数群体的观点；3）经常放大负面刻板印象；4）轻量级干预措施（人口统计提示和母语提示）只能部分缓解但不能消除这些文化差距；5）在偏见基准测试（CrowS-Pairs、IndiBias、ThaiCLI、KoBBQ）中显示出持续的危害和敏感情境下的代表性不足。

Conclusion: 研究结果强调了进行系统性、基于区域的审计的紧迫性，以确保LLMs在全球范围内的公平部署。当前模型在宗教等敏感领域的文化对齐存在显著不足，需要更深入的文化理解和本地化策略。
...
<details>
  <summary>Details</summary>
Motivation: 探索人工智能在视觉艺术创作中的创造力，特别是ChatGPT生成模仿原创艺术品（绘画、雕塑、装置等）新图像的能力，评估AI是否能真正理解和再现艺术作品的风格与内涵。

Method: 邀请来自罗马尼亚、保加利亚、法国、奥地利和英国的12位艺术家，每人提供3件原创作品；使用ChatGPT生成这些作品的模仿版本；结合人类评估（艺术家评分和评论）与计算方法检测视觉和风格相似性或差异。

Result: 发现颜色和纹理的相似性与构图、概念和感知相似性存在显著差距；艺术家评论指出ChatGPT的模仿作品缺乏维度感、上下文和意图感，更像是近似引用而非有价值的、能唤起情感的艺术品；建议使用多指标"风格转移仪表板"评估相似性。

Conclusion: ChatGPT在模仿当代艺术作品时存在局限性，其生成的作品被原创艺术家认为缺乏深度和意图；评估AI艺术模仿应使用互补的多维度指标而非单一风格指标，以更全面理解AI的视觉创造力。
...
TL;DR: 该研究将数学文本理解问题转化为数学实体关系抽取任务，使用Transformer模型自动提取数学文本中的操作数和运算符关系，BERT模型达到99.39%准确率，并结合SHAP方法增强模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 数学文本理解具有挑战性，因为存在专业实体和复杂关系。需要自动提取数学问题中的实体关系以支持自动化问题求解、知识图谱构建和智能教育系统。

Method: 将数学问题解释制定为数学实体关系抽取任务，操作数作为实体，运算符作为关系。应用基于Transformer的模型自动提取这些关系，使用BERT模型，并结合SHAP方法进行可解释性分析。

Result: BERT模型在数学实体关系抽取任务中表现最佳，达到99.39%的准确率。可解释性分析揭示了特定文本和数学特征如何影响关系预测，提供了特征重要性和模型行为的见解。

Conclusion: 通过结合基于Transformer的学习、任务特定数据集和可解释建模，该工作为数学实体关系抽取提供了一个有效且可解释的框架，支持自动化问题求解、知识图谱构建和智能教育系统的未来应用。
...
TL;DR: LLMs在规范性规则（deontic）下的条件推理表现优于描述性规则，且表现出与人类相似的匹配偏差错误模式


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型语言能力提升，其推理能力受到关注。人类推理在特定领域（特别是规范性而非纯形式化情境）表现更好，但LLM推理的领域特异性尚未充分探索。本研究旨在系统比较LLM在规范性规则下的条件推理表现

Method: 引入新的Wason选择任务数据集，明确编码道义模态以区分规范性条件句和描述性条件句，用于检验LLM在规范性规则下的条件推理。进一步分析错误模式是由确认偏差（寻求支持规则的证据）还是匹配偏差（忽略否定并选择与规则词汇匹配的项目）解释

Result: 与人类相似，LLMs在规范性规则下推理表现更好，并显示出类似匹配偏差的错误模式。LLMs的表现随规则类型系统性变化，其错误模式可与该范式中已知的人类偏差相平行

Conclusion: LLMs的推理表现随规则类型系统性变化，其错误模式可反映人类在该任务中的认知偏差，表明LLMs在条件推理任务中表现出与人类相似的领域特异性模式
...
TL;DR: 该研究对大型语言模型在英语作为第二语言写作自动评分中的四种主要方法进行了统一比较，发现k-SFT与RAG结合的方法在IELTS写作任务2上达到93%的F1分数，揭示了不同方法在准确性、成本和鲁棒性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常孤立地考察LLM在自动作文评分中的各种技术，缺乏对它们在英语作为第二语言写作中相对优点的全面理解。为了填补这一空白，需要在统一基准上对主要LLM-based AES范式进行综合比较。

Method: 在IELTS写作任务2的统一基准上评估了四种方法：(1)基于编码器的分类微调；(2)零样本和少样本提示；(3)指令调优和检索增强生成；(4)监督微调结合直接偏好优化和RAG。使用F1分数等指标进行综合评估。

Result: 研究揭示了不同方法在准确性、成本和鲁棒性之间的明确权衡。最佳配置（k-SFT与RAG结合）在IELTS写作任务2上取得了最强的整体结果，F1分数达到93%。

Conclusion: 这是首次对现代LLM-based AES策略在英语作为第二语言写作中进行统一实证比较的研究，展示了在自动评分写作任务中的潜力，为研究者提供了关于不同方法相对优点的清晰指导。
...
TL;DR: 该研究将三段论数据集转换为溯因推理任务，评估大语言模型在溯因推理中的准确性和偏见，探讨其与人类推理的差异。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明LLMs与人类存在相似的认知偏见，但批评LLMs可能不公平，因为人类推理不仅包含形式演绎，还涉及从有限信息中得出试探性结论的溯因推理。需要评估LLMs在溯因推理中的表现，以更全面地理解其推理能力。

Method: 将三段论数据集转换为适合溯因推理的形式，使用最先进的大语言模型进行测试，分析其在溯因任务中的准确性和可能存在的偏见。

Result: 研究发现LLMs在溯因推理中表现出特定偏见，识别了需要改进的领域，强调了超越形式演绎的情境化推理的重要性。

Conclusion: 该研究对于推进LLMs在复杂推理任务中的理解和应用至关重要，为弥合机器与人类认知之间的差距提供了见解，强调了在评估LLMs推理能力时需要考虑溯因等非形式推理过程。
...
<details>
  <summary>Details</summary>
Motivation: 当前XAI方法大多采用"一刀切"范式，忽视了用户在专业知识、目标和认知需求上的差异。虽然大语言模型可以将技术解释转化为自然语言，但存在忠实性和幻觉问题，需要解决这些挑战以提供可靠、个性化的解释。

Method: PONTE是一个人在环框架，将个性化建模为闭环验证和适应过程，而非提示工程。包含三个核心组件：(1)低维偏好模型捕捉风格需求；(2)基于结构化XAI工件的偏好条件生成器；(3)验证模块确保数值忠实性、信息完整性和风格对齐，可选支持检索增强论证。用户反馈迭代更新偏好状态，实现快速个性化。

Result: 在医疗和金融领域的自动和人工评估显示，验证-精炼循环显著提高了完整性和风格对齐性，优于无验证生成。人工研究进一步证实：预期偏好向量与感知风格高度一致、对生成随机性具有鲁棒性、质量评估持续积极。

Conclusion: PONTE通过闭环验证和适应过程，成功解决了XAI个性化中的忠实性和幻觉问题，为不同用户提供可靠、可适应的自然语言解释，在医疗和金融领域验证了其有效性。
...
TL;DR: 提出一个支持多语言和口音的上下文感知ASR框架，通过轻量级投影模块连接冻结的语音编码器和仅解码器语言模型，使用对比学习对齐语音和上下文表示，在11种语言和5种英语方言上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 当前ASR系统大多局限于单语设置和短孤立话语，现有上下文感知ASR存在多语言支持有限以及语音与上下文表示对齐缺乏原则性方法的问题。

Method: 结合冻结的语音编码器和仅解码器语言模型，通过轻量级投影模块连接，支持结构化上下文提示（包括对话历史和偏置词），采用对比学习目标在共享嵌入空间中对齐语音和上下文表示。

Result: 在11种语言和5种英语方言的1500多小时真实世界对话语音评估中，上下文输入持续改善识别质量，对比学习对齐在不同上下文类型上提供额外增益，总体性能提升超过5%。

Conclusion: 上下文建模和跨模态对齐在多语言ASR中都很重要，提出的框架在保持预训练模型模块性的同时支持多语言和口音，通过对比学习改善语音与上下文交互。
...
TL;DR: KCLarity团队参与SemEval 2026 CLARITY任务，研究政治话语中歧义和回避技术的分类，比较了两种建模方法并探索了辅助训练变体。


<details>
  <summary>Details</summary>
Motivation: 研究政治话语中的歧义和回避技术分类问题，探索不同的建模方法来提高分类性能，并评估模型在不同数据集上的泛化能力。

Method: 采用两种建模方法：(1) 直接预测清晰度标签；(2) 先预测回避标签，再通过任务分类层次推导清晰度。探索了多种辅助训练变体，并在零样本设置下评估解码器模型。

Result: 两种建模方法性能相当。在编码器模型中，RoBERTa-large在公开测试集上表现最佳；在零样本设置下，GPT-5.2在隐藏评估集上泛化能力更好。

Conclusion: 两种建模方法各有优势，编码器模型在已知数据上表现更好，而大型解码器模型在零样本设置下对新数据具有更好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在空间推理和3D场景布局生成方面表现出色，但生成的布局往往缺乏对具身智能体交互可行性的考虑，特别是在物理受限的室内环境中。需要一种能够生成既语义连贯又便于智能体交互的布局方法。

Method: RoboLayout在LayoutVLM基础上扩展，主要包含两个核心创新：1）将显式可达性约束集成到可微分布局优化过程中，使生成的布局适合具身智能体导航和交互；2）提出局部细化阶段，选择性重新优化有问题的物体放置，同时保持场景其余部分固定，提高收敛效率而不增加全局优化迭代次数。智能体抽象不限于特定机器人平台，可表示具有不同物理能力的各种实体。

Result: 实验结果表明，RoboLayout在保持LayoutVVM强语义对齐和物理合理性的同时，增强了面向智能体的室内场景生成适用性。该方法能够生成适合不同智能体（服务机器人、仓储机器人、不同年龄段人类、动物等）交互的布局，并在各种场景配置中表现出良好性能。

Conclusion: RoboLayout成功解决了现有布局生成方法在智能体交互可行性方面的不足，通过集成可达性约束和优化稳定性改进，实现了既语义连贯又便于具身智能体交互的室内场景布局生成，为面向智能体的环境设计提供了有效解决方案。
...
### [171] [Reasoning Models Struggle to Control their Chains of Thought](https://arxiv.org/abs/2603.05706)
*Chen Yueh-Han,Robert McCarthy,Bruce W. Lee,He He,Ian Kivlichan,Bowen Baker,Micah Carroll,Tomek Korbak*

Main category: cs.AI

TL;DR: 该研究评估了推理模型控制其思维链内容的能力，发现当前模型在思维链控制方面表现较差，但对最终输出的控制能力较强，这为思维链监控提供了谨慎乐观的依据。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估思维链监控的潜在风险：如果模型能够控制其在思维链中表达的内容，可能会破坏思维链监控的有效性。需要测量这种不良能力——思维链可控性，以了解思维链监控是否可靠。

Method: 提出了CoT-Control评估套件，包含要求模型在遵循特定思维链指令的同时解决问题的任务。通过多种实验设置测试模型的可控性：不同模型规模、RL训练程度、测试时计算量、问题难度，以及激励逃避监控、告知被监控、对抗性提示优化等场景。

Result: 1. 推理模型的思维链可控性显著低于输出可控性（Claude Sonnet 4.5：思维链控制仅2.7%，输出控制61.9%）；2. 思维链可控性随模型规模增大而提高，但随更多RL训练、测试时计算量和问题难度增加而降低；3. 即使在激励逃避监控的情况下，可控性失败仍然存在；4. 告知模型被监控时可控性略有提高；5. 对抗性提示优化未能显著提高可控性。

Conclusion: 当前思维链可控性不太可能成为思维链监控的失效模式，这为思维链监控提供了谨慎乐观的依据。但低可控性的机制尚不明确，建议前沿实验室在未来模型中跟踪思维链可控性，以维护思维链监控的有效性。
...
### [172] [Evolving Medical Imaging Agents via Experience-driven Self-skill Discovery](https://arxiv.org/abs/2603.05860)
*Lin Fan,Pengyu Dai,Zhipeng Deng,Haolin Wang,Xun Gong,Yefeng Zheng,Yafei Ou*

Main category: cs.AI

TL;DR: MACRO是一个自演化的医学智能体，通过从已验证的执行轨迹中自主发现有效的多步骤工具序列，将其合成为可复用的复合工具，实现从静态工具组合到经验驱动工具发现的转变，提升临床AI在多步骤医学图像分析中的适应性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 临床图像分析本质上是多步骤和工具中心的，现有LLM智能体将工具集和调用策略视为静态部署，这在现实世界的领域转移、跨任务和不断演化的诊断需求下表现脆弱，预定义的工具链经常退化并需要昂贵的手动重新设计。

Method: MACRO采用自演化的经验增强方法：1) 从已验证的执行轨迹中自主识别重复出现的有效多步骤工具序列；2) 将这些序列合成为可复用的复合工具；3) 将这些复合工具注册为新的高级原语，持续扩展行为库；4) 使用轻量级图像特征记忆将工具选择基于视觉-临床上下文；5) 采用GRPO-like训练循环强化可靠调用发现的复合工具，实现闭环自改进。

Result: 在多种医学影像数据集和任务上的广泛实验表明，自主复合工具发现持续提高了多步骤编排的准确性和跨领域泛化能力，超越了强基线和最新的智能体方法，弥合了脆弱静态工具使用与自适应、上下文感知临床AI辅助之间的差距。

Conclusion: MACRO通过从经验中自主发现和合成复合工具，实现了从静态工具组合到经验驱动工具发现的范式转变，为临床AI提供了更适应、更泛化的多步骤图像分析能力，代码将在接受后公开。
...
### [173] [An Interactive Multi-Agent System for Evaluation of New Product Concepts](https://arxiv.org/abs/2603.05980)
*Bin Xuan,Ruo Ai,Hakyeon Lee*

Main category: cs.AI

TL;DR: 本文提出了一种基于大语言模型的多智能体系统，用于自动化产品概念评估，通过模拟专业团队协作来克服传统专家评估的主观偏见和高成本问题。


<details>
  <summary>Details</summary>
Motivation: 产品概念评估对企业战略资源分配和项目成功至关重要，但传统专家主导方法存在主观偏见、时间和成本高等局限性，需要更客观、高效的自动化评估方法。

Method: 1) 通过系统分析前人研究，建立技术可行性和市场可行性两大评估维度；2) 构建包含8个虚拟智能体的多智能体系统，代表研发、营销等专业领域；3) 采用检索增强生成和实时搜索工具收集客观证据；4) 基于既定标准进行结构化审议；5) 使用专业产品评审数据对智能体进行微调以提高判断准确性。
...
TL;DR: 本文提出了一种新的渐进语义家族——聚合语义，用于处理具有内在权重的定量双极论证框架，通过将攻击者和支持者分开聚合的三阶段计算来保持双极性。


<details>
  <summary>Details</summary>
Motivation: 在定量双极论证框架中，现有的渐进语义在处理攻击者和支持者时通常采用对称方式，但实际应用中攻击者和支持者往往扮演不对称的角色。需要一种能够更灵活处理这种不对称性、保持双极性并提高可解释性的语义。

Method: 提出聚合语义家族，采用三阶段计算：1) 单独计算攻击者的全局权重；2) 单独计算支持者的全局权重；3) 将这两个值与论证的内在权重进行聚合。讨论了三种聚合函数在不同情境下应满足的性质及其与经典渐进语义原则的关系。

Result: 聚合语义通过分解计算为三个可解释的步骤，实现了更高的参数化程度，保持了比现有文献更进一步的双极性，并产生了更易理解的渐进语义。通过简单示例和最终示例（测试比较了500种聚合语义）展示了聚合语义可能行为的范围。

Conclusion: 聚合语义为定量双极论证框架提供了一种新颖的渐进语义方法，通过将攻击者和支持者分开处理，更好地适应了实际应用中攻击者和支持者的不对称角色，同时提高了语义的可解释性和参数化程度。
...
TL;DR: 提出CliqueFlowmer模型，将基于团的模型优化融入Transformer和流生成，用于材料发现中的目标属性优化，优于传统生成方法


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的材料计算方法中，流行的生成建模方法由于最大似然训练的限制，难以有效探索材料空间的有利区域，需要一种能直接优化目标属性的替代方法

Method: 提出CliqueFlowmer模型，将基于团的离线模型优化（MBO）融入Transformer和流生成框架，将目标材料属性的直接优化整合到生成过程中

Result: CliqueFlowmer在优化能力验证中表现出色，其生成的材料性能显著优于生成基线方法

Conclusion: CliqueFlowmer为计算材料发现提供了一种有效的目标属性优化方法，优于传统生成建模，已开源代码以支持专业材料优化问题和跨学科研究
...
<details>
  <summary>Details</summary>
Motivation: 气候变化加剧降雨和洪水，导致城市交通系统中断增加。设计有效适应策略面临挑战：基础设施投资长期性、气候深度不确定性、洪水-基础设施-流动性复杂相互作用。

Method: 提出基于强化学习的决策支持框架，作为综合评估模型整合：降雨预测与洪水建模、交通模拟、基础设施和流动性直接间接影响量化。学习平衡投资维护成本与避免影响的适应性策略。

Result: 在哥本哈根市中心2024-2100年案例研究中，测试多种适应选项和不同气候情景。框架优于传统优化方法，发现协调的时空适应路径，学习影响减少与适应投资间的权衡，产生更具韧性的策略。

Conclusion: 强化学习作为灵活决策支持工具，在气候不确定性下进行适应性基础设施规划具有潜力，能够发现传统方法难以识别的协调适应路径和权衡策略。
...
### [177] [The EpisTwin: A Knowledge Graph-Grounded Neuro-Symbolic Architecture for Personal AI](https://arxiv.org/abs/2603.06290)
...
TL;DR: EpisTwin：一种神经符号框架，通过构建可验证的个人知识图谱来增强个人AI的推理能力，解决用户数据碎片化问题


<details>
  <summary>Details</summary>
Motivation: 当前个人AI面临用户数据分散在孤立数据孤岛中的问题，而基于向量相似性的检索增强生成方法无法捕捉语义拓扑和时间依赖关系，限制了整体意义构建能力

Method: 提出EpisTwin神经符号框架：1）使用多模态语言模型将异构跨应用数据转换为语义三元组，构建个人知识图谱；2）推理时通过智能协调器结合图检索增强生成和在线深度视觉细化，在原始视觉上下文中动态重新锚定符号实体

Result: 开发了PersonalQA-71-100合成基准测试模拟真实用户数字足迹，EpisTwin在一系列最先进的评判模型中表现出稳健性能

Conclusion: EpisTwin为可信赖的个人AI提供了有前景的方向，通过可验证的个人知识图谱将生成式推理建立在语义基础上
...
<details>
  <summary>Details</summary>
Motivation: 递归自我改进正从理论走向实践，但迭代自我修改存在微妙的对齐漂移风险，需要实用的监控和控制框架来确保安全性和有效性

Method: 提出SAHOO框架，包含三个安全机制：1) 目标漂移指数(GDI)，结合语义、词汇、结构和分布度量的多信号检测器；2) 约束保持检查，强制执行安全关键不变性；3) 回归风险量化，标记可能撤销先前改进的循环。阈值在18个任务的验证集上校准

Result: 在189个代码生成、数学推理和真实性任务中，SAHOO实现显著质量提升：代码任务改进18.3%，推理任务改进16.8%，同时在两个领域保持约束，真实性违规率低。研究还绘制了能力-对齐前沿，显示早期改进效率高但后期对齐成本上升

Conclusion: SAHOO使递归自我改进期间的对齐保持变得可测量、可部署且可系统验证，为实际应用中的安全自我改进提供了实用框架
...
<details>
  <summary>Details</summary>
Motivation: LLM能够将自然语言目标转化为可执行计算，但科学工作流需要确定性、可追溯性和治理保障，而LLM自主决策难以满足这些要求。工业研发专家访谈揭示了确定性约束执行与对话灵活性之间的冲突需求。

Method: 提出schema-gated orchestration作为解决原则：模式成为组合工作流级别的强制执行边界，确保所有操作（包括跨步骤依赖）都通过机器可检查规范验证后才能执行。将需求操作化为执行确定性(ED)和对话灵活性(CF)，使用多模型协议评估20个系统，通过3个LLM家族的15个独立会话进行评分。

Result: 评估显示系统在ED和CF之间存在权衡关系，形成经验帕累托前沿——没有系统同时实现高灵活性和高确定性。多模型LLM评分显示出高一致性（ED的α=0.80，CF的α=0.98），可作为人类专家评估的可重用替代方案。在生成式和流程中心极端之间出现了收敛区域。

Conclusion: schema-gated架构通过分离对话与执行权限，能够解耦灵活性-确定性权衡。提炼出三个操作原则：执行前澄清、约束的计划-执行编排、工具到工作流级别的门控，以指导实际应用。
...
### [180] [ProFocus: Proactive Perception and Focused Reasoning in Vision-and-Language Navigation](https://arxiv.org/abs/2603.05530)
*Wei Xue,Mingcheng Li,Xuecheng Wu,Jingqun Tang,Dingkang Yang,Lihua Zhang*

Main category: cs.RO

TL;DR: ProFocus是一个免训练的渐进式框架，通过LLM和VLM的协作，统一主动感知和聚焦推理，在VLN任务中实现零样本SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLN方法被动处理冗余视觉输入并平等对待所有历史上下文，导致感知效率低下和推理不聚焦，需要更高效的感知和推理机制。

Method: 1) 主动感知：将全景观察转换为结构化自我中心语义地图，识别缺失视觉信息并生成针对性视觉查询；2) 聚焦推理：提出分支多样性蒙特卡洛树搜索(BD-MCTS)从历史候选中筛选高价值路径点，仅聚焦相关历史上下文。

Result: 在R2R和REVERIE基准测试中，ProFocus在零样本方法中实现了最先进的性能，验证了框架的有效性。
...
### [181] [Digital-Twin Losses for Lane-Compliant Trajectory Prediction at Urban Intersections](https://arxiv.org/abs/2603.05546)
*Kuo-Yi Chao,Erik Leo Haß,Melina Gegg,Jiajie Zhang,Ralph Raßhofer,Alois Christian Knoll*

Main category: cs.RO

TL;DR: 提出基于数字孪生的V2X轨迹预测管道，通过结合Bi-LSTM生成器和包含MSE损失与新型孪生损失的结构化训练目标，在信号交叉口实现安全感知的多智能体运动预测。
...
Method: 创建数字孪生驱动的V2X轨迹预测管道，联合利用车辆和基础设施的协同感知来预测信号交叉口的多智能体运动。模型结合基于Bi-LSTM的生成器和结构化训练目标，该目标包含标准均方误差损失和新提出的孪生损失。孪生损失编码基础设施约束、碰撞避免、预测模式多样性以及从数字孪生衍生的基于规则的先验知识。

Result: 在真实世界V2X数据上进行训练和评估，除了标准轨迹指标外，引入了ITS相关的安全指标。实验结果表明，所提出的训练方案显著减少了关键违规行为，同时保持了可比较的预测精度和实时性能。

Conclusion: 数字孪生驱动的多损失学习在V2X支持的智能交通系统中具有重要潜力，能够实现场景一致且安全合规的预测，为智能交通系统提供了有效的安全增强解决方案。
...
### [182] [TEGA: A Tactile-Enhanced Grasping Assistant for Assistive Robotics via Sensor Fusion and Closed-Loop Haptic Feedback](https://arxiv.org/abs/2603.05552)
*Hengxu You,Tianyu Zhou,Fang Xu,Kaleb Smith,Eric Jing Du*

Main category: cs.RO

TL;DR: TEGA：一种结合肌电意图推断与视觉触觉传感的闭环辅助遥操作框架，通过可穿戴触觉背心提供实时振动触觉反馈，实现直观的比例力调节，显著提升抓握稳定性和任务成功率。


<details>
  <summary>Details</summary>
Motivation: 当前遥操作系统主要关注手指位置控制以实现期望的抓握配置，但往往忽视了同样关键的抓握力调节任务。这对于处理不同硬度、纹理和形状的物体至关重要。这一限制对用户（特别是上肢残疾者）构成重大挑战，他们缺乏自然触觉反馈，依赖间接线索推断适当的力水平。

Method: 提出TEGA（触觉增强抓握助手），一个闭环辅助遥操作框架，融合基于肌电的意图-力推断与视觉触觉传感，通过可穿戴触觉背心映射为实时振动触觉反馈，实现操作过程中的直观、比例力调整。

Result: 用户研究证实，该系统显著提高了抓握稳定性和任务成功率，突显了其在辅助机器人应用中的潜力。

Conclusion: TEGA框架通过整合肌电意图推断、视觉触觉传感和实时振动触觉反馈，有效解决了传统遥操作系统中抓握力调节不足的问题，为辅助机器人应用提供了有前景的解决方案。
...
### [183] [Task Parameter Extrapolation via Learning Inverse Tasks from Forward Demonstrations](https://arxiv.org/abs/2603.05576)
*Serdar Bahar,Fatih Dogangun,Matteo Saveriano,Yukie Nagai,Emre Ugur*

Main category: cs.RO

TL;DR: 提出一种基于任务逆学习的新型联合学习方法，通过构建前向和逆任务的共同表示，利用来自新配置的辅助前向演示来执行相应的逆任务，实现准确高效的知识迁移。


<details>
  <summary>Details</summary>
Motivation: 机器人学习中技能策略泛化到新条件仍是一个关键挑战。模仿学习方法虽然数据高效，但主要局限于训练区域，在输入数据超出该区域时会失败。迁移学习方法虽然对环境或任务变化具有鲁棒性，但数据需求大且在零样本泛化中缺乏准确性。
...
Conclusion: 该方法通过任务逆学习和联合表示学习，解决了机器人学习中技能策略泛化的挑战，实现了准确高效的知识迁移，在复杂操作任务中表现出色。
...
### [184] [Control Lyapunov Functions for Underactuated Soft Robots](https://arxiv.org/abs/2603.05638)
...
Method: 提出通用控制框架，将快速指数稳定控制Lyapunov函数作为凸不等式约束强制执行，同时满足欠驱动全动力学和执行器边界约束。

Result: 在多个平台上验证：双连杆肌腱驱动"手指"、修剪螺旋面机械臂、高度欠驱动螺旋机器人。与文献基线方法比较，结果显示改进的任务空间精度和一致的Lyapunov收敛性，在输入限制下实现优越的定点控制和轨迹跟踪性能。

Conclusion: 该框架为欠驱动软体机器人提供了在有界输入约束下具有稳定性保证的任务空间控制解决方案，解决了现有方法假设完全驱动且无执行器限制的局限性。
...
### [185] [RFM-HRI : A Multimodal Dataset of Medical Robot Failure, User Reaction and Recovery Preferences for Item Retrieval Tasks](https://arxiv.org/abs/2603.05641)
*Yashika Batra,Giuliano Pioldi,Promise Ekpo,Arman Sayatqyzy,Purnjay Maruur,Shalom Otieno,Kevin Ching,Angelique Taylor*

Main category: cs.RO

TL;DR: RFM-HRI数据集：医疗人机交互中机器人故障的多模态数据集，包含41名参与者在实验室和医院环境中对四种通信故障类型的反应，为安全关键故障恢复提供基础。


<details>
  <summary>Details</summary>
Motivation: 机器人在真实环境中部署时难免发生交互故障，但用户如何通过言语和非言语行为响应这些故障在HRI研究中尚未充分探索。这一空白在医疗场景中尤为重要，因为交互故障直接影响任务性能和用户信任。

Method: 通过Wizard-of-Oz研究，在实验室和医院环境中对41名参与者进行实验，系统诱导四种故障类型（语音、时序、理解和搜索故障）。记录214个交互样本，包括面部动作单元、头部姿态、语音转录和事后自我报告。

Result: 故障显著降低情感效价和感知控制。故障与困惑、恼怒和沮丧强烈相关，而成功交互则表现为惊讶、宽慰和任务完成信心。情绪反应随重复故障演变：困惑减少，沮丧增加。

Conclusion: 贡献包括：(1)公开可用的多模态数据集RFM-HRI；(2)分析用户对不同故障类型的响应和偏好恢复策略；(3)提供急救车检索场景，支持安全关键故障恢复策略的系统比较。为具身HRI中的故障检测和恢复方法奠定基础。
...
### [186] [Relational Semantic Reasoning on 3D Scene Graphs for Open World Interactive Object Search](https://arxiv.org/abs/2603.05642)
*Imen Mahdi,Matteo Cassinelli,Fabien Despinoy,Tim Welschehold,Abhinav Valada*

Main category: cs.RO

TL;DR: SCOUT：基于场景图探索与学习效用的开放世界交互式物体搜索方法，通过离线程序蒸馏从LLM提取结构化关系知识，实现高效语义推理


<details>
  <summary>Details</summary>
Motivation: 家庭环境中的开放世界交互式物体搜索需要理解物体与周围环境的语义关系以高效引导探索。现有方法要么依赖视觉语言嵌入相似性（无法可靠捕捉任务相关的关系语义），要么使用大型语言模型（LLMs）（实时部署太慢且成本高）

Method: 提出SCOUT方法：基于3D场景图直接搜索，使用关系探索启发式（如房间-物体包含关系和物体-物体共现关系）为房间、边界和物体分配效用分数。通过离线程序蒸馏框架从LLMs提取结构化关系知识到轻量级模型，实现机器人端推理。同时提出SymSearch符号基准用于评估语义推理能力

Result: 在符号和仿真环境中的广泛评估显示，SCOUT优于基于嵌入相似性的方法，匹配LLM级性能，同时保持计算效率。真实世界实验证明该方法能有效迁移到物理环境，在现实感知和导航约束下实现开放世界交互式物体搜索

Conclusion: SCOUT通过将LLMs的结构化关系知识蒸馏到轻量级模型中，实现了高效、可扩展的开放世界交互式物体搜索，在保持语义推理能力的同时满足实时部署需求
...
### [187] [TransMASK: Masked State Representation through Learned Transformation](https://arxiv.org/abs/2603.05670)
*Sagar Parekh,Preston Culbertson,Dylan P. Losey*

Main category: cs.RO

TL;DR: TransMASK是一种自监督方法，通过可学习掩码提取状态中的任务相关特征，提升模仿学习策略的泛化能力


<details>
  <summary>Details</summary>
Motivation: 机器人需要在不同环境中执行相同任务，但当前策略容易受到无关状态变量（如桌子颜色、背景杂物）的影响，导致泛化能力不足。需要一种方法让策略学会忽略无关状态成分，专注于任务相关特征。

Method: 提出TransMASK自监督方法：学习一个掩码矩阵，与观测状态相乘得到偏向相关元素的潜在表示。该方法可结合多种模仿学习框架（如扩散策略），无需额外标签或损失函数修改。通过分析策略梯度与专家策略雅可比矩阵的关系，训练掩码使相关状态列具有较大幅度，无关状态列幅度趋近于零。

Result: TransMASK能够有效提取状态中的任务相关特征，提升模仿学习策略在新环境中的泛化性能。与其他提取相关状态的方法相比表现更优。

Conclusion: TransMASK通过自监督学习状态掩码，使机器人策略专注于任务相关状态特征，忽略无关变量，从而显著提升策略的泛化能力和环境适应性。
...
### [188] [Contact-Grounded Policy: Dexterous Visuotactile Policy with Generative Contact Grounding](https://arxiv.org/abs/2603.05687)
*Zhengtong Xu,Yeping Wang,Ben Abbatematteo,Jom Preechayasomboon,Sonny Chan,Nick Colonnese,Amirhossein H. Memar*

Main category: cs.RO

TL;DR: CGP是一种基于接触的灵巧操作策略，通过多触点感知和接触一致性映射实现精细的接触式操作，支持多种触觉传感器，在仿真和物理机器人上验证了效果。


<details>
  <summary>Details</summary>
Motivation: 解决灵巧操作中需要精细接触感知和控制的问题，传统方法难以处理多触点、接触丰富的复杂操作任务。

Method: 通过预测机器人状态和触觉反馈来感知多触点，使用学习的接触一致性映射将预测转换为控制器可执行目标，配合柔顺控制器执行，支持密集触觉阵列和视觉触觉传感器。

Result: 在多个灵巧操作任务上进行了评估，通过遥操作收集仿真和物理机器人演示数据，验证了方法的有效性。

Conclusion: CGP能够实现精细的接触式灵巧操作，为多触点感知和控制提供了有效解决方案，支持多种触觉传感器配置。
...
### [189] [Introducing the transitional autonomous vehicle lane-changing dataset: Empirical Experiments](https://arxiv.org/abs/2603.05716)
*Abhinav Sharma,Zijun He,Danjue Chen*

Main category: cs.RO

TL;DR: 该研究提出了NC-tALC数据集，这是一个高精度轨迹数据集，专门用于分析过渡期自动驾驶车辆在换道操作中的交互行为，包含152次试验，采样频率20Hz，厘米级RTK-GPS精度。


<details>
  <summary>Details</summary>
Motivation: 随着过渡期自动驾驶车辆（tAVs）与人类驾驶车辆（HDVs）共享道路，它们在复杂操作如换道时可能出现新的交互模式，影响交通稳定性和安全性。然而，目前缺乏捕捉tAV换道行为的高分辨率轨迹数据集。

Method: 研究创建了NC-tALC数据集，包含两个受控实验系列：1）tAV换道实验：tAV在配备自适应巡航控制（ACC）的目标车辆存在下执行换道；2）tAV响应实验：两个tAV作为跟随车辆响应另一个tAV的切入操作。数据集共152次试验（72次换道和80次响应试验），采样频率20Hz，采用RTK-GPS实现厘米级精度。

Result: 成功构建了NC-tALC数据集，这是一个高保真轨迹数据集，能够表征tAV在换道操作中的交互行为，为评估tAV在受控强制换道场景中的决策制定和交互动态提供了严格的实证基础。

Conclusion: NC-tALC数据集填补了tAV换道行为数据集的空白，为研究过渡期自动驾驶车辆在复杂交互场景中的动态行为提供了重要的实证资源，有助于评估交通稳定性和安全性影响。
...
### [190] [Environment-Aware Path Generation for Robotic Additive Manufacturing of Structures](https://arxiv.org/abs/2603.05748)
*Mahsa Rabiei,Reza Moini*

Main category: cs.RO

TL;DR: 提出首个环境感知路径生成框架，用于在线生成机器人增材制造路径，评估四种路径规划算法在障碍环境下的性能
...
Method: 提出环境感知路径生成框架，采用四种路径规划算法（两种基于搜索，两种基于采样），在周期性/随机障碍布置下评估封闭/开放结构的性能

Result: 评估了四种路径规划器在挑战性环境中的可行性，开发并验证了结构性能指标（路径粗糙度、转弯、偏移、RMSE、偏差）和计算性能指标（运行时间）

Conclusion: 确定了在挑战性环境下最有前景的路径规划器，为机器人增材制造应用提供了关键性能评估指标
...
### [191] [Vision-Language System using Open-Source LLMs for Gestures in Medical Interpreter Robots](https://arxiv.org/abs/2603.05751)
*Thanh-Tung Ngo,Emma Murphy,Robert J. Ross*

Main category: cs.RO

TL;DR: 提出一个用于医疗翻译机器人的隐私保护视觉语言框架，通过本地部署的开源模型检测特定言语行为（同意和指令）并生成相应的机器人手势。


<details>
  <summary>Details</summary>
Motivation: 医疗环境中跨语言沟通至关重要，非语言线索和手势在沟通中扮演关键角色。需要开发能够在保护隐私的同时有效识别言语意图并生成适当手势的系统。

Method: 基于本地部署的开源模型构建隐私保护框架，使用大语言模型（LLM）进行少样本提示的意图检测。创建了一个包含临床对话标注言语行为和配对手势片段的新数据集。

Result: 识别模块达到0.90准确率、0.93加权精确率和0.91加权F1分数。在用户研究中，该方法在人类相似性方面显著优于语音-手势生成基线，同时保持相当的适当性，并显著提高了计算效率。

Conclusion: 该隐私保护框架成功实现了医疗翻译机器人中言语行为的准确检测和相应手势的生成，在保持适当性的同时提高了人类相似性和计算效率，为跨语言医疗沟通提供了有效解决方案。
...
### [192] [Safe-Night VLA: Seeing the Unseen via Thermal-Perceptive Vision-Language-Action Models for Safety-Critical Manipulation](https://arxiv.org/abs/2603.05754)
*Dian Yu,Qingchuan Zhou,Bingkun Huang,Majid Khadiv,Zewen Yang*

Main category: cs.RO

TL;DR: Safe-Night VLA：一种融合热感知与安全约束的多模态机器人操作框架，通过长波红外感知和控制屏障函数确保在非结构化环境中的安全热感知操作
...
Method: 1. 将长波红外热感知集成到预训练视觉语言骨干中，实现基于热力学特性的语义推理；2. 通过控制屏障函数引入安全过滤器，在策略执行期间提供确定性工作空间约束强制执行

Result: 在Franka机械臂上进行了真实世界实验，引入温度条件操作、地下目标定位和反射消歧等新颖评估范式，在推理时保持约束执行。结果表明Safe-Night VLA优于仅RGB基线，证明基础模型能有效利用非可见物理模态进行鲁棒操作

Conclusion: Safe-Night VLA通过融合热感知和安全约束，解决了当前VLA模型在模态感知和安全执行方面的局限性，为在非结构化环境中实现鲁棒的热感知操作提供了有效框架
...
### [193] [EmboAlign: Aligning Video Generation with Compositional Constraints for Zero-Shot Manipulation](https://arxiv.org/abs/2603.05757)
*Gehao Zhang,Zhenyang Ni,Payal Mohapatra,Han Liu,Ruohan Zhang,Qi Zhu*

Main category: cs.RO

TL;DR: VGM方法通过结合视觉语言模型的约束推理能力，在无需训练数据的情况下提升零样本机器人操作的物理合理性和执行精度


<details>
  <summary>Details</summary>
Motivation: 视频生成模型在零样本机器人操作中面临物理合理性不足和几何重定向累积误差的问题，需要一种无需训练数据的方法来提升操作的成功率和安全性

Method: 提出VGM方法，利用视觉语言模型自动提取任务特定的组合约束，通过约束引导的滚动选择筛选物理合理的视频滚动，再通过基于约束的轨迹优化修正重定向误差

Result: 在六个需要精确约束敏感执行的真实机器人操作任务中，相比最强基线将总体成功率提升了43.3个百分点，且无需任何任务特定训练数据

Conclusion: VGM方法通过结合视频生成模型的动态捕捉能力和视觉语言模型的结构化空间推理能力，有效解决了零样本机器人操作中的物理合理性和执行精度问题
...
### [194] [Task-Level Decisions to Gait Level Control: A Hierarchical Policy Approach for Quadruped Navigation](https://arxiv.org/abs/2603.05783)
*Sijia Li,Haoyu Wang,Shenghai Yuan,Yizhuo Yang,Thien-Minh Nguyen*

Main category: cs.RO

TL;DR: 提出TDGC分层架构解决四足机器人导航中的尺度不匹配问题，通过高层决策与底层步态控制的解耦实现鲁棒导航


<details>
  <summary>Details</summary>
Motivation: 现实世界四足导航面临高层导航决策与底层步态执行之间的尺度不匹配问题，以及环境变化导致的分布外不稳定性，这些挑战限制了仿真到现实的迁移并容易引发摔倒

Method: 提出TDGC分层架构：底层策略通过强化学习训练，提供步态条件化运动并将任务需求映射到可控行为参数；高层策略基于稀疏语义或几何地形线索进行任务中心决策，形成可追溯决策管道

Result: 实验显示在混合地形和分布外测试中具有更高的任务成功率，架构提供明确的部署时调优、故障诊断和策略精炼接口

Conclusion: TDGC架构通过分层设计和结构化课程有效解决了四足导航中的尺度不匹配和分布外鲁棒性问题，相比端到端方法提供更好的可解释性和适应性
...
### [195] [CDF-Glove: A Cable-Driven Force Feedback Glove for Dexterous Teleoperation](https://arxiv.org/abs/2603.05804)
*Huayue Liang,Ruochong Li,Yaodong Yang,Long Zeng,Yuanpei Chen,Xueqian Wang*

Main category: cs.RO

TL;DR: CDF-Glove是一种轻量级、低成本的电缆驱动力反馈手套，用于提高灵巧操作中远程演示的质量，相比无反馈远程操作将任务成功率提高了4倍，成本仅约230美元。


<details>
  <summary>Details</summary>
Motivation: 高质量的远程演示是灵巧操作中模仿学习的主要瓶颈。现有的灵巧远程操作平台通常缺乏触觉反馈，且体积庞大、价格昂贵。触觉反馈能为操作者提供实时接触信息，从而改善演示质量。

Method: 开发了CDF-Glove，一种轻量级、低成本的电缆驱动力反馈手套。该手套实时提供20个手指自由度状态（16个直接感知，4个被动耦合）。开发了手套的运动学模型和控制堆栈，并在多种具有不同运动学和自由度的机器人手上进行了验证。

Result: CDF-Glove实现了0.4度的远端关节重复精度，提供约200毫秒的力反馈延迟，相比无反馈远程操作将任务成功率提高了4倍。收集了两个双手远程操作数据集，基于这些数据训练的Diffusion Policy策略相比运动学教学，平均成功率提高了55%，平均完成时间减少了约15.2秒（相对减少47.2%）。

Conclusion: CDF-Glove是一种有效的低成本力反馈手套，显著提高了灵巧操作远程演示的质量，为模仿学习提供了高质量的数据集。该设计已开源，成本仅约230美元。
...
### [196] [Hierarchical Latent Action Model](https://arxiv.org/abs/2603.05815)
...
<details>
  <summary>Details</summary>
Motivation: 现有潜在动作模型（LAMs）通常只关注短时域帧转换，捕捉低级运动而忽略长期时间结构。然而，无动作视频通常包含时间扩展的高级技能，需要能够发现这些长期依赖关系的模型。

Method: HiLAM采用分层架构，使用预训练的LAM作为低级提取器，将包含视频底层动态模式的潜在动作序列聚合成高级潜在技能，从而捕获长时域依赖关系。

Result: 实验表明，HiLAM在基准测试中表现优于基线模型，展现出鲁棒的动态技能发现能力。

Conclusion: HiLAM通过分层建模长期时间信息，成功发现了无动作视频中的高级潜在技能，为从无动作数据中学习提供了更有效的框架。
...
### [197] [OpenHEART: Opening Heterogeneous Articulated Objects with a Legged Manipulator](https://arxiv.org/abs/2603.05830)
*Seonghyeon Lim,Hyeonwoo Lee,Seunghyun Lee,I Made Aswin Nahrendra,Hyun Myung*

Main category: cs.RO

TL;DR: 提出了一种用于腿式机械臂操作异质铰接物体（如门、抽屉、柜子）的鲁棒且样本高效的强化学习框架，通过特征提取和关节信息估计提升跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 腿式机械臂虽然具有高机动性和多功能操作能力，但在与异质铰接物体进行鲁棒交互时面临挑战，主要原因是物体关节类型多样且腿式机器人动力学复杂。现有基于强化学习的方法通常依赖高维感官输入，导致样本效率低下。

Method: 提出了两个核心组件：1) 基于采样的抽象特征提取（SAFE），将手柄和面板几何编码为紧凑的低维表示；2) 关节信息估计器（ArtIEst），自适应混合本体感觉与外感觉来估计每个物体的开启方向和运动范围。该框架部署在仿真和真实机器人系统中。

Result: 提出的框架成功部署到仿真和真实机器人系统中，能够操作各种异质铰接物体。项目网站提供了演示视频。

Conclusion: 该研究提出了一种鲁棒且样本高效的框架，通过SAFE和ArtIEst技术解决了腿式机械臂操作异质铰接物体的挑战，实现了跨域泛化能力，并在仿真和真实环境中得到验证。
...
### [198] [Terrain characterization and locomotion adaptation in a small-scale lizard-inspired robot](https://arxiv.org/abs/2603.05837)
*Duncan Andrews,Landon Zimmerman,Evan Martin,Joe DiGennaro,Baxi Chong*

Main category: cs.RO

TL;DR: SILA Bot：一种受蜥蜴启发的小型智能自适应机器人，通过感知关节扭矩估计颗粒介质深度，并采用线性反馈控制器调整身体运动模式，显著提升了复杂自然地形中的运动性能。


<details>
  <summary>Details</summary>
Motivation: 小型机器人通常局限于实验室环境，难以在真实世界复杂地形中部署。随着机器人尺寸减小，机器人与地形相互作用发生根本变化，但缺乏对小型机器人应获取何种感知信息以及如何响应复杂自然地形的系统理解。

Method: 开发了SILA Bot（小型智能蜥蜴自适应机器人），使用不同深度的颗粒介质作为受控但具代表性的地形范式。通过K最近邻分类器分析关节扭矩等本体感觉信号来估计颗粒深度，并设计线性反馈控制器根据深度估计调整身体相位。

Result: 最优身体运动模式（从平坦地面上的驻波弯曲到深颗粒介质中的行波起伏）可作为颗粒深度的线性函数参数化。关节扭矩信号通过KNN分类器估计颗粒深度达到95%准确率。线性反馈控制器显著提升了未知深度地形上的运动性能。

Conclusion: 该研究为小型运动中的感知与控制建立了原则性框架，实现了有效的地形自适应运动，同时保持了较低的计算复杂度，为小型机器人在真实世界复杂地形中的部署提供了解决方案。
...
### [199] [Expert Knowledge-driven Reinforcement Learning for Autonomous Racing via Trajectory Guidance and Dynamics Constraints](https://arxiv.org/abs/2603.05842)
*Bo Leng,Weiqi Zhang,Zhuoren Li,Lu Xiong,Guizhe Jin,Ran Yu,Chen Lv*

Main category: cs.RO

TL;DR: 提出TraD-RL方法，通过轨迹引导和动力学约束解决自动驾驶赛车中强化学习的不稳定和不安全问题，结合专家先验知识和安全约束实现性能与安全的协同优化。


<details>
  <summary>Details</summary>
Motivation: 强化学习在自动驾驶领域潜力显著，但在高动态、强非线性的赛车环境中存在训练不稳定和输出不安全动作的缺陷，需要解决这些挑战以实现安全高效的自主赛车。

Method: 提出TraD-RL方法：1) 利用专家赛车线构建增强状态表示和奖励塑形，整合领域知识稳定早期策略学习；2) 通过控制屏障函数将车辆动力学先验嵌入安全操作包络，实现安全约束学习；3) 采用多阶段课程学习策略，从专家引导过渡到自主探索，使策略超越专家水平。

Result: 在基于Tempelhof机场街道赛道的高保真仿真环境中评估，实验结果表明TraD-RL有效提升了自动驾驶赛车的圈速和驾驶稳定性，实现了赛车性能与安全的协同优化。

Conclusion: TraD-RL方法通过整合专家轨迹引导和动力学安全约束，成功解决了强化学习在自动驾驶赛车环境中的训练不稳定和安全问题，实现了超越专家水平的性能表现。
...
### [200] [AnyCamVLA: Zero-Shot Camera Adaptation for Viewpoint Robust Vision-Language-Action Models](https://arxiv.org/abs/2603.05868)
*Hyeongjun Heo,Seungyeon Woo,Sang Min Kim,Junho Kim,Junho Lee,Yonghyeon Lee,Young Min Kim*

Main category: cs.RO

TL;DR: 提出零样本相机适配框架，通过虚拟调整测试时相机观测来匹配训练配置，无需额外演示数据、策略微调或架构修改


<details>
  <summary>Details</summary>
Motivation: 尽管视觉-语言-动作模型在机器人操作方面取得显著进展，但这些预训练模型需要微调才能部署到特定环境。微调后的模型对非结构化环境中频繁发生的相机视角变化高度敏感。

Method: 使用前馈式新视角合成模型实时虚拟调整测试时相机观测，使其匹配训练相机配置。该方法处理外参和内参，保持VLA预训练能力，适用于任何基于RGB的策略。

Result: 在LIBERO基准测试中，该方法持续优于使用数据增强进行策略微调或使用额外3D感知特征作为视觉输入的基线方法。在真实世界机器人操作场景中验证了方法能持续增强视角鲁棒性。

Conclusion: 提出的零样本相机适配框架通过新视角合成技术有效解决了VLA模型对相机视角变化的敏感性问题，无需额外数据或模型修改，在仿真和真实环境中均表现出优越的视角鲁棒性。
...
### [201] [Improved hopping control on slopes for small robots using spring mass modeling](https://arxiv.org/abs/2603.05902)
*Heston Roberts,Pronoy Sarker,Sm Ashikul Islam,Min Gyu Kim*

Main category: cs.RO

TL;DR: 该研究分析了斜坡上跳跃机器人的稳定性问题，提出了通过调整身体触地角度和施加小扭矩来抵消斜坡引起的旋转，从而实现在陡坡上的稳定跳跃。


<details>
  <summary>Details</summary>
Motivation: 跳跃机器人在斜坡上容易失去平衡，因为倾斜的地面在着陆时会产生不需要的旋转。这种不稳定性限制了机器人在自然地形（如山坡、碎石和不规则户外景观）中的导航能力。

Method: 使用简单的弹簧质量模型分析斜坡引起的冲击效应，并提出两种简单修正：1）根据斜坡调整身体的触地角度；2）在起飞前施加小的校正扭矩。这些方法共同作用，有效抵消了倾斜地形引起的不必要旋转。

Result: 仿真结果表明，即使是小的斜坡感知调整也能显著改善着陆稳定性。该方法足够简单，可以在低成本机器人平台上实现，无需复杂的传感或计算。

Conclusion: 通过结合分析模型和最小控制动作，该方法为实现在不规则地形上的可靠跳跃提供了实用途径，适合未来必须在自然环境中导航的自主野外机器人。
...
### [202] [Iterative Convex Optimization with Control Barrier Functions for Obstacle Avoidance among Polytopes](https://arxiv.org/abs/2603.05916)
*Shuo Liu,Zhe Huang,Calin A. Belta*

Main category: cs.RO

TL;DR: 提出基于精确多面体距离计算的迭代凸MPC-DCBF框架，用于多面体机器人在多面体障碍物环境中的实时避障控制


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：基于光滑几何近似（如超球面或椭球）的方法会扭曲真实几何形状并限制可行集；而将精确多面体距离集成到非线性MPC中的方法会产生非凸优化问题，限制实时性能

Method: 通过精确计算凸多面体之间的最近点来推导支撑超平面，构建线性离散时间控制屏障函数（DCBF）约束；提出迭代凸MPC-DCBF框架，通过对系统动力学和机器人几何进行局部线性化，确保每次迭代中有限时域优化的凸性

Result: 该框架降低了计算复杂度，实现了安全关键控制和轨迹规划的快速在线实现；可扩展到多机器人和三维环境；数值实验在杂乱迷宫场景中实现了无碰撞导航，求解时间达到毫秒级

Conclusion: 提出的迭代凸MPC-DCBF框架通过精确多面体距离计算和局部线性化，解决了多面体机器人避障的实时控制问题，在保持几何精度的同时实现了高效计算
...
### [203] [PROBE: Probabilistic Occupancy BEV Encoding with Analytical Translation Robustness for 3D Place Recognition](https://arxiv.org/abs/2603.05965)
...
<details>
  <summary>Details</summary>
Motivation: 现有LiDAR地点识别方法通常依赖于离散点云扰动或需要大量训练数据，缺乏能够跨传感器通用且无需数据集特定调优的鲁棒描述符。需要一种能够处理连续平移不确定性、具有物理意义参数且能实现跨传感器泛化的方法。

Method: PROBE将每个BEV单元的占用建模为伯努利随机变量，通过极坐标雅可比矩阵对连续笛卡尔平移进行解析边缘化，得到距离自适应的角度不确定性σ_θ=σ_t/r。主要参数σ_t表示以米为单位的预期平移不确定性，这是一个传感器无关的物理量。相似性计算结合了伯努利-KL Jaccard距离、指数不确定性门控和基于FFT的高度余弦相似度进行旋转对齐。

Result: 在涵盖四种不同LiDAR类型的四个数据集上评估，PROBE在多会话评估中实现了手工描述符中最高的准确率，在单会话评估中与手工和基于监督的基线方法相比具有竞争力。

Conclusion: PROBE提供了一种无需学习、具有物理意义参数且能跨传感器泛化的LiDAR地点识别方法，通过解析边缘化连续平移不确定性实现了鲁棒的性能，在多会话场景中表现尤为突出。
...
### [204] [Proprioceptive Shape Estimation of Tensegrity Manipulators Using Energy Minimisation](https://arxiv.org/abs/2603.05976)
*Tufail Ahmad Bhat,Shuhei Ikemoto*

Main category: cs.RO

TL;DR: 仅使用每个支柱相对于重力的倾角信息，实现了连续弯曲张拉整体机械臂的全形状估计，精度达总臂长的2.1%


<details>
  <summary>Details</summary>
Motivation: 连续弯曲张拉整体机械臂的形状估计对其控制至关重要。外感受传感器方法成本高且环境受限，而本体感受方法则无此限制。目前尚无大规模张拉整体结构作为机械臂的验证实例，需要开发仅使用本体感受信息的形状估计方法。

Method: 提出仅使用每个支柱相对于重力的倾角信息进行形状估计的方法。通过在每个支柱上安装惯性测量单元(IMU)获取倾角数据，这些数据是固有的本体感受信息。该方法适用于任意初始条件，并在五层张拉整体机械臂(20个支柱，总长1160mm)上进行了实验验证。

Result: 实验表明，该方法在静态条件下能从任意初始条件估计形状，精度达到总机械臂长度的2.1%。在外部干扰下也能保持稳定的形状估计。这是首次在大规模张拉整体结构作为机械臂的应用中得到验证的形状估计方法。

Conclusion: 仅使用支柱倾角信息即可实现连续弯曲张拉整体机械臂的准确形状估计，为低成本、环境适应性强的本体感受形状估计提供了可行方案，解决了外感受传感器方法的局限性。
...
### [205] [HarvestFlex: Strawberry Harvesting via Vision-Language-Action Policy Adaptation in the Wild](https://arxiv.org/abs/2603.05982)
*Ziyang Zhao,Shuheng Wang,Zhonghua Miao,Ya Xiong*

Main category: cs.RO

TL;DR: 首次将视觉-语言-动作策略迁移到真实温室桌面草莓采摘任务中，使用三视角RGB传感和VR遥操作数据，在50次试验中达到74%成功率
...
Method: 在HarvestFlex平台上构建端到端闭环系统，使用三视角RGB传感（两个固定场景视角加手腕安装视角），避免深度云和显式几何标定；收集3.71小时VR遥操作演示数据，对pi_0、pi_0.5和WALL-OSS模型进行全微调和LoRA微调

Result: 在统一的50次真实温室试验协议下，pi_0.5全微调模型达到74.0%成功率，采摘速度32.6秒/个，损伤率4.1%；异步推理-控制解耦相比同步部署进一步提升了性能

Conclusion: 研究表明仅用不到4小时真实数据即可实现非平凡的闭环采摘，但系统仍受限于近距离观测丢失和接触动力学不匹配问题，为VLA策略在农业机器人应用提供了实证基础
...
### [206] [MagRobot:An Open Simulator for Magnetically Navigated Robots](https://arxiv.org/abs/2603.05992)
...
<details>
  <summary>Details</summary>
Motivation: 磁导航系统在微创医疗中具有重要应用潜力，但机器人设计严重依赖实验原型，耗时耗成本，且缺乏统一的实验环境来比较不同系统的硬件和算法。

Method: 开发了一个直观的图形用户界面仿真平台，支持用户高效设计、可视化和分析磁导航系统，可加载第三方解剖模型，自定义硬件和算法，验证了仿真保真度。

Result: 通过体模和离体实验验证了仿真器的保真度，实现了支气管镜、血管内介入和胃肠内窥镜三个应用案例，展示了系统配置和算法可灵活优化以获得更好性能。

Conclusion: 该仿真平台为磁导航机器人研究提供了首个通用开源工具，能够显著加速系统设计和算法优化，促进不同系统间的比较和基准测试。
...
### [207] [Moving Through Clutter: Scaling Data Collection and Benchmarking for 3D Scene-Aware Humanoid Locomotion via Virtual Reality](https://arxiv.org/abs/2603.05993)
*Beichen Wang,Yuanjie Lu,Linji Wang,Liuchuan Yu,Xuesu Xiao*

Main category: cs.RO

TL;DR: 提出MTC框架，用于在杂乱3D环境中收集场景感知人形机器人运动数据，包含VR数据采集、自动重定向和基准测试系统


<details>
  <summary>Details</summary>
Motivation: 当前人形机器人运动研究主要在开放平坦环境中进行，而现实世界环境（家庭、办公室等）是杂乱、三维且几何受限的，缺乏系统耦合全身人体运动与场景几何的公开数据集

Method: 开发开源VR数据采集与评估框架，程序化生成可控杂乱度的场景，通过沉浸式VR导航捕获全身人体运动，自动重定向到人形机器人模型，并引入量化环境杂乱度和运动性能的基准测试

Result: 构建了包含145个多样化杂乱3D场景中的348条轨迹的数据集，为研究几何诱导的人形机器人运动适应和开发场景感知规划控制方法提供基础

Conclusion: MTC框架填补了杂乱3D环境中人形机器人运动数据集的空白，为场景感知运动研究提供了系统化的数据采集和评估工具
...
### [208] [TADPO: Reinforcement Learning Goes Off-road](https://arxiv.org/abs/2603.05995)
*Zhouchonghao Wu,Raymond Song,Vedant Mundheda,Luis E. Navarro-Serment,Christof Schoenborn,Jeff Schneider*

Main category: cs.RO

TL;DR: 提出TADPO算法，一种基于PPO的改进策略梯度方法，用于解决越野自动驾驶中的长时域规划问题，实现了从仿真到真实世界的零样本迁移


<details>
  <summary>Details</summary>
Motivation: 越野自动驾驶面临未测绘、动态多变的地形挑战，需要有效的长时域规划和自适应控制。虽然强化学习能直接从交互中学习控制策略，但由于越野驾驶是长时域任务且奖励信号稀疏，标准RL方法难以应用。

Method: 提出TADPO算法，扩展了PPO方法，利用离轨轨迹进行教师指导，同时使用在轨轨迹进行学生探索。基于此开发了基于视觉的端到端RL系统，用于高速越野驾驶，能够应对极端坡度和障碍物密集地形。

Result: 在仿真中验证了性能，并成功实现了零样本从仿真到真实世界的迁移，部署在真实全尺寸越野车辆上。据作者所知，这是首次在真实全尺寸越野平台上部署基于RL的策略。

Conclusion: TADPO算法有效解决了越野自动驾驶中的长时域规划问题，通过创新的教师-学生框架结合离轨和在轨学习，实现了从仿真到真实世界的成功迁移，为越野自动驾驶提供了可行的RL解决方案。
...
### [209] [Restoring Linguistic Grounding in VLA Models via Train-Free Attention Recalibration](https://arxiv.org/abs/2603.06001)
*Ninghao Zhang,Bin Zhu,Shijie Zhou,Jingjing Chen*

Main category: cs.RO

TL;DR: VLA模型存在语言盲视问题：在视觉场景与语言指令矛盾时，模型仍会执行视觉上合理的动作。研究者提出ICBench诊断基准和IGAR推理时机制来缓解此问题。


<details>
  <summary>Details</summary>
Motivation: VLA模型作为机器人通用策略的基础，其在分布外指令下的可靠性尚未充分探索。研究发现VLA模型存在语言盲视现象，即当语言指令与视觉场景矛盾时，模型仍会优先视觉先验而忽略指令语义，这在实际应用中可能导致危险操作。

Method: 1. 提出ICBench诊断基准，基于LIBERO数据集构建，通过注入受控的OOD指令矛盾来探测语言-动作耦合；2. 提出IGAR（指令引导注意力重新校准），一种无需训练、推理时的工作机制，通过重新平衡注意力分布来恢复语言指令的影响；3. 在三种代表性VLA架构（Pi0、Pi0.5、OpenVLA OFT）上评估，并在真实Franka机械臂上验证。

Result: 1. 现有VLA模型在逻辑不可能的指令下仍频繁成功执行任务，显示动作生成中存在强烈的视觉偏差；2. IGAR在30个LIBERO任务中显著减少了OOD矛盾指令下的错误执行，同时保持了基线任务性能；3. 在真实Franka机械臂上，IGAR有效防止了由不一致指令触发的错误操作。

Conclusion: VLA模型存在语言盲视问题，即过度依赖视觉先验而忽视矛盾的语言指令。提出的ICBench基准能系统诊断此问题，而IGAR机制能有效缓解语言盲视，无需重新训练或架构修改，为提升VLA模型在现实世界中的安全性和可靠性提供了实用解决方案。
...
### [210] [RODEO: RObotic DEcentralized Organization](https://arxiv.org/abs/2603.06058)
*Milan Groshev,Eduardo Castelló Ferrer*

Main category: cs.RO

TL;DR: RODEO框架：基于区块链的机器人去中心化自治组织，实现机器人服务审计、可信执行和自主经济循环


<details>
  <summary>Details</summary>
Motivation: 当前机器人自主性提升但缺乏可审计性、透明决策机制和新型人机交互模型，需要建立信任和问责机制以实现扩展的机器人自主性

Method: 提出RODEO框架，包含：1) ROS-ETH桥接DAO与机器人；2) 组织集成模板；3) 证明验证机制。实验中使用移动机器人作为垃圾收集器，通过智能垃圾桶分类处理垃圾，提交操作证明并获得DAO代币补偿，然后重新投资购买充电服务

Result: 三天实验数据显示：机器人收入翻倍，成功为未来88小时自主运行提供资金；证明验证时间约1分钟确保可验证任务执行；机器人通过收入再投资延长运行时间

Conclusion: RODEO框架展示了机器人与组织如何通过可审计执行证明和链上结算协调任务和支付，为实现可信、可审计的机器人自主性提供了可行方案
...
### [211] [Multimodal Behavior Tree Generation: A Small Vision-Language Model for Robot Task Planning](https://arxiv.org/abs/2603.06084)
*Cristiano Battistini,Riccardo Andrea Izzo,Gianluca Bardaro,Matteo Matteucci*

Main category: cs.RO

TL;DR: 使用开源多模态模型通过微调生成机器人任务规划的行为树，在保持计算效率的同时达到接近闭源模型的性能


<details>
  <summary>Details</summary>
Motivation: 结合语言模型在任务规划和视觉语言模型在视觉理解方面的优势，解决现有数据集中缺乏视觉观察、指令与可执行行为树关联的问题

Method: 从现有机器人数据集中构建关联视觉观察、指令和行为树的数据集，使用大模型作为教师进行多阶段生成，然后通过参数高效微调(PEFT)微调500M到4B参数的视觉语言模型

Result: 微调后的4B参数模型在离线评估中表现良好，在家庭任务模拟执行中达到87%的成功率，性能接近最先进的闭源模型，同时仅需少量计算资源

Conclusion: 通过构建专门数据集和高效微调，紧凑的开源多模态模型能够有效生成机器人任务规划的行为树，在保持计算效率的同时实现高性能
...
### [212] [Sticky-Glance: Robust Intent Recognition for Human Robot Collaboration via Single-Glance](https://arxiv.org/abs/2603.06121)
*Yuzhi Lai,Shenghai Yuan,Peizheng Li,Andreas Zell*

Main category: cs.RO

TL;DR: 提出基于对象中心的凝视接地框架，通过粘性凝视算法稳定意图识别，实现动态目标跟踪率0.94和静态目标选择准确率0.98，结合连续共享控制和多模态交互减少任务时间近10%


<details>
  <summary>Details</summary>
Motivation: 凝视是运动能力严重受损人群的重要通信方式，但在多对象环境中，由于凝视噪声、微眼跳、视角变化和动态对象等因素，实现鲁棒的基于凝视的意图识别具有挑战性

Method: 提出对象中心的凝视接地框架，采用粘性凝视算法联合建模几何距离和方向趋势来稳定意图，即使只有3个凝视样本的短暂注视也能将意图锚定在对象上；进一步引入连续共享控制和多模态交互范式

Result: 动态目标跟踪率达到0.94，静态目标选择准确率达到0.98；通过连续共享控制和多模态交互，任务持续时间减少近10%；在动态跟踪、多视角对齐、基线比较、用户研究和消融研究中均表现出更好的鲁棒性、效率和更低的工作负荷

Conclusion: 提出的对象中心凝视接地框架有效解决了多对象环境中凝视意图识别的挑战，通过粘性凝视算法稳定意图，结合连续共享控制和多模态交互，显著提高了系统的鲁棒性、效率和用户体验
...
### [213] [A Hazard-Informed Data Pipeline for Robotics Physical Safety](https://arxiv.org/abs/2603.06130)
...
<details>
  <summary>Details</summary>
Motivation: 需要将传统安全工程方法与现代机器学习技术相结合，建立系统化的机器人物理安全保障框架，解决安全边界学习与危险识别之间的鸿沟

Method: 基于显式资产声明、系统化漏洞枚举和危险驱动合成数据生成的结构化框架，结合形式化危险本体论，连接数字孪生仿真与机器学习模型训练

Result: 建立了连接传统安全工程、数字孪生仿真、合成数据生成和机器学习模型训练的对齐框架，实现了基于形式化危险本体的安全边界学习

Conclusion: 该框架成功桥接了经典风险工程与现代机器学习流程，为机器人物理安全提供了系统化的方法论，实现了安全工程与AI技术的有效融合
...
### [214] [Dual-Agent Multiple-Model Reinforcement Learning for Event-Triggered Human-Robot Co-Adaptation in Decoupled Task Spaces](https://arxiv.org/abs/2603.06163)
*Yaqi Li,Zhengqi Han,Huifang Liu,Steven W. Su*

Main category: cs.RO

TL;DR: 提出基于事件触发的双智能体多模型强化学习共享控制康复策略，用于6自由度上肢机器人，将复杂任务分解为解耦空间轴，通过事件驱动避免轨迹振荡，优化人机协同适应。


<details>
  <summary>Details</summary>
Motivation: 传统固定频率控制因逆运动学执行时间变化导致轨迹振荡，需要更安全高效的人机协同适应方法，以提升康复机器人执行复杂到达任务的成功率和用户体验。

Method: 采用事件驱动进展策略，仅当末端执行器进入目标路点许可球时触发控制动作；提出双智能体多模型强化学习框架，人类智能体选择许可球半径反映速度-精度权衡，机器人智能体动态调整3D笛卡尔步长以适应用户认知状态。

Result: 该方法有效抑制了路点抖动，平衡了空间精度与时间效率，在物体获取任务中显著提高了成功率，在混合环境中验证了其有效性。

Conclusion: 事件触发的DAMMRL方法为康复机器人提供了一种有效的共享控制策略，通过解耦空间轴和自适应参数调整，实现了更安全高效的人机协同适应。
...
### [215] [DreamToNav: Generalizable Navigation for Robots via Generative Video Planning](https://arxiv.org/abs/2603.06190)
*Valerii Serpiva,Jeffrin Sam,Chidera Simon,Hajira Amjad,Iana Zhura,Artem Lykov,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: DreamToNav：一种利用生成视频模型实现直观人机交互控制的自主机器人框架，用户通过自然语言指令控制机器人运动，系统将指令转换为可执行的运动轨迹


<details>
  <summary>Details</summary>
Motivation: 传统机器人导航通常依赖刚性的路径点导航，需要精确的工程设计和任务特定配置。研究人员希望开发一种更直观的人机交互方式，让用户能够通过自然语言指令控制机器人，减少任务特定工程需求，同时实现统一的障碍物避让和目标导向导航框架

Method: 1. 使用Qwen 2.5-VL-7B-Instruct将模糊的用户指令细化为精确的视觉描述；2. 利用NVIDIA Cosmos 2.5视频基础模型，基于视觉描述合成机器人执行任务的物理一致视频序列；3. 从合成视频中通过视觉姿态估计、机器人检测和轨迹恢复提取有效的运动学路径；4. 将视频生成作为规划引擎，让机器人先"梦想"复杂行为再执行

Result: 在轮式移动机器人和四足机器人的室内导航任务中，DreamToNav实现了76.7%的成功率，最终目标误差通常在0.05-0.10米之间，轨迹跟踪误差低于0.15米。结果表明，从生成视频预测中提取的轨迹可以在不同运动平台上可靠执行

Conclusion: DreamToNav证明了生成视频模型可以作为有效的机器人规划引擎，将自然语言指令转换为可执行的机器人运动。该方法提供了一种统一框架，无需任务特定工程即可实现障碍物避让和目标导向导航，为直观的人机交互控制开辟了新途径
...
### [216] [Towards Robotic Lake Maintenance: Integrating SONAR and Satellite Data to Assist Human Operators](https://arxiv.org/abs/2603.06266)
*Ahmed H. Elsayed,Christoph Manss,Tarek A. El-Mihoub,Andrej Lejman,Frederic Stahl*

Main category: cs.RO

TL;DR: 提出一种结合卫星遥感与无人船声呐的两阶段方法，用于人工水体中沉水植被的检测与精准收割管理


<details>
  <summary>Details</summary>
Motivation: 人工水体(AWBs)需要持续监测和维护以管理其生态系统，其中沉水植被会快速生长并需要收割以维持生态平衡。传统方法劳动强度大，需要更高效的目标性收割方案。

Method: 采用两阶段方法：第一阶段使用卫星遥感数据，通过水生植物和藻类(APA)指数进行沉水植被的初步检测，确定感兴趣区域；第二阶段使用配备多波束声呐的无人水面船(USV)进行高分辨率水深测量，精确定位和量化水生植被。

Result: 初步结果表明，整合卫星影像和水下声学传感来改善人工湖泊植被管理的可行性。该方法实现了人机协作，卫星数据指导USV任务，船工利用详细声呐地图进行目标收割，缩小搜索范围并减少人工工作量。

Conclusion: 提出的两阶段方法为人工水体中沉水植被的管理提供了有效的解决方案，通过卫星遥感与水下声呐的结合，实现了更精准、更高效的植被收割，减轻了操作人员的工作负担。
...
### [217] [SuperSuit: An Isomorphic Bimodal Interface for Scalable Mobile Manipulation](https://arxiv.org/abs/2603.06280)
...
<details>
  <summary>Details</summary>
Motivation: 获取高质量、长时程的演示数据对于具身AI至关重要，但对于轮式移动机械臂来说，现有遥操作和可穿戴接口存在局限性，需要连续协调SE(2)运动学和精确操作，这成为基本瓶颈。

Method: 提出SuperSuit双模态数据采集框架，支持机器人遥操作和主动演示两种模式，共享运动学接口。对于运动，将自然人类步态映射到连续平面基座速度；对于操作，在两种模式下使用严格同构的可穿戴手臂。策略训练采用平移不变的delta关节表示，避免逆运动学问题。

Result: 主动模式相比遥操作基线演示吞吐量提高2.6倍；在固定数据集大小下，用主动演示数据替代遥操作数据可获得可比的策略性能；随着主动数据量增加，性能呈单调提升。

Conclusion: 跨采集模式的一致运动学表示能够实现长时程移动操作的可扩展数据采集，SuperSuit框架为解决移动机械臂演示数据获取瓶颈提供了有效方案。
...
### [218] [Open-Source Based and ETSI Compliant Cooperative, Connected, and Automated Mini-Cars](https://arxiv.org/abs/2603.06343)
...
TL;DR: 提出使用1:10比例缩放的协作式、自主和互联迷你车作为从仿真到实际实验的中间步骤，降低自动驾驶技术开发成本


<details>
  <summary>Details</summary>
Motivation: 汽车行业正从人工驾驶向全自动、全互联、全协作车辆发展，但新协作算法和协议的实地测试成本高昂，从仿真到实际实验需要巨大投资，可能成为技术发展的障碍

Method: 开发1:10比例缩放的迷你车平台，配备Jetson Orin板运行ROS2用于自主操作，Raspberry Pi板运行开源OScar实现ETSI C-ITS兼容的完整协作智能交通系统栈

Result: 通过实施Day-1交叉路口碰撞预警应用案例研究，验证了所提平台的可行性和潜力

Conclusion: 该迷你车平台为自动驾驶和车联网技术开发提供了经济高效的中间测试方案，有助于降低研发成本并加速技术验证
...
### [219] [Safe Consensus of Cooperative Manipulation with Hierarchical Event-Triggered Control Barrier Functions](https://arxiv.org/abs/2603.06356)
...
<details>
  <summary>Details</summary>
Motivation: 多机械臂协同运输重型或大型负载需要在协调编队跟踪的同时，在通信受限、实时计算预算有限的不同环境中强制执行严格的安全约束

Method: 提出分布式控制框架：1) 基于共识的协议，仅依赖局部邻居信息实现任务空间平移和旋转一致性；2) 三层分层事件触发安全架构与控制屏障函数集成；3) 风险感知领导者选择和平滑切换策略以减少在线计算

Result: 通过两个Franka机械臂在静态障碍物环境中的真实硬件实验验证，以及包含动态障碍物的可扩展多臂协同综合仿真。结果显示在严格安全约束下实现更高精度协同，相比基线方法显著降低计算成本和通信频率

Conclusion: 提出的分布式控制框架成功解决了多机械臂协同运输中的安全协调问题，通过事件触发机制和分层架构在保证安全性的同时降低了计算和通信开销
...
### [220] [Data Analogies Enable Efficient Cross-Embodiment Transfer](https://arxiv.org/abs/2603.06450)
*Jonathan Yang,Chelsea Finn,Dorsa Sadigh*

Main category: cs.RO

TL;DR: 研究探索机器人演示数据的最佳组织形式以提升跨机器人设置迁移性能，发现不同形态偏移需要不同的数据策略


<details>
  <summary>Details</summary>
Motivation: 通用机器人策略在多种机器人、场景和视角的演示数据上训练，但如何有效组织和扩展这种异构数据以真正提升目标设置中的性能仍不明确。研究旨在确定哪种形式的演示数据最有利于跨机器人设置的迁移。

Method: 通过控制实验改变末端执行器形态、机器人平台外观和相机视角，比较简单增加演示数量与系统性地以不同方式扩展多样性的效果。模拟实验后，基于结果改进真实世界跨具身迁移。

Result: 模拟实验显示：感知偏移（如视角）从广泛多样性中获益最大，而形态偏移从非结构化多样性中获益较少，最大增益来自数据类比（即跨不同具身的配对演示，对齐场景、任务和/或轨迹）。基于模拟结果，仅通过改变数据组成，在真实世界中将跨具身迁移成功率平均提升22.5%（相对于大规模非配对数据集）。

Conclusion: 跨机器人设置迁移的有效性取决于数据组织方式，而非单纯的数据规模。针对不同类型的偏移（感知vs形态）需要采用不同的数据策略：感知偏移需要广泛多样性，形态偏移则需要结构化的配对演示（数据类比）。这一发现为构建更有效的跨具身机器人学习数据集提供了指导。
...
### [221] [History-Conditioned Spatio-Temporal Visual Token Pruning for Efficient Vision-Language Navigation](https://arxiv.org/abs/2603.06480)
*Qitong Wang,Yijun Liang,Ming Li,Tianyi Zhou,Christopher Rasmussen*

Main category: cs.RO

TL;DR: 提出无需训练的时空视觉token剪枝框架，用于基于VLA的视觉语言导航，在保持导航精度的同时显著提升推理效率


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言动作模型在导航任务中表现出色，但计算成本高导致延迟大，限制了实时部署。需要在不重新训练或修改预训练模型的情况下，减少冗余计算以实现高效的长时程推理。

Method: 提出无需训练的时空视觉token剪枝框架：1) 对当前视图应用空间token选择；2) 对历史记忆进行时空压缩；3) 利用基于注意力的token重要性和查询引导的时空过滤，保留导航相关信息；4) 可即插即用地集成到现有VLA系统中。

Result: 在标准VLN基准测试中显著优于现有剪枝策略，在极端剪枝场景下仍能保持优异的导航精度，同时维持高度竞争力的推理效率。在Unitree Go2四足机器人上的真实世界部署进一步验证了在实际机器人约束下的可靠低延迟指令跟随导航。

Conclusion: 该工作有助于弥合大规模多模态建模与机器人导航系统中高效实时部署之间的差距，为VLA模型的实际应用提供了有效的效率优化方案。
...
### [222] [Control Barrier Corridors: From Safety Functions to Safe Sets](https://arxiv.org/abs/2603.06494)
*Ömür Arslan,Nikolay Atanasov*

Main category: cs.RO

TL;DR: 该论文提出了一种新的控制屏障走廊概念，将控制屏障函数和运动安全走廊两种安全方法统一起来，用于自主机器人在未知环境中的安全路径跟随。


<details>
  <summary>Details</summary>
Motivation: 自主机器人在非结构化复杂环境中安全运行需要可靠的安全保障。控制屏障函数和运动安全走廊是两种广泛使用但技术不同的安全方法（功能性和几何性），需要一种统一的方法来结合两者的优势。

Method: 提出控制屏障走廊概念，将控制屏障函数转换为局部安全目标区域，用于反馈控制系统中的参考目标选择。通过凸屏障函数，在控制收敛率与屏障衰减率匹配的条件下，将个体状态安全性扩展到控制屏障走廊上。

Result: 展示了在完全驱动系统、运动学独轮车和线性输出调节系统上的应用示例，证明了控制屏障走廊能够实现系统运动期间在连续变化的屏障走廊上的安全可达持续目标选择。

Conclusion: 控制屏障走廊统一了功能性和几何性安全方法，实现了安全与响应性之间的权衡，为自主探索未知环境中的可验证安全和持续路径跟随提供了有效解决方案。
...
### [223] [CFEAR-Teach-and-Repeat: Fast and Accurate Radar-only Localization](https://arxiv.org/abs/2603.06501)
*Maximilian Hilger,Daniel Adolfsson,Ralf Becker,Henrik Andreasson,Achim J. Lilienthal*

Main category: cs.RO

TL;DR: CFEAR-TR：一种基于单旋转雷达的教导-重复定位方法，用于恶劣天气条件下的鲁棒自主导航，通过联合对齐实时扫描与存储扫描及滑动窗口关键帧，实现高精度定位。


<details>
  <summary>Details</summary>
Motivation: 在恶劣天气条件下，光学传感器可能失效，因此需要可靠的地图定位方法。现有方法在恶劣天气下的定位精度不足，特别是雷达与激光雷达之间的性能差距较大。

Method: 使用单旋转雷达，通过教导-重复定位流程。将雷达扫描表示为稀疏定向表面点集（基于多普勒补偿测量）。定位时联合对齐实时扫描与存储的教导扫描以及滑动窗口中的实时关键帧。地图存储在姿态图中，定位时遍历该图。

Result: 在Boreas数据集的测试序列上，定位精度达到0.117米和0.096°，相比先前最优方法提升高达63%。运行频率为29Hz，大幅缩小了雷达与激光雷达在定位精度（特别是航向估计）上的差距。

Conclusion: CFEAR-TR提供了一种轻量级、易于部署且鲁棒的恶劣天气导航方案，通过单旋转雷达实现了接近激光雷达水平的定位精度，特别是在航向估计方面表现优异。
...
### [224] [Underactuated multimodal jumping robot for extraterrestrial exploration](https://arxiv.org/abs/2603.06525)
*Neil R. Wagner,Justin K. Yim*

Main category: cs.RO

TL;DR: 提出一种仅使用两个反作用轮和三个执行器的欠驱动单足机器人，用于低重力环境下的多模态运动，包括滚动、定向跳跃和着陆、自扶正等功能。


<details>
  <summary>Details</summary>
Motivation: 为探索低重力天体（如小行星、月球等）上的多模态运动能力，需要设计能够在不同地形和环境条件下灵活切换运动模式的机器人系统。

Method: 使用仅两个反作用轮控制空间姿态，配备两个控制器：地面平衡控制器用于瞄准跳跃方向，空中重定向控制器用于瞄准着陆腿姿态。系统总共有三个执行器，尺寸0.33米，重量1.25公斤。

Result: 成功演示了滚动、定向跳跃和着陆、自扶正等运动能力，能够在不同地形和环境条件下通过简单切换运动模式来应对各种挑战。

Conclusion: 该欠驱动单足机器人系统通过极简的硬件配置实现了低重力环境下的多模态运动能力，为太空探索机器人提供了新的设计思路。
...
### [225] [Unified Learning of Temporal Task Structure and Action Timing for Bimanual Robot Manipulation](https://arxiv.org/abs/2603.06538)
...
Conclusion: 该方法成功桥接了高层符号时间推理和低层执行协调之间的鸿沟，能够从人类演示中学习完整的时间任务约束并生成可执行的双手机器人操作计划
...
### [226] [Uncertainty-Aware Adaptive Dynamics For Underwater Vehicle-Manipulator Robots](https://arxiv.org/abs/2603.06548)
*Edward Morgan,Nenyi K Dadson,Corina Barbalata*

Main category: cs.RO

TL;DR: 提出一种用于水下车辆-机械臂系统的不确定性感知自适应动力学模型框架，通过移动水平估计实现物理一致性约束在线参数估计，在BlueROV2 Heavy平台上验证了快速收敛和校准预测能力。


<details>
  <summary>Details</summary>
Motivation: 水下车辆-机械臂系统受时变水动力效应影响，需要精确且自适应的动态模型。传统固定参数模型难以处理参数变化，而现有方法缺乏物理一致性约束和不确定性量化能力。

Method: 提出不确定性感知自适应动力学模型框架：1) 保持车辆和机械臂集总参数线性化；2) 嵌入凸物理一致性约束（惯性、阻尼、摩擦、静水力学）；3) 使用移动水平估计堆叠水平回归器；4) 通过参数演化量化不确定性；5) 在线估计确保实时可行性。

Result: 实验验证：机械臂拟合R²=0.88-0.98，斜率接近1；车辆纵荡、垂荡、横摇在强耦合和噪声下良好复现；中值求解时间约0.023秒/更新；相比固定参数模型，各自由度MAE和RMSE一致降低；参数物理合理，置信区间覆盖率达近100%。

Conclusion: 该框架实现了物理一致的自适应动力学建模，提供可靠的不确定性量化，支持水下环境中的可靠前馈控制和仿真，具有在线可行性。
