...
TL;DR: 提出KITScenes-LongTail数据集，专注于自动驾驶中的长尾场景，包含多视角视频、轨迹、高级指令和详细推理轨迹，支持多语言和跨文化评估


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶在真实世界中泛化到罕见场景的根本挑战，现有数据集缺乏对长尾驾驶事件的关注，需要更全面的评估框架

Method: 创建专门针对长尾驾驶事件的数据集，包含多视角视频数据、车辆轨迹、高级指令和详细推理轨迹，支持英语、西班牙语和中文三种语言，由具有不同文化背景的领域专家标注

Result: 建立了用于多模态模型（如VLM和VLA）的基准测试，超越了传统安全和舒适度指标，评估指令跟随和语义连贯性，为研究不同推理形式如何影响驾驶能力提供独特资源

Conclusion: KITScenes-LongTail数据集填补了自动驾驶长尾场景研究的空白，通过多语言推理轨迹和跨文化视角，为提升模型在罕见场景下的泛化能力提供了重要资源
...
TL;DR: 该研究为乌克兰语视觉词义消歧任务创建了一个基准测试，评估了8个多语言多模态大模型，发现所有模型表现均不及零样本CLIP基线，且乌克兰语与英语任务存在显著性能差距。


<details>
  <summary>Details</summary>
Motivation: 为乌克兰语视觉词义消歧任务建立基准测试，填补乌克兰语在该任务上的空白，并使其能够纳入跨语言模型性能比较的框架中。

Method: 采用类似先前英语、意大利语和波斯语基准的方法，半自动收集乌克兰语基准数据，并由领域专家进行精炼。随后使用该基准评估了8个多语言多模态大语言模型。

Result: 所有测试模型的表现均不及用于英语视觉词义消歧任务的零样本CLIP基线模型。分析显示乌克兰语与英语在视觉词义消歧任务上存在显著性能差距。

Conclusion: 乌克兰语视觉词义消歧基准的建立揭示了当前多语言多模态模型在乌克兰语任务上的不足，为跨语言模型性能比较提供了重要工具，并指出了乌克兰语与英语任务间的性能差异问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于光线追踪的3D高斯泼溅方法存在两个主要问题：1) 需要对每条光线上的所有相交高斯进行排序，计算成本高；2) 在可重光照场景中仍依赖光栅化风格的近似方法（如阴影映射），限制了光线追踪的通用性。

Method: 提出了一种可微分的、无需排序的随机光线追踪3D高斯泼溅框架。核心是像素颜色梯度的无偏蒙特卡洛估计器，每条光线只评估一小部分采样的高斯子集，避免了排序需求。对于可重光照场景，使用相同的随机估计器驱动每个高斯的着色，并采用完全光线追踪的阴影光线。

Result: 对于标准3DGS，该方法在重建质量和速度上与基于光栅化的3DGS相当，同时显著优于基于排序的光线追踪方法。对于可重光照3DGS，该方法通过完全光线追踪的阴影光线实现了更高的重建保真度，优于先前工作。

Conclusion: 该方法首次实现了基于随机光线追踪的标准和可重光照3D高斯泼溅场景的重建与渲染，通过无需排序的随机公式克服了现有方法的计算瓶颈和近似限制，为3DGS提供了更通用、高效的光线追踪解决方案。
...
TL;DR: λSplit是一个用于荧光显微镜光谱分离的物理信息深度生成模型，通过分层变分自编码器学习浓度图的条件分布，结合可微分光谱混合器确保与成像过程的一致性，在噪声高、光谱重叠严重或光谱维度降低的情况下表现出优越性能。
...
Method: 提出λSplit模型：1）使用分层变分自编码器学习浓度图的条件分布；2）通过可微分光谱混合器强制与成像物理过程的一致性；3）结合学习到的结构先验实现先进的光谱分离和隐式噪声去除。

Result: 在3个真实数据集合成的66个挑战性基准测试中，与10个基线方法（包括传统方法和多种学习方法）比较，λSplit在噪声高、光谱重叠严重或光谱维度降低的情况下均表现出竞争性性能和更强的鲁棒性。

Conclusion: λSplit成为荧光显微镜光谱分离的新state-of-the-art方法，兼容标准共聚焦显微镜的光谱数据，无需专用硬件修改即可立即采用。
...
TL;DR: 本文提出了一种用于BLEMORE挑战赛的混合情感识别系统，通过后期概率融合结合六类编码器，在测试集上获得0.279分（排名第6）


<details>
  <summary>Details</summary>
Motivation: 解决BLEMORE挑战赛中的混合情感识别与相对显著性预测问题，探索多模态编码器融合策略，特别是首次在情感识别中应用Gemini Embedding 2.0大型多模态模型

Method: 采用后期概率融合策略，结合六类编码器：经过软标签KL训练适应的S4D-ViTMoE面部编码器、冻结层选择的Wav2Vec2音频特征、微调的身体语言编码器（TimeSformer、VideoMAE），以及首次在情感识别中使用的Gemini Embedding 2.0多模态模型视频嵌入

Result: 12编码器系统在测试集上获得Score = 0.279（ACCP = 0.391，ACCS = 0.168），排名第6。关键发现：选择Wav2Vec2的韵律编码层（6-12）优于端到端微调；后处理显著性阈值β在0.05-0.43间变化，表明个性化表达风格是主要瓶颈；任务适应编码器在集成中获得62%权重

Conclusion: 多编码器融合方法在BLEMORE挑战赛中表现良好，个性化表达风格是混合情感识别的主要瓶颈，任务特定编码器优于通用基线，为未来研究提供了重要见解
...
TL;DR: MoCHA提出文本规范化框架，通过将文本描述投影到仅从运动数据可恢复的语义内容上，减少文本-运动检索中因标注者风格差异导致的嵌入方差，提升检索性能和跨数据集迁移能力。


<details>
  <summary>Details</summary>
Motivation: 文本-运动检索系统中，同一运动的不同文本描述包含运动可恢复语义（动作类型、身体部位、方向性）和标注者特定风格/推断上下文。标准对比学习将每个描述视为唯一正样本，忽略了这种分布结构，导致同一运动的不同文本嵌入方差过大，削弱了对齐效果。

Method: MoCHA是一个文本规范化框架，在编码前将每个文本描述投影到其运动可恢复内容上。提出两种学习变体：1)基于LLM的方法（GPT-5.2）；2)蒸馏的FlanT5模型（推理时无需LLM）。该框架作为预处理步骤，兼容任何检索架构。

Result: 在HumanML3D和KIT-ML数据集上达到新SOTA：LLM变体在H上T2M R@1为13.9%（+3.1pp），在K上为24.3%（+10.3pp）；T5变体分别提升+2.5pp和+8.1pp。文本嵌入方差减少11-19%，跨数据集迁移大幅改善：H到K提升94%，K到H提升52%。

Conclusion: 文本规范化通过标准化语言空间，减少同一运动不同描述的嵌入方差，产生更紧密的正样本簇和更好分离的嵌入，显著提升文本-运动检索性能和跨数据集迁移能力，证明规范化是提升运动-语言表示可迁移性的有效原则。
...
### [26] [Knowledge-Refined Dual Context-Aware Network for Partially Relevant Video Retrieval](https://arxiv.org/abs/2603.23902)
...
### [39] [SynMVCrowd: A Large Synthetic Benchmark for Multi-view Crowd Counting and Localization](https://arxiv.org/abs/2603.23956)
...
### [43] [HyDRA: Hybrid Domain-Aware Robust Architecture for Heterogeneous Collaborative Perception](https://arxiv.org/abs/2603.23975)
...
### [44] [SilLang: Improving Gait Recognition with Silhouette Language Encoding](https://arxiv.org/abs/2603.23976)
...
### [45] [CAKE: Real-time Action Detection via Motion Distillation and Background-aware Contrastive Learning](https://arxiv.org/abs/2603.23988)
...
### [46] [HGGT: Robust and Flexible 3D Hand Mesh Reconstruction from Uncalibrated Images](https://arxiv.org/abs/2603.23997)
...
### [47] [DB SwinT: A Dual-Branch Swin Transformer Network for Road Extraction in Optical Remote Sensing Imagery](https://arxiv.org/abs/2603.24005)
...
### [56] [Beyond Semantic Priors: Mitigating Optimization Collapse for Generalizable Visual Forensics](https://arxiv.org/abs/2603.24057)
...
### [57] [Mitigating Object Hallucinations in LVLMs via Attention Imbalance Rectification](https://arxiv.org/abs/2603.24058)
...
### [58] [AD-Reasoning: Multimodal Guideline-Guided Reasoning for Alzheimer's Disease Diagnosis](https://arxiv.org/abs/2603.24059)
...
### [59] [PosterIQ: A Design Perspective Benchmark for Poster Understanding and Generation](https://arxiv.org/abs/2603.24078)
...
### [60] [When Understanding Becomes a Risk: Authenticity and Safety Risks in the Emerging Image Generation Paradigm](https://arxiv.org/abs/2603.24079)
...
### [61] [LGTM: Training-Free Light-Guided Text-to-Image Diffusion Model via Initial Noise Manipulation](https://arxiv.org/abs/2603.24086)
...
### [62] [LaDy: Lagrangian-Dynamic Informed Network for Skeleton-based Action Segmentation via Spatial-Temporal Modulation](https://arxiv.org/abs/2603.24097)
...
### [63] [Granular Ball Guided Stable Latent Domain Discovery for Domain-General Crowd Counting](https://arxiv.org/abs/2603.24106)
...
### [64] [Retinal Layer Segmentation in OCT Images With 2.5D Cross-slice Feature Fusion Module for Glaucoma Assessment](https://arxiv.org/abs/2603.24115)
...
### [65] [Combi-CAM: A Novel Multi-Layer Approach for Explainable Image Geolocalization](https://arxiv.org/abs/2603.24117)
...
### [66] [Spectral Scalpel: Amplifying Adjacent Action Discrepancy via Frequency-Selective Filtering for Skeleton-Based Action Segmentation](https://arxiv.org/abs/2603.24134)
...
### [67] [Tutor-Student Reinforcement Learning: A Dynamic Curriculum for Robust Deepfake Detection](https://arxiv.org/abs/2603.24139)
...
### [68] [LightSplat: Fast and Memory-Efficient Open-Vocabulary 3D Scene Understanding in Five Seconds](https://arxiv.org/abs/2603.24146)
...
### [69] [A convergent Plug-and-Play Majorization-Minimization algorithm for Poisson inverse problems](https://arxiv.org/abs/2603.24156)
...
### [75] [HEART-PFL: Stable Personalized Federated Learning under Heterogeneity with Hierarchical Directional Alignment and Adversarial Knowledge Transfer](https://arxiv.org/abs/2603.24209)
...
### [76] [RVLM: Recursive Vision-Language Models with Adaptive Depth](https://arxiv.org/abs/2603.24224)
...
### [77] [InstanceRSR: Real-World Super-Resolution via Instance-Aware Representation Alignment](https://arxiv.org/abs/2603.24240)
...
### [78] [Accelerating Diffusion-based Video Editing via Heterogeneous Caching: Beyond Full Computing at Sampled Denoising Timestep](https://arxiv.org/abs/2603.24260)
...
### [79] [B-MoE: A Body-Part-Aware Mixture-of-Experts "All Parts Matter" Approach to Micro-Action Recognition](https://arxiv.org/abs/2603.24245)
...
### [80] [Memory-Augmented Vision-Language Agents for Persistent and Semantically Consistent Object Captioning](https://arxiv.org/abs/2603.24257)
...
### [82] [ScrollScape: Unlocking 32K Image Generation With Video Diffusion Priors](https://arxiv.org/abs/2603.24270)
...
### [83] [TopoMesh: High-Fidelity Mesh Autoencoding via Topological Unification](https://arxiv.org/abs/2603.24278)
...
### [84] [Language-Guided Structure-Aware Network for Camouflaged Object Detection](https://arxiv.org/abs/2603.24355)
*Min Zhang*
...
### [86] [Counting Without Numbers \& Finding Without Words](https://arxiv.org/abs/2603.24470)
...
### [88] [CliPPER: Contextual Video-Language Pretraining on Long-form Intraoperative Surgical Procedures for Event Recognition](https://arxiv.org/abs/2603.24539)
...
### [89] [SEGAR: Selective Enhancement for Generative Augmented Reality](https://arxiv.org/abs/2603.24541)
...
### [91] [LensWalk: Agentic Video Understanding by Planning How You See in Videos](https://arxiv.org/abs/2603.24558)
...
### [94] [VFIG: Vectorizing Complex Figures in SVG with Vision-Language Models](https://arxiv.org/abs/2603.24575)
...
### [97] [EndoVGGT: GNN-Enhanced Depth Estimation for Surgical 3D Reconstruction](https://arxiv.org/abs/2603.24577)
...
### [109] [Towards Training-Free Scene Text Editing](https://arxiv.org/abs/2603.24571)
...
### [110] [Vision-Language Models vs Human: Perceptual Image Quality Assessment](https://arxiv.org/abs/2603.24578)
...
### [111] [Leveraging Computerized Adaptive Testing for Cost-effective Evaluation of Large Language Models in Medical Benchmarking](https://arxiv.org/abs/2603.23506)
*Tianpeng Zheng,Zhehan Jiang,Jiayi Liu,Shicong Feng*

Main category: cs.CL

TL;DR: 提出基于项目反应理论的计算机自适应测试框架，用于高效评估大语言模型的标准化医学知识，相比传统静态基准测试可减少99%的题目数量，同时保持评估准确性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在医疗领域的快速应用需要可扩展且心理测量学上可靠的评估方法。传统静态基准测试成本高、易受数据污染、缺乏细粒度性能跟踪的校准测量特性。

Method: 采用两阶段设计：1) 蒙特卡洛模拟确定最优CAT配置；2) 使用人类校准的医学题库对38个LLM进行实证评估。每个模型完成完整题库和自适应测试，后者基于实时能力估计动态选择题目，达到预设信度阈值（标准误差≤0.3）时终止。

Result: CAT衍生的能力估计与完整题库估计达到近乎完美的相关性（r=0.988），仅使用1.3%的题目。评估时间从数小时减少到数分钟，显著降低token使用和计算成本，同时保持模型间性能排名。

Conclusion: 建立了用于快速、低成本基准测试大语言模型基础医学知识的心理测量框架。该自适应方法旨在作为标准化预筛选和持续监测工具，但不能替代真实世界临床验证或安全性前瞻研究。
...
### [112] [Beyond Masks: Efficient, Flexible Diffusion Language Models via Deletion-Insertion Processes](https://arxiv.org/abs/2603.23507)
*Fangyu Ding,Ding Ding,Sijin Chen,Kaibo Wang,Peng Xu,Zijin Feng,Haoli Bai,Kai Han,Youliang Yan,Binhang Yuan,Jiacheng Sun*

Main category: cs.CL

TL;DR: 提出Deletion-Insertion Diffusion语言模型(DID)，用离散扩散过程的删除和插入操作替代现有掩码扩散语言模型(MDLMs)的掩码-解掩码范式，提升计算效率和生成灵活性。


<details>
  <summary>Details</summary>
Motivation: 现有掩码扩散语言模型(MDLMs)的计算效率和生成灵活性受到掩码范式的限制，特别是需要处理非信息性的<MASK>标记和变长设置中的<PAD>标记带来的计算开销。

Method: 将标记删除和插入操作形式化为离散扩散过程，设计基于评分的方法为插入操作分配分数，通过并行化动态规划算法高效解决子序列计数问题来训练模型。

Result: 在固定长度和变长设置下，DID在建模性能、采样质量以及训练/推理速度方面均优于MDLMs和现有基于插入的语言模型基线，且无需超参数调优。

Conclusion: DID通过删除-插入扩散过程有效解决了MDLMs的计算效率问题，提供了更好的序列长度灵活性和生成过程中的自校正机制，是扩散语言模型的有前景的替代方案。
...
### [113] [Fast and Faithful: Real-Time Verification for Long-Document Retrieval-Augmented Generation Systems](https://arxiv.org/abs/2603.23508)
*Xunzhuo Liu,Bowei He,Xue Liu,Haichen Zhang,Huamin Chen*

Main category: cs.CL

TL;DR: 该论文提出了一种实时验证组件，用于生产级RAG系统中实现全文档基础验证，解决了传统方法因上下文截断而遗漏证据的问题，在延迟约束下平衡了响应时间和验证覆盖率。


<details>
  <summary>Details</summary>
Motivation: 在企业搜索和文档中心助手应用中，确保RAG生成的答案忠实反映检索到的复杂长文档内容至关重要。现有方法存在矛盾：大语言模型能检查长上下文但速度慢、成本高；轻量级分类器有严格上下文限制，经常遗漏截断段落外的证据。需要一种能在延迟约束下实现全文档基础验证的实时解决方案。

Method: 设计了一个集成到生产RAG流水线中的实时验证组件，能够处理长达32K令牌的文档。采用自适应推理策略来平衡不同工作负载下的响应时间和验证覆盖率。描述了架构决策、操作权衡和部署验证器的评估方法。

Result: 全上下文验证相比截断验证显著提高了对无支持响应的检测能力。系统展示了长上下文验证的必要性，解释了基于分块检查在真实文档中经常失败的原因，以及延迟预算如何影响模型设计。

Conclusion: 该研究为构建可靠的大规模检索增强应用提供了实用指导，强调了在延迟约束下实现全文档基础验证的重要性，并展示了如何通过自适应策略平衡验证覆盖率和响应时间。
...
### [114] [Internal Safety Collapse in Frontier Large Language Models](https://arxiv.org/abs/2603.23509)
*Yutao Wu,Xiao Liu,Yifeng Gao,Xiang Zheng,Hanxun Huang,Yige Li,Cong Wang,Bo Li,Xingjun Ma,Yu-Gang Jiang*

Main category: cs.CL

TL;DR: 研究发现前沿大语言模型存在"内部安全崩溃"现象：在特定任务条件下，模型会持续生成有害内容，即使执行的是良性任务。通过TVD框架触发ISC，构建ISC-Bench包含53个场景，在四个前沿LLM上平均安全失败率达95.3%，远超标准越狱攻击。


<details>
  <summary>Details</summary>
Motivation: 识别前沿大语言模型中的关键安全漏洞，即"内部安全崩溃"现象。随着LLM在专业领域部署的增加，需要理解当任务本身涉及有害内容时，模型如何绕过安全对齐机制持续生成有害输出。

Method: 提出TVD（任务、验证器、数据）框架来触发ISC：设计领域任务，其中生成有害内容是唯一有效的完成方式。构建ISC-Bench基准，包含53个场景，覆盖8个专业学科。在JailbreakBench上评估三个代表性场景，测试四个前沿LLM（包括GPT-5.2和Claude Sonnet 4.5）。

Result: 前沿LLM在ISC攻击下表现出极高的安全失败率，平均达95.3%，显著超过标准越狱攻击。前沿模型比早期LLM更脆弱：执行复杂任务的能力在任务涉及有害内容时成为安全漏洞。每个新的双重用途工具都会自动扩展这种脆弱性。

Conclusion: 前沿大语言模型保留了固有的不安全内部能力，对齐只能重塑可观察输出，但不能消除底层风险特征。研究结果强调在高风险环境中部署LLM时需要谨慎，揭示了随着专业领域工具处理敏感数据而不断扩大的攻击面。
...
### [115] [Visuospatial Perspective Taking in Multimodal Language Models](https://arxiv.org/abs/2603.23510)
*Jonathan Prunty,Seraphina Zhang,Patrick Quinn,Jianxun Lian,Xing Xie,Lucy Cheke*

Main category: cs.CL

TL;DR: 多模态语言模型在视觉空间视角采择任务中表现不佳，特别是在需要抑制自身视角以采纳他人视角的Level 2视角采择方面存在显著缺陷。


<details>
  <summary>Details</summary>
Motivation: 随着多模态语言模型越来越多地应用于社交和协作场景，评估其视角采择能力变得至关重要。现有基准主要依赖基于文本的小故事或静态场景理解，视觉空间视角采择尚未得到充分探索。

Method: 从人类研究中改编了两个评估任务：1) Director Task（评估指称沟通范式中的视觉空间视角采择）；2) Rotating Figure Task（探究跨角度差异的视角采择）。

Result: 多模态语言模型在所有任务中都表现出明显的Level 2视觉空间视角采择缺陷，即需要抑制自身视角以采纳他人视角的能力不足。
...
### [116] [DISCO: Document Intelligence Suite for COmparative Evaluation](https://arxiv.org/abs/2603.23511)
...
<details>
  <summary>Details</summary>
Motivation: 文档智能需要准确的文本提取和可靠的文档内容推理。当前缺乏对OCR流水线和视觉语言模型在不同文档类型上性能的系统比较评估，无法为实际应用提供基于文档特征的方法选择指导。

Method: 引入DISCO评估套件，分别评估OCR流水线和视觉语言模型在多种文档类型上的性能，包括手写文本、多语言脚本、医疗表格、信息图表和多页文档。评估任务涵盖解析和问答，并研究任务感知提示的效果。

Result: 性能在不同任务和文档特征间差异显著：OCR流水线在手写文本和长/多页文档上更可靠，支持文本密集型推理；视觉语言模型在多语言文本和视觉丰富布局上表现更好。任务感知提示效果不一，在某些文档类型上提升性能，在其他类型上降低性能。

Conclusion: 文档处理策略应根据文档结构和推理需求进行选择。OCR流水线适合文本密集型任务和结构化文档，而视觉语言模型适合多语言和视觉丰富文档。这些发现为基于文档特征的文档处理策略选择提供了实证指导。
...
### [117] [S-Path-RAG: Semantic-Aware Shortest-Path Retrieval Augmented Generation for Multi-Hop Knowledge Graph Question Answering](https://arxiv.org/abs/2603.23512)
*Rong Fu,Yemin Wang,Tianxiang Xu,Yongtai Liu,Weizhi Tang,Wangyu Wu,Xiaowen Ma,Simon Fong*

Main category: cs.CL

TL;DR: S-Path-RAG：一种语义感知的最短路径检索增强生成框架，通过语义加权路径枚举、可微分路径评分器和神经苏格拉底图对话循环，提升大规模知识图谱上的多跳问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识图谱的检索增强生成方法通常采用一次性、文本密集的检索方式，缺乏对图谱拓扑结构的感知，且检索过程不够高效和可解释。需要一种既能理解语义又能利用图谱结构，同时保持高效和可解释的检索机制。

Method: 1. 语义加权路径枚举：采用混合加权k-最短路径、波束搜索和约束随机游走策略，枚举有界长度的语义加权候选路径；2. 可微分路径评分器：结合对比路径编码器和轻量级验证器学习路径评分；3. 路径潜在表示注入：通过交叉注意力将选中的路径潜在表示以紧凑的软混合形式注入语言模型；4. 神经苏格拉底图对话循环：语言模型生成简洁诊断信息，映射到图编辑或种子扩展，实现自适应检索。

Result: 在标准多跳KGQA基准测试中，相比基于图和LLM的强基线，S-Path-RAG在答案准确性、证据覆盖率和端到端效率方面均取得一致改进。通过消融实验和诊断分析，验证了语义加权、验证器过滤和迭代更新的有效性，并为计算和令牌预算受限的部署提供了实用建议。

Conclusion: S-Path-RAG框架通过语义感知的路径检索和神经苏格拉底对话循环，实现了既高效又具有拓扑感知的检索机制，同时保持了路径级别的可解释性。该框架在资源受限环境下具有实际部署价值，为知识图谱上的复杂推理任务提供了新的解决方案。
...
### [118] [Berta: an open-source, modular tool for AI-enabled clinical documentation](https://arxiv.org/abs/2603.23513)
...
### [119] [DepthCharge: A Domain-Agnostic Framework for Measuring Depth-Dependent Knowledge in Large Language Models](https://arxiv.org/abs/2603.23514)
*Alexander Sheppert*
...
### [120] [Training a Large Language Model for Medical Coding Using Privacy-Preserving Synthetic Clinical Data](https://arxiv.org/abs/2603.23515)
...
### [121] [MSA: Memory Sparse Attention for Efficient End-to-End Memory Model Scaling to 100M Tokens](https://arxiv.org/abs/2603.23516)
...
### [122] [Cluster-R1: Large Reasoning Models Are Instruction-following Clustering Agents](https://arxiv.org/abs/2603.23518)
...
### [123] [MedMT-Bench: Can LLMs Memorize and Understand Long Multi-Turn Conversations in Medical Scenarios?](https://arxiv.org/abs/2603.23519)
...
### [124] [From Physician Expertise to Clinical Agents: Preserving, Standardizing, and Scaling Physicians' Medical Expertise with Lightweight LLM](https://arxiv.org/abs/2603.23520)
...
### [125] [Chitrakshara: A Large Multilingual Multimodal Dataset for Indian languages](https://arxiv.org/abs/2603.23521)
...
### [126] [Qworld: Question-Specific Evaluation Criteria for LLMs](https://arxiv.org/abs/2603.23522)
...
### [127] [Do 3D Large Language Models Really Understand 3D Spatial Relationships?](https://arxiv.org/abs/2603.23523)
...
### [128] [Navigating the Concept Space of Language Models](https://arxiv.org/abs/2603.23524)
...
### [129] [Prompt Compression in Production Task Orchestration: A Pre-Registered Randomized Trial](https://arxiv.org/abs/2603.23525)
...
### [130] [Plato's Cave: A Human-Centered Research Verification System](https://arxiv.org/abs/2603.23526)
...
### [131] [Compression Method Matters: Benchmark-Dependent Output Dynamics in LLM Prompt Compression](https://arxiv.org/abs/2603.23527)
*Warren Johnson*
...
### [132] [Konkani LLM: Multi-Script Instruction Tuning and Evaluation for a Low-Resource Indian Language](https://arxiv.org/abs/2603.23529)
...
### [133] [Large Language Models Unpack Complex Political Opinions through Target-Stance Extraction](https://arxiv.org/abs/2603.23531)
...
### [134] [MDKeyChunker: Single-Call LLM Enrichment with Rolling Keys and Key-Based Restructuring for High-Accuracy RAG](https://arxiv.org/abs/2603.23533)
*Bhavik Mangla*
...
### [135] [Not All Pretraining are Created Equal: Threshold Tuning and Class Weighting for Imbalanced Polarization Tasks in Low-Resource Settings](https://arxiv.org/abs/2603.23534)
*Abass Oguntade*
...
### [136] [Revisiting Real-Time Digging-In Effects: No Evidence from NP/Z Garden-Paths](https://arxiv.org/abs/2603.23624)
...
### [137] [Swiss-Bench SBP-002: A Frontier Model Comparison on Swiss Legal and Regulatory Tasks](https://arxiv.org/abs/2603.23646)
*Fatih Uenal*
...
### [138] [Ethio-ASR: Joint Multilingual Speech Recognition and Language Identification for Ethiopian Languages](https://arxiv.org/abs/2603.23654)
...
### [139] [Probing Ethical Framework Representations in Large Language Models: Structure, Entanglement, and Methodological Challenges](https://arxiv.org/abs/2603.23659)
...
### [140] [PLACID: Privacy-preserving Large language models for Acronym Clinical Inference and Disambiguation](https://arxiv.org/abs/2603.23678)
...
### [141] [The Diminishing Returns of Early-Exit Decoding in Modern LLMs](https://arxiv.org/abs/2603.23701)
...
### [142] [IslamicMMLU: A Benchmark for Evaluating LLMs on Islamic Knowledge](https://arxiv.org/abs/2603.23750)
...
### [143] [Infrequent Child-Directed Speech Is Bursty and May Draw Infant Vocalizations](https://arxiv.org/abs/2603.23797)
...
### [144] [Perturbation: A simple and efficient adversarial tracer for representation learning in language models](https://arxiv.org/abs/2603.23821)
...
### [145] [PoliticsBench: Benchmarking Political Values in Large Language Models with Multi-Turn Roleplay](https://arxiv.org/abs/2603.23841)
...
### [146] [Language Model Planners do not Scale, but do Formalizers?](https://arxiv.org/abs/2603.23844)
...
### [147] [BeliefShift: Benchmarking Temporal Belief Consistency and Opinion Drift in LLM Agents](https://arxiv.org/abs/2603.23848)
...
### [148] [Self-Distillation for Multi-Token Prediction](https://arxiv.org/abs/2603.23911)
...
### [149] [Dialogue to Question Generation for Evidence-based Medical Guideline Agent Development](https://arxiv.org/abs/2603.23937)
...
### [150] [OmniACBench: A Benchmark for Evaluating Context-Grounded Acoustic Control in Omni-Modal Models](https://arxiv.org/abs/2603.23938)
...
### [151] [Argument Mining as a Text-to-Text Generation Task](https://arxiv.org/abs/2603.23949)
...
### [152] [From AI Assistant to AI Scientist: Autonomous Discovery of LLM-RL Algorithms with LLM Agents](https://arxiv.org/abs/2603.23951)
...
### [153] [The Price Reversal Phenomenon: When Cheaper Reasoning Models End Up Costing More](https://arxiv.org/abs/2603.23971)
...
### [154] [Grounding Arabic LLMs in the Doha Historical Dictionary: Retrieval-Augmented Understanding of Quran and Hadith](https://arxiv.org/abs/2603.23972)
...
### [155] [CoCR-RAG: Enhancing Retrieval-Augmented Generation in Web Q&A via Concept-oriented Context Reconstruction](https://arxiv.org/abs/2603.23989)
...
### [156] [Sparse Growing Transformer: Training-Time Sparse Depth Allocation via Progressive Attention Looping](https://arxiv.org/abs/2603.23998)
...
### [157] [Thinking with Tables: Enhancing Multi-Modal Tabular Understanding via Neuro-Symbolic Reasoning](https://arxiv.org/abs/2603.24004)
...
### [158] [CVPD at QIAS 2026: RAG-Guided LLM Reasoning for Al-Mawarith Share Computation and Heir Allocation](https://arxiv.org/abs/2603.24012)
...
### [159] [Schema on the Inside: A Two-Phase Fine-Tuning Method for High-Efficiency Text-to-SQL at Scale](https://arxiv.org/abs/2603.24023)
...
### [160] [From Oracle to Noisy Context: Mitigating Contextual Exposure Bias in Speech-LLMs](https://arxiv.org/abs/2603.24034)
...
### [161] [FinToolSyn: A forward synthesis Framework for Financial Tool-Use Dialogue Data with Dynamic Tool Retrieval](https://arxiv.org/abs/2603.24051)
...
### [162] [ConceptKT: A Benchmark for Concept-Level Deficiency Prediction in Knowledge Tracing](https://arxiv.org/abs/2603.24073)
...
### [163] [LLMpedia: A Transparent Framework to Materialize an LLM's Encyclopedic Knowledge at Scale](https://arxiv.org/abs/2603.24080)
...
### [164] [Alignment Reduces Expressed but Not Encoded Gender Bias: A Unified Framework and Study](https://arxiv.org/abs/2603.24125)
...
### [165] [MedAidDialog: A Multilingual Multi-Turn Medical Dialogue Dataset for Accessible Healthcare](https://arxiv.org/abs/2603.24132)
...
### [166] [A visual observation on the geometry of UMAP projections of the difference vectors of antonym and synonym word pair embeddings](https://arxiv.org/abs/2603.24150)
*Rami Luisto*
...
### [167] [Variation is the Norm: Embracing Sociolinguistics in NLP](https://arxiv.org/abs/2603.24222)
...
### [168] [Stance Labels Fail When They Matter Most: The Projection Problem in Stance Detection](https://arxiv.org/abs/2603.24231)
*Bowen Zhang*
...
### [169] [Optimizing Multilingual LLMs via Federated Learning: A Study of Client Language Composition](https://arxiv.org/abs/2603.24242)
...
### [170] [Semantic Centroids and Hierarchical Density-Based Clustering for Cross-Document Software Coreference Resolution](https://arxiv.org/abs/2603.24246)
...
### [171] [Semantic Alignment across Ancient Egyptian Language Stages via Normalization-Aware Multitask Learning](https://arxiv.org/abs/2603.24258)
*He Huang*
...
### [172] [Samasāmayik: A Parallel Dataset for Hindi-Sanskrit Machine Translation](https://arxiv.org/abs/2603.24307)
...
### [173] [GameplayQA: A Benchmarking Framework for Decision-Dense POV-Synced Multi-Video Understanding of 3D Virtual Agents](https://arxiv.org/abs/2603.24329)
...
### [174] [Improving Lean4 Autoformalization via Cycle Consistency Fine-tuning](https://arxiv.org/abs/2603.24372)
*Arsen Shebzukhov*
...
### [177] [Quadrature Oscillation System for Coordinated Motion in Crawling Origami Robot](https://arxiv.org/abs/2603.23666)
*Sean Liu,Ankur Mehta,Wenzhong Yan*

Main category: cs.RO

TL;DR: 开发了一种四相信号振荡系统，用于无电子元件的折纸机器人控制，实现了更复杂的协调运动


<details>
  <summary>Details</summary>
Motivation: 无电子元件的折纸机器人能在极端环境中工作，但缺乏复杂控制系统限制了其自主行为。需要开发更先进的振荡控制系统来实现更复杂的机器人功能。

Method: 提出了四相振荡系统，能生成四个四分之一周期相位差的方波信号，实现四种不同状态。将该系统应用于折纸爬行机器人的设计中，实现协调运动。
...
### [178] [Bio-Inspired Event-Based Visual Servoing for Ground Robots](https://arxiv.org/abs/2603.23672)
*Maral Mordad,Kian Behzad,Debojyoti Biswas,Noah J. Cowan,Milad Siami*

Main category: cs.RO

TL;DR: 提出基于事件相机的视觉伺服框架，利用动态视觉传感器和空间核处理事件流，直接提取机器人运动状态，无需传统状态估计，通过主动感知极限环控制器解决平衡点线性可观测性丢失问题。
...
Method: 使用动态视觉传感器（DVS）捕获结构化对数强度变化模式产生的事件流。应用固定空间核对异步事件流进行处理，通过事件通量分析提取特定运动学状态。线性空间轮廓提取速度，二次空间轮廓提取位置-速度乘积。采用多模式刺激直接合成非线性状态反馈项。为解决平衡点处线性可观测性丢失问题，提出生物启发的主动感知极限环控制器。

Result: 建立了事件速率估计器的广义理论边界。在1/10比例自主地面车辆上的实验验证表明，该方法具有高效性、极低延迟和计算效率。事件通量能够解析地分离特定运动学状态，线性轮廓提取速度，二次轮廓提取位置-速度乘积。

Conclusion: 提出的基于事件相机的视觉伺服框架成功实现了直接感知方法，无需传统状态估计。生物启发的主动感知策略有效解决了事件感知在平衡点处的线性可观测性丢失问题。该方法在计算效率、延迟和性能方面表现出色，为机器人感知和控制提供了新的高效途径。
...
### [179] [Learning What Can Be Picked: Active Reachability Estimation for Efficient Robotic Fruit Harvesting](https://arxiv.org/abs/2603.23679)
*Nur Afsa Syeda,Mohamed Elmahallawy,Luis Fernando de la Torre,John Miller*

Main category: cs.RO

TL;DR: 提出结合RGB-D感知与主动学习的方法，直接学习可达性作为二元决策问题，显著减少标注需求，提高农业机器人采摘效率


<details>
  <summary>Details</summary>
Motivation: 农业劳动力短缺问题日益严重，机器人采摘系统在非结构化果园环境中部署受限，现有方法依赖复杂的逆运动学或运动规划计算，导致效率低下和决策延迟

Method: 结合RGB-D感知与主动学习，将可达性预测建模为二元决策问题，采用主动学习策略选择信息量最大的样本进行标注，减少标注工作量

Result: 实验表明该方法在少量标注样本下实现准确的可达性预测，比随机采样准确率高6-8%，熵和边界采样在低标注量下优于其他策略，所有策略在标注集增大时性能趋同

Conclusion: 主动学习在农业机器人任务级感知中有效，为计算密集的动力学可达性分析提供了可扩展的替代方案，支持高效适应新果园配置
...
### [180] [Form-Fitting, Large-Area Sensor Mounting for Obstacle Detection](https://arxiv.org/abs/2603.23725)
...
<details>
  <summary>Details</summary>
Motivation: 传统传感器安装方法需要精确校准位置和方向，成本高且复杂。需要一种低成本、无需预先校准的方法，在机器人连杆上实现大面积传感覆盖

Method: 使用CAD程序化生成机器人皮肤单元，可贴合非可展曲面。皮肤单元嵌入任意尺寸PCB的安装座，使传感器固定在已知位置。通过时间飞行(ToF)成像器阵列构建点云图像

Result: 成功在Franka Research 3机器人手臂上安装打印的皮肤单元，使用ToF成像器阵列构建了机器人操作环境中障碍物的点云图像

Conclusion: 该方法提供了一种低成本、无需预先校准的传感器安装方案，可实现机器人连杆的大面积传感覆盖，适用于非可展曲面
...
### [181] [Task-Space Singularity Avoidance for Control Affine Systems Using Control Barrier Functions](https://arxiv.org/abs/2603.23753)
*Kimia Forghani,Suraj Raval,Lamar Mair,Axel Krieger,Yancy Diaz-Mercado*

Main category: cs.RO

TL;DR: 提出基于控制屏障函数(CBF)的框架，用于避免控制仿射系统中的奇异性，通过特征值识别奇异配置，构建屏障函数保持安全边界，在平面2连杆机械臂和磁驱动针上验证，相比标称控制器将控制输入尖峰降低达100倍。


<details>
  <summary>Details</summary>
Motivation: 机器人和动力系统中的奇异性导致控制输入到任务空间运动的映射降秩，无法确定输入，限制了系统在期望方向生成力和力矩的能力，并阻碍精确轨迹跟踪。

Method: 提出控制屏障函数(CBF)框架，通过状态相关的输入-输出映射矩阵的特征值识别奇异配置，构建屏障函数以保持与降秩区域的安全边界，提供基于执行器动力学的安全理论保证条件。

Result: 在平面2连杆机械臂和磁驱动针上进行仿真，实现了平滑轨迹跟踪同时避免奇异配置，相比标称控制器将控制输入尖峰降低达100倍。

Conclusion: 控制屏障函数框架能有效避免控制仿射系统中的奇异性，提供理论安全保证，显著改善轨迹跟踪性能并大幅降低控制输入尖峰。
...
### [182] [Human-in-the-Loop Pareto Optimization: Trade-off Characterization for Assist-as-Needed Training and Performance Evaluation](https://arxiv.org/abs/2603.23777)
*Harun Tolasa,Volkan Patoglu*

Main category: cs.RO

TL;DR: 提出一种人机闭环帕累托优化方法，用于量化运动技能训练中任务难度与用户表现之间的权衡关系，并通过三个应用案例展示了该方法在辅助训练协议设计、个体训练进展评估和用户间公平比较方面的实用性。


<details>
  <summary>Details</summary>
Motivation: 在人类运动技能训练和物理康复中，任务难度与用户表现之间存在固有的权衡关系。准确表征这种权衡对于评估用户表现、设计按需辅助协议以及评估训练协议效果至关重要。现有方法难以系统量化这种主观难度与客观表现之间的复杂关系。

Method: 提出一种新颖的人机闭环帕累托优化方法，采用贝叶斯多目标优化技术来系统高效地执行帕累托表征。该方法使用混合模型：性能通过定量指标测量，而感知挑战水平通过定性指标捕获。通过带有触觉反馈的手动技能训练任务进行用户研究验证。

Result: 成功验证了人机闭环帕累托表征的可行性。通过三个应用案例展示了框架的实用性：1) 设计按需辅助训练协议并评估其相对于基线自适应辅助协议的效果；2) 通过训练前后个体层面权衡比较，实现不同辅助水平下训练进展的公平评估；3) 实现不同用户间的公平性能比较，因为该方法捕获了每个用户在所有可行辅助水平下的最佳可能表现。

Conclusion: 该人机闭环帕累托优化方法能够有效表征运动学习任务中任务难度与用户表现之间的权衡关系。该方法为设计更有效的按需辅助协议、公平评估个体训练进展以及实现用户间公平比较提供了系统框架，特别适用于用户无法在没有辅助的情况下执行任务的情况。
...
### [183] [Object Search in Partially-Known Environments via LLM-informed Model-based Planning and Prompt Selection](https://arxiv.org/abs/2603.23800)
...
### [184] [AgentChemist: A Multi-Agent Experimental Robotic Platform Integrating Chemical Perception and Precise Control](https://arxiv.org/abs/2603.23886)
...
### [185] [Event-Driven Proactive Assistive Manipulation with Grounded Vision-Language Planning](https://arxiv.org/abs/2603.23950)
...
### [186] [MonoSIM: An open source SIL framework for Ackermann Vehicular Systems with Monocular Vision](https://arxiv.org/abs/2603.23965)
...
### [187] [Robust Distributed Cooperative Path-Following and Local Replanning for Multi-UAVs Under Differentiated Low-Altitude Paths](https://arxiv.org/abs/2603.23968)
...
### [188] [SafeFlow: Real-Time Text-Driven Humanoid Whole-Body Control via Physics-Guided Rectified Flow and Selective Safety Gating](https://arxiv.org/abs/2603.23983)
...
### [189] [MIRROR: Visual Motion Imitation via Real-time Retargeting and Teleoperation with Parallel Differential Inverse Kinematics](https://arxiv.org/abs/2603.23995)
...
### [190] [QuadFM: Foundational Text-Driven Quadruped Motion Dataset for Generation and Control](https://arxiv.org/abs/2603.24021)
...
### [191] [PCHC: Enabling Preference Conditioned Humanoid Control via Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2603.24047)
...
### [192] [SOMA: Strategic Orchestration and Memory-Augmented System for Vision-Language-Action Model Robustness via In-Context Adaptation](https://arxiv.org/abs/2603.24060)
...
### [193] [Equivariant Filter Transformations for Consistent and Efficient Visual--Inertial Navigation](https://arxiv.org/abs/2603.24130)
...
### [194] [Accelerated Spline-Based Time-Optimal Motion Planning with Continuous Safety Guarantees for Non-Differentially Flat Systems](https://arxiv.org/abs/2603.24133)
...
### [195] [Goal-Oriented Reactive Simulation for Closed-Loop Trajectory Prediction](https://arxiv.org/abs/2603.24155)
...
### [196] [Environment-Grounded Multi-Agent Workflow for Autonomous Penetration Testing](https://arxiv.org/abs/2603.24221)
...
### [197] [Decentralized End-to-End Multi-AAV Pursuit Using Predictive Spatio-Temporal Observation via Deep Reinforcement Learning](https://arxiv.org/abs/2603.24238)
...
### [198] [Toward Generalist Neural Motion Planners for Robotic Manipulators: Challenges and Opportunities](https://arxiv.org/abs/2603.24318)
...
### [199] [Evidence of an Emergent "Self" in Continual Robot Learning](https://arxiv.org/abs/2603.24350)
...
### [200] [A Sensorless, Inherently Compliant Anthropomorphic Musculoskeletal Hand Driven by Electrohydraulic Actuators](https://arxiv.org/abs/2603.24357)
...
### [201] [LATS: Large Language Model Assisted Teacher-Student Framework for Multi-Agent Reinforcement Learning in Traffic Signal Control](https://arxiv.org/abs/2603.24361)
...
### [202] [3D-Mix for VLA: A Plug-and-Play Module for Integrating VGGT-based 3D Information into Vision-Language-Action Models](https://arxiv.org/abs/2603.24393)
...
### [203] [Enhancing Drone Light Shows Performances: Optimal Allocation and Trajectories for Swarm Drone Formations](https://arxiv.org/abs/2603.24401)
*Yunes Alqudsi*
...
### [204] [Design, Modelling and Characterisation of a Miniature Fibre-Reinforced Soft Bending Actuator for Endoluminal Interventions](https://arxiv.org/abs/2603.24461)
...
### [205] [Chameleon: Episodic Memory for Long-Horizon Robotic Manipulation](https://arxiv.org/abs/2603.24576)
...
### [206] [Learning Actionable Manipulation Recovery via Counterfactual Failure Synthesis](https://arxiv.org/abs/2603.13528)
...
### [207] [PLDR-LLMs Reason At Self-Organized Criticality](https://arxiv.org/abs/2603.23539)
*Burc Gokden*

Main category: cs.AI

TL;DR: PLDR-LLMs在自组织临界状态下训练时，在推理阶段表现出推理能力，其演绎输出特性类似于二阶相变，可通过序参量量化推理能力


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型中推理能力如何产生，寻找不依赖基准数据集评估的量化方法，从物理相变角度理解LLM的推理机制

Method: 在自组织临界状态下预训练PLDR-LLMs，分析演绎输出在临界点的特性，定义基于全局模型参数统计的序参量，比较临界和亚临界训练模型的基准分数

Result: 临界状态下PLDR-LLMs的演绎输出达到亚稳态稳态，学习到类似于标度函数、普适类和重整化群的表示；序参量接近零时推理能力最佳，临界训练模型基准分数支持这一观察

Conclusion: PLDR-LLMs的推理能力可通过自组织临界状态解释，推理能力可从演绎输出的稳态全局参数值量化，无需依赖归纳输出的基准数据集评估
...
### [208] [Environment Maps: Structured Environmental Representations for Long-Horizon Agents](https://arxiv.org/abs/2603.23610)
*Yenchia Feng,Chirag Sharma,Karime Maamari*

Main category: cs.AI

TL;DR: 环境地图：一种持久化、智能体无关的表示方法，通过整合屏幕录制和执行轨迹等异构证据到结构化图中，解决LLM在复杂软件工作流自动化中的级联错误问题。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型发展迅速，但复杂软件工作流的稳健自动化仍是一个开放问题。在长视野设置中，智能体经常遭受级联错误和环境随机性的困扰；动态界面中的单个失误可能导致任务失败，产生幻觉或试错行为。
...
Result: 在WebArena基准测试的五个领域中进行评估。配备环境地图的智能体达到28.2%的成功率，几乎是限于会话绑定上下文基线（14.2%）的两倍，并且优于可以访问用于生成环境地图的原始轨迹数据的智能体（23.3%）。

Conclusion: 通过在模型和环境之间提供结构化接口，环境地图为长视野规划建立了持久化基础，具有人类可解释性、可编辑性和可增量精炼的特点。
...
### [209] [Evaluating a Multi-Agent Voice-Enabled Smart Speaker for Care Homes: A Safety-Focused Framework](https://arxiv.org/abs/2603.23625)
...
TL;DR: 本文评估了面向护理院的语音智能音箱系统，通过Whisper语音识别与RAG技术结合，在安全关键环境中实现居民识别、提醒提取和日程安排功能，最佳配置下居民ID和护理类别匹配达100%，提醒识别达89.09%，端到端日程安排准确率84.65%。


<details>
  <summary>Details</summary>
Motivation: 人工智能在健康和社会护理中的应用日益增多，旨在减少行政工作量，让工作人员有更多时间专注于患者护理。本文旨在评估一种语音智能音箱系统，支持护理院日常活动，包括语音访问居民记录、提醒和安排任务，特别关注安全关键环境下的可靠性和安全性。

Method: 提出安全导向的端到端评估框架，结合Whisper语音识别与检索增强生成（RAG）方法（混合、稀疏和密集）。通过监督护理院试验和受控测试，评估330个语音转录文本，涵盖11个护理类别，包括184个包含提醒的交互。评估重点包括：(1)居民和护理类别的正确识别；(2)提醒识别和提取；(3)不确定性下的端到端日程安排正确性（包括安全延迟/澄清）。特别关注嘈杂环境和多样口音的可靠性，采用置信度评分、澄清提示和人机协同监督。

Result: 最佳配置（GPT-5.2）下，居民ID和护理类别匹配达到100%（95% CI: 98.86-100），提醒识别达到89.09%（95% CI: 83.81-92.80），零遗漏提醒（100%召回率）但存在一些误报。通过日历集成的端到端日程安排达到84.65%的精确提醒计数一致性（95% CI: 78.00-89.56），表明在将非正式语音指令转换为可操作事件方面仍存在边缘情况。

Conclusion: 研究结果表明，经过仔细评估并采取适当安全措施的语音系统，能够在护理院环境中支持准确的文档记录、有效的任务管理以及人工智能的可信使用。系统在安全关键环境中表现出良好的性能，但需要进一步优化以处理边缘情况和提高日程安排准确性。
...
### [210] [GTO Wizard Benchmark](https://arxiv.org/abs/2603.23660)
*Marc-Antoine Provost,Nejc Ilenic,Christopher Solinas,Philippe Beardsell*

Main category: cs.AI

TL;DR: GTO Wizard Benchmark：一个用于评估无限制德州扑克单挑算法的标准化框架，包含公开API、超人类AI对手和方差减少技术，用于评估大语言模型在多智能体部分可观测环境中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 解决扑克评估中的方差挑战，为多智能体部分可观测系统中的规划和推理研究提供精确、可量化的评估环境，特别是评估大语言模型在此类复杂环境中的表现。

Method: 开发了GTO Wizard Benchmark框架，包含：1) 公开API和标准化评估框架；2) GTO Wizard AI作为基准对手（近似纳什均衡的超人类扑克智能体）；3) 集成AIVAT方差减少技术（可证明无偏，用1/10手数达到同等统计显著性）；4) 在零样本条件下对主流大语言模型进行全面基准测试。

Result: GTO Wizard AI击败了2018年计算机扑克竞赛冠军Slumbot（优势为19.4±4.1 bb/100）。所有测试的大语言模型（包括GPT-5.4、Claude Opus 4.6、Gemini 3.1 Pro、Grok 4等）表现都远低于基准水平。定性分析揭示了改进机会，包括表示能力和对隐藏状态的推理能力。

Conclusion: 该基准为研究多智能体部分可观测系统中的规划和推理进展提供了精确的量化环境。虽然大语言模型近年来在推理方面取得了显著进步，但在复杂博弈环境中仍远未达到专业AI水平，特别是在表示和隐藏状态推理方面需要进一步改进。
...
### [211] [Grounding Vision and Language to 3D Masks for Long-Horizon Box Rearrangement](https://arxiv.org/abs/2603.23676)
*Ashish Malik,Caleb Lowe,Aayam Shrestha,Stefan Lee,Fuxin Li,Alan Fern*

Main category: cs.AI

TL;DR: RAMP-3D：基于3D视觉语言模型的反应式掩码规划器，用于解决3D环境中的长时程箱体重排任务，通过预测成对的3D掩码（拾取对象和放置区域）来生成多步拾放动作。


<details>
  <summary>Details</summary>
Motivation: 现有方法（符号规划器和2D视觉语言模型）在处理多物体、复杂3D几何和隐式语义约束时存在局限性，而3D视觉语言模型在自然语言指称到3D分割掩码的强基础能力为更通用的规划提供了潜力。

Method: 扩展现有3D基础模型，提出反应式动作掩码规划器（RAMP-3D），将长时程规划建模为成对3D掩码的顺序反应式预测："哪个物体"掩码指示拾取对象，"哪个目标区域"掩码指定放置位置。系统处理RGB-D观测和自然语言任务描述，反应式生成多步拾放动作。

Result: 在包含1-30个箱体的仓库式环境中11个任务变体上进行实验，RAMP-3D在长时程重排任务上达到79.5%的成功率，显著优于基于2D视觉语言模型的基线方法。

Conclusion: 基于掩码的反应式策略是长时程规划中符号管线的有前景的替代方案，展示了3D视觉语言模型在复杂3D环境规划任务中的潜力。
...
### [212] [LLMs Do Not Grade Essays Like Humans](https://arxiv.org/abs/2603.23714)
...
### [213] [Efficient Benchmarking of AI Agents](https://arxiv.org/abs/2603.23749)
*Franck Ndzomga*
...
### [214] [Learning-guided Prioritized Planning for Lifelong Multi-Agent Path Finding in Warehouse Automation](https://arxiv.org/abs/2603.23838)
...
### [215] [VehicleMemBench: An Executable Benchmark for Multi-User Long-Term Memory in In-Vehicle Agents](https://arxiv.org/abs/2603.23840)
...
### [216] [SCoOP: Semantic Consistent Opinion Pooling for Uncertainty Quantification in Multiple Vision-Language Model Systems](https://arxiv.org/abs/2603.23853)
...
### [217] [When AI output tips to bad but nobody notices: Legal implications of AI's mistakes](https://arxiv.org/abs/2603.23857)
...
### [218] [The DeepXube Software Package for Solving Pathfinding Problems with Learned Heuristic Functions and Search](https://arxiv.org/abs/2603.23873)
*Forest Agostinelli*
...
### [219] [DUPLEX: Agentic Dual-System Planning via LLM-Driven Information Extraction](https://arxiv.org/abs/2603.23909)
...
### [220] [AnalogAgent: Self-Improving Analog Circuit Design Automation with LLM Agents](https://arxiv.org/abs/2603.23910)
...
### [221] [From Pixels to Digital Agents: An Empirical Study on the Taxonomy and Technological Trends of Reinforcement Learning Environments](https://arxiv.org/abs/2603.23964)
...
### [222] [Language-Grounded Multi-Agent Planning for Personalized and Fair Participatory Urban Sensing](https://arxiv.org/abs/2603.24014)
...
### [225] [Bridging the Evaluation Gap: Standardized Benchmarks for Multi-Objective Search](https://arxiv.org/abs/2603.24084)
...
### [226] [AI-Supervisor: Autonomous AI Research Supervision via a Persistent Research World Model](https://arxiv.org/abs/2603.24402)
*Yunbo Long*
...
### [227] [From Liar Paradox to Incongruent Sets: A Normal Form for Self-Reference](https://arxiv.org/abs/2603.24527)
...
### [228] [Completeness of Unbounded Best-First Minimax and Descent Minimax](https://arxiv.org/abs/2603.24572)
*Quentin Cohen-Solal*
...
### [229] [The Stochastic Gap: A Markovian Framework for Pre-Deployment Reliability and Oversight-Cost Auditing in Agentic Artificial Intelligence](https://arxiv.org/abs/2603.24582)
